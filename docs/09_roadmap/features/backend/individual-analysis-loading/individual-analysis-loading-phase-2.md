# Individual Analysis Loading – Phase 2: Frontend Lazy Loading Infrastructure

## Overview
Create the frontend infrastructure for lazy loading individual analysis components, including reusable components, custom hooks, and API repository enhancements.
...
- [ ] File: `frontend/src/infrastructure/repositories/APIChatRepository.jsx` - Enhanced with individual analysis methods
...
- [ ] APIChatRepository has individual analysis methods
...
const useIndividualAnalysis = (analysisType, projectId = null, cacheKey = null) => {
  ...
  const preloadAnalysis = useCallback(async () => {
    try {
      const currentProjectId = projectId || await apiRepository.current.getCurrentProjectId();
      await apiRepository.current.preloadAnalysis(currentProjectId, analysisType);
      logger.info(`🔍 [useIndividualAnalysis] Preloaded ${analysisType} analysis`);
    } catch (err) {
      logger.warn(`⚠️ [useIndividualAnalysis] Failed to preload ${analysisType}:`, err);
    }
...
async getAnalysisStatus(projectId, analysisType) {
  const currentProjectId = projectId || await this.getCurrentProjectId();
  return apiCall(`/api/projects/${currentProjectId}/analysis/${analysisType}/status`, {}, currentProjectId);
}

async preloadAnalysis(projectId, analysisType) {
  const currentProjectId = projectId || await this.getCurrentProjectId();
  return apiCall(`/api/projects/${currentProjectId}/analysis/${analysisType}/preload`, {
    method: 'POST'
  }, currentProjectId);
}
```
...
describe('useIndividualAnalysis', () => {
  ...
  it('should load analysis data successfully', async () => {
    ...
    mockAPIRepository.getIndividualAnalysis.mockResolvedValue({
      success: true,
      data: mockData
    });
  ...
  it('should use cached data when available', async () => {
    ...
    expect(mockAPIRepository.getIndividualAnalysis).not.toHaveBeenCalled();
  });
});
```
...
This phase provides the frontend infrastructure that Phase 3 (Component Refactoring) will use to refactor existing analysis components to use lazy loading. 