...
function RightPanelComponent({ eventBus }) {
  ...
  useEffect(() => {
    console.log('🔄 RightPanelComponent initializing...');
    setupEventListeners();
    loadPanelData();
  ...
  const setupEventListeners = () => {
    if (eventBus) {
      ...
      eventBus.on('chat-info-updated', handleChatInfoUpdate);
  ...
  const loadPanelData = async () => {
    try {
      // Load chat info
      const chatData = await apiCall(API_CONFIG.endpoints.chat.info);
      if (chatData.success) {
        setChatInfo(chatData.info || {});
      }
  ...
  const handleChatInfoUpdate = (info) => {
    setChatInfo(info);
  };

  const handleFilesAttached = (files) => {
    setAttachedFiles(prevFiles => [...prevFiles, ...files]);
  };

  const handleFilesRemoved = (fileIds) => {
    setAttachedFiles(prevFiles => prevFiles.filter(f => !fileIds.includes(f.id)));
  };
  ...
  const handleQuickPromptSelected = (prompt) => {
    if (eventBus) {
      eventBus.emit('prompt-selected', { prompt });
    }
  };
  ...
  const handleRightPanelSend = () => {
    ...
    if (eventBus) {
      eventBus.emit('right-panel-message', { message: inputValue });
    }
    ...
    <div className="chat-tab">
      <div className="chat-info">
        ...
        <div className="info-item">
          ...
          <span className="value">{chatInfo.totalMessages || 0}</span>
        </div>
        <div className="info-item">
          <span className="label">Session Duration:</span>
          <span className="value">{chatInfo.duration || '0:00'}</span>
        </div>
        <div className="info-item">
          <span className="label">Last Activity:</span>
          <span className="value">{chatInfo.lastActivity || 'Never'}</span>
        </div>
      </div>
      ...
      <div className="quick-prompts">
        ...
        <div className="prompt-list">
          {quickPrompts.map(prompt => (
            <button
              key={prompt.id}
              onClick={() => handleQuickPromptSelected(prompt)}
    ...
    <div ref={containerRef} className="chat-right-panel">
      <div className="chat-right-panel-content">
        ...
        <div className="panel-header">
          ...
          <button onClick={handleToggle} className="btn-icon" title="Toggle Right Panel">
            ✕
          </button>
        </div>
        ...
        <div className="right-panel-input-area">
          ...
          />
          <button
            id="rightPanelSendBtn"
            onClick={handleRightPanelSend}
