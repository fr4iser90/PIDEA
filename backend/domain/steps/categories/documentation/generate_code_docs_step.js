...
class GenerateCodeDocsStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const includeScreenshots =
        options.includeScreenshots || config.settings.includeScreenshots;
      ...
      result.documentation = await this.generateCodeDocumentation(
      ...
      logger.info(
        `✅ Code documentation generated successfully. Analyzed ${result.filesAnalyzed.length} files.`,
      );
  ...
  async analyzeCodeFiles(projectPath) {
    ...
    try {
      ...
      logger.warn(
        `Warning: Could not scan directory ${projectPath}: ${error.message}`,
      );
    }
  ...
  calculateStats(files) {
    ...
    return {
      ...
      totalLines,
      averageComplexity:
        files.length > 0 ? (totalComplexity / files.length).toFixed(2) : 0,
    };
  }
