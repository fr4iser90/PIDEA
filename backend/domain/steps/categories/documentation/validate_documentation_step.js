...
class ValidateDocumentationStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const validateExamples =
        options.validateExamples || config.settings.validateExamples;
      const checkSpelling =
        options.checkSpelling || config.settings.checkSpelling;
      ...
      logger.info(
        `✅ Documentation validation completed. Score: ${result.validation.score}/100`,
      );
  ...
  async findDocumentationFiles(projectPath) {
    ...
    try {
      ...
      logger.warn(
        `Warning: Could not scan for documentation files: ${error.message}`,
      );
    }
  ...
  async scanForDocumentation(
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          await this.scanForDocumentation(
            itemPath,
            docFiles,
            docExtensions,
            currentDepth + 1,
  ...
  async validateDocumentationFile(docFile, options) {
    ...
    try {
      ...
      validation.recommendations = this.generateFileRecommendations(
        validation.checks,
        docFile,
      );
    } catch (error) {
