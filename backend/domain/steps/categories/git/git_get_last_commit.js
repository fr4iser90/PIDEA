...
class GitGetLastCommitStep {
  ...
  async execute(context = {}) {
    ...
    try {
      ...
      const messageResult = await execAsync("git log -1 --pretty=format:%s", {
        cwd: projectPath,
      });
      const message = messageResult.stdout.trim();

      // Get commit author using execAsync
      const authorResult = await execAsync("git log -1 --pretty=format:%an", {
        cwd: projectPath,
      });
      const author = authorResult.stdout.trim();

      // Get commit date using execAsync
      const dateResult = await execAsync("git log -1 --pretty=format:%cd", {
        cwd: projectPath,
      });
