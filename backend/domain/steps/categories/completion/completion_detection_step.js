...
class CompletionDetectionStep {
  ...
  async execute(context) {
    try {
      ...
      logger.info(
        `Detecting completion for project: ${projectId}${taskId ? `, task: ${taskId}` : ""}`,
      );
      ...
      if (taskId) {
        ...
        tasksToCheck = await taskRepository.findByProjectIdAndStatus(
          projectId,
          "pending",
        );
      }
      ...
      const completionCriteria = await this.checkCompletionCriteria(
        terminalService,
        workspacePath,
      );
      ...
      const completedTasks = results.filter(
        (r) => r.success && r.completed,
      ).length;

      logger.info(
        `CompletionDetectionStep completed: ${completedTasks}/${tasksToCheck.length} tasks completed`,
      );
  ...
  async checkCompletionCriteria(terminalService, workspacePath) {
    try {
      ...
      const testResults = await terminalService.executeCommand(
      ...
      const gitStatus = await terminalService.executeCommand(
