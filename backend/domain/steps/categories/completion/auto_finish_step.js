...
class AutoFinishStep {
  constructor() {
    ...
    this.description =
      "Automatically finish tasks based on completion detection";
  ...
  async execute(context = {}) {
    ...
    try {
      ...
      logger.info(
        `Auto-finishing project: ${projectId}${taskId ? `, task: ${taskId}` : ""}`,
      );
      ...
      if (taskId) {
        ...
        tasksToFinish = await taskRepository.findByProjectIdAndStatus(
          projectId,
          "completed",
        );
      }
      ...
      logger.info(
        `AutoFinishStep completed: ${successfulFinishes}/${tasksToFinish.length} tasks finished`,
      );
      ...
      return {
        ...
        data: {
          ...
          results,
          committed:
            gitStatus.success && gitStatus.output.trim() ? true : false,
        },
      };
    } catch (error) {
