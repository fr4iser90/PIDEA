...
class IDESendMessageStep {
  ...
  async execute(context = {}) {
    ...
    try {
      ...
      logger.info(
        `📤 Sending message to IDE for project ${projectId}${ideType ? ` (${ideType})` : ""}`,
      );
      ...
      if (clickNewChat) {
        ...
        if (browserManager) {
          ...
          logger.warn(
            "🔘 [IDESendMessageStep] BrowserManager not available for New Chat click",
          );
        }
      } else {
        logger.info(
          "🔘 [IDESendMessageStep] Skipping New Chat click (clickNewChat: false)",
        );
      }
      ...
      logger.info(
        "📝 Executing SendMessageHandler (Business Logic + Browser Automation)...",
      );
      ...
      if (waitForResponse) {
        ...
        if (!browserManager) {
          logger.warn(
            "BrowserManager not available, skipping AI response waiting",
          );
        } else {
          try {
            ...
            if (page) {
              ...
              logger.info("🔍 Debug: IDE selectors retrieved:", {
                ...
                chatSelectors: ideSelectors?.chatSelectors
                  ? Object.keys(ideSelectors.chatSelectors)
                  : [],
              });
      ...
      const aiResponseSuccessful =
        !waitForResponse || (aiResponse && aiResponse.success !== false);

      return {
        success: aiResponseSuccessful,
        message: aiResponseSuccessful
          ? "Message sent to IDE via Handler"
