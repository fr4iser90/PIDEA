...
class GetChatHistoryStep {
  ...
  async execute(context) {
    ...
    try {
      ...
      if (
        context.sessionId &&
        !isPortNumber &&
        context.sessionId !== context.port
      ) {
        ...
        try {
          ...
          logger.warn(
            `Session ${context.sessionId} not found, falling back to IDE extraction`,
          );
          messages = [];
        }
      }
      ...
      if (
        ...
        const cachedMessages = chatCacheService
          ? await chatCacheService.getChatHistory(port)
          : null;
        if (cachedMessages) {
          logger.info(
            `Cache hit: Retrieved ${cachedMessages.length} messages from cache for port ${port}`,
          );
          ...
          logger.info(
            `Cache miss: Extracting live chat from IDE for port ${port}`,
          );
          ...
          if (cursorIDEService) {
            try {
              ...
              if (chatCacheService) {
                messages = await chatCacheService.executeWithDeduplication(
                  port,
                  async () => {
                    const extractedMessages =
                      await cursorIDEService.extractChatHistory();
                    logger.info(
                      `Extracted ${extractedMessages.length} messages from IDE on port ${port}`,
                    );
                ...
                logger.info(
                  `Extracted ${messages.length} messages from IDE on port ${port}`,
                );
              }
            } catch (error) {
              logger.error(
                `Failed to extract chat from IDE on port ${port}:`,
                error,
              );
              messages = [];
            }
      ...
      if (eventBus) {
        eventBus
          .publish("chat.history.retrieval.failed", {
            stepId,
            userId: context.userId,
            sessionId: context.sessionId,
            error: originalError,
  ...
  invalidateCache(port) {
    ...
    logger.warn(
      "invalidateCache method is deprecated - use DI container instead",
    );
  }
  ...
  getCacheStats() {
    // This method is deprecated - use DI container to get cache service
    logger.warn(
      "getCacheStats method is deprecated - use DI container instead",
    );
    return {};
  }
