...
class ZapSecurityStep {
  ...
  async execute(context = {}) {
    ...
    try {
      ...
      logger.info(
        `📊 Starting ZAP web application security testing for: ${projectPath}`,
      );
      ...
      logger.error(
        `❌ ZAP web application security testing failed: ${error.message}`,
      );
  ...
  detectWebSecurityIssues(content, filePath) {
    ...
    webSecurityPatterns.forEach(
      ({ pattern, severity, message, category, suggestion }) => {
    ...
    webBestPracticePatterns.forEach(
      ...
      case "package.json":
        try {
          ...
          if (
            pkg.scripts &&
            pkg.scripts.start &&
            pkg.scripts.start.includes("--inspect")
          ) {
            vulnerabilities.push({
              type: "web-security",
              severity: "high",
