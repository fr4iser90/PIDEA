...
class TrivySecurityStep {
  ...
  detectSecurityIssues(content, filePath) {
    ...
    const dangerousPatterns = [
      ...
      {
        pattern: /sessionStorage\.[a-zA-Z_][a-zA-Z0-9_]*\s*=/,
        severity: "medium",
    ...
    if (result.vulnerabilities && result.vulnerabilities.length > 0) {
      issues.push(
        ...result.vulnerabilities.map((vuln) => ({
          ...
          description:
            vuln.description || vuln.message || "Security vulnerability found",
    ...
    if (result.bestPractices && result.bestPractices.length > 0) {
      issues.push(
        ...result.bestPractices.map((bp) => ({
          ...
          description:
            bp.description || "Security best practice recommendation",
  ...
  generateIssues(result) {
    ...
    if (
      ...
      issues.push({
        ...
        description:
          "High severity security vulnerabilities found in the codebase",
        severity: "high",
        priority: "high",
        category: "security",
        source: "TrivySecurityStep",
    ...
    if (
      result.vulnerabilities &&
      result.vulnerabilities.some((v) => v.message.includes("eval()"))
    ) {
      issues.push({
        type: "eval-usage",
        ...
        location: "security-analysis",
        suggestion:
          "Replace eval() with safer alternatives to prevent code injection",
      });
    }
