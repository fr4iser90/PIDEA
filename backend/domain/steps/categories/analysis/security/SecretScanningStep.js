...
class SecretScanningStep {
  ...
  async analyzeSecrets(projectPath, options = {}) {
    try {
      ...
      logger.info(
        `Scanning ${Math.min(allFiles.length, maxFiles)} files for secrets`,
      );
  ...
  detectSecrets(content, filePath) {
    ...
    const secretPatterns = [
      ...
      {
        pattern:
          /aws[_-]?access[_-]?key[_-]?id\s*[:=]\s*['"`][^'"`]{20,}['"`]/i,
        severity: "high",
      ...
      {
        pattern:
          /aws[_-]?secret[_-]?access[_-]?key\s*[:=]\s*['"`][^'"`]{20,}['"`]/i,
        severity: "high",
    ...
    try {
      ...
      for (const envFile of envFiles) {
        try {
          ...
          const envSecrets = this.analyzeEnvFile(
            content,
