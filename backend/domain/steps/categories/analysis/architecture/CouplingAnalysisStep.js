...
class CouplingAnalysisStep {
  ...
  async analyzeCoupling(projectPath, options = {}) {
    try {
      ...
      const packageDependencies =
        await this.analyzePackageDependencies(projectPath);
  ...
  generateCouplingRecommendations(data) {
    ...
    if (coupling.high.length > 5) {
      recommendations.push({
        ...
        action:
          "Consider refactoring to reduce dependencies between components",
        files: coupling.high.slice(0, 3).map((f) => f.file),
      });
    }

    // Check for external dependencies
    const externalDeps = packageDependencies.filter(
    ...
    if (externalDeps.length > 20) {
      recommendations.push({
        ...
        action:
          "Consider if all dependencies are necessary and look for alternatives",
        count: externalDeps.length,
      });
    }

    // Check for relative imports
    const highRelativeImports = coupling.high.filter(
      (f) => f.importTypes.relative > 5,
    );
    if (highRelativeImports.length > 0) {
      recommendations.push({
        ...
        action:
          "Consider using absolute imports or barrel exports to reduce coupling",
  ...
  calculateConfidence(result) {
    ...
    return Math.min(
      fileConfidence + dependencyConfidence + couplingConfidence,
      100,
    );
  }
  ...
  generateIssues(result) {
    ...
    if (result.score < 70) {
      issues.push({
        ...
        suggestion:
          "Improve coupling management and reduce tight coupling between components",
      });
    }
    ...
    if (result.metrics && result.metrics.highCouplingFiles > 5) {
      issues.push({
        ...
        location: "coupling-analysis",
        suggestion:
          "Refactor high coupling files to reduce dependencies and improve maintainability",
      });
    }
    ...
    if (
      ...
      result.packageDependencies.some((dep) => dep.circular)
    ) {
      issues.push({
        ...
        location: "package-dependencies",
        suggestion:
          "Break circular dependencies to prevent build and runtime issues",
      });
    }
