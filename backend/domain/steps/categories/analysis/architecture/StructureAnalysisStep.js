...
class StructureAnalysisStep {
    ...
    try {
      ...
      if (this.hasMVCStructure(directories)) {
        ...
        layers.push(
          ...
          { name: "View", type: "presentation", directories: structure.views },
          {
            name: "Controller",
            type: "logic",
            directories: structure.controllers,
          },
        );
      }
      ...
      if (this.hasMicroservicesStructure(directories)) {
        patterns.push({
          ...
          evidence:
            "Multiple service directories with independent configurations",
        });
      }
  ...
  analyzeDirectoryStructure(directories, projectPath) {
    ...
    directories.forEach((dir) => {
      ...
      if (dirName.includes("model") || dirName.includes("entity")) {
        ...
        dirName.includes("controller") ||
        dirName.includes("handler")
      ) {
  ...
  hasMVCStructure(directories) {
    ...
    const hasModels = dirNames.some(
      (name) => name.includes("model") || name.includes("entity"),
    );
    const hasViews = dirNames.some(
      (name) =>
        name.includes("view") ||
        name.includes("page") ||
        name.includes("component"),
    );
    const hasControllers = dirNames.some(
      (name) => name.includes("controller") || name.includes("handler"),
    );
  ...
  hasMicroservicesStructure(directories) {
    const dirNames = directories.map((dir) => path.basename(dir).toLowerCase());
    const serviceDirs = dirNames.filter(
      (name) => name.includes("service") && !name.includes("test"),
    );
  ...
  generateIssues(result) {
    ...
    if (result.score < 70) {
      issues.push({
        ...
        suggestion:
          "Consider reorganizing project structure following architectural best practices",
      });
    }
    ...
    if (result.patterns && result.patterns.length === 0) {
      issues.push({
        ...
        description:
          "No clear architectural patterns were identified in the project structure",
        severity: "high",
        priority: "high",
        category: "architecture",
        source: "StructureAnalysisStep",
        location: "project-structure",
        suggestion:
          "Implement a clear architectural pattern (MVC, DDD, Layered Architecture, etc.)",
      });
    }
    ...
    if (result.layers && result.layers.length === 0) {
      issues.push({
        ...
        description:
          "No clear layer separation was identified in the project structure",
        severity: "medium",
        priority: "medium",
        category: "architecture",
        source: "StructureAnalysisStep",
        location: "project-structure",
        suggestion:
          "Organize code into logical layers (presentation, business, data, etc.)",
      });
    }
    ...
    if (
      result.metrics &&
      result.metrics.totalDirectories > 50 &&
      result.patterns &&
      ...
      issues.push({
        ...
        location: "project-structure",
        suggestion:
          "Reorganize directories following architectural principles and best practices",
      });
    }
  ...
  generateRecommendations(result) {
    ...
    if (result.score < 80) {
      recommendations.push({
        ...
        source: "StructureAnalysisStep",
        action:
          "Implement architectural best practices to improve structure score",
  ...
  async createImplementationDoc(result, docsDir) {
    const implementationPath = path.join(
      docsDir,
      `structure-analysis-step-implementation.md`,
    );
