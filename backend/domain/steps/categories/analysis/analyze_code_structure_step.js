...
const logger = new Logger("AnalyzeCodeStructureStep");
...
class AnalyzeCodeStructureStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const analyzeImports =
        options.analyzeImports || config.settings.analyzeImports;
      ...
      result.analysis.structure = await this.analyzeCodeStructure(
        projectPath,
        scanDepth,
        includeHidden,
      );
      ...
      if (analyzeImports) {
        result.analysis.imports = await this.analyzeImports(
          result.analysis.structure,
        );
      }

      // Analyze organization
      result.analysis.organization = await this.analyzeOrganization(
        result.analysis.structure,
      );
  ...
  async analyzeCodeStructure(
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          const subStructure = await this.analyzeCodeStructure(
            itemPath,
            maxDepth,
            includeHidden,
            currentDepth + 1,
  ...
  extractClasses(content) {
    const classes = [];
    const patterns = [/class\s+(\w+)/g, /interface\s+(\w+)/g, /type\s+(\w+)/g];
  ...
  async analyzeOrganization(structure) {
    ...
    if (
      ...
      directories.has("services") &&
      directories.has("repositories")
    ) {
