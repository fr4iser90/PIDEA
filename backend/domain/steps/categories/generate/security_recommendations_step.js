...
const config = {
  ...
  type: "generate",
  description:
    "Generates security-focused recommendations based on security analysis",
  category: "generate",
  version: "1.0.0",
...
class SecurityRecommendationsStep {
  ...
  async execute(context) {
    try {
      ...
      if (!security_analysis) {
        throw new Error(
          "Security analysis required for security recommendations generation",
        );
      }
      ...
      recommendations.recommendations =
        await this._generateSecurityRecommendations(security_analysis);
      ...
      this.logger.info(
        `Generated ${recommendations.recommendations.length} security recommendations`,
      );
  ...
  async _generateSecurityRecommendations(securityAnalysis) {
    ...
    if (
      ...
      const mediumVulns = securityAnalysis.vulnerabilities.filter(
        (v) => v.severity === "medium",
      );
    ...
    if (securityAnalysis.configuration) {
      if (
        ...
        securityAnalysis.configuration.missingSecurity.length > 0
      ) {
        recommendations.push({
          ...
          details: securityAnalysis.configuration.missingSecurity.map(
            (missing) => ({
              component: missing,
      ...
      if (
        securityAnalysis.configuration.weakConfigurations &&
        securityAnalysis.configuration.weakConfigurations.length > 0
      ) {
        recommendations.push({
    ...
    if (
      securityAnalysis.dependencies &&
      securityAnalysis.dependencies.securityIssues &&
      securityAnalysis.dependencies.securityIssues.length > 0
    ) {
      recommendations.push({
  ...
  async _generateInsights(recommendations) {
    ...
    const vulnerabilityRecs = recommendations.filter((r) =>
      r.tags.includes("vulnerabilities"),
    );
