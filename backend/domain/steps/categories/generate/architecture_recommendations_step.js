...
const config = {
  ...
  type: "generate",
  description:
    "Generates architecture-focused recommendations based on architecture analysis",
  category: "generate",
  version: "1.0.0",
...
class ArchitectureRecommendationsStep {
  ...
  async execute(context) {
    try {
      ...
      if (!architecture_analysis) {
        throw new Error(
          "Architecture analysis required for architecture recommendations generation",
        );
      }
      ...
      recommendations.recommendations =
        await this._generateArchitectureRecommendations(architecture_analysis);
      ...
      this.logger.info(
        `Generated ${recommendations.recommendations.length} architecture recommendations`,
      );

      return recommendations;
    } catch (error) {
      this.logger.error(
        "Architecture recommendations generation failed:",
        error,
      );
      throw error;
    }
  ...
  async _generateArchitectureRecommendations(architectureAnalysis) {
    ...
    if (
      ...
      architectureAnalysis.detectedPatterns &&
      architectureAnalysis.detectedPatterns.length > 1
    ) {
      recommendations.push({
    ...
    if (
      ...
      const highViolations = architectureAnalysis.violations.filter(
        (v) => v.severity === "high",
      );
    ...
    if (architectureAnalysis.coupling) {
      const highInstability = Object.entries(
        architectureAnalysis.coupling.instability || {},
      ...
      const highCoupling = Object.entries(
        architectureAnalysis.coupling.afferentCoupling || {},
    ...
    // Layer violations
    if (
      architectureAnalysis.layerViolations &&
      architectureAnalysis.layerViolations.length > 0
    ) {
      recommendations.push({
  ...
  async _generateInsights(recommendations) {
    ...
    // Pattern focus
    const patternRecs = recommendations.filter((r) =>
    ...
    // Dependency focus
    const dependencyRecs = recommendations.filter((r) =>
      r.tags.includes("dependencies"),
    );
