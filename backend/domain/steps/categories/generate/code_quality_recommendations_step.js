...
const config = {
  ...
  type: "generate",
  description:
    "Generates code quality-focused recommendations based on code quality analysis",
  category: "generate",
  version: "1.0.0",
...
class CodeQualityRecommendationsStep {
  ...
  async execute(context) {
    try {
      ...
      if (!code_quality_analysis) {
        throw new Error(
          "Code quality analysis required for code quality recommendations generation",
        );
      }
      ...
      recommendations.recommendations =
        await this._generateCodeQualityRecommendations(code_quality_analysis);
      ...
      this.logger.info(
        `Generated ${recommendations.recommendations.length} code quality recommendations`,
      );

      return recommendations;
    } catch (error) {
      this.logger.error(
        "Code quality recommendations generation failed:",
        error,
      );
      throw error;
    }
  ...
  async _generateCodeQualityRecommendations(codeQualityAnalysis) {
    ...
    if (codeQualityAnalysis.metrics?.complexity) {
      ...
      const veryHighComplexity = Object.entries(
        codeQualityAnalysis.metrics.complexity.cyclomaticComplexity,
    ...
    if (codeQualityAnalysis.metrics?.maintainability) {
      if (
        codeQualityAnalysis.metrics.maintainability.largeFiles &&
        codeQualityAnalysis.metrics.maintainability.largeFiles.length > 0
      ) {
        recommendations.push({
    ...
    if (codeQualityAnalysis.issues && codeQualityAnalysis.issues.length > 0) {
      const consoleLogIssues = codeQualityAnalysis.issues.filter(
        (i) => i.type === "console-log",
      );
  ...
  async _generateInsights(recommendations) {
    ...
    // Testing focus
    const testingRecs = recommendations.filter((r) =>
      r.tags.includes("testing"),
    );
    ...
    // Complexity focus
    const complexityRecs = recommendations.filter((r) =>
      r.tags.includes("complexity"),
    );
