...
class RecommendationsStep {
  ...
  async execute(context) {
    try {
      ...
      if (!analysis_results) {
        throw new Error(
          "Analysis results required for recommendations generation",
        );
      }
      ...
      const securityRecs = await this._generateSecurityRecommendations(
        analysis_results.security,
      );
      const qualityRecs = await this._generateCodeQualityRecommendations(
        analysis_results.codeQuality,
      );
      const architectureRecs = await this._generateArchitectureRecommendations(
        analysis_results.architecture,
      );
      const performanceRecs = await this._generatePerformanceRecommendations(
        analysis_results.performance,
      );
      const techStackRecs = await this._generateTechStackRecommendations(
        analysis_results.techStack,
      );
      const manifestRecs = await this._generateManifestRecommendations(
        analysis_results.manifest,
      );
      const dependencyRecs = await this._generateDependencyRecommendations(
      ...
      // Generate insights
      recommendations.insights = await this._generateInsights(
        recommendations.recommendations,
      );

      // Calculate metadata
      recommendations.metadata = this._calculateMetadata(
        recommendations.recommendations,
      );
  ...
  async _generateSecurityRecommendations(securityAnalysis) {
    ...
    if (
      ...
      const highVulns = securityAnalysis.vulnerabilities.filter(
        (v) => v.severity === "high",
      );
    ...
    if (
      securityAnalysis.configuration &&
      securityAnalysis.configuration.missingSecurity
    ) {
      recommendations.push({
    ...
    // Secrets detection
    if (
      securityAnalysis.secrets &&
      securityAnalysis.secrets.found &&
      securityAnalysis.secrets.found.length > 0
    ) {
      recommendations.push({
  ...
  async _generateCodeQualityRecommendations(codeQualityAnalysis) {
    ...
    if (
      codeQualityAnalysis.configuration &&
      !codeQualityAnalysis.configuration.linting?.hasESLint
    ) {
      recommendations.push({
    ...
    if (codeQualityAnalysis.metrics?.complexity) {
      const highComplexity = Object.entries(
        codeQualityAnalysis.metrics.complexity.cyclomaticComplexity,
    ...
    // Testing
    if (
      codeQualityAnalysis.metrics?.testability &&
      !codeQualityAnalysis.metrics.testability.hasTests
    ) {
      recommendations.push({
  ...
  async _generateArchitectureRecommendations(architectureAnalysis) {
    ...
    // Pattern detection
    if (
      architectureAnalysis.detectedPatterns &&
      architectureAnalysis.detectedPatterns.length === 0
    ) {
      recommendations.push({
    ...
    if (
      architectureAnalysis.dependencies?.circularDependencies &&
      architectureAnalysis.dependencies.circularDependencies.length > 0
    ) {
      recommendations.push({
    ...
    // Violations
    if (
      architectureAnalysis.violations &&
      architectureAnalysis.violations.length > 0
    ) {
      const criticalViolations = architectureAnalysis.violations.filter(
        (v) => v.severity === "critical",
      );
  ...
  async _generatePerformanceRecommendations(performanceAnalysis) {
    ...
    if (performanceAnalysis.issues && performanceAnalysis.issues.length > 0) {
      const criticalIssues = performanceAnalysis.issues.filter(
        (i) => i.severity === "critical",
      );
      const highIssues = performanceAnalysis.issues.filter(
        (i) => i.severity === "high",
      );
  ...
  async _generateTechStackRecommendations(techStackAnalysis) {
    ...
    if (
      ...
      techStackAnalysis.languages.includes("JavaScript") &&
      !techStackAnalysis.languages.includes("TypeScript")
    ) {
      recommendations.push({
    ...
    // Testing framework
    if (
      techStackAnalysis.tools &&
      !techStackAnalysis.tools.some((t) =>
        ["jest", "mocha", "vitest"].includes(t.name.toLowerCase()),
      )
    ) {
      recommendations.push({
  ...
  async _generateManifestRecommendations(manifestAnalysis) {
    ...
    if (
      manifestAnalysis.missingConfigs &&
      manifestAnalysis.missingConfigs.length > 0
    ) {
      recommendations.push({
  ...
  _calculateMetadata(recommendations) {
    const metadata = {
      ...
      mediumCount: recommendations.filter((r) => r.priority === "medium")
        .length,
