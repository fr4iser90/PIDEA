...
class TaskStatusSyncStep {
  ...
  getRequiredServices(context) {
    ...
    const statusTransitionService = context.getService(
      "statusTransitionService",
    );
  ...
  async syncSingleTask(context, services, options) {
    ...
    if (!isValidTransition) {
      throw new Error(
        `Invalid status transition from ${task.status?.value || task.status} to ${targetStatus}`,
      );
    }

    // Perform sync
    const result = await this.performStatusSync(
      task,
      targetStatus,
      services,
      options,
  ...
  async validateSync(context, services, options) {
    ...
    logger.info(
      `✅ Validation completed: ${validationResult.validTasks} valid, ${validationResult.invalidTasks} invalid`,
    );
  ...
  async rollbackSync(context, services, options) {
    ...
    logger.info(
      `✅ Rollback completed: ${rollbackResult.successfulRollbacks} successful, ${rollbackResult.failedRollbacks} failed`,
    );
    ...
    try {
      ...
      switch (newStatus) {
        ...
        case "pending":
          ...
          break;
        case "cancelled":
          await services.statusTransitionService.moveTaskToCancelled(
            task.id,
            "Task cancelled via sync",
          );
          break;
        default:
