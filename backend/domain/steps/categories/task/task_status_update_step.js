...
class TaskStatusUpdateStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      logger.info(
        `✅ Task status updated successfully: ${taskId} -> ${newStatus}`,
      );
      return result;
    } catch (error) {
  ...
  async updateTaskStatus(taskId, newStatus, metadata, taskRepository) {
    try {
      logger.info(
        `📊 Updating database record for task ${taskId}: status=${newStatus}`,
      );
    ...
    try {
      ...
      switch (newStatus) {
        ...
        case "pending":
          ...
          break;
        case "cancelled":
          await statusTransitionService.moveTaskToCancelled(
            taskId,
            metadata.reason || "Task cancelled",
          );
          break;
        default:
  ...
  async updateFileReferences(taskId, currentTask, newStatus) {
    try {
      ...
      logger.info(
        `📝 Would update file references for task ${taskId} with new status: ${newStatus}`,
      );
    } catch (error) {
