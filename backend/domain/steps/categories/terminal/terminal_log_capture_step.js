...
class TerminalLogCaptureStep {
  ...
  validateContext(context) {
    ...
    if (
      ...
      typeof context.includeTimestamps !== "boolean"
    ) {
    ...
    if (context.filterLevel && !validLevels.includes(context.filterLevel)) {
      throw new Error(
        `Invalid filter level: ${context.filterLevel}. Valid levels: ${validLevels.join(", ")}`,
      );
    }
    ...
    if (context.format && !validFormats.includes(context.format)) {
      throw new Error(
        `Invalid format: ${context.format}. Valid formats: ${validFormats.join(", ")}`,
      );
    }
  }
...
module.exports = {
  config,
  execute: TerminalLogCaptureStep.prototype.execute.bind(
    new TerminalLogCaptureStep(),
  ),
