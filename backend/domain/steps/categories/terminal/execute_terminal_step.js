...
class ExecuteTerminalStep {
  ...
  async execute(context = {}) {
    ...
    try {
      logger.info("Starting terminal command execution", {
        ...
        command:
          context.command?.substring(0, 50) +
          (context.command?.length > 50 ? "..." : ""),
  ...
  async executeTerminalCommand(services, command, options) {
    ...
    try {
      const result = await services.ideAutomationService.executeTerminalCommand(
        command,
        {
  ...
  validateContext(context) {
    ...
    if (
      context.waitTime &&
      (typeof context.waitTime !== "number" ||
        context.waitTime < 0 ||
...
module.exports = {
  config,
  execute: ExecuteTerminalStep.prototype.execute.bind(
    new ExecuteTerminalStep(),
  ),
  ExecuteTerminalStep,
};
