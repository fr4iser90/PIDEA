...
class ListTerminalProcessesStep {
  ...
  validateContext(context) {
    ...
    if (
      context.sortBy &&
      !["cpu", "memory", "pid", "name", "user"].includes(context.sortBy)
    ) {
    ...
    if (context.filter) {
      ...
      for (const key of Object.keys(context.filter)) {
        if (!validFilterKeys.includes(key)) {
          throw new Error(
            `Invalid filter key: ${key}. Valid keys: ${validFilterKeys.join(", ")}`,
          );
        }
      }
    }
    ...
    if (
      ...
      (typeof context.refreshInterval !== "number" ||
        context.refreshInterval < 0)
    ) {
  ...
  validate(context) {
    ...
    if (context.refreshInterval && context.refreshInterval < 1000) {
      warnings.push(
        "Very short refresh intervals may impact system performance",
      );
    }
...
module.exports = {
  config,
  execute: ListTerminalProcessesStep.prototype.execute.bind(
    new ListTerminalProcessesStep(),
  ),
