...
const config = {
  ...
  category: "terminal",
  description:
    "Manage terminal sessions with creation, management, and cleanup capabilities",
  version: "1.0.0",
...
class TerminalSessionManagementStep {
  ...
  validateContext(context) {
    ...
    if (!validActions.includes(context.action)) {
      throw new Error(
        `Invalid action: ${context.action}. Valid actions: ${validActions.join(", ")}`,
      );
    }
    ...
    if (
      actionsRequiringSessionId.includes(context.action) &&
      !context.sessionId
    ) {
    ...
    if (context.sessionConfig) {
      ...
      for (const key of Object.keys(context.sessionConfig)) {
        if (!validConfigKeys.includes(key)) {
          throw new Error(
            `Invalid session config key: ${key}. Valid keys: ${validConfigKeys.join(", ")}`,
          );
        }
      }
    }
    ...
    if (
      ...
      throw new Error(
        "Timeout must be a number between 1000 and 120000 milliseconds",
      );
    }
  }
  ...
  validate(context) {
    ...
    if (context.action === "delete") {
      warnings.push(
        "Delete action will permanently remove the terminal session",
      );
    }
    ...
    if (context.cleanup) {
      warnings.push(
        "Cleanup enabled - sessions will be terminated after completion",
      );
    }
...
module.exports = {
  config,
  execute: TerminalSessionManagementStep.prototype.execute.bind(
    new TerminalSessionManagementStep(),
  ),
