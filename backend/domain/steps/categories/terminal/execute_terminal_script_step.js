...
const config = {
  ...
  category: "terminal",
  description:
    "Execute terminal scripts with validation, environment setup, and error handling",
  version: "1.0.0",
...
class ExecuteTerminalScriptStep {
  ...
  validateContext(context) {
    ...
    if (
      context.interpreter &&
      !validInterpreters.includes(context.interpreter)
    ) {
      throw new Error(
        `Invalid interpreter: ${context.interpreter}. Valid interpreters: ${validInterpreters.join(", ")}`,
      );
    }
    ...
    if (context.script) {
      ...
      for (const pattern of dangerousPatterns) {
        if (lowerScript.includes(pattern)) {
          warnings.push(
            `Potentially dangerous script pattern detected: ${pattern}`,
          );
        }
      }
    }
...
module.exports = {
  config,
  execute: ExecuteTerminalScriptStep.prototype.execute.bind(
    new ExecuteTerminalScriptStep(),
  ),
