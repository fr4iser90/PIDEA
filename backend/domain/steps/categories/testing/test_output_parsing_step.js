...
class TestOutputParsingStep {
  constructor() {
    ...
    this.description =
      "Parse existing test output files for failing tests and coverage issues";
  ...
  validateServices(context) {
    ...
    try {
      ...
      logger.warn(
        "⚠️ eventBus not available, continuing without event publishing",
      );
    }
  ...
  async parseAllTestOutputs(projectPath, options = {}) {
    try {
      ...
      const testFiles = await this.findTestOutputFiles(
        projectPath,
        testOutputPatterns,
      );
      ...
      // Remove duplicates
      parsedData.failingTests = this.removeDuplicates(
        parsedData.failingTests,
        "testName",
      );
      parsedData.coverageIssues = this.removeDuplicates(
        parsedData.coverageIssues,
        "file",
      );
      parsedData.legacyTests = this.removeDuplicates(
        parsedData.legacyTests,
        "testName",
      );
      parsedData.complexTests = this.removeDuplicates(
        parsedData.complexTests,
        "testName",
      );
