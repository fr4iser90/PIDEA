...
class TestingStep {
  constructor() {
    ...
    this.legacyPatterns = [/describe\(/g, /it\(/g, /test\(/g, /expect\(/g];
    ...
    try {
      ...
      for (const testFile of testFiles) {
        try {
          ...
          logger.debug(
            `Failed to analyze test file: ${testFile}`,
            error.message,
          );
        }
      }
    ...
    try {
      ...
      for (const testFile of testFiles) {
        try {
          ...
          logger.debug(
            `Failed to analyze legacy test: ${testFile}`,
            error.message,
          );
        }
      }
    ...
    try {
      ...
      for (const testFile of testFiles) {
        try {
          ...
          logger.debug(
            `Failed to analyze complex test: ${testFile}`,
            error.message,
          );
        }
      }
  ...
  async runTestFile(testFile) {
    try {
      ...
      if (!testCommand) {
        return { error: "No test command found" };
      }
  ...
  async runCoverageAnalysis(projectPath, context) {
    try {
      ...
      const coveredLines = await this.estimateCoveredLines(
        sourceFiles,
        testFiles,
      );
  ...
  async runAutoTestFix(projectPath, context) {
    try {
      ...
      const failingTests = await this.analyzeFailingTests(projectPath);
      const coverageAnalysis = await this.runCoverageAnalysis(
        projectPath,
        context,
      );

      const tasksGenerated =
        failingTests.length + (coverageAnalysis.needsImprovement ? 1 : 0);
  ...
  generateSummary(results) {
    ...
    if (summary.totalIssues > 0) {
      summary.recommendations.push(
        `Found ${summary.totalIssues} test issues that need attention`,
      );
    }
    if (summary.testsGenerated > 0) {
      summary.recommendations.push(
        `Generated ${summary.testsGenerated} new tests`,
      );
    }
