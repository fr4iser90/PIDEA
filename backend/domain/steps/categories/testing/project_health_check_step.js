...
class ProjectHealthCheckStep {
  ...
  async execute(context = {}) {
    ...
    try {
      ...
      if (!projectRepo) {
        logger.warn(
          "ProjectRepository not available in context, using basic health check",
        );
      ...
      if (config.settings.includeBuildCheck) {
        healthChecks.build = await this.performBuildCheck(
          workspacePath,
          packageManager,
        );
      ...
      if (config.settings.includeTestCheck) {
        healthChecks.test = await this.performTestCheck(
          workspacePath,
          packageManager,
        );
        healthChecks.summary.total++;
      ...
      if (config.settings.includeLintCheck) {
        healthChecks.lint = await this.performLintCheck(
          workspacePath,
          packageManager,
        );
        healthChecks.summary.total++;
      ...
      logger.info(
        `✅ Health checks completed. Status: ${healthChecks.summary.status}`,
      );
