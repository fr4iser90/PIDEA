...
class ValidateAIOutputStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const validateSyntax =
        options.validateSyntax || config.settings.validateSyntax;
      ...
      logger.info(
        `✅ AI output validation completed. Score: ${result.validation.score}/100`,
      );
    ...
    try {
      ...
      if (file.content) {
        ...
        if (issues.length > 0) {
          ...
          fileCheck.score = Math.max(
            0,
            100 -
              highSeverityIssues.length * 30 -
              mediumSeverityIssues.length * 10,
          );
          fileCheck.status =
            fileCheck.score >= 80
              ? "pass"
  ...
  async checkQuality(context) {
    ...
    try {
      ...
      qualityCheck.score = this.calculateQualityScore(
  ...
  calculateQualityMetrics(codeFiles) {
    ...
    if (metrics.totalFiles > 0) {
      metrics.averageLinesPerFile = Math.round(
  ...
  async runTests(context) {
    ...
    try {
      ...
      if (testCheck.results.total > 0) {
        testCheck.score = Math.round(
          (testCheck.results.passed / testCheck.results.total) * 100,
        );
        testCheck.status =
          testCheck.score >= 80
            ? "pass"
