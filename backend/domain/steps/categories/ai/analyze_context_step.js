...
class AnalyzeContextStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const contextWindow =
        options.contextWindow || config.settings.contextWindow;
      const includeCodebase =
        options.includeCodebase || config.settings.includeCodebase;
      const analysisDepth =
        options.analysisDepth || config.settings.analysisDepth;
      ...
      if (includeCodebase) {
        result.analysis.codebase = await this.analyzeCodebaseContext(
          projectPath,
          analysisDepth,
        );
      }
      ...
      result.analysis.requirements =
        await this.analyzeRequirementsContext(projectPath);

      // Generate insights
      result.analysis.insights = await this.generateInsights(result.analysis);

      this.logger.info(
        `✅ Context analysis completed. Generated ${result.analysis.insights.length} insights.`,
      );
  ...
  async analyzeProjectContext(projectPath) {
    ...
    try {
      ...
      context.complexity = this.determineComplexity(
        context.size,
        context.technology.length,
      );
    } catch (error) {
      this.logger.warn(
        `Warning: Could not analyze project context: ${error.message}`,
      );
    }
  ...
  async analyzeCodebaseContext(projectPath, depth) {
    ...
    try {
      ...
      this.logger.warn(
        `Warning: Could not analyze codebase context: ${error.message}`,
      );
    }
  ...
  async findCodeFiles(projectPath, depth) {
    ...
    try {
      ...
      this.logger.warn(
        `Warning: Could not scan for code files: ${error.message}`,
      );
    }
  ...
  analyzeStructurePatterns(codeFiles) {
    ...
    const hasModels = codeFiles.some((f) =>
      f.name.toLowerCase().includes("model"),
    );
    const hasViews = codeFiles.some((f) =>
      f.name.toLowerCase().includes("view"),
    );
    const hasControllers = codeFiles.some((f) =>
    ...
    const hasServices = codeFiles.some((f) =>
      f.name.toLowerCase().includes("service"),
    );
    const hasRepositories = codeFiles.some((f) =>
  ...
  analyzeNamingPatterns(codeFiles) {
    ...
    // Check for camelCase
    const camelCaseFiles = codeFiles.filter((f) =>
    ...
    const kebabCaseFiles = codeFiles.filter((f) =>
      /^[a-z][a-z0-9-]*\./.test(f.name),
    );
  ...
  determineArchitecture(codeFiles) {
    ...
    if (dirArray.some((d) => d.includes("microservices")))
  ...
  async analyzeRequirementsContext(projectPath) {
    ...
    try {
      ...
      requirements.devDependencies = Object.keys(
      ...
      const totalDeps =
        requirements.dependencies.length + requirements.devDependencies.length;
      ...
      this.logger.warn(
        `Warning: Could not analyze requirements: ${error.message}`,
      );
    }
  ...
  async generateInsights(analysis) {
    ...
    if (analysis.codebase.quality === "needs-improvement") {
      insights.push({
        ...
        confidence: "medium",
        recommendation:
          "Consider refactoring large files and improving code organization",
      });
    }

    // Architecture insights
    if (
      analysis.codebase.architecture === "monolithic" &&
      analysis.codebase.files.length > 100
    ) {
      insights.push({
        type: "architecture",
        ...
        confidence: "high",
        recommendation:
          "Consider modularizing the codebase or migrating to microservices",
      });
    }
    ...
    if (analysis.requirements.complexity === "complex") {
      insights.push({
        ...
        confidence: "medium",
        recommendation:
          "Review and potentially reduce dependencies to improve maintainability",
      });
    }
  ...
  async calculateProjectSize(projectPath) {
    ...
    try {
      ...
      this.logger.warn(
        `Warning: Could not calculate project size: ${error.message}`,
      );
    }
