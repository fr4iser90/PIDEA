...
class ParallelExecutionEngine {
  constructor(options = {}) {
    this.logger =
      options.logger || new ServiceLogger("ParallelExecutionEngine");
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => {
          reject(
            new Error(
              `Step "${stepName}" execution timeout after ${this.timeout}ms`,
            ),
          );
        }, this.timeout);
      });
      ...
      const executionPromise = this.executeStepWithRetry(
        stepName,
        context,
        options,
      );
  ...
  async executeStepWithRetry(stepName, context, options) {
    ...
    for (let attempt = 1; attempt <= this.retryAttempts + 1; attempt++) {
      try {
        ...
        const stepRegistry =
          this.stepRegistry || new (require("../StepRegistry"))();
        ...
        if (attempt <= this.retryAttempts) {
          this.logger.warn(
            `Step "${stepName}" attempt ${attempt} failed, retrying...`,
            error.message,
          );
    ...
    throw (
      lastError ||
      new Error(
        `Step "${stepName}" execution failed after ${this.retryAttempts + 1} attempts`,
      )
    );
  }
  ...
  getStatistics() {
    return {
      ...
      timeoutRate:
        this.stats.totalExecutions > 0
          ? (
              (this.stats.timeoutExecutions / this.stats.totalExecutions) *
              100
            ).toFixed(2) + "%"
          : "0%",
    };
