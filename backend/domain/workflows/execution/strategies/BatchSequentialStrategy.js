    ...
    try {
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        if (!batchResult.success) {
          this.logger.warn(
            "BatchSequentialStrategy: Batch failed, stopping execution",
            {
              batchIndex: i + 1,
      ...
      const executionResult = {
        ...
        parallelBatches: this.enableParallelBatches
          ? this.maxParallelBatches
          : 1,
      };
  ...
  async executeBatch(batch, context, executionContext, batchIndex) {
    ...
    try {
      ...
      if (canExecuteParallel && this.enableParallelBatches) {
        results = await this.executeBatchParallel(
          batch,
          context,
          executionContext,
        );
      } else {
        results = await this.executeBatchSequential(
          batch,
          context,
          executionContext,
        );
