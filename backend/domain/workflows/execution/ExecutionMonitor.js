...
class ExecutionMonitor extends EventEmitter {
   ...
   */
  calculateErrorRate() {
    const recentExecutions = Array.from(this.executionHistory.values()).filter(
      (execution) => Date.now() - execution.metrics.endTime < 3600000, // Last hour
    );
    ...
    const failedExecutions = recentExecutions.filter(
      (execution) => !execution.metrics.success,
    );
