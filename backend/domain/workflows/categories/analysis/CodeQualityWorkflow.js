...
class CodeQualityWorkflow extends BaseWorkflowStep {
  ...
  async initialize(context) {
    try {
      ...
      logger.error(
        "❌ Failed to initialize Code Quality Workflow:",
        error.message,
      );
      throw error;
    }
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      if (results.summary.qualityScore === 0) {
        results.summary.qualityScore = this.calculateOverallQualityScore(
          results.steps,
        );
      }

      logger.info(
        `✅ Code Quality Workflow completed. Quality Score: ${results.summary.qualityScore}/100`,
      );
      return results;
    } catch (error) {
  ...
  async runCodeQualityFramework(context, options) {
    try {
      ...
      const framework = this.frameworkRegistry.getFramework(
  ...
  calculateOverallQualityScore(steps) {
    const completedSteps = steps.filter(
      (step) => step.status === "completed" && step.qualityScore,
    );
    ...
    const totalScore = completedSteps.reduce(
      (sum, step) => sum + step.qualityScore,
      0,
    );
