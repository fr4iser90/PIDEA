...
class ConfidenceCalculator {
  ...
  async calculate(task, context) {
    ...
    scores.historicalSuccess = await this.calculateHistoricalSuccess(
      task,
      context,
    );
  ...
  async calculateHistoricalSuccess(task, context) {
    ...
    try {
      ...
      const historicalTasks = await taskRepository.findByType(
        task.type?.value || "",
        {
          limit: 50,
          status: "completed",
      ...
      const successfulTasks = historicalTasks.filter(
        (t) => t.metadata?.result?.success === true,
      );
  ...
  async calculateUserExperience(task, context) {
    ...
    try {
      ...
      const automationPreference =
        user.metadata?.automationPreference || "balanced";

      return this.calculateUserExperienceScore(
        experienceLevel,
  ...
  normalizeQualityMetrics(metrics) {
    ...
    return (
      Object.values(normalizedMetrics).reduce((sum, score) => sum + score, 0) /
      Object.keys(normalizedMetrics).length
    );
  }
