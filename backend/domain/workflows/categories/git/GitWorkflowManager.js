...
class GitWorkflowManager {
  constructor(dependencies = {}) {
    ...
    this.branchStrategy =
      dependencies.branchStrategy || new BranchStrategy(dependencies);
    this.mergeStrategy =
      dependencies.mergeStrategy || new MergeStrategy(dependencies);
  ...
  async executeWorkflow(task, context = {}) {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        throw GitWorkflowException.validationFailed(
      ...
      const reviewResult = await this.performAutoReview(
        task,
        gitContext,
        prResult,
      );
      ...
      const mergeResult = await this.mergeChanges(
        task,
        gitContext,
        reviewResult,
      );
      ...
      this.logger.info(
        "GitWorkflowManager: Git workflow completed successfully",
        {
          taskId: task.id,
          duration,
  ...
  async createBranch(task, context) {
    try {
      ...
      context.setBranchInfo(
        branchName,
        context.get("baseBranch") || "pidea-agent",
        {
          strategy: strategy.constructor.name,
  ...
  async executeWorkflowSteps(task, context) {
    try {
      ...
      if (!workflow) {
        this.logger.warn(
          "GitWorkflowManager: No workflow found in context, using default execution",
        );
      ...
      throw GitWorkflowException.workflowExecutionFailed(
        "workflow_execution",
        error.message,
        {
          taskId: task.id,
  ...
  async createPullRequest(task, context, workflowResult) {
    ...
    if (automationLevel === "full_auto") {
      this.logger.info(
        "GitWorkflowManager: Skipping PR creation for full auto mode",
        {
          taskId: task.id,
        },
      );
    ...
    if (!this.pullRequestManager) {
      this.logger.warn(
        "GitWorkflowManager: No pull request manager available, skipping PR creation",
      );
      return {
        skipped: true,
    ...
    try {
      ...
      context.setPullRequestInfo(result.id, prData.title, result.url, {
  ...
  async performAutoReview(task, context, prResult) {
    ...
    if (!this.autoReviewService) {
      this.logger.warn(
        "GitWorkflowManager: No auto review service available, skipping review",
      );
      return {
        skipped: true,
  ...
  async mergeChanges(task, context, reviewResult) {
    ...
    try {
      ...
      const mergeConfig = this.mergeStrategy.getMergeConfiguration(
        task,
        context,
      );
      ...
      if (!this.mergeStrategy.canAutomateMerge(mergeConfig, context)) {
        this.logger.info(
          "GitWorkflowManager: Merge cannot be automated, manual intervention required",
          {
            taskId: task.id,
...
${task.description || ""}`;
  ...
  generatePRLabels(task) {
    const labels = [`type-${task.type?.value || "task"}`, "automated"];
