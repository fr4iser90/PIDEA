...
class GitWorkflowValidator {
  ...
  validateTask(task) {
    ...
    if (
      task.type?.value &&
      !this.validationRules.taskType.allowedValues.includes(task.type.value)
    ) {
    ...
    if (task.metadata) {
      ...
      if (
        ...
        !this.validationRules.automationLevel.allowedValues.includes(
          task.metadata.automationLevel,
        )
      ) {
        warnings.push(
          `Unknown automation level: ${task.metadata.automationLevel}`,
        );
      }
    }
    ...
    try {
      ...
      if (projectPath) {
        ...
        if (!Array.isArray(branches)) {
          warnings.push(
            "Git service getBranches method should return an array",
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          warnings.push(
            `Failed to cleanup test branch: ${cleanupError.message}`,
          );
        }
      } catch (error) {
        errors.push(
          `Insufficient permissions to create branches: ${error.message}`,
        );
      }
    } catch (error) {
