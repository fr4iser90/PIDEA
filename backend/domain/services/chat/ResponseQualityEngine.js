...
class ResponseQualityEngine {
  ...
  assessResponse(response, context = {}) {
    try {
      ...
      this.logger.info(
        `Response quality assessment completed: ${Math.round(overallScore * 100)}%`,
      );
      return result;
    } catch (error) {
  ...
  assessCompleteness(response, context) {
    ...
    const hasCompletionKeyword = this.completionIndicators.positive.some(
    ...
    const hasPartialKeyword = this.completionIndicators.negative.some(
      (keyword) => response.toLowerCase().includes(keyword),
    );
  ...
  assessRelevance(response, context) {
    ...
    const technicalTerms = this.relevancePatterns.technicalTerms.filter(
    ...
    if (context.previousMessages && context.previousMessages.length > 0) {
      ...
      const contextWords = lastKeywords.filter((word) =>
  ...
  assessCodeQuality(response, context) {
    ...
    for (const block of codeBlocks) {
      ...
      const patterns =
        this.codeQualityPatterns[language] ||
        this.codeQualityPatterns.javascript;

      // Count good patterns
      const goodPatterns = patterns.good.filter((pattern) =>
        pattern.test(block.content),
      );
      const badPatterns = patterns.bad.filter((pattern) =>
        pattern.test(block.content),
      );
      ...
      if (
        block.content.includes("try") ||
        block.content.includes("catch") ||
        block.content.includes("except")
      ) {
        score += 0.2;
  ...
  calculateOverallScore(assessments) {
    const { completeness, relevance, codeQuality, errorDetection } =
      assessments;
  ...
  detectLanguage(text) {
    const languagePatterns = {
      javascript:
        /(const|let|var|function|=>|console\.|\.then\(|\.catch\(|async|await)/i,
