...
class DOMCodeBlockDetector {
  ...
  async detectCodeBlocks(page) {
    try {
      this.logger.info(
        "🔍 [DOMCodeBlockDetector] Starting code block detection",
      );
      ...
      return await page.evaluate((selectors) => {
        ...
        const detectLanguageFromElement = (
          languageElement,
          filenameElement,
        ) => {
          if (languageElement) {
            const classList = languageElement.className;
            if (classList.includes("javascript-lang-file-icon"))
              return "javascript";
            if (classList.includes("typescript-lang-file-icon"))
              return "typescript";
        ...
        const calculateConfidence = (container, selectors) => {
          ...
          if (container.querySelector(selectors.chatSelectors.monacoEditor))
            confidence += 0.3;

          // Check for syntax highlighting (medium confidence)
          if (container.querySelector(selectors.chatSelectors.syntaxTokens))
            confidence += 0.2;

          // Check for filename (low confidence)
          if (
            container.querySelector(selectors.chatSelectors.codeBlockFilename)
          )
            confidence += 0.1;

          // Check for language indicator (medium confidence)
          if (
            container.querySelector(selectors.chatSelectors.codeBlockLanguage)
          )
            confidence += 0.15;
        ...
        const validateSyntax = (codeText, language) => {
          ...
          const patterns = {
            javascript: {
              valid: /^[\s\S]*$/,
              hasErrors:
                /(SyntaxError|ReferenceError|TypeError|console\.error)/i,
        ...
        containers.forEach((container, index) => {
          const content = container.querySelector(
            selectors.chatSelectors.codeBlockContent,
          );
          const header = container.querySelector(
            selectors.chatSelectors.codeBlockHeader,
          );
          const filename = container.querySelector(
            selectors.chatSelectors.codeBlockFilename,
          );
          const language = container.querySelector(
            selectors.chatSelectors.codeBlockLanguage,
          );
          const editor = container.querySelector(
            selectors.chatSelectors.monacoEditor,
          );
          ...
          if (content && editor) {
            ...
            let codeLines = editor.querySelectorAll(
              selectors.chatSelectors.codeLines,
            );
            ...
            const detectedLanguage = detectLanguageFromElement(
              language,
              filename,
            );
            ...
            codeBlocks.push({
              ...
              hasApplyButton:
                !!container.querySelector(
                  selectors.chatSelectors.codeBlockApplyButton,
                ) &&
                container
                  .querySelector(selectors.chatSelectors.codeBlockApplyButton)
      ...
      // Log the results
      this.logger.info(
        `🔍 [DOMCodeBlockDetector] Found ${codeBlocks.length} code blocks`,
      );

      return codeBlocks;
    } catch (error) {
      this.logger.error(
        "❌ [DOMCodeBlockDetector] Error detecting code blocks:",
        error.message,
      );
      return [];
    }
  ...
  async extractCodeBlocksFromContainer(page, containerSelector) {
    try {
      return await page.evaluate(
        (selectors, containerSelector) => {
          ...
          editors.forEach((editor, index) => {
            const codeLines = editor.querySelectorAll(
              selectors.chatSelectors.codeLines,
            );
      ...
      this.logger.error(
        "❌ [DOMCodeBlockDetector] Error extracting from container:",
        error.message,
      );
      return [];
    }
