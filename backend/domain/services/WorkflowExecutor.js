  ...
  async run(taskId, options = {}) {
    try {
      ...
      if (
        ...
        this.logger.info(
          "WorkflowExecutor: Task creation workflow detected, loading real task from database",
        );
        task = await this.loadTask(taskId);
        if (!task) {
          throw new Error(`Task ${taskId} not found`);
        }
        this.logger.info("WorkflowExecutor: Real task loaded successfully", {
          taskId,
          taskTitle: task.title,
        });
      } else {
        // Load task from database for execution workflows
        this.logger.info("WorkflowExecutor: Loading task from database", {
          taskId,
        });
        task = await this.loadTask(taskId);
        if (!task) {
        ...
        this.logger.info("WorkflowExecutor: Task loaded successfully", {
          taskId,
          taskTitle: task.title,
        });
      }
      ...
      this.logger.info("WorkflowExecutor: Determined workflow name", {
        workflowName,
        taskMode: options.taskMode,
      });

      // Load workflow definition
      this.logger.info("WorkflowExecutor: Loading workflow definition", {
        workflowName,
      });
      ...
      this.logger.info("WorkflowExecutor: Workflow loaded successfully", {
        workflowName,
      ...
      this.logger.info(
        "WorkflowExecutor: Passing context to WorkflowExecutionService",
        {
          taskId: taskId,
        ...
        try {
          ...
          if (step.enabled === false) {
            ...
            results.push({
              ...
              step: step,
              result: { skipped: true, reason: "Step disabled" },
            });
            continue;
          }
          ...
          if (
            step.type === "ide_send_message_step" &&
            step.options?.useTaskPrompt &&
            context.task
          ) {
            if (this.taskService) {
              ...
              stepMessage = await this.taskService.buildTaskPromptForStep(
                context.task,
                {
              ...
              this.logger.warn(
                "WorkflowExecutor: TaskService not available for prompt building",
              );
            }
          }
          ...
          const stepSucceeded =
            stepResult && stepResult.success !== false && !stepResult.error;
          ...
          if (stepSucceeded) {
            ...
            this.logger.error(
              "WorkflowExecutor: Step FAILED - STOPPING WORKFLOW",
              {
                stepName: step.step,
                stepType: step.type,
            ...
            throw new Error(
              `Workflow stopped due to step failure: ${step.step} - ${stepResult?.error || "Unknown error"}`,
            );
          }
        } catch (error) {
          this.logger.error(
            "WorkflowExecutor: Step execution FAILED - STOPPING WORKFLOW",
            {
              stepName: step.step,
              stepType: step.type,
              error: error.message,
          ...
          throw new Error(
            `Workflow stopped due to step exception: ${step.step} - ${error.message}`,
          );
        }
      }
