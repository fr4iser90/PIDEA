...
class BrowserEnvironmentService {
  constructor() {
    ...
    this.playwrightCacheDir = path.join(
      os.homedir(),
      ".cache",
      "ms-playwright",
    );
    ...
    try {
      ...
      for (const browser of playwrightBrowsers) {
        ...
        if (browser.name === "chromium_headless_shell") {
          const headlessShellDirs = entries.filter((entry) =>
          ...
          const browserVersionDirs = entries.filter((entry) =>
            entry.startsWith(browser.name + "-"),
          );
  ...
  async detectSystemBrowsers() {
    ...
    for (const browser of systemBrowserCommands) {
      try {
        ...
        logger.info(
          `✅ Found system browser: ${browser.name} at ${browser.command}`,
        );
      } catch (error) {
        // Don't add unavailable browsers to the list
        logger.debug(
          `❌ System browser ${browser.name} not available: ${browser.command}`,
        );
      }
    }

    logger.info(
      `System browser detection completed: ${browsers.length} browsers found`,
    );
    return browsers;
  }
