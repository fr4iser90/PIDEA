...
class TerminalService {
  ...
  async executeCommandWithInput(command, input, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      childProcess.on("close", (code) => {
        ...
        if (success) {
          logger.info(
            `✅ Command with input completed successfully: ${command}`,
          );
        } else {
          logger.warn(
            `⚠️ Command with input completed with code ${code}: ${command}`,
          );
        }
  ...
  async executeCommandInteractive(command, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      logger.info(
        `🔄 Interactive command started with PID: ${childProcess.pid}`,
      );
  ...
  killAllProcesses() {
    logger.info(
      `🛑 Killing all active processes: ${this.activeProcesses.size}`,
    );
