...
class TerminalMonitor {
  ...
  async monitorTerminalOutput() {
    try {
      ...
      const terminalExists = await page.evaluate(() => {
        ...
        const canvasElements = xtermScreen
          ? xtermScreen.querySelectorAll("canvas")
          : [];
        const hasTerminalContent =
          canvasElements.length > 0 &&
          Array.from(canvasElements).some(
      ...
      const statusChanged =
        JSON.stringify(terminalExists) !==
        JSON.stringify(this.lastTerminalStatus);
      if (statusChanged) {
      ...
      const terminalOutput = await page.evaluate(() => {
        ...
        if (!terminalText.trim()) {
          const terminalTabs = document.querySelectorAll(
            ".terminal-tab, .tab-content",
          );
          terminalTabs.forEach((tab) => {
            const tabText = tab.innerText || tab.textContent || "";
            if (
              tabText.trim() &&
              !tabText.includes(".xterm-") &&
              tabText.length > 10
            ) {
        ...
        if (!terminalText.trim()) {
          const terminalArea = document.querySelector(
            ".terminal, .xterm, .xterm-screen",
          );
          if (terminalArea) {
            const areaText =
              terminalArea.innerText || terminalArea.textContent || "";
            if (
              areaText.trim() &&
              !areaText.includes(".xterm-") &&
              areaText.length > 10
            ) {
  ...
  async startTerminalMonitoring() {
    ...
    */
    ...
    logger.info(
      "Terminal monitoring setup complete (no continuous monitoring to avoid filesystem errors)",
    );
  }
