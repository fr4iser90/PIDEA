...
class TaskStatusValidator {
  ...
  async validateTaskStatusConsistency(taskId, options = {}) {
    try {
      this.logger.info("Starting task status consistency validation", {
        taskId,
      });
      ...
      if (task.filePath) {
        try {
          fileContent = await this.fileSystemService.readFile(
            task.filePath,
            "utf8",
          );
          fileStatus =
            await this.contentHashService.extractStatusFromContent(fileContent);
          contentHash =
            await this.contentHashService.generateContentHash(fileContent);
        } catch (error) {
   ...
   * @returns {Promise<Object>} Validation result
   */
  async performConsistencyValidation(
    task,
    fileContent,
    fileStatus,
    contentHash,
    ...
    if (fileContent && fileStatus && task.status) {
      ...
      if (fileStatus !== dbStatus) {
        issues.push(
          `Status mismatch: file shows '${fileStatus}', database shows '${dbStatus}'`,
        );
    ...
    if (fileContent && task.contentHash) {
      const isHashValid = await this.contentHashService.validateContentHash(
        fileContent,
        task.contentHash,
      );
      if (!isHashValid) {
    ...
    if (task.lastSyncedAt) {
      ...
      if (hoursSinceSync > 24) {
        issues.push(
          `Stale sync: last synced ${Math.round(hoursSinceSync)} hours ago`,
        );
  ...
  async validateBatchTaskStatusConsistency(taskIds, options = {}) {
    try {
      ...
      for (const taskId of taskIds) {
        try {
          const result = await this.validateTaskStatusConsistency(
            taskId,
            options,
          );
          results.push({
      ...
      this.logger.info(
        "Batch task status consistency validation completed",
        summary,
      );
  ...
  async getValidationStatistics(options = {}) {
    try {
      ...
      stats.contentHashCoverage =
        (stats.withContentHashes / stats.totalTasks) * 100;
  ...
  async autoFixConsistencyIssues(taskId, options = {}) {
    try {
      this.logger.info("Starting auto-fix for task consistency issues", {
        taskId,
      });

      const validationResult = await this.validateTaskStatusConsistency(
        taskId,
        options,
      );
      ...
      if (task.filePath && !task.contentHash) {
        try {
          const fileContent = await this.fileSystemService.readFile(
            task.filePath,
            "utf8",
          );
          const contentHash =
      ...
      if (task.filePath && validationResult.details.fileStatus) {
        try {
          ...
          if (fileStatus !== (task.status?.value || task.status)) {
            ...
            fixesApplied.push(
              `Updated status from '${task.status?.value || task.status}' to '${fileStatus}'`,
            );
          }
        } catch (error) {
