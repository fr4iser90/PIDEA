...
class TaskValidationService {
  ...
  async validateTaskExecution(taskId, options = {}) {
    ...
    try {
      ...
      validationResult.errors.push(
        `Execution validation error: ${error.message}`,
      );
      return validationResult;
    }
  }
  ...
  async validateTaskDependencies(dependencyIds) {
    ...
    try {
      ...
      await this.validateDependencyCompatibility(
        dependencyIds,
        validationResult,
      );
      ...
      validationResult.isValid = false;
      validationResult.errors.push(
        `Dependency validation error: ${error.message}`,
      );
      return validationResult;
    }
  }
  ...
  async validateProjectConfiguration(projectPath, options = {}) {
    ...
    try {
      ...
      validationResult.isValid = false;
      validationResult.errors.push(
        `Project validation error: ${error.message}`,
      );
      return validationResult;
    }
  }
  ...
  validateFieldFormats(params, validationResult) {
    ...
    if (
      ...
      (params.estimatedTime < 1 || params.estimatedTime > 1440)
    ) {
      validationResult.errors.push(
    ...
    if (
      params.priority &&
      !Object.values(TaskPriority).includes(params.priority)
    ) {
  ...
  async validateBusinessRules(params, validationResult) {
    ...
    const existingTasks = await this.taskRepository.findByProjectPath(
      params.projectPath,
    );
    ...
    if (duplicateTask) {
      validationResult.warnings.push(
        "Similar task already exists in this project",
      );
    }
    ...
    if (
      ...
      params.estimatedTime > 480
    ) {
      validationResult.warnings.push(
        "Critical tasks should typically be completed within 8 hours",
      );
    }
  }
  ...
  async validateProjectContext(params, validationResult) {
    ...
    const projectExists = await this.fileSystemService.exists(
      params.projectPath,
    );
    if (!projectExists) {
      validationResult.errors.push(
    ...
    if (!projectStructure.isValid) {
      validationResult.warnings.push(
        "Project structure may not be optimal for this task type",
      );
    }

    // Check project permissions
    const hasWritePermission = await this.fileSystemService.hasWritePermission(
      params.projectPath,
    );
  ...
  generateSuggestions(params, validationResult) {
    ...
    if (!params.estimatedTime) {
      validationResult.suggestions.push(
        "Consider adding estimated time for better planning",
      );
    }
  ...
  async validateExecutionPrerequisites(task, validationResult) {
    ...
    if (task.dependencies && task.dependencies.length > 0) {
      for (const depId of task.dependencies) {
        ...
        if (depTask && depTask.status.value !== TaskStatus.COMPLETED) {
          validationResult.errors.push(
            `Dependency task not completed: ${depTask.title}`,
          );
        }
      }
    }
    ...
    if (!resourceCheck.available) {
      validationResult.errors.push(
        `Required resources not available: ${resourceCheck.missing.join(", ")}`,
      );
    }
  }
  ...
  async validateResourceRequirements(task, options, validationResult) {
    ...
    if (memoryRequired > availableMemory) {
      validationResult.errors.push(
        `Insufficient memory: required ${memoryRequired}MB, available ${availableMemory}MB`,
      );
    }
    ...
    if (diskRequired > availableDisk) {
      validationResult.errors.push(
        `Insufficient disk space: required ${diskRequired}MB, available ${availableDisk}MB`,
      );
    }
    ...
    if (cpuRequired > availableCPU) {
      validationResult.warnings.push(
        `High CPU usage expected: ${cpuRequired}% of available ${availableCPU}%`,
      );
    }
  }
  ...
  async validateExecutionDependencies(task, validationResult) {
    ...
    if (conflictingTasks.length > 0) {
      validationResult.warnings.push(
        `Conflicting tasks detected: ${conflictingTasks.map((t) => t.title).join(", ")}`,
      );
    }
    ...
    if (blockingTasks.length > 0) {
      validationResult.errors.push(
        `Blocking tasks detected: ${blockingTasks.map((t) => t.title).join(", ")}`,
      );
    }
  }
  ...
  async validateProjectState(task, validationResult) {
    ...
    if (projectState.hasMergeConflicts) {
      validationResult.errors.push(
        "Project has merge conflicts that must be resolved",
      );
    }
  ...
  validateExecutionOptions(options, validationResult) {
    ...
    if (
      options.timeout &&
      (options.timeout < 1000 || options.timeout > 3600000)
    ) {
      validationResult.errors.push(
    ...
    if (
      options.retryCount &&
      (options.retryCount < 0 || options.retryCount > 10)
    ) {
  ...
  validateUpdatePermissions(task, updates, validationResult) {
    ...
    if (task.status.value === TaskStatus.COMPLETED) {
      validationResult.warnings.push(
        "Updating completed task may affect historical data",
      );
    }
  ...
  async validateUpdateConstraints(task, updates, validationResult) {
    ...
    if (updates.status) {
      ...
      if (!validTransitions.includes(updates.status)) {
        validationResult.errors.push(
          `Invalid status transition from ${task.status.value} to ${updates.status}`,
        );
      }
    }
    ...
    if (updates.priority && updates.priority !== task.priority.value) {
      const dependencyImpact = await this.analyzePriorityChangeImpact(
        task,
        updates.priority,
      );
      if (dependencyImpact.hasConflicts) {
        validationResult.warnings.push(
          "Priority change may affect dependent tasks",
        );
      }
    }
  }
  ...
  async validateUpdateImpact(task, updates, validationResult) {
    ...
    for (const depTask of dependentTasks) {
      ...
      if (impact.hasIssues) {
        validationResult.warnings.push(
          `Update may affect dependent task: ${depTask.title}`,
        );
      }
    }
  }
  ...
  async validateDependencyStates(dependencyIds, validationResult) {
    for (const depId of dependencyIds) {
      ...
      if (depTask && depTask.status.value === TaskStatus.CANCELLED) {
        validationResult.warnings.push(
          `Dependency task is cancelled: ${depTask.title}`,
        );
      }
    }
  }
  ...
  async validateDependencyCompatibility(dependencyIds, validationResult) {
    ...
    if (uniqueTypes.size !== taskTypes.length) {
      validationResult.warnings.push(
        "Multiple dependencies of the same type may cause conflicts",
      );
    }
  }
  ...
  async detectCircularDependencies(dependencyIds) {
    return [];
  }
  ...
  calculateMemoryRequirement(task, options) {
    return 512;
  }
  async getAvailableMemory() {
    return 8192;
  }
  calculateDiskRequirement(task, options) {
    return 100;
  }
  async getAvailableDiskSpace(projectPath) {
    return 10000;
  }
  calculateCPURequirement(task, options) {
    return 50;
  }
  async getAvailableCPU() {
    return 100;
  }
  async findConflictingTasks(task) {
    return [];
  }
  async findBlockingTasks(task) {
    return [];
  }
  async analyzeProjectState(projectPath) {
    return {
  ...
  async analyzePriorityChangeImpact(task, newPriority) {
    return { hasConflicts: false };
  }
  async findDependentTasks(taskId) {
    return [];
  }
  analyzeUpdateImpact(task, updates, depTask) {
