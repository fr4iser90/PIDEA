...
class TaskSecurityService {
  ...
  async performComprehensiveScan(task, options) {
    ...
    results.securityScore = this.calculateSecurityScore(
      results.vulnerabilities,
    );
  ...
  async performVulnerabilityScan(task, options) {
    ...
    const aiVulnerabilities = await this.generateAIVulnerabilityAnalysis(
      task,
      options,
    );
  ...
  async performCodeSecurityScan(task, options) {
    ...
    const aiCodeVulnerabilities = await this.generateAICodeAnalysis(
      task,
      options,
    );
    ...
    // Generate recommendations
    recommendations.push(
      ...this.generateCodeSecurityRecommendations(vulnerabilities),
    );
  ...
  async performDependencyScan(task, options) {
    ...
    if (task.projectPath) {
      ...
      if (await this.fileSystemService.exists(packageJsonPath)) {
        const packageJson = await this.fileSystemService.readFile(
          packageJsonPath,
          "utf8",
        );
        const dependencies = JSON.parse(packageJson);

        const depVulnerabilities = await this.scanDependencies(
          dependencies,
          options,
        );
    ...
    const aiDepVulnerabilities = await this.generateAIDependencyAnalysis(
      task,
      options,
    );
    ...
    // Generate recommendations
    recommendations.push(
      ...this.generateDependencyRecommendations(vulnerabilities),
    );
  ...
  async performConfigurationScan(task, options) {
    ...
    const aiConfigVulnerabilities = await this.generateAIConfigurationAnalysis(
      task,
      options,
    );
    ...
    // Generate recommendations
    recommendations.push(
      ...this.generateConfigurationRecommendations(vulnerabilities),
    );
  ...
  async performCustomScan(task, scanType, options) {
    ...
    const aiPrompt = this.buildCustomSecurityScanPrompt(
      task,
      scanType,
      options,
    );
    ...
    const recommendations =
      this.generateCustomSecurityRecommendations(vulnerabilities);
