...
class TaskStatusTransitionService {
  ...
  async moveTaskToInProgress(taskId) {
    try {
      ...
      const newPath = this.fileLocationService.getTaskFilePathForStatus(
        task,
        "in_progress",
      );
      ...
      if (oldPath) {
        ...
        const moveResult = await this.fileLocationService.moveTaskFiles(
          oldPath,
          newPath,
        );
        ...
        this.logger.warn(
          "⚠️ No existing task files found, skipping file movement",
          { taskId },
        );
      }
    ...
    try {
      ...
      if (task.status.value === "completed") {
        ...
        const expectedPath = path.join(
          projectRoot,
          `docs/09_roadmap/completed/${quarter}/${task.category}/${this.normalizeTaskName(task.title || "unknown-task")}/`,
        );

        try {
          await fs.access(expectedPath);
          this.logger.info(
            "✅ Task already completed and files in correct location",
            { taskId, expectedPath },
          );
          return {
           
            message: "Task already completed and synchronized",
          };
        } catch (error) {
          this.logger.info(
            "🔄 Task completed but files not in correct location, fixing...",
            { taskId, expectedPath },
          );
      ...
      const taskCategory =
        task.metadata?.category || task.category || "unknown";
      // ✅ FIXED: Use correct project root (parent directory of backend)
      const projectRoot = path.resolve(process.cwd(), "..");
      const newPath = path.join(
        projectRoot,
        `docs/09_roadmap/completed/${quarter}/${taskCategory}/${taskName}/`,
      );
      ...
      const possibleOldPaths = [
        path.join(
          projectRoot,
          `docs/09_roadmap/in-progress/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/pending/${priorityDir}/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/pending/medium/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/pending/high/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/pending/low/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/pending/critical/${taskCategory}/${taskName}/`,
        ),
        path.join(
          projectRoot,
          `docs/09_roadmap/planning/${priorityDir}/${taskCategory}/${taskName}/`,
        ),
      ];
      ...
      for (const testPath of possibleOldPaths) {
        try {
          ...
          if (actualFiles.length > 0) {
            ...
            this.logger.debug(`📁 Directory exists but is empty: ${testPath}`, {
              taskId,
            });
          }
        } catch (error) {
          ...
          try {
            ...
            if (actualFiles.length === 0) {
              try {
                ...
                this.logger.debug("🗑️ Cleaned up empty directory", {
                  path: currentPath,
                });
              } catch (rmError) {
  ...
  async updateFileReferences(taskId, oldPath, newPath) {
    try {
      ...
      if (task) {
        ...
        this.logger.warn("⚠️ Task not found for file reference update", {
          taskId,
        });
      }
    } catch (error) {
  ...
  async moveTaskToBlocked(taskId, reason = "Blocked by external dependency") {
    try {
      ...
      const oldPath = path.join(
        projectRoot,
        `docs/09_roadmap/in-progress/${task.category}/${taskName}/`,
      );
      const newPath = path.join(
        projectRoot,
        `docs/09_roadmap/blocked/${priorityDir}/${task.category}/${taskName}/`,
      );
  ...
  async moveTaskToCancelled(taskId, reason = "Task cancelled") {
    try {
      ...
      const newPath = path.join(
        projectRoot,
        `docs/09_roadmap/cancelled/${priorityDir}/${task.category}/${taskName}/`,
      );
