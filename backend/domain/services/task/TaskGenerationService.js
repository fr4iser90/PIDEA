...
class TaskGenerationService {
  ...
  async generateTasks(params) {
    ...
    try {
      ...
      switch (mode) {
        case "human":
          tasks = await this.generateHumanTasks(
            projectAnalysis,
            context,
            options,
          );
        ...
        case "collaborative":
        default:
          tasks = await this.generateCollaborativeTasks(
            projectAnalysis,
            context,
            options,
          );
  ...
  async getGenerationStatistics(projectPath, startDate, endDate) {
    ...
    for (const task of filteredTasks) {
      ...
      // Count by priority
      stats.byPriority[task.priority.value] =
        (stats.byPriority[task.priority.value] || 0) + 1;

      // Count by status
      stats.byStatus[task.status.value] =
        (stats.byStatus[task.status.value] || 0) + 1;
    ...
    stats.averageEstimatedTime =
      filteredTasks.length > 0 ? totalTime / filteredTasks.length : 0;
