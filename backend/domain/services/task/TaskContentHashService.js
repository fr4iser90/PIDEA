...
class TaskContentHashService {
  ...
  async generateFileHash(filePath, options = {}) {
    try {
      ...
      if (cachedHash) {
        this.logger.debug("Using cached file hash", {
          filePath,
          hash: cachedHash,
        });
        return cachedHash;
  ...
  async extractStatusFromContent(content) {
    try {
      ...
      if (progressMatches.length > 0) {
        const completedCount = progressMatches.filter(
          (match) => match[1] === "x",
        ).length;
        const totalCount = progressMatches.length;

        if (completedCount === totalCount && totalCount > 0) {
          this.logger.debug("Status inferred from progress: completed", {
            completedCount,
            totalCount,
          });
          return "completed";
        } else if (completedCount > 0) {
          this.logger.debug("Status inferred from progress: in_progress", {
            completedCount,
            totalCount,
          });
          return "in_progress";
