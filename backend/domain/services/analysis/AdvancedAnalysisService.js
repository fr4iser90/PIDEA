...
class AdvancedAnalysisService {
  constructor(dependencies = {}) {
    this.logger =
      dependencies.logger || new ServiceLogger("AdvancedAnalysisService");
    this.layerValidationService =
      dependencies.layerValidationService ||
      new LayerValidationService(this.logger);
    this.logicValidationService =
      dependencies.logicValidationService ||
      new LogicValidationService(this.logger);
    this.taskAnalysisService =
      dependencies.taskAnalysisService || new TaskAnalysisService();
  }
  ...
  async performAdvancedAnalysis(projectPath, options = {}) {
    this.logger.info(
      "Starting advanced analysis with layer and logic validation for project",
    );
    ...
    try {
      ...
      analysis.standardAnalysis = await this.taskAnalysisService.analyzeProject(
        projectPath,
        options,
      );
      ...
      if (options.includeLayerValidation !== false) {
        ...
        analysis.layerValidation =
          await this.layerValidationService.validateLayers(
            projectPath,
            options,
          );
      } else {
        this.logger.info(
          "Phase 2: Skipping layer validation (disabled) for project",
        );
        analysis.layerValidation = {};
      }
      ...
      if (options.includeLogicValidation !== false) {
        ...
        analysis.logicValidation =
          await this.logicValidationService.validateLogic(projectPath, options);
      } else {
        this.logger.info(
          "Phase 3: Skipping logic validation (disabled) for project",
        );
        analysis.logicValidation = {};
      }
      ...
      analysis.integratedInsights =
        await this.generateIntegratedInsights(analysis);

      // Phase 5: Generate Comprehensive Recommendations
      this.logger.info(
        "Phase 5: Generating comprehensive recommendations for project",
      );
      analysis.recommendations =
        await this.generateComprehensiveRecommendations(analysis);
      ...
      const layerValid =
        options.includeLayerValidation !== false
          ? analysis.layerValidation.overall
          : true;
      const logicValid =
        options.includeLogicValidation !== false
          ? analysis.logicValidation.overall
          : true;
      analysis.overall =
        layerValid && logicValid && analysis.metrics.overallScore >= 70;

      this.logger.info(
        `Advanced analysis completed successfully - Overall Score: ${analysis.metrics.overallScore}, Layer Score: ${analysis.metrics.layerScore}, Logic Score: ${analysis.metrics.logicScore}, Valid: ${analysis.overall} for project`,
      );
  ...
  async generateIntegratedInsights(analysis) {
      ...
      for (const logicViolation of logicViolations) {
        if (this.areViolationsRelated(layerViolation, logicViolation)) {
          insights.push({
            ...
            suggestion:
              "Address both architectural and logical issues together for better code quality",
          });
        }
      }
    }
    ...
    const businessLogicInWrongLayer =
      this.findBusinessLogicInWrongLayer(analysis);
    if (businessLogicInWrongLayer.length > 0) {
      insights.push({
        ...
        suggestion:
          "Move business logic to appropriate domain or application layers",
      });
    }
  ...
  areViolationsRelated(layerViolation, logicViolation) {
    ...
    return (
      layerPatterns.includes(layerViolation.type) &&
  ...
  findBusinessLogicInWrongLayer(analysis) {
    ...
    if (
      layerValidation.layers.presentation &&
      layerValidation.layers.presentation.files
    ) {
      for (const file of layerValidation.layers.presentation.files) {
        const logicViolations = (logicValidation?.violations || []).filter(
          (v) => v.file === file,
        );
    ...
    if (
      layerValidation.layers.infrastructure &&
      layerValidation.layers.infrastructure.files
    ) {
  ...
  analyzeErrorHandlingPatterns(analysis) {
    ...
    if (errorHandling.metrics.tryCatchBlocks === 0) {
      insights.push({
        ...
        suggestion:
          "Implement comprehensive error handling with try-catch blocks and proper error types",
      });
    }
  ...
  analyzeSecurityPatterns(analysis) {
    ...
    if (security.metrics.securityChecks === 0) {
      insights.push({
        ...
        suggestion:
          "Implement comprehensive security measures including authentication, authorization, and input validation",
      });
    }
  ...
  analyzeDataFlowPatterns(analysis) {
    ...
    if (dataFlow.metrics.dataTransformations === 0) {
      insights.push({
        ...
        suggestion:
          "Consider implementing data transformation patterns for better data handling",
      });
    }
    ...
    if (dataFlow.metrics.dataValidations === 0) {
      insights.push({
        ...
        suggestion:
          "Implement comprehensive data validation for all data inputs and transformations",
      });
    }
  ...
  async generateComprehensiveRecommendations(analysis) {
    ...
    if (criticalViolations.length > 0) {
      recommendations.push({
        ...
        action:
          "Fix all critical violations before proceeding with development",
    ...
    const logicRecommendations =
      analysis.logicValidation?.recommendations || [];
    ...
    if (analysis.standardAnalysis?.performance) {
      const performanceIssues = this.extractPerformanceIssues(
        analysis.standardAnalysis.performance,
      );
  ...
  async calculateOverallMetrics(analysis) {
    ...
    if (analysis.standardAnalysis) {
      metrics.standardScore = this.calculateStandardScore(
        analysis.standardAnalysis,
      );
    }

    // Calculate breakdown scores
    metrics.breakdown.architecture = Math.round(
      metrics.layerScore * 0.6 + metrics.standardScore * 0.4,
    );
    metrics.breakdown.quality = Math.round(
      metrics.logicScore * 0.7 + metrics.standardScore * 0.3,
    );
  ...
  calculateStandardScore(standardAnalysis) {
    ...
    if (
      standardAnalysis.security &&
      standardAnalysis.security.vulnerabilities
    ) {
  ...
  calculateSecurityScore(analysis) {
    ...
    const securityViolations =
      analysis.logicValidation?.security?.violations || [];
    score -=
      securityViolations.filter((v) => v.severity === "critical").length * 20;
    score -=
      securityViolations.filter((v) => v.severity === "high").length * 10;
    score -=
      securityViolations.filter((v) => v.severity === "medium").length * 5;

    // Add points for security measures
    const securityChecks =
      analysis.logicValidation?.security?.metrics?.securityChecks || 0;
  ...
  generateAnalysisReport(analysis) {
    return {
      summary: {
        ...
        totalViolations:
          analysis.layerValidation.violations.length +
          analysis.logicValidation.violations.length,
      },
      breakdown: {
        architecture: {
        ...
        performance: {
          ...
          issues: analysis.standardAnalysis.performance
            ? analysis.standardAnalysis.performance.issues
            : [],
        },
      },
