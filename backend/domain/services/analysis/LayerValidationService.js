...
class LayerValidationService {
  ...
  async validateLayers(projectPath, options = {}) {
    ...
    try {
      ...
      for (const [layerName, layerInfo] of Object.entries(detectedLayers)) {
        const layerValidation = await this.validateLayer(
          projectPath,
          layerName,
          layerInfo,
          options,
      ...
      validation.recommendations =
        await this.generateLayerRecommendations(validation);
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stats.isDirectory()) {
          ...
          await this.scanDirectoryForLayers(
            itemPath,
            projectPath,
            layers,
            depth + 1,
          ...
          const detectedLayer = await this.detectLayerFromFile(
            itemPath,
            relativePath,
          );
          if (detectedLayer) {
  ...
  async detectLayerFromFile(filePath, relativePath) {
    try {
      ...
      if (
        ...
        lowerContent.includes("res.json") ||
        lowerContent.includes("req.body")
      ) {
        return "presentation";
      }
      ...
      if (
        ...
        lowerContent.includes("class.*service") ||
        lowerContent.includes("class.*handler") ||
        lowerContent.includes("business logic") ||
        lowerContent.includes("application service")
      ) {
        return "application";
      }
      ...
      if (
        ...
        lowerContent.includes("class.*entity") ||
        lowerContent.includes("class.*model") ||
        lowerContent.includes("domain logic") ||
        lowerContent.includes("business rules")
      ) {
        return "domain";
      }
  ...
  async validateLayer(projectPath, layerName, layerInfo, options) {
    ...
    try {
      ...
      const boundaryViolations = await this.validateLayerBoundaries(
        projectPath,
        layerName,
        layerInfo,
      );
      ...
      const importViolations = await this.validateImportDependencies(
        projectPath,
        layerName,
        layerInfo,
      );
      ...
      const logicViolations = await this.validateBusinessLogicPlacement(
        projectPath,
        layerName,
        layerInfo,
      );
      ...
      validation.metrics = await this.calculateLayerMetrics(
        projectPath,
        layerName,
        layerInfo,
      );
      ...
      try {
        ...
        for (const importPath of imports) {
          ...
          for (const forbiddenImport of rules.forbiddenImports) {
            if (
              importPath.toLowerCase().includes(forbiddenImport.toLowerCase())
            ) {
              violations.push({
  ...
  async calculateLayerMetrics(projectPath, layerName, layerInfo) {
    ...
    if (layerInfo.files.length > 0) {
      ...
      metrics.complexityScore =
        metrics.complexityScore / layerInfo.files.length;
    }
  ...
  async generateLayerRecommendations(validation) {
    ...
    if (missingLayers.length > 0) {
      recommendations.push({
        ...
        suggestion:
          "Consider implementing proper layered architecture with all required layers",
      });
    }

    // Check for layer distribution
    const totalFiles = Object.values(validation.layers).reduce(
      ...
      for (const [layerName, layerInfo] of Object.entries(validation.layers)) {
        ...
        if (percentage > 50) {
          recommendations.push({
            ...
            suggestion:
              "Consider redistributing responsibilities across layers",
          });
        }
      }
    }
    ...
    if (criticalViolations.length > 0) {
      recommendations.push({
        ...
        suggestion:
          "Address critical violations to maintain architectural integrity",
      });
    }
  ...
  async calculateValidationMetrics(validation) {
    ...
    for (const [layerName, layerInfo] of Object.entries(validation.layers)) {
      ...
      if (layerInfo.validation && layerInfo.validation.metrics) {
        metrics.complexityScores[layerName] =
          layerInfo.validation.metrics.complexityScore;
      }
    }
