...
class AnalysisOutputService {
  ...
  async saveAnalysisResult(projectId, analysisType, data) {
    return await this.fileSystemService.saveAnalysisResult(
      projectId,
      analysisType,
      data,
    );
  ...
  async generateMarkdownReport(projectId, analysisResults) {
    ...
    return await this.reportGenerator.generateMarkdownReport(
      projectId,
      analysisResults,
      projectsPath,
    );
  ...
  filterAnalysisResultsForPackage(analysisResults, pkg) {
    return this.packageExtractor.filterAnalysisResultsForPackage(
      analysisResults,
      pkg,
    );
  }
  ...
  async generateMonorepoReports(
    ...
    return await this.reportGenerator.generateMonorepoReports(
      projectId,
      analysisResults,
      packages,
      baseFilename,
  ...
  async generateSinglePackageReport(projectId, analysisResults, baseFilename) {
    const projectsPath = this.fileSystemService.getProjectsPath();
    const projectDir = require("path").join(projectsPath, projectId);
    return await this.reportGenerator.generateSinglePackageReport(
      projectId,
      analysisResults,
      baseFilename,
      projectDir,
  ...
  generateComprehensiveSuggestions(analysisResults) {
    return this.reportGenerator.generateComprehensiveSuggestions(
      analysisResults,
    );
  }
  ...
  formatCodeQualityData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatCodeQualityData(
      data,
      metrics,
      recommendations,
    );
  ...
  formatSecurityData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatSecurityData(
      data,
      metrics,
      recommendations,
    );
  ...
  formatPerformanceData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatPerformanceData(
      data,
      metrics,
      recommendations,
    );
  ...
  formatArchitectureData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatArchitectureData(
      data,
      metrics,
      recommendations,
    );
  ...
  formatProjectStructureData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatProjectStructureData(
      data,
      metrics,
      recommendations,
    );
  ...
  formatDependenciesData(data, metrics = null, recommendations = null) {
    return this.markdownFormatter.formatDependenciesData(
      data,
      metrics,
      recommendations,
    );
  ...
  async getAnalysisRecommendations(projectId) {
    try {
      ...
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not injected, returning empty recommendations",
        );
        return [];
      }
  ...
  async getAnalysisMetrics(projectId) {
    try {
      ...
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not injected, returning empty metrics",
        );
        return {
          totalAnalyses: 0,
          completedAnalyses: 0,
      ...
      const lastAnalysis =
        analyses.length > 0
          ? new Date(
              Math.max(
                ...analyses.map((a) => new Date(a.completedAt || a.createdAt)),
              ),
            )
          : null;
  ...
  async getAnalysisTechStack(projectId) {
    try {
      ...
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not injected, returning empty tech stack",
        );
        return {
          languages: [],
          frameworks: [],
      ...
      const techStackAnalysis = analyses
        .filter(
          (a) =>
            a.status === "completed" &&
            a.result &&
            (a.analysisType === "tech-stack" ||
              a.analysisType === "TechStackAnalysisStep"),
        )
        .sort(
          (a, b) =>
            new Date(b.completedAt || b.createdAt) -
      ...
      const techStack =
        techStackAnalysis.result.techStack || techStackAnalysis.result;
  ...
  async getAnalysisArchitecture(projectId) {
    try {
      ...
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not injected, returning empty architecture",
        );
        return {
          structure: null,
          patterns: [],
      ...
      const architectureAnalysis = analyses
        .filter(
          (a) =>
            a.status === "completed" &&
            a.result &&
            (a.analysisType === "architecture" ||
              a.analysisType === "ArchitectureAnalysisOrchestrator"),
        )
        .sort(
          (a, b) =>
            new Date(b.completedAt || b.createdAt) -
  ...
  async getAnalysisCharts(projectId, type) {
    try {
      ...
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not injected, returning empty charts",
        );
        return {
          charts: [],
      ...
      const analyses = await this.analysisRepository.findByProjectId(
        projectId,
        {
          limit: 50,
          status: "completed",
