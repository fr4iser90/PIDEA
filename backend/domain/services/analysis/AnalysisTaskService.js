...
class AnalysisTaskService {
  ...
  async createTasksFromAnalysis(analysisResult, context, analysisType) {
    try {
      ...
      this.logger.info(
        `✅ Created ${tasks.length} tasks from ${analysisType} analysis`,
      );
      return tasks;
    } catch (error) {
   ...
   */
  async createMainImprovementTask(
    analysisType,
    projectId,
    issues,
    recommendations,
    ...
    return {
      ...
      description: this.generateMainTaskDescription(
        analysisType,
        issues,
        recommendations,
        score,
   ...
   */
  async createRecommendationTasks(
    parentTaskId,
    analysisType,
    projectId,
    recommendations,
    ...
    const mediumPriorityRecommendations = recommendations.filter(
  ...
  calculateEstimatedHours(issues, recommendations) {
    ...
    const highRecCount = recommendations.filter(
      (rec) => rec.priority === "high",
    ).length;
    const mediumRecCount = recommendations.filter(
      (rec) => rec.priority === "medium",
    ).length;
