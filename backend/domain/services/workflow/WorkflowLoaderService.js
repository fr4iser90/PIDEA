...
class WorkflowLoaderService {
  constructor() {
    ...
    this.workflowsPaths = [
      ...
      path.join(
        __dirname,
        "../../../framework/workflows/task-create-workflows.json",
      ),
      path.join(
        __dirname,
        "../../../framework/workflows/analysis-workflows.json",
      ),
    ];
  ...
  async loadWorkflows() {
    try {
      ...
      for (const workflowPath of this.workflowsPaths) {
        try {
          ...
          for (const [workflowId, workflow] of Object.entries(
            ...
            this.logger.debug(
              `Loaded workflow: ${workflowId} - ${workflow.name} from ${path.basename(workflowPath)}`,
            );
          }
          ...
          if (workflowsConfig.taskTypeMapping) {
            this.taskTypeMapping = {
              ...this.taskTypeMapping,
              ...workflowsConfig.taskTypeMapping,
            };
          }
          ...
          if (workflowsConfig.contentLibrary) {
            this.contentLibrary = {
              ...this.contentLibrary,
              ...workflowsConfig.contentLibrary,
            };
          }
        } catch (error) {
          this.logger.warn(
            `Failed to load workflows from ${workflowPath}:`,
            error.message,
          );
        }
      }
      ...
      this.logger.info(
        `Successfully loaded ${this.workflows.size} workflows from ${this.workflowsPaths.length} files`,
      );
      return true;
    } catch (error) {
  ...
  getWorkflowByTaskType(taskType) {
    const workflowId =
      this.taskTypeMapping[taskType] || this.taskTypeMapping.default;
  ...
  resolveWorkflow(workflow) {
    ...
    return {
      ...resolvedBase,
      ...workflow,
      steps: [...(resolvedBase.steps || []), ...(workflow.steps || [])],
    };
  }
