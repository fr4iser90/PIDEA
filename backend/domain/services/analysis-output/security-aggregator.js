...
class SecurityAggregator {
  ...
  aggregateSecurityData(monorepoSecurityData) {
    ...
    Object.values(monorepoSecurityData.packageSecurityAnalyses || {}).forEach(
      (pkgSec) => {
        ...
        if (securityAnalysis.configuration) {
          if (securityAnalysis.configuration.securityMiddleware) {
            aggregated.configuration.securityMiddleware.push(
              ...securityAnalysis.configuration.securityMiddleware,
            );
          }
          if (securityAnalysis.configuration.missingSecurity) {
            aggregated.configuration.missingSecurity.push(
        ...
        if (securityAnalysis.dependencies) {
          aggregated.dependencies.total +=
            securityAnalysis.dependencies.total || 0;
          aggregated.dependencies.critical +=
            securityAnalysis.dependencies.critical || 0;
          aggregated.dependencies.high +=
            securityAnalysis.dependencies.high || 0;
          aggregated.dependencies.medium +=
            securityAnalysis.dependencies.medium || 0;
          aggregated.dependencies.low += securityAnalysis.dependencies.low || 0;
          if (securityAnalysis.dependencies.vulnerable) {
            aggregated.dependencies.vulnerable.push(
              ...securityAnalysis.dependencies.vulnerable,
            );
          }
        }
        ...
        if (securityAnalysis.secrets) {
          ...
          if (securityAnalysis.secrets.patterns) {
            aggregated.secrets.patterns.push(
              ...securityAnalysis.secrets.patterns,
            );
          }
        }
      },
    ...
    aggregated.configuration.securityMiddleware = [
      ...new Set(aggregated.configuration.securityMiddleware),
    ];
    aggregated.configuration.missingSecurity = [
      ...new Set(aggregated.configuration.missingSecurity),
    ];

    // Remove features from missingSecurity if they are present in securityMiddleware
    aggregated.configuration.missingSecurity =
      aggregated.configuration.missingSecurity.filter(
        (missing) =>
          !aggregated.configuration.securityMiddleware.includes(missing),
      );

    // Update metrics
    aggregated.metrics.totalSecrets = aggregated.secrets.found.length;
    aggregated.metrics.totalMissingSecurity =
      aggregated.configuration.missingSecurity.length;
    aggregated.metrics.criticalVulnerabilities =
      UTILS.countCriticalVulnerabilities(aggregated.vulnerabilities);
    aggregated.metrics.highVulnerabilities = UTILS.countHighVulnerabilities(
      aggregated.vulnerabilities,
    );
    aggregated.metrics.mediumVulnerabilities = UTILS.countMediumVulnerabilities(
      aggregated.vulnerabilities,
    );
    aggregated.metrics.lowVulnerabilities = UTILS.countLowVulnerabilities(
      aggregated.vulnerabilities,
    );
    ...
    aggregated.recommendations = aggregated.recommendations.filter((rec) => {
      ...
      if (
        rec.title === "Add security middleware" &&
        aggregated.configuration.missingSecurity.length === 0
      ) {
        return false;
      }
      return true;
