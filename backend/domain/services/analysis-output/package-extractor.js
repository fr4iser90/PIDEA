...
class PackageExtractor {
  ...
  extractPackagesFromAnalysis(analysisResults) {
    ...
    logger.debug(
      "DEBUG: extractPackagesFromAnalysis called with:",
    ...
    if (
      ...
      analysisResults.Dependencies.data &&
      analysisResults.Dependencies.data.packages
    ) {
    ...
    if (
      ...
      analysisResults.structure.data &&
      analysisResults.structure.data.dependenciesAnalysis &&
      analysisResults.structure.data.dependenciesAnalysis.packages
    ) {
      logger.debug(
        "DEBUG: Found packages in structure.data.dependenciesAnalysis.packages",
      );
      packages.push(
        ...analysisResults.structure.data.dependenciesAnalysis.packages,
      );
    ...
    if (analysisResults.Architecture && analysisResults.Architecture.data) {
      ...
      if (
        analysisResults.Architecture.data.isMonorepo &&
        analysisResults.Architecture.data.packages
      ) {
        logger.debug(
          "DEBUG: Found packages in Architecture.data.packages (monorepo)",
        );
      ...
      // Check for old structure
        ...
        logger.debug(
          "DEBUG: Found packages in Architecture.data.packages (legacy)",
        );
        packages.push(...analysisResults.Architecture.data.packages);
      }
    }
    ...
    Object.entries(analysisResults).forEach(([type, result]) => {
      if (result && result.data) {
        ...
        if (type === "Dependencies" && result.data.packages) {
          const packageData = result.data.packages.find(
            (p) => p.path === pkg.path,
          );
          if (packageData) {
            packageResults[type] = {
        ...
        // For code quality, use package-specific data if available
          ...
          if (
            result.data.isMonorepo &&
            result.data.packageQualityAnalyses &&
            result.data.packageQualityAnalyses[pkg.name]
          ) {
            ...
            packageQualityData.metrics = {
              ...
              averageComplexity:
                packageQualityData.realMetrics.averageComplexity || 0,
              maintainabilityIndex:
                packageQualityData.realMetrics.maintainabilityIndex || 0,
              testCoverage: packageQualityData.realMetrics.testCoverage || 0,
              codeDuplicationPercentage:
                packageQualityData.realMetrics.codeDuplicationPercentage || 0,
              codeStyleIssues:
                packageQualityData.realMetrics.codeStyleIssues || 0,
              documentationCoverage:
                packageQualityData.realMetrics.documentationCoverage || 0,
              performanceIssues:
                packageQualityData.realMetrics.performanceIssues || 0,
              overallQualityScore:
                packageQualityData.realMetrics.overallQualityScore || 0,
            };
        ...
        // For security, use package-specific data ONLY if available
          ...
          if (
            ...
            packageSecurityData.metrics = {
              ...
              highVulnerabilities: this.countHighVulnerabilities(
                packageSecurityData.vulnerabilities,
              ),
              mediumVulnerabilities: this.countMediumVulnerabilities(
                packageSecurityData.vulnerabilities,
              ),
              lowVulnerabilities: this.countLowVulnerabilities(
                packageSecurityData.vulnerabilities,
              ),
            };
