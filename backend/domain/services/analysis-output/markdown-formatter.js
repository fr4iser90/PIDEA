...
class MarkdownFormatter {
  ...
  formatSecurityData(data, metrics = null, recommendations = null) {
    ...
    if (
      ...
      typeof data.securityAnalysis === "object"
    ) {
    ...
    if (
      securityData.configuration &&
      Array.isArray(securityData.configuration.securityMiddleware)
    ) {
    ...
    if (
      securityData.configuration &&
      Array.isArray(securityData.configuration.missingSecurity)
    ) {
    ...
    if (securityData.secrets) {
      ...
      if (
        securityData.secrets.patterns &&
        securityData.secrets.patterns.length > 0
      ) {
    ...
    if (perfMetrics && Object.keys(perfMetrics).length > 0) {
      ...
      Object.entries(perfMetrics).forEach(([metric, value]) => {
        ...
        if (
          ...
          value > CONSTANTS.PERFORMANCE_THRESHOLDS.SLOW_TIME_MS
        )
          status = "Slow";
        else if (
          metric.includes("size") &&
          value > CONSTANTS.PERFORMANCE_THRESHOLDS.LARGE_SIZE_BYTES
        )
          status = "Large";
  ...
  formatProjectStructureData(data, metrics = null, recommendations = null) {
    ...
    if (data.structure) {
      ...
      if (
        data.structure.fileTypes &&
        Object.keys(data.structure.fileTypes).length > 0
      ) {
      ...
      // Largest files
      if (
        data.structure.largestFiles &&
        data.structure.largestFiles.length > 0
      ) {
        md += "#### Largest Files\n\n";
  ...
  formatDependenciesData(data, metrics = null, recommendations = null) {
    ...
    if (
      ...
      data.dependenciesAnalysis.packages &&
      data.dependenciesAnalysis.packages.length > 0
    ) {
      md += "### Packages\n\n";
      md += CONSTANTS.MARKDOWN_TEMPLATES.PACKAGE_TABLE_HEADER;
  ...
  formatAnalysisData(type, data) {
    ...
    if (
      ...
      switch (type) {
        case "codeQuality":
        case "Code Quality":
          return this.formatCodeQualityData(
            analysisData,
            metrics,
            recommendations,
          );
        case "security":
        case "Security":
          return this.formatSecurityData(
            analysisData,
            metrics,
            recommendations,
          );
        ...
        case "architecture":
        case "Architecture":
          return this.formatArchitectureData(
            analysisData,
            metrics,
            recommendations,
          );
        ...
        case "Repository Structure":
          return this.formatProjectStructureData(
            analysisData,
            metrics,
            recommendations,
          );
        case "Dependencies":
          return this.formatDependenciesData(
            analysisData,
            metrics,
            recommendations,
          );
        case "Tech Stack":
          return this.formatTechStackData(
            analysisData,
            metrics,
            recommendations,
          );
        case "Refactoring":
          return this.formatRefactoringData(
            analysisData,
            metrics,
            recommendations,
          );
        case "Generation":
          return this.formatGenerationData(
            analysisData,
            metrics,
            recommendations,
          );
        default:
          return this.formatGenericAnalysisData(
            type,
            analysisData,
            metrics,
            recommendations,
