...
class ReportGenerator {
  ...
  async generateMarkdownReport(projectId, analysisResults, projectsPath) {
    ...
    const packages =
      this.packageExtractor.extractPackagesFromAnalysis(analysisResults);

    if (packages.length > 1) {
      // Generate separate reports for each package
      return await this.generateMonorepoReports(
        projectId,
        analysisResults,
        packages,
        baseFilename,
      ...
      return await this.generateSinglePackageReport(
        projectId,
        analysisResults,
        baseFilename,
        projectDir,
  ...
  async generateMonorepoReports(
    ...
    for (const pkg of packages) {
      ...
      const filteredResults =
        this.packageExtractor.filterAnalysisResultsForPackage(
          analysisResults,
          pkg,
        );
      ...
      for (const section of sections) {
        const data =
          filteredResults[section.key] ||
          filteredResults[section.key.toLowerCase()];
        if (data) {
          ...
          switch (section.key) {
            case "Repository Structure":
              packageMarkdown +=
                this.markdownFormatter.formatProjectStructureData(
                  data.data || data,
                  data.metrics,
              ...
              break;
            case "Architecture":
              packageMarkdown += this.markdownFormatter.formatArchitectureData(
                data.data || data,
                data.metrics,
                data.recommendations,
              );
              break;
            case "Code Quality":
              packageMarkdown += this.markdownFormatter.formatCodeQualityData(
                data.data || data,
                data.metrics,
                data.recommendations,
              );
              break;
            case "Dependencies":
              packageMarkdown += this.markdownFormatter.formatDependenciesData(
                data.data || data,
                data.metrics,
                data.recommendations,
              );
              break;
            case "Tech Stack":
              packageMarkdown += this.markdownFormatter.formatTechStackData(
                data.data || data,
              );
              break;
          }
      ...
      const securityData =
        analysisResults["Security"] || analysisResults["security"];

      if (
        securityData &&
        securityData.data &&
        securityData.data.isMonorepo &&
        securityData.data.packageSecurityAnalyses &&
        securityData.data.packageSecurityAnalyses[pkg.name]
      ) {
        ...
        packageSecurityData.metrics = {
          ...
          missingSecurityCount:
            packageSecurityData.configuration.missingSecurity?.length || 0,
          criticalVulnerabilities:
            this.packageExtractor.countCriticalVulnerabilities(
              packageSecurityData.vulnerabilities,
            ),
          highVulnerabilities: this.packageExtractor.countHighVulnerabilities(
            packageSecurityData.vulnerabilities,
          ),
          mediumVulnerabilities:
            this.packageExtractor.countMediumVulnerabilities(
              packageSecurityData.vulnerabilities,
            ),
          lowVulnerabilities: this.packageExtractor.countLowVulnerabilities(
            packageSecurityData.vulnerabilities,
          ),
        };

        const secMd = this.markdownFormatter.formatSecurityData(
          packageSecurityData,
          packageSecurityData.metrics,
          packageSecurityData.recommendations,
        );
        packageMarkdown += secMd;
      } else if (
        filteredResults["Security"] &&
        filteredResults["Security"].data
      ) {
        ...
        packageMarkdown +=
          "**No security analysis available for this package.**\n\n";
      }
    ...
    if (sharedStructureData) {
      ...
      sharedMarkdown += this.markdownFormatter.formatProjectStructureData(
        sharedStructureData.data || sharedStructureData,
        sharedStructureData.metrics,
        sharedStructureData.recommendations,
      );
    ...
    for (const section of systemSections) {
      const data =
        analysisResults[section.key] ||
        analysisResults[section.key.toLowerCase()];
      if (data) {
        ...
        switch (section.key) {
          case "Performance":
            sectionMarkdown += this.markdownFormatter.formatPerformanceData(
              data.data || data,
              data.metrics,
              data.recommendations,
            );
          ...
          case "Security":
            ...
            if (
              data.data &&
              data.data.isMonorepo &&
              data.data.packageSecurityAnalyses
            ) {
              // Create aggregated security data from all packages
              const aggregatedSecurityData =
                this.securityAggregator.aggregateSecurityData(data.data);
              sectionMarkdown += this.markdownFormatter.formatSecurityData(
                aggregatedSecurityData,
                aggregatedSecurityData.metrics,
                aggregatedSecurityData.recommendations,
              );
            } else {
              sectionMarkdown += this.markdownFormatter.formatSecurityData(
                data.data || data,
   ...
   * @returns {Promise<Object>} Report generation result
   */
  async generateSinglePackageReport(
    projectId,
    analysisResults,
    baseFilename,
    projectDir,
      ...
      if (data) {
        ...
        switch (section.key) {
          case "Repository Structure":
            ...
            break;
          case "Architecture":
            sectionMarkdown += this.markdownFormatter.formatArchitectureData(
              data.data || data,
              data.metrics,
              data.recommendations,
            );
            break;
          case "Code Quality":
            sectionMarkdown += this.markdownFormatter.formatCodeQualityData(
              data.data || data,
              data.metrics,
              data.recommendations,
            );
            break;
          case "Dependencies":
            sectionMarkdown += this.markdownFormatter.formatDependenciesData(
              data.data || data,
              data.metrics,
              data.recommendations,
            );
            break;
          case "Tech Stack":
            sectionMarkdown += this.markdownFormatter.formatTechStackData(
              data.data || data,
            );
            break;
          case "Performance":
          ...
          case "Security":
            ...
            break;
          case "Refactoring":
            sectionMarkdown += this.markdownFormatter.formatRefactoringData(
              data.data || data,
            );
            break;
          case "Generation":
            sectionMarkdown += this.markdownFormatter.formatGenerationData(
              data.data || data,
            );
            break;
        }
  ...
  generateComprehensiveSuggestions(analysisResults) {
    ...
    if (allRecommendations.length === 0) {
      ...
      // Group by priority
      const highPriority = allRecommendations.filter(
        (r) =>
      ...
      const mediumPriority = allRecommendations.filter(
        (r) =>
      ...
      const lowPriority = allRecommendations.filter(
        (r) => r.recommendation.priority === "low",
      );
