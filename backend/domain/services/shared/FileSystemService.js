...
class FileSystemService {
  ...
  async getDirectorySize(dirPath) {
    try {
      ...
      throw new Error(
        `Failed to get directory size for ${dirPath}: ${error.message}`,
      );
    }
  }
  ...
  async getRelativePath(basePath, targetPath) {
    try {
      ...
      throw new Error(
        `Failed to get relative path from ${basePath} to ${targetPath}: ${error.message}`,
      );
    }
  }
  ...
  async getAbsolutePath(relativePath, basePath = process.cwd()) {
    try {
      ...
      throw new Error(
        `Failed to get absolute path for ${relativePath}: ${error.message}`,
      );
    }
  }
  ...
  async validatePath(filePath) {
    try {
      ...
      throw new Error(
        `Path validation failed for ${filePath}: ${error.message}`,
      );
    }
  }
