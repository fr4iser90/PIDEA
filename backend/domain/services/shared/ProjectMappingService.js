...
class ProjectMappingService {
  ...
  async getProjectIdFromWorkspace(workspacePath) {
    ...
    if (this.monorepoStrategy) {
      try {
        const isMonorepo =
          await this.monorepoStrategy.isMonorepo(workspacePath);
        if (isMonorepo) {
        ...
        logger.warn(
          "ProjectMappingService: Failed to check monorepo status:",
          error.message,
        );
      }
    }
  ...
  async getProjectIdFromWorkspacePath(workspacePath) {
    return (
      this.workspaceMappings.get(workspacePath) ||
      (await this.getProjectIdFromWorkspace(workspacePath))
    );
  }
  ...
  registerWorkspace(workspacePath, projectId = null) {
    ...
    const calculatedProjectId =
      projectId || this.getProjectIdFromWorkspace(workspacePath);
