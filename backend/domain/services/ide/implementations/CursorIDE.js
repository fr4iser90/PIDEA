...
class CursorIDE extends BaseIDE {
  ...
  async detect() {
    try {
      ...
      const cursorIDEs = availableIDEs.filter(
        (ide) =>
  ...
  async sendTask(task, workspacePath = null) {
    try {
      ...
      if (!workspacePath) {
        ...
        logger.info(
          "🔍 [CursorIDE] Using active IDE workspace path:",
          workspacePath,
        );
      }
...
Please provide a summary when you complete the task.`;
  ...
  async sendAutoModeTasks(tasks, projectAnalysis, workspacePath = null) {
    try {
      logger.info(
        "🔍 [CursorIDE] Sending auto mode tasks to Cursor IDE:",
        tasks.length,
        "tasks",
      );
...
`;
      ...
      logger.info(
        "✅ [CursorIDE] Created auto mode file at:",
        autoModeFilePath,
      );
  ...
  async switchToSession(session) {
    try {
      ...
      if (activeIDE && activeIDE.port === session.idePort) {
        return { message: "Already connected to the right IDE" };
      }
