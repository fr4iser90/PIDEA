...
class SelectorCollectionBot {
  ...
  async collectSelectors(ideType, version, port) {
    try {
      this.logger.info(
        `Collecting selectors for ${ideType} version ${version} on port ${port}`,
      );
      ...
      if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
        this.logger.info(
          `Using cached selectors for ${ideType} version ${version}`,
        );
      ...
      const collectedSelectors =
        await this.selectorCollector.collectSelectors(port);

      if (!collectedSelectors || Object.keys(collectedSelectors).length === 0) {
        throw new Error(
          `No selectors collected for ${ideType} version ${version}`,
        );
      }

      // Validate collected selectors
      const validatedSelectors =
        await this.validateSelectors(collectedSelectors);

      // Categorize selectors
      const categorizedSelectors =
        await this.categorizeSelectors(validatedSelectors);
      ...
      this.logger.info(
        `Successfully collected ${Object.keys(categorizedSelectors).length} selector categories for ${ideType} version ${version}`,
      );
      return categorizedSelectors;
    } catch (error) {
      this.logger.error(
        `Selector collection failed for ${ideType} version ${version}:`,
        error.message,
      );
      throw error;
    }
  ...
  async categorizeSelectors(selectors) {
    try {
      ...
      this.logger.info(
        `Categorized selectors into ${Object.keys(result).length} categories`,
      );
      return result;
    } catch (error) {
  ...
  async collectChatSelectors(ideType, version, port) {
    try {
      this.logger.info(
        `Collecting comprehensive chat selectors for ${ideType} version ${version}`,
      );
      ...
      const chatKeys = Object.keys(allSelectors).filter(
        (key) =>
          key.includes("chat") ||
      ...
      const validatedChatSelectors =
        await this.validateChatSelectors(chatSelectors);

      this.logger.info(
        `Successfully collected ${Object.keys(validatedChatSelectors).length} chat selectors`,
      );
      return validatedChatSelectors;
    } catch (error) {
      this.logger.error(
        `Chat selector collection failed for ${ideType} version ${version}:`,
        error.message,
      );
      throw error;
    }
  ...
  async validateChatSelectors(chatSelectors) {
    try {
      ...
      const missingRequired = chatRequiredSelectors.filter(
        (selector) => !validated[selector],
      );
      if (missingRequired.length > 0) {
        this.logger.warn(
          `Missing required chat selectors: ${missingRequired.join(", ")}`,
        );
      }

      this.logger.info(
        `Validated ${Object.keys(validated).length} chat selectors`,
      );
      return validated;
    } catch (error) {
  ...
  async testChatSelectors(ideType, version, chatSelectors, port) {
    try {
      this.logger.info(
        `Testing chat selectors for ${ideType} version ${version}`,
      );
      ...
      this.logger.info(
        `Chat selector testing completed: ${results.passed}/${results.tested} passed`,
      );
      return results;
    } catch (error) {
      this.logger.error(
        `Chat selector testing failed for ${ideType} version ${version}:`,
        error.message,
      );
      throw error;
    }
  ...
  async saveSelectors(ideType, version, selectors) {
    try {
      ...
      if (this.ideTypesUpdater) {
        ...
        this.logger.info(
          `Successfully updated IDETypes.js with ${ideType} version ${version}`,
        );
        return updateResult;
      } else {
        this.logger.warn(
          `IDETypesUpdater not available, selectors not saved to IDETypes.js for ${ideType} version ${version}`,
        );
        return {
      ...
      this.logger.error(
        `Failed to save selectors for ${ideType} version ${version}:`,
        error.message,
      );
      throw error;
    }
  ...
  async testSelectors(ideType, version, selectors, port) {
    try {
      ...
      this.logger.info(
        `Selector testing completed: ${results.passed}/${results.tested} passed`,
      );
      return results;
    } catch (error) {
      this.logger.error(
        `Selector testing failed for ${ideType} version ${version}:`,
        error.message,
      );
      throw error;
    }
