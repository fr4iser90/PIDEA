...
class IDEMirrorService {
  ...
  async connectToIDE() {
    ...
    if (!activePort) {
      ...
      if (availableIDEs.length === 0) {
        throw new Error(
          "No Cursor IDE instances found. Please start Cursor IDE first.",
        );
      }
  ...
  async captureCompleteIDEState() {
    ...
    try {
          ...
          Array.from(document.styleSheets).forEach((sheet) => {
            try {
              if (sheet.href) {
                ...
                if (
                  sheet.href.includes("vscode-file://") ||
                  sheet.href.includes("electron://")
                ) {
                  try {
                    ...
                    if (rules.trim()) {
                      ...
                      logger.info(
                        "✅ Extracted CSS from Electron URL:",
                        sheet.href,
                      );
                    }
                  } catch (e) {
      ...
      logger.info(
        `📸✅ Captured IDE: Screenshot + ${totalElements} clickable elements`,
      );
      return domStructure;
    } catch (error) {
      ...
      for (const chatSelector of chatSelectors) {
        try {
          ...
          if (elementInfo && elementInfo.visible) {
            logger.info(
              `🎯 Targeting input element: ${chatSelector} (${elementInfo.type})`,
            );
          ...
          logger.debug(
            `⚠️ Focus attempt failed for ${chatSelector}: ${e.message}`,
          );
        }
      }
    } else {
