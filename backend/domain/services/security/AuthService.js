...
class AuthService {
  ...
  async authenticateUser(email, password) {
    ...
    logger.info("✅ User found in DB:", {
      ...
      passwordHashStart: user.passwordHash
  ...
  async validateAccessToken(accessToken) {
    ...
    try {
      ...
      const session =
        await this.userSessionRepository.findByAccessToken(accessToken);
      if (!session) {
      ...
      const validationResult = this.tokenValidator.validateSessionToken(
        accessToken,
        session,
      );
      ...
      if (!session.isActive()) {
        ...
        try {
          await this.userSessionRepository.delete(session.id);
          logger.info(
            "🧹 [AuthService] Automatically cleaned up expired session:",
            session.id,
          );
        } catch (cleanupError) {
          logger.error(
            "❌ [AuthService] Failed to cleanup expired session:",
      ...
      if (!user) {
        ...
        try {
          await this.userSessionRepository.delete(session.id);
          logger.info(
            "🧹 [AuthService] Automatically cleaned up orphaned session:",
            session.id,
          );
        } catch (cleanupError) {
          logger.error(
            "❌ [AuthService] Failed to cleanup orphaned session:",
            cleanupError.message,
          );
        }
  ...
  async refreshToken() {
    ...
    return { message: "Authentication refreshed" };
  }
  ...
  async cleanupOrphanedSessions() {
    try {
      ...
      for (const session of allSessions) {
        try {
          ...
          if (!user) {
            ...
            orphanedCount++;
            logger.debug(
              `🧹 [AuthService] Cleaned up orphaned session for non-existent user: ${session.userId}`,
            );
          }
        } catch (error) {
          ...
          orphanedCount++;
          logger.debug(
            `🧹 [AuthService] Cleaned up session with lookup error: ${session.id}`,
          );
        }
      }
      ...
      logger.error(
        "❌ [AuthService] Orphaned session cleanup failed:",
        error.message,
      );
      return 0;
    }
  ...
  async cleanupUserSessions(userId) {
    ...
    logger.info(
      "🧹 [AuthService] Cleaning up expired sessions for user:",
      userId,
    );
    ...
    try {
      ...
      const expiredCount =
        await this.userSessionRepository.deleteExpiredByUserId(userId);
      logger.info(
        "✅ [AuthService] Cleaned up",
        expiredCount,
      ...
      if (activeSessions.length >= maxSessions) {
        ...
        for (const session of sessionsToDelete) {
          await this.userSessionRepository.delete(session.id);
          logger.info(
            "🧹 [AuthService] Removed old session due to limit:",
            session.id,
          );
        }

        logger.info(
          "✅ [AuthService] Session limit enforced, removed",
      ...
      logger.error(
        "❌ [AuthService] Failed to cleanup sessions for user:",
        userId,
        error,
      );
