  ...
  async analyzePackageJsonInPath(workspacePath) {
    try {
        ...
        try {
          ...
          for (const file of files) {
            ...
            if (file.isDirectory()) {
              ...
              logger.info(
                "Recursing into subdirectory, depth:",
                currentDepth + 1,
              );
      ...
      if (candidates.length > 0) {
        ...
        if (best) {
          return await this.parsePackageJson(
            path.join(best.dir, "package.json"),
            best.dir,
          );
        }
      }
  ...
  async techstackScore(packageJsonPath) {
    try {
      ...
      for (const dep of Object.keys(allDeps)) {
        if (this.frontendTechs.some((t) => dep.toLowerCase().includes(t)))
          techScore += 10;
      }
      return techScore;
    } catch (e) {
    ...
    try {
      ...
      for (const [scriptName, scriptCommand] of Object.entries(scripts)) {
        for (const pattern of devPatterns) {
          if (
            pattern.test(scriptName.toLowerCase()) ||
            pattern.test(scriptCommand.toLowerCase())
          ) {
            // Port-Extraktion
            const portMatch = scriptCommand.match(
              /--port\s+(\d+)|-p\s+(\d+)|port\s*=\s*(\d+)|--port=(\d+)/i,
            );
