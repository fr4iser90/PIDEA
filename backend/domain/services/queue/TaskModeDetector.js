...
class TaskModeDetector {
    ...
    try {
      ...
      if (!workflowData || !workflowData.steps) {
        throw new InvalidWorkflowDataError(
          "Workflow data and steps are required",
        );
      }
      ...
      if (!this.knownTypes.has(detectedMode)) {
        throw new UnknownTaskModeError(
          `Unknown task mode detected: ${detectedMode}`,
        );
      }
  ...
  calculateConfidence(stepAnalysis, detectedMode) {
    ...
    const unexpectedSteps = stepTypes.filter(
      (stepType) =>
  ...
  validateWorkflowData(workflowData) {
    try {
      ...
      this.logger.error("Error validating workflow data", {
        error: error.message,
      });
      return false;
    }
