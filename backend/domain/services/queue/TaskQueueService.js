...
class TaskQueueService {
  ...
  async enqueue(taskId, userId, options = {}) {
    this.logger.info("🔍 [TaskQueueService] enqueue called:", {
      taskId,
      options,
    });
    ...
    try {
      ...
      if (
        ...
        this.logger.info(
          "🔍 [TaskQueueService] Task creation workflow detected, skipping task validation",
        );
      } else {
      ...
      this.logger.info(
        "✅ [TaskQueueService] Task added to queue successfully",
        {
          taskId,
      ...
      this.logger.error(
        "❌ [TaskQueueService] Failed to add task to queue:",
        error.message,
      );
      throw error;
    }
