...
class StepProgressService {
  ...
  async getTaskStepProgress(projectId, taskId, userId) {
    try {
      ...
      const stepProgress =
        this.taskStepProgress.get(taskKey) || this.initializeTaskStepProgress();
      ...
      const progress = {
        ...
        estimatedTimeRemaining:
          this.calculateEstimatedTimeRemaining(stepProgress),
   ...
   */
  async initializeTaskStepProgress(
    projectId,
    taskId,
    steps = [],
    options = {},
    ...
    try {
      this.logger.info("Initializing step progress", {
        projectId,
        taskId,
  ...
  async failStep(projectId, taskId, stepId, error) {
    try {
      this.logger.error("Failing step", {
        projectId,
        taskId,
        stepId,
        error: error.message,
