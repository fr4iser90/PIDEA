...
class TaskQueueStore {
    ...
    try {
      ...
      const status = {
        ...
        queue: {
          ...
          running: activeItems.filter((item) => item.status === "running")
            .length,
          queued: activeItems.filter((item) => item.status === "queued").length,
          completed: completedItems.filter(
            (item) => item.status === "completed",
          ).length,
          failed: completedItems.filter((item) => item.status === "failed")
            .length,
        },
        statistics,
  ...
  async updateQueueItem(projectId, itemId, updates) {
    try {
      ...
      this.logger.debug("Queue item updated", {
        projectId,
        itemId,
        status: item.status,
      });
  ...
  async updateStepProgress(projectId, workflowId, stepId, progress) {
    try {
      ...
      const projectQueue = this.getProjectQueue(projectId);
      const workflowIndex = projectQueue.findIndex(
      ...
      const stepIndex = workflow.workflow?.steps?.findIndex(
        (step) => step.id === stepId,
      );
      if (stepIndex === -1) {
      ...
      const completedSteps = workflow.workflow.steps.filter(
        (step) => step.status === "completed",
      ).length;
      const totalSteps = workflow.workflow.steps.length;
      workflow.workflow.progress = Math.round(
  ...
  async getQueueStatistics(projectId, userId) {
    try {
      const projectQueue = this.getProjectQueue(projectId);
      const stats =
        this.queueStats.get(projectId) || this.initializeQueueStats();

      const currentStats = {
        totalItems: projectQueue.length,
        running: projectQueue.filter((item) => item.status === "running")
          .length,
        queued: projectQueue.filter((item) => item.status === "queued").length,
        completed: projectQueue.filter((item) => item.status === "completed")
          .length,
        failed: projectQueue.filter((item) => item.status === "failed").length,
        cancelled: projectQueue.filter((item) => item.status === "cancelled")
          .length,
        ...
        successRate:
          stats.itemsProcessed > 0
            ? (stats.completed / stats.itemsProcessed) * 100
            : 0,
        lastUpdated: new Date().toISOString(),
      };
  ...
  async clearCompletedItems(projectId, userId) {
    try {
      ...
      const itemsToRemove = projectQueue.filter(
        (item) =>
          item.status === "completed" ||
          item.status === "failed" ||
          item.status === "cancelled",
  ...
  calculateQueueHealth(activeItems) {
    ...
    const runningItems = activeItems.filter(
