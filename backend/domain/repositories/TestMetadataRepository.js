...
class TestMetadataRepository {
  ...
  /**
   * Find all flaky test metadata
   * @returns {Promise<TestMetadata[]>} - Array of flaky test metadata
   */
  async findFlakyTests() {
    const ids = this._flakyIndex.get(true) || [];
    return ids.map(id => this._testMetadata.get(id)).filter(Boolean);
  }
  /**
  ...
  async getStatistics() {
    ...
    return {
      ...
      skipped,
      pending,
      flaky: allTests.filter(test => test.isFlaky()).length,
  ...
  _addToIndexes(testMetadata) {
    ...
    if (testMetadata.isFlaky) {
      if (!this._flakyIndex.has(true)) {
        this._flakyIndex.set(true, []);
      }
      this._flakyIndex.get(true).push(testMetadata.id);
    }
    // Tag index
  ...
  _removeFromIndexes(testMetadata) {
    ...
    if (testMetadata.isFlaky) {
      const flakyIds = this._flakyIndex.get(true);
      if (flakyIds) {
        const index = flakyIds.indexOf(testMetadata.id);
        if (index > -1) {
          flakyIds.splice(index, 1);
        }
        if (flakyIds.length === 0) {
          this._flakyIndex.delete(true);
        }
      }
    }
    // Tag index
