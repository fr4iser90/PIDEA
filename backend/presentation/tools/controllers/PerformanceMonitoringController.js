...
class PerformanceMonitoringController {
  ...
  async getStatus(req, res) {
    try {
      ...
      res.success({
        ...
        performanceMonitor:
          status.performanceMonitoring?.performanceMonitor || null,
        queryMonitor: status.performanceMonitoring?.queryMonitor || null,
        queryCache: status.performanceMonitoring?.queryCache || null,
      });
    } catch (error) {
      this.logger.error(
        "Error getting performance monitoring status:",
        error.message,
      );
      res.error("Failed to get performance monitoring status", 500, {
  ...
  async getStats(req, res) {
    try {
      const performanceMonitor =
        this.databaseConnection.getPerformanceMonitor();
      ...
      res.error("Failed to get performance statistics", 500, {
        details: error.message,
      });
    }
  }
  ...
  async getTopQueriesByTime(req, res) {
    try {
      ...
      res.error("Failed to get top queries by time", 500, {
        details: error.message,
      });
    }
  }
  ...
  async getMostFrequentQueries(req, res) {
    try {
      ...
      res.error("Failed to get most frequent queries", 500, {
        details: error.message,
      });
    }
  }
  ...
  async clearQueryHistory(req, res) {
    try {
      ...
      res.error("Failed to clear query history", 500, {
        details: error.message,
      });
    }
  }
  ...
  async exportData(req, res) {
    try {
      ...
      if (format === "csv") {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="performance_data.csv"',
        );
        res.send(data);
      } else {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="performance_data.json"',
        );
        res.send(data);
      }
    } catch (error) {
      this.logger.error("Error exporting performance data:", error.message);
      res.error("Failed to export performance data", 500, {
        details: error.message,
      });
    }
  }
  ...
  async enableMonitoring(req, res) {
    try {
      ...
      this.logger.error(
        "Error enabling performance monitoring:",
        error.message,
      );
      res.error("Failed to enable performance monitoring", 500, {
        details: error.message,
      });
    }
  }
  ...
  async disableMonitoring(req, res) {
    try {
      ...
      this.logger.error(
        "Error disabling performance monitoring:",
        error.message,
      );
      res.error("Failed to disable performance monitoring", 500, {
        details: error.message,
      });
    }
  }
