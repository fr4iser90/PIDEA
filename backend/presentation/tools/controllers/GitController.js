...
class GitController {
  constructor(dependencies = {}) {
    ...
    if (!this.gitApplicationService) {
      throw new Error(
        "GitController requires gitApplicationService dependency",
      );
    }
    if (!this.gitService) {
  ...
  async getStatus(req, res) {
    try {
      ...
      if (shouldLog) {
        this.logger.info("GitController: Getting Git status", {
          projectId,
          userId,
        });
  ...
  async getBranches(req, res) {
    try {
      ...
      this.logger.info("GitController: Getting branches", {
        projectId,
        userId,
      });
      ...
      res.success(
        ...
        200,
        { meta: { message: "Branches retrieved successfully" } },
      );
    } catch (error) {
  ...
  async validate(req, res) {
    try {
      ...
      const result = await this.gitApplicationService.getStatus(
        projectId,
        projectPath,
        userId,
      );
      ...
      if (validation.untrackedFiles > 10) {
        validation.warnings.push(
          `Many untracked files (${validation.untrackedFiles}). Consider adding to .gitignore`,
        );
      }

      if (validation.modifiedFiles > 20) {
        validation.warnings.push(
          `Many modified files (${validation.modifiedFiles}). Consider committing in smaller chunks`,
        );
      }

      if (currentBranch === "main" && validation.hasChanges) {
        validation.warnings.push(
          "You have uncommitted changes on main branch. Consider creating a feature branch",
        );
      }
  ...
  async compare(req, res) {
    try {
      ...
      const sourceHistory = await this.gitService.getCommitHistory(
        projectPath,
        {
          branch: sourceBranch,
          limit: 5,
      ...
      const targetHistory = await this.gitService.getCommitHistory(
        projectPath,
        {
          branch: targetBranch,
          limit: 5,
      ...
      res.success(
        ...
        200,
        { meta: { message: "Branch comparison completed successfully" } },
      );
    } catch (error) {
  ...
  async pull(req, res) {
    try {
      ...
      // Pull changes
      const result = await this.gitService.pullChanges(
        projectPath,
        {
          remote,
          branch,
      ...
      res.success(
        ...
        200,
        { meta: { message: "Changes pulled successfully" } },
      );
    } catch (error) {
  ...
  async checkout(req, res) {
    try {
      ...
      res.success(
        ...
        200,
        { meta: { message: "Switched to branch: ${branch}" } },
      );
    } catch (error) {
  ...
  async merge(req, res) {
    try {
      ...
      // Pull latest changes
      await this.gitService.pullChanges(
        projectPath,
        { branch: targetBranch },
        userId,
        projectId,
  ...
  async getRepositoryInfo(req, res) {
    try {
      ...
      this.logger.info("GitController: Getting repository info", {
        projectId,
        userId,
      });

      // ✅ OPTIMIZATION: Use GitApplicationService with direct Commands/Handlers
      const result = await this.gitApplicationService.getGitInfo(
        projectPath,
        userId,
      );
      ...
      res.error("Failed to get repository info", 500, {
  ...
  async pullPideaAgent(req, res) {
    try {
      ...
      res.success(
        ...
        200,
        { meta: { message: "Successfully pulled from pidea-agent branch" } },
      );
    } catch (error) {
      this.logger.error(
        "GitController: Failed to pull from pidea-agent branch",
        {
          projectPath: req.body.projectPath,
      ...
      res.error("Failed to pull from pidea-agent branch", 500, {
        details: error.message,
      });
    }
  }
    ...
    try {
      ...
      res.success(
        ...
        200,
        {
          meta: {
            message:
              "Successfully merged ${sourceBranch} into pidea-agent branch",
          },
        },
      );
    } catch (error) {
      this.logger.error(
        "GitController: Failed to merge to pidea-agent branch",
        {
          projectPath: req.body.projectPath,
          sourceBranch: req.body.sourceBranch,
          error: error.message,
      ...
      res.error("Failed to merge to pidea-agent branch", 500, {
        details: error.message,
      });
    }
  }
  ...
  async getPideaAgentStatus(req, res) {
    try {
      ...
      this.logger.info("GitController: Getting pidea-agent branch status", {
        projectId,
        userId,
        projectPath,
      });
      ...
      this.logger.info(
        "GitController: Getting branches via GitApplicationService",
        { projectPath },
      );
      const branchesResult = await this.gitApplicationService.getBranches(
        projectPath,
        userId,
      );
      this.logger.info(
        "GitController: GitApplicationService.getBranches() returned",
        { branchesResult },
      );
      ...
      const branches =
        ...
        branchesResult.data.branches.all
          ? branchesResult.data.branches.all
          : [];
      this.logger.info("GitController: Extracted branches.all", { branches });

      const pideaAgentExists =
        branches.includes("pidea-agent") ||
        branches.includes("remotes/origin/pidea-agent");
      this.logger.info("GitController: Branch search result", {
        pideaAgentExists,
        "pidea-agent found in local": branches.includes("pidea-agent"),
        "pidea-agent found in remote": branches.includes(
          "remotes/origin/pidea-agent",
        ),
      });

      if (!pideaAgentExists) {
        this.logger.warn("GitController: Pidea-agent branch not found", {
          branches,
        });
        return res.notFound("Pidea-agent branch does not exist", {
      ...
      const currentBranchResult =
        await this.gitApplicationService.getCurrentBranch(projectPath, userId);
      const currentBranch = currentBranchResult.data.currentBranch;
      this.logger.info("GitController: Current branch", { currentBranch });

      // Get pidea-agent branch status
      const statusResult = await this.gitApplicationService.getStatus(
        projectId,
        projectPath,
        userId,
      );
      ...
      const lastCommit =
        await this.gitApplicationService.getLastCommitDirect(projectPath);
      ...
      this.logger.error(
        "GitController: Failed to get pidea-agent branch status",
        {
          projectPath: req.body.projectPath,
          error: error.message,
          userId: req.user?.id,
      ...
      res.error("Failed to get pidea-agent branch status", 500, {
        details: error.message,
      });
    }
  }
  ...
  async compareWithPideaAgent(req, res) {
    try {
      ...
      // Check if pidea-agent branch exists
      const branchesResult = await this.gitService.getBranches(
        projectPath,
        { includeRemote: true, includeLocal: true },
        userId,
        projectId,
      );
      const branches =
        branchesResult && branchesResult.result && branchesResult.result.all
          ? branchesResult.result.all
          : [];
      const pideaAgentExists =
        branches.includes("pidea-agent") ||
        branches.includes("remotes/origin/pidea-agent");
      ...
      const pideaAgentHistory = await this.gitService.getCommitHistory(
        projectPath,
        {
          branch: "pidea-agent",
          limit: 5,
      ...
      res.success(
        ...
        200,
        {
          meta: {
            message:
              "Comparison with pidea-agent branch completed successfully",
          },
        },
      );
    } catch (error) {
      this.logger.error(
        "GitController: Failed to compare with pidea-agent branch",
        {
          projectPath: req.body.projectPath,
          sourceBranch: req.body.sourceBranch,
          error: error.message,
      ...
      res.error("Failed to compare with pidea-agent branch", 500, {
        details: error.message,
      });
    }
  }
