...
class TaskWebSocket {
  ...
  setupEventListeners() {
    ...
    this.eventBus.on(
      "task:sync:completed",
      this.handleTaskSyncCompleted.bind(this),
    );

    // Task execution events
    this.eventBus.on(
      "task:execution:start",
      this.handleTaskExecutionStart.bind(this),
    );
    this.eventBus.on(
      "task:execution:progress",
      this.handleTaskExecutionProgress.bind(this),
    );
    this.eventBus.on(
      "task:execution:complete",
      this.handleTaskExecutionComplete.bind(this),
    );
    this.eventBus.on(
      "task:execution:error",
      this.handleTaskExecutionError.bind(this),
    );
    ...
    this.eventBus.on(
      "step:progress",
      this.handleAnalysisStepProgress.bind(this),
    );
    this.eventBus.on(
      "step:completed",
      this.handleAnalysisStepCompleted.bind(this),
    );
    this.eventBus.on("step:failed", this.handleAnalysisStepFailed.bind(this));
    this.eventBus.on(
      "step:cancelled",
      this.handleAnalysisStepCancelled.bind(this),
    );

    // Suggestion events
    this.eventBus.on(
      "suggestion:generated",
      this.handleSuggestionGenerated.bind(this),
    );
    this.eventBus.on(
      "suggestion:applied",
    ...
    this.eventBus.on(
      "suggestion:rejected",
      this.handleSuggestionRejected.bind(this),
    );
    ...
    this.eventBus.on(
      "ide:status:changed",
      this.handleIDEStatusChanged.bind(this),
    );
    this.eventBus.on("ide:switched", this.handleIDESwitched.bind(this));
    this.eventBus.on(
      "ide:features:updated",
      this.handleIDEFeaturesUpdated.bind(this),
    );
    this.eventBus.on(
      "ide:mirror:status:changed",
      this.handleIDEMirrorStatusChanged.bind(this),
    );
    this.eventBus.on("ide:dom:updated", this.handleIDEDOMUpdated.bind(this));
    this.eventBus.on(
      "ide:element:interaction",
      this.handleIDEElementInteraction.bind(this),
    );
    ...
    this.eventBus.on(
      ...
      this.handleQueueItemCancelled.bind(this),
    );
    this.eventBus.on(
      "queue:item:priority_updated",
      this.handleQueueItemPriorityUpdated.bind(this),
    );
    this.eventBus.on(
      "queue:items:cleared",
      this.handleQueueItemsCleared.bind(this),
    );

    // Step progress events
    this.eventBus.on(
      "task:step:progress",
      this.handleTaskStepProgress.bind(this),
    );
    this.eventBus.on(
      "task:step:started",
      this.handleTaskStepStarted.bind(this),
    );
    this.eventBus.on(
      "task:step:completed",
      this.handleTaskStepCompleted.bind(this),
    );
    ...
    this.eventBus.on(
      "task:step:resumed",
      this.handleTaskStepResumed.bind(this),
    );
    this.eventBus.on(
      "task:step:progress:initialized",
      this.handleTaskStepProgressInitialized.bind(this),
    );
  }
  ...
  async handleTaskStatusRequest(socket, user, data) {
    try {
      ...
      const status = await this.taskMonitoringService.getTaskStatus(
        taskId,
        user.id,
      );
  ...
  async handleExecutionLogsRequest(socket, user, data) {
    try {
      ...
      const logs = await this.taskMonitoringService.getExecutionLogs(
        executionId,
        user.id,
        limit,
      );
  ...
  handleIDEUnsubscription(socket, user, data) {
    try {
      ...
      socket.emit("error", {
        message: "Failed to unsubscribe from IDE updates",
      });
    }
  }
  ...
  handleIDEMirrorSubscription(socket, user, data) {
    try {
      ...
      this.logger.info(
        "TaskWebSocket: Client subscribed to IDE mirror updates",
        {
          socketId: socket.id,
          userId: user.id,
          roomName,
      ...
      this.logger.error(
        "TaskWebSocket: Failed to handle IDE mirror subscription",
        {
          socketId: socket.id,
          userId: user.id,
          error: error.message,
      ...
      socket.emit("error", {
        message: "Failed to subscribe to IDE mirror updates",
      });
    }
  }
  ...
  handleIDEMirrorUnsubscription(socket, user, data) {
    try {
      ...
      this.logger.info(
        "TaskWebSocket: Client unsubscribed from IDE mirror updates",
        {
          socketId: socket.id,
          userId: user.id,
          roomName,
      ...
      this.logger.error(
        "TaskWebSocket: Failed to handle IDE mirror unsubscription",
        {
          socketId: socket.id,
          userId: user.id,
          error: error.message,
      ...
      socket.emit("error", {
        message: "Failed to unsubscribe from IDE mirror updates",
      });
    }
  }
  ...
  async handleIDEStatusRequest(socket, user, data) {
    try {
      ...
      if (!port) {
        socket.emit("error", {
          message: "Port is required for IDE status request",
        });
        return;
      }
  ...
  async handleIDEFeaturesRequest(socket, user, data) {
    try {
      ...
      if (!port) {
        socket.emit("error", {
          message: "Port is required for IDE features request",
        });
        return;
      }
      ...
      this.logger.error(
        "TaskWebSocket: Failed to handle IDE features request",
        {
          socketId: socket.id,
          userId: user.id,
          error: error.message,
