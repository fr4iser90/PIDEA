...
class TaskAnalysisController {
  ...
  async analyzeProject(req, res) {
    try {
      ...
      res.success(
        ...
        200,
        { meta: { message: "Project analysis completed successfully" } },
      );
    } catch (error) {
  ...
  async getProjectAnalysis(req, res) {
    try {
      ...
      this.logger.error(
        "TaskAnalysisController: Failed to get project analysis",
        {
      ...
      res.error("Failed to get project analysis", 500, {
  ...
  async aiProjectAnalysis(req, res) {
    try {
      ...
      res.success(
        ...
        200,
        { meta: { message: "AI analysis completed successfully" } },
      );
    } catch (error) {
      this.logger.error(
        "TaskAnalysisController: Failed to perform AI analysis",
        {
          projectPath: req.body.projectPath,
          error: error.message,
          userId: req.user?.id,
      ...
      res.error("Failed to perform AI analysis", 500, {
        details: error.message,
      });
    }
  }
  ...
  async getAnalysisHistory(req, res) {
    try {
      ...
      this.logger.error(
        "TaskAnalysisController: Failed to get analysis history",
        {
          error: error.message,
          userId: req.user?.id,
        },
  ...
  async compareAnalyses(req, res) {
    try {
      ...
      const result = await this.commandBus.execute(
        "CompareAnalysesCommand",
        command,
      );

      this.logger.info(
        "TaskAnalysisController: Analysis comparison completed",
        {
          analysisIds,
      ...
      res.success(
        ...
        200,
        { meta: { message: "Analysis comparison completed successfully" } },
      );
    } catch (error) {
  ...
  async exportAnalysis(req, res) {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="analysis-${id}-${Date.now()}.${format}"`,
      );
  ...
  async getAnalysisStats(req, res) {
    try {
      ...
      this.logger.info(
        "TaskAnalysisController: Analysis statistics retrieved",
        {
          userId,
        },
      );
      ...
      this.logger.error(
        "TaskAnalysisController: Failed to get analysis statistics",
        {
          error: error.message,
          userId: req.user?.id,
        },
  ...
  async scheduleAnalysis(req, res) {
    try {
      const { projectPath, schedule, analysisType, options = {} } = req.body;
  ...
  async healthCheck(req, res) {
    try {
      ...
      res.error("Analysis service is unhealthy", 500, {
        details: error.message,
      });
    }
  }
