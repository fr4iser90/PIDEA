...
class TaskController {
  constructor(taskApplicationService, eventBus = null, application = null) {
    ...
    this.logger.debug(
      "🔍 [TaskController] taskApplicationService methods:",
      Object.getOwnPropertyNames(
        Object.getPrototypeOf(this.taskApplicationService),
      ),
    );
    this.logger.debug(
      "🔍 [TaskController] syncManualTasks exists:",
      typeof this.taskApplicationService.syncManualTasks,
    );
    this.logger.debug(
      "🔍 [TaskController] taskApplicationService type:",
      typeof this.taskApplicationService,
    );
    this.logger.debug(
      "🔍 [TaskController] taskApplicationService constructor:",
      this.taskApplicationService.constructor.name,
    );
    this.logger.debug(
      "🔍 [TaskController] taskApplicationService keys:",
      Object.keys(this.taskApplicationService),
    );
  }
  ...
  async getProjectTasks(req, res) {
    try {
      ...
      const tasks = await this.taskApplicationService.getProjectTasks(
        projectId,
        {
  ...
  async updateTask(req, res) {
    try {
      ...
      const task = await this.taskApplicationService.updateTask(
        id,
        projectId,
        updateData,
        userId,
  ...
  async executeTask(req, res) {
    try {
      ...
      this.logger.warn(
        "🚨 [TaskController] executeTask endpoint is DEPRECATED! Redirecting to enqueueTask",
      );
      ...
      if (
        ...
        res.notFound("Task not found", {
          message: error.message,
          deprecationWarning:
            "This endpoint is deprecated. Use /api/projects/:projectId/tasks/enqueue instead.",
        });
      } else {
        res.error("Task execution failed", 500, {
  ...
  async syncManualTasks(req, res) {
    try {
      ...
      const result = await this.taskApplicationService.syncManualTasks(
        projectId,
        userId,
      );

      this.logger.info(
        "✅ [TaskController] Manual tasks sync completed successfully",
      );
      ...
      try {
        ...
        const allTasks =
          await this.taskApplicationService.taskRepository.findByProjectId(
            projectId,
          );
        ...
        if (allTasks && allTasks.length > 0) {
          ...
          this.logger.info(
            "🔍 [TaskController] Task status validation completed",
            {
              projectId,
          ...
          if (validationResult.invalidTasks > 0) {
            const invalidTasks = validationResult.results.filter(
              (r) => !r.valid,
            );
            this.logger.warn(
              "⚠️ [TaskController] Found invalid task statuses:",
              {
                projectId,
        ...
        this.logger.warn(
          "⚠️ [TaskController] Task status validation failed (non-critical):",
          syncError,
        );
      }
      ...
      if (this.eventBus) {
        ...
        this.logger.info(
          "📡 [TaskController] Emitted task:sync:completed event",
        );
      }
      ...
      this.logger.error(
        "❌ [TaskController] Failed to sync manual tasks:",
        error,
      );
  ...
  async cleanManualTasks(req, res) {
    try {
      ...
      this.logger.info(
        `🧹 [TaskController] cleanManualTasks called for project: ${projectId}`,
      );

      // Use Application Service for manual tasks cleanup
      const result = await this.taskApplicationService.cleanManualTasks(
        projectId,
        userId,
      );

      this.logger.info(
        "✅ [TaskController] Manual tasks cleanup completed successfully",
      );

      res.success(result);
    } catch (error) {
      this.logger.error(
        "❌ [TaskController] Failed to clean manual tasks:",
        error,
      );
      res.error("Failed to clean manual tasks", 500, {
        details: error.message,
      });
    }
  }
  ...
  async analyzeProjectForTasks(req, res) {
    try {
      ...
      const result = await this.taskApplicationService.analyzeProjectForTasks(
        projectId,
        options,
      );

      this.logger.info(
        "✅ [TaskController] Project analysis completed successfully",
      );

      res.success(result);
    } catch (error) {
      this.logger.error(
        "❌ [TaskController] Failed to analyze project for tasks:",
        error,
      );
      res.error("Failed to analyze project for tasks", 500, {
        details: error.message,
      });
    }
  }
  ...
  async validateTaskConsistency(req, res) {
    try {
      ...
      const result = await this.taskApplicationService.validateTaskConsistency(
        taskId,
        { autoFix },
      );
      ...
      res.error("Failed to validate task consistency", 500, {
        details: error.message,
      });
    }
  }
  ...
  async getValidationStatistics(req, res) {
    try {
      ...
      res.error("Failed to get validation statistics", 500, {
        details: error.message,
      });
    }
  }
  ...
  async batchSyncTasks(req, res) {
    try {
      ...
      const result = await this.taskApplicationService.batchSyncTasks(taskIds, {
        autoFix,
      });
      ...
      res.success(result, 200, {
        meta: {
          message:
            "Batch synchronization completed for ${taskIds.length} tasks",
        },
      });
    } catch (error) {
  ...
  async enqueueTask(req, res) {
    try {
      ...
      if (req.body.taskId || (task && task.id)) {
            ...
            if (ideManager) {
              ...
              if (activeIDE && activeIDE.port) {
                ...
                const createChatHandler = new CreateChatHandler({
                  chatSessionService:
                    this.application?.chatSessionService ||
                    this.application?.getChatHistoryHandler,
                ...
                this.logger.info(
                  "TaskController: Creating new chat with timeout",
                );
                // Add timeout to prevent hanging
                const clickPromise = createChatHandler.handle(
                  createChatCommand,
                  {},
                  activeIDE.port,
                );
                const timeoutPromise = new Promise((_, reject) =>
                  setTimeout(
                    () => reject(new Error("New Chat creation timeout")),
                    10000,
                  ),
                );
                ...
                if (result && result.success) {
                  this.logger.info(
                    "TaskController: New chat created successfully",
                    {
                      port: activeIDE.port,
                ...
                this.logger.warn(
                  "TaskController: No active IDE found for New Chat",
                );
              }
            }
          } catch (error) {
        ...
        this.logger.info(
          "TaskController: New Chat process completed, proceeding to task execution",
        );

        // Queue the task for execution using TaskApplicationService
        try {
          const taskResult = await this.taskApplicationService.executeTask(
            taskId,
            projectId,
            userId,
            {
          ...
          res.error("Failed to queue task for execution", 500, {
            details: error.message,
          });
          return;
        }
      ...
      if (workflow === "task-check-state-workflow") {
        try {
          ...
          for (let i = 0; i < tasks.length; i++) {
            ...
            try {
              this.logger.info(
                `TaskController: Adding task ${i + 1}/${tasks.length} to check-state queue`,
                {
                  taskId: task.id,
              ...
              const queueResult = await this.taskApplicationService.executeTask(
                task.id,
                projectId,
                userId,
                {
              ...
              this.logger.info(
                `TaskController: Task ${i + 1} added to check-state queue`,
                {
                  taskId: task.id,
                  queueItemId: queueResult.queueItemId,
                  position: queueResult.position,
              ...
              this.logger.error(
                `TaskController: Failed to add task ${i + 1} to check-state queue`,
                {
                  taskId: task.id,
                  error: error.message,
                  projectId,
          ...
          this.logger.info(
            "TaskController: Task check-state workflow completed",
            {
          ...
          this.logger.error(
            "TaskController: Failed to execute task check-state workflow",
            {
              error: error.message,
              projectId,
              userId,
          ...
          return res.error(
            "Task check-state workflow failed: ${error.message}",
            500,
          );
        }
      }
      ...
      if (workflow === "task-review-workflow") {
        try {
          ...
          for (let i = 0; i < tasks.length; i++) {
            ...
            try {
              this.logger.info(
                `TaskController: Adding task ${i + 1}/${tasks.length} to review queue`,
                {
                  taskId: task.id,
                  taskTitle: task.title || task.name,
                  projectId,
              ...
              this.logger.info(
                `TaskController: Task ${i + 1} added to review queue`,
                {
                  taskId: task.id,
                  queueItemId: queueResult.queueItemId,
                  position: queueResult.position,
              ...
              this.logger.error(
                `TaskController: Failed to add task ${i + 1} to queue`,
                {
                  taskId: task.id,
                  error: error.message,
                  projectId,
      ...
      if (workflow === "task-create-workflow") {
        try {
          ...
          if (creationOptions.creationMode === "advanced" && !taskData.title) {
            throw new Error(
              "Task title is required for advanced task creation",
            );
          }
          ...
          if (creationOptions.autoExecute !== false) {
            try {
              ...
              this.logger.error(
                "TaskController: Failed to queue created task",
                {
                  taskId: createdTask.id,
              ...
              return res.success({
                message:
                  "Task created successfully but failed to queue for execution",
                data: {
                  task: createdTask,
                  workflowId: createdTask.id,
                  queueItemId: null,
          ...
          return res.error(
            "Task creation workflow failed: ${error.message}",
            500,
          );
        }
      }
