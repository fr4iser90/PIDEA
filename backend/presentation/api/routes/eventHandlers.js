...
class EventHandlers {
  ...
  setupIDEEvents() {
    ...
    this.eventBus.subscribe("activeIDEChanged", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting activeIDEChanged",
        );
      }
    });
    ...
    this.eventBus.subscribe("ideListUpdated", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting ideListUpdated",
        );
      }
    });
  }
  ...
  setupChatEvents() {
    ...
    this.eventBus.subscribe("ChatHistoryUpdated", (data) => {
      this.logger.info(
        "Chat history updated event:",
        "[REDACTED_HISTORY_DATA]",
      );
      if (this.webSocketManager) {
  ...
  setupQueueEvents() {
    this.eventBus.subscribe("queue:item:added", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting queue:item:added",
        );
      }
    });
    ...
    this.eventBus.subscribe("queue:item:updated", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting queue:item:updated",
        );
      }
    });
    ...
    this.eventBus.subscribe("queue:item:completed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting queue:item:completed",
        );
      }
    });
  }
  ...
  setupWorkflowEvents() {
    this.eventBus.subscribe("workflow:step:progress", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting workflow:step:progress",
        );
      }
    });
    ...
    this.eventBus.subscribe("workflow:step:completed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting workflow:step:completed",
        );
      }
    });
    ...
    this.eventBus.subscribe("workflow:step:failed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting workflow:step:failed",
        );
      }
    });
    ...
    this.eventBus.subscribe("analysis:completed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting analysis:completed",
        );
      }
    });
  }
  ...
  setupGitEvents() {
    this.eventBus.subscribe("git:checkout:completed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting git-branch-changed",
        );
      }
    });
    ...
    this.eventBus.subscribe("git:pull:completed", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "No WebSocket manager available for broadcasting git-status-updated",
        );
      }
    });
  ...
  setupTestEvents() {
    this.eventBus.subscribe("playwright:config:saved", (data) => {
      ...
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "WebSocket manager not available for playwright:config:saved broadcast",
        );
      }
    });
    ...
    this.eventBus.subscribe("playwright:config:failed", (data) => {
      this.logger.info(
        "Playwright config failed event:",
        "[REDACTED_ERROR_DATA]",
      );
      if (this.webSocketManager) {
        ...
        this.logger.warn(
          "WebSocket manager not available for playwright:config:failed broadcast",
        );
      }
    });
  }
  ...
  setupVersionEvents() {
    ...
    this.eventBus.subscribe("ai-version-analysis-completed", (data) => {
      this.logger.info(
        "AI version analysis completed event:",
        "[REDACTED_ANALYSIS_DATA]",
      );
      if (this.webSocketManager) {
        this.webSocketManager.broadcastToAll(
          "ai-version-analysis-completed",
          data,
        );
        this.logger.info(
          "Broadcasting ai-version-analysis-completed to all clients",
        );
      } else {
        this.logger.warn(
          "WebSocket manager not available for ai-version-analysis-completed broadcast",
        );
      }
    });
  }
