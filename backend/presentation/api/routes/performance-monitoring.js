...
function createPerformanceMonitoringRoutes(databaseConnection) {
  ...
  router.get(
    "/queries/top-by-time",
    controller.getTopQueriesByTime.bind(controller),
  );

  // GET /api/performance-monitoring/queries/most-frequent
  router.get(
    "/queries/most-frequent",
    controller.getMostFrequentQueries.bind(controller),
  );

  // DELETE /api/performance-monitoring/queries/history
  router.delete(
    "/queries/history",
    controller.clearQueryHistory.bind(controller),
  );
