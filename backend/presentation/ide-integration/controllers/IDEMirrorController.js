...
const StreamingController = require("../../system/controllers/StreamingController");
...
class IDEMirrorController {
  constructor(dependencies = {}) {
    ...
    if (!this.ideMirrorApplicationService) {
      throw new Error(
        "IDEMirrorController requires ideMirrorApplicationService dependency",
      );
    }
  ...
  initializeStreamingServices(registry) {
    try {
      ...
      if (webSocketManager) {
        webSocketManager.setScreenshotStreamingService(
  ...
  async getAvailableIDEs(req, res) {
    try {
      const userId = req.user?.id;
      const result =
        await this.ideMirrorApplicationService.getAvailableIDEs(userId);
  ...
  async clickElement(req, res) {
    try {
      ...
      const userId = req.user?.id;
      const result = await this.ideMirrorApplicationService.clickElement(
        selector,
        coordinates,
        userId,
      );
  ...
  async switchIDE(req, res) {
    try {
      ...
      const userId = req.user?.id;
      const result = await this.ideMirrorApplicationService.switchIDE(
        port,
        userId,
      );
      ...
      res.success(newState, 200, {
        meta: { message: "Switched to IDE on port ${port}" },
      });
    } catch (error) {
  ...
  async typeText(req, res) {
    try {
      ...
      const userId = req.user?.id;
      const result = await this.ideMirrorApplicationService.typeText(
        text,
        selector,
        userId,
      );
  ...
  async focusAndType(req, res) {
    try {
      ...
      const userId = req.user?.id;
      const result = await this.ideMirrorApplicationService.focusAndType(
        selector,
        text,
        clearFirst,
        userId,
  ...
  async sendChatMessage(req, res) {
    try {
      ...
      const userId = req.user?.id;
      const result = await this.ideMirrorApplicationService.sendChatMessage(
        message,
        userId,
      );
  ...
  handleWebSocketConnection(ws) {
    ...
    this.ideMirrorApplicationService
      .isIDEConnected()
      .then((result) => {
  ...
  async handleWebSocketType(ws, payload) {
    ...
    const result = await this.ideMirrorApplicationService.typeTextAdvanced(
      text,
      selector,
      key,
      modifiers,
  ...
  async handleWebSocketTypeBatch(ws, payload) {
    ...
    logger.info(
      `⚡ Processing batch: "${text}" (${text.length} chars) for ${selector}`,
    );
  ...
  async handleWebSocketRefresh(ws) {
    const result =
      await this.ideMirrorApplicationService.handleWebSocketInteraction(
        "refresh",
        {},
        ws.userId,
      );
  ...
  setupRoutes(app) {
    ...
    if (this.streamingController) {
      ...
      app.post("/api/ide-mirror/:port/stream/start", (req, res) =>
        this.streamingController.startStreaming(req, res),
      );
      app.post("/api/ide-mirror/:port/stream/stop", (req, res) =>
        this.streamingController.stopStreaming(req, res),
      );
      app.get("/api/ide-mirror/:port/stream/status", (req, res) =>
        this.streamingController.getPortStatus(req, res),
      );
      app.put("/api/ide-mirror/:port/stream/config", (req, res) =>
        this.streamingController.updatePortConfig(req, res),
      );
      app.post("/api/ide-mirror/:port/stream/pause", (req, res) =>
        this.streamingController.pauseStreaming(req, res),
      );
      app.post("/api/ide-mirror/:port/stream/resume", (req, res) =>
        this.streamingController.resumeStreaming(req, res),
      );

      // Global streaming routes
      app.get("/api/ide-mirror/stream/ports", (req, res) =>
        this.streamingController.getAllPorts(req, res),
      );
      app.get("/api/ide-mirror/stream/stats", (req, res) =>
        this.streamingController.getStats(req, res),
      );
      app.post("/api/ide-mirror/stream/stop-all", (req, res) =>
        this.streamingController.stopAllStreaming(req, res),
      );
      app.get("/api/ide-mirror/stream/health", (req, res) =>
        this.streamingController.healthCheck(req, res),
      );

      logger.info("Port-based streaming routes registered successfully");
    } else {
      logger.info(
        "Streaming controller not available, skipping streaming routes",
      );
    }
  }
}
