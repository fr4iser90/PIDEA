...
class TaskNotificationService {
  ...
  setupNotificationTemplates() {
    ...
    this.notificationTemplates.set("suggestion:generated", {
      ...
      message:
        "{count} new AI suggestions have been generated for your project",
      type: "info",
      icon: "ai",
      priority: "normal",
    });
    ...
    this.notificationTemplates.set("suggestion:applied", {
      ...
      message:
        'AI suggestion "{suggestionTitle}" has been applied successfully',
      type: "success",
      icon: "check",
      priority: "normal",
    });
    ...
    this.notificationTemplates.set("auto:complete", {
      ...
      message:
        'VibeCoder auto mode has completed successfully for project "{projectName}"',
      type: "success",
      icon: "check",
      priority: "normal",
    });
    ...
    this.notificationTemplates.set("auto:error", {
      ...
      message:
        'VibeCoder auto mode has failed for project "{projectName}": {error}',
      type: "error",
      icon: "error",
      priority: "high",
    });
  ...
  createNotification(type, data) {
    try {
      ...
      if (!template) {
        this.logger.warn(
          "TaskNotificationService: No template found for notification type",
          { type },
        );
        return;
      }
      ...
      this.logger.error(
        "TaskNotificationService: Failed to create notification",
        {
          type,
          error: error.message,
        },
  ...
  sendNotificationToRecipients(notification, recipients) {
    try {
      ...
      this.logger.error(
        "TaskNotificationService: Failed to send notifications",
        {
          notificationId: notification.id,
          error: error.message,
        },
  ...
  storeUserNotification(userId, notification) {
    try {
      ...
      this.logger.debug(
        "TaskNotificationService: Notification stored for user",
        {
          userId,
          notificationId: notification.id,
        },
      ...
      this.logger.error(
        "TaskNotificationService: Failed to store user notification",
        {
          userId,
          notificationId: notification.id,
          error: error.message,
  ...
  subscribeUser(userId, types) {
    try {
      ...
      this.logger.info(
        "TaskNotificationService: User subscribed to notifications",
        {
          userId,
          types,
        },
  ...
  unsubscribeUser(userId, types) {
    try {
      ...
      this.logger.info(
        "TaskNotificationService: User unsubscribed from notifications",
        {
          userId,
          types,
        },
  ...
  markNotificationAsRead(userId, notificationId) {
    try {
      ...
      this.logger.debug(
        "TaskNotificationService: Notification marked as read",
        {
          userId,
          notificationId,
        },
      ...
      this.logger.error(
        "TaskNotificationService: Failed to mark notification as read",
        {
          userId,
          notificationId,
          error: error.message,
  ...
  dismissNotification(userId, notificationId) {
    try {
      ...
      this.logger.error(
        "TaskNotificationService: Failed to dismiss notification",
        {
          userId,
          notificationId,
          error: error.message,
