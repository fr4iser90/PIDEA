...
async function detectPIDEAProjectInfo() {
  try {
    ...
    let projectInfo = {
      ...
      languages: ['javascript', 'jsx', 'css', 'sass', 'sql', 'markdown', 'json', 'yaml', 'shell', 'dockerfile'],
      packageManager: 'npm',
      status: 'active',
      priority: 1,
      createdBy: 'me',
    ...
    try {
      ...
      if (dependencies['@playwright/test']) techStack.push('Playwright Testing');
      ...
      if (detectedLanguages.length > 0) {
        projectInfo.languages = [...new Set([...projectInfo.languages, ...detectedLanguages])];
        projectInfo.metadata = {
          ...projectInfo.metadata,
    ...
    try {
      const backendPackageJsonPath = path.join(workspacePath, 'backend', 'package.json');
      const backendPackageJsonContent = await fs.readFile(backendPackageJsonPath, 'utf8');
    ...
    try {
      const frontendPackageJsonPath = path.join(workspacePath, 'frontend', 'package.json');
      const frontendPackageJsonContent = await fs.readFile(frontendPackageJsonPath, 'utf8');
  ...
  try {
    ...
    try {
      ...
      logger.info('🔍 Check by path result:', { 
        ...
        checkByPathResultHasRows: !!(checkByPathResult && checkByPathResult.rows),
        ...
        if (pathRows && pathRows.length > 0) {
          ...
          if (existingProject.workspace_path !== correctWorkspacePath) {
            ...
            await databaseConnection.execute(
              ...
              [correctWorkspacePath, new Date().toISOString(), existingProject.id]
            );
