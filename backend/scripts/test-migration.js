...
function migrateContent(content) {
  ...
  // Pattern 1: res.json({ success: true, data: ... })
  content = content.replace(
    /res\.json\(\s*\{\s*success:\s*true\s*,\s*data:\s*([^}]+)\s*\}\s*\)/g,
    'res.success($1)'
  );

  // Pattern 2: res.status(201).json({ success: true, data: ... })
  content = content.replace(
    /res\.status\(\s*201\s*\)\.json\(\s*\{\s*success:\s*true\s*,\s*data:\s*([^}]+)\s*\}\s*\)/g,
    'res.created($1)'
  );

  // Pattern 3: res.status(500).json({ success: false, error: ... })
  content = content.replace(
    /res\.status\(\s*500\s*\)\.json\(\s*\{\s*success:\s*false\s*,\s*error:\s*['"`]([^'"`]+)['"`]\s*\}\s*\)/g,
    'res.error(\'$1\', 500)'
  );

  // Pattern 4: res.status(400).json({ success: false, error: ... })
  content = content.replace(
    /res\.status\(\s*400\s*\)\.json\(\s*\{\s*success:\s*false\s*,\s*error:\s*['"`]([^'"`]+)['"`]\s*\}\s*\)/g,
    'res.badRequest(\'$1\')'
  );

  // Pattern 5: res.status(404).json({ success: false, error: ... })
  content = content.replace(
