...
class TaskProgressUI extends EventEmitter {
  ...
  displayRealTimeStats(stats) {
    ...
    logger.info(
      chalk.gray(
        `📊 Tasks: ${stats.activeTasks}/${stats.totalTasks} | Executions: ${stats.activeExecutions}/${stats.totalExecutions} | Progress: ${progressBar} ${Math.round(stats.avgProgress)}%`,
      ),
    );
  }
  ...
  displaySessionSummary() {
    ...
    logger.info(
      chalk.gray(`   Completed: ${chalk.green(stats.completedTasks)}`),
    );
    ...
    logger.info(
      chalk.gray(`   Active: ${chalk.yellow(stats.activeExecutions)}`),
    );
    logger.info(
      chalk.gray(`   Completed: ${chalk.green(stats.completedExecutions)}`),
    );
  ...
  showTaskDetails(taskId) {
    ...
    logger.info(
      chalk.gray(
        `Created: ${chalk.white(new Date(task.createdAt).toLocaleString())}`,
      ),
    );
    ...
    if (executions.length > 0) {
      ...
      executions.forEach((execution) => {
        ...
        logger.info(
          chalk.gray(
            `   ${statusIcon} ${execution.id.substring(0, 8)} - ${execution.status}`,
          ),
        );
      });
    }
  ...
  showExecutionDetails(executionId) {
    ...
    logger.info(
      chalk.gray(`Progress: ${chalk.white(execution.progress || 0)}%`),
    );
    logger.info(
      chalk.gray(
        `Current Step: ${chalk.white(execution.currentStep || "N/A")}`,
      ),
    );
    logger.info(
      chalk.gray(
        `Started: ${chalk.white(new Date(execution.startedAt).toLocaleString())}`,
      ),
    );
    ...
    if (execution.completedAt) {
      logger.info(
        chalk.gray(
          `Completed: ${chalk.white(new Date(execution.completedAt).toLocaleString())}`,
        ),
      );
    }
  ...
  showLiveDashboard() {
    ...
    const updateDashboard = () => {
      ...
      logger.info(
        chalk.gray(
          `   Progress: ${taskProgressBar} ${stats.completedTasks}/${stats.totalTasks}`,
        ),
      );
      logger.info(
        chalk.gray(
          `   Active: ${chalk.yellow(stats.activeTasks)} | Completed: ${chalk.green(stats.completedTasks)} | Failed: ${chalk.red(stats.failedTasks)}`,
        ),
      );

      // Executions overview
      logger.info(chalk.blue("\n▶️  Executions Overview:"));
      const executionProgressBar = this.createMiniProgressBar(
        stats.avgProgress,
      );
      logger.info(
        chalk.gray(
          `   Progress: ${executionProgressBar} ${Math.round(stats.avgProgress)}%`,
        ),
      );
      logger.info(
        chalk.gray(
          `   Active: ${chalk.yellow(stats.activeExecutions)} | Completed: ${chalk.green(stats.completedExecutions)} | Failed: ${chalk.red(stats.failedExecutions)}`,
        ),
      );
  ...
  displayRecentActivity() {
    ...
    if (recentExecutions.length > 0) {
      ...
      recentExecutions.forEach((execution) => {
        ...
        logger.info(
          chalk.gray(`   ${statusIcon} ${execution.taskTitle} - ${timeAgo}`),
        );
      });
    }
  }
