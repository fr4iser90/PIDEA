...
class TaskCommands {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const result = await this.aiService.optimizeCode(content, {
          ...
          requirements: options.performance
            ? ["improve performance", "reduce complexity"]
            : ["improve quality", "enhance maintainability"],
        });
  ...
  async securityAnalysis(target, options = {}) {
    try {
      ...
      const securityResult = await this.aiService.performSecurityAnalysis(
        projectData,
        {
          aiModel: "gpt-4",
        },
  ...
  async executeSequentialTasks(options) {
    try {
      ...
      // Initialize services
      const workflowOrchestrationService = this.serviceContainer.get(
      ...
      logger.info(
        `📈 Average Duration: ${Math.round(result.averageDuration / 1000)}s per task`,
      );
  ...
  async getTasksFromTestReports(options) {
    ...
    try {
      ...
      const testReportFullPath =
        options.testReportFullPath || "test-report-full.md";
  ...
  parseTasksFromTestReportFull(testReportFull) {
    ...
    const failingTestsSection = testReportFull.match(
      /❌ Failing Tests.*?(?=##|$)/s,
    );
    if (failingTestsSection) {
      const failingTestLines = failingTestsSection[0].match(
        /\| `([^`]+)` \| `([^`]+)` \| `([^`]+)` \|/g,
      );
  ...
  parseTasksFromCoverageReport(coverageReport) {
    ...
    const lowCoverageMatch = coverageReport.match(
      /Files with ≥80% Coverage.*?(\d+)/s,
    );
    if (lowCoverageMatch) {
      const lowCoverageCount = parseInt(lowCoverageMatch[1]);

      // Find files with low coverage
      const fileLines = coverageReport.match(
        /\| `([^`]+)` \| \d+% \| \d+% \| \d+% \| \d+% \| ❌ \|/g,
      );
  ...
  async monitorTaskExecution(executionId) {
    ...
    while (!completed) {
      try {
        const result = await this.queryBus.execute("GetTaskExecutionQuery", {
          executionId,
        });
  ...
  async detectTestFramework(targetPath) {
    try {
      ...
      if (files.includes("pytest.ini") || files.includes("requirements.txt"))
        return "pytest";
  ...
  displayTaskExecution(result) {
    ...
    logger.info(
      chalk.gray(`Execution ID: ${chalk.white(result.execution.id)}`),
    );
  ...
  displayScriptInfo(result) {
    ...
    logger.info(
      chalk.gray(`Target: ${chalk.white(result.requirements.target)}`),
    );
    logger.info(
      chalk.gray(
        `Generated: ${chalk.white(new Date(result.timestamp).toLocaleString())}`,
      ),
    );
  }
  ...
  displayRefactoringResults(results, options) {
    ...
    results.forEach((result, index) => {
      ...
      logger.info(
        chalk.gray(`   Recommendations: ${result.recommendations.length}`),
      );
    });
  }
  ...
  displaySecurityResults(results, options) {
    ...
    logger.info(
      chalk.gray(`Vulnerabilities found: ${results.vulnerabilities.length}`),
    );
    logger.info(
      chalk.gray(`Recommendations: ${results.recommendations.length}`),
    );
  }
  ...
  displayDeploymentResults(results) {
    ...
    logger.info(
      chalk.gray(
        `Status: ${chalk.white(results.success ? "Success" : "Failed")}`,
      ),
    );
    if (results.url) {
  ...
  displaySystemStats(stats) {
    ...
    logger.info(
      chalk.gray(`AI Models: ${chalk.white(stats.ai.models.length)}`),
    );
    logger.info(
      chalk.gray(
        `Active Executions: ${chalk.white(stats.execution.activeExecutions)}`,
      ),
    );
  }
  ...
  displayHealthStatus(healthChecks) {
    ...
    Object.entries(healthChecks).forEach(([service, health]) => {
      const status =
        health.status === "healthy" ? chalk.green("✅") : chalk.red("❌");
  ...
  displayCleanupResults(results) {
    logger.info(chalk.blue("\n🧹 Cleanup Results:"));
    logger.info(
      chalk.gray(`Deleted Tasks: ${chalk.white(results.deletedTasks)}`),
    );
    logger.info(
      chalk.gray(
        `Deleted Executions: ${chalk.white(results.deletedExecutions)}`,
      ),
    );
