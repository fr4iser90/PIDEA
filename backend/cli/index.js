...
class TaskCLIMain {
  ...
  async runAutoMode(args) {
    ...
    try {
      ...
      if (options.dryRun) {
        ...
        const analysis = await this.application.commandBus.execute(
      ...
      const result = await this.application.commandBus.execute(
        "AutoModeCommand",
        command,
      );
  ...
  displayAutoModePreview(analysis, projectPath) {
    ...
    logger.info(
      chalk.gray(
        `   Type: ${chalk.white(analysis.projectStructure?.type || "Unknown")}`,
      ),
    );
    logger.info(
      chalk.gray(
        `   Files: ${chalk.white(analysis.projectStructure?.files?.length || 0)}`,
      ),
    );
  ...
  displaySequentialModeHelp() {
    ...
    logger.info(
      "  --timeout, -t <seconds>  Timeout per task in seconds (default: 300)",
    );
    logger.info(
      "  --no-commit              Disable auto-commit after each task",
    );
    ...
    logger.debug(
      "  node cli/index.js sequential --from-test-reports --project /path/to/project",
    );
  }
  ...
  displaySequentialModeResults(result) {
    ...
    logger.info(
      `  Total Duration: ${chalk.yellow(Math.round(result.totalDuration / 1000))}s`,
    );
    logger.info(
      `  Average Duration: ${chalk.yellow(Math.round(result.averageDuration / 1000))}s per task`,
    );
    ...
    for (const demoTask of demoTasks) {
      ...
      for (let progress = 0; progress <= 100; progress += 20) {
        await new Promise((resolve) =>
          setTimeout(resolve, demoTask.duration / 5),
        );
