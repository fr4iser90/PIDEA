...
class MockResponseProcessor {
        ...
        try {
          ...
          if (this.selectors.codeBlockContent) {
            const contentElements = await block.$$(
              this.selectors.codeBlockContent,
            );
          ...
          if (this.selectors.codeBlockLanguage) {
            try {
              const langElements = await block.$$(
                this.selectors.codeBlockLanguage,
              );
  ...
  async extractAIResponse(page) {
    try {
      ...
      const strategies = [
        ...
        async () => {
          if (this.selectors?.aiMessages) {
            ...
            console.log(
              `🔍 [MockResponseProcessor] Found ${messages.length} AI messages`,
            );
            if (messages && messages.length > 0) {
              ...
              for (let i = messages.length - 1; i >= 0; i--) {
                ...
                console.log(
                  `🔍 [MockResponseProcessor] Checking message ${i}: "${text.substring(0, 50)}..."`,
                );

                if (text && this.isValidAIResponse(text)) {
                  console.log(
                    `✅ [MockResponseProcessor] Valid AI response found in message ${i}`,
                  );
                  return text;
                }
              }
              console.log(
                `🚫 [MockResponseProcessor] No valid AI response found in any message`,
              );
            }
          }
          return null;
      ...
      for (const strategy of strategies) {
        try {
          ...
          if (result && result.length > 0) {
            console.log(
              `📋 Strategy erfolgreich: ${result.length} characters extrahiert`,
            );
            return result;
          }
        } catch (error) {
      ...
      console.log(
        `❌ Alle Extrakions-Strategien fehlgeschlagen: ${error.message}`,
      );
      return null;
    }
  }
  ...
  isValidAIResponse(text) {
    ...
    for (const pattern of errorPatterns) {
      if (trimmed.includes(pattern)) {
        console.log(
          `🚫 [MockResponseProcessor] Filtered out error message: ${pattern}`,
        );
        return false;
      }
    }
...
async function testIDEMessageSending() {
  ...
  try {
    ...
    if (!activePort) {
      ...
      console.log(
        "   Please start Cursor, VSCode, or Windsurf with --remote-debugging-port=9222",
      );
      process.exit(1);
    }
    ...
    const versionResult = await versionDetectionService.detectVersion(
      activePort,
      ideType,
    );
    ...
    if (!versionResult.isKnownVersion) {
      ...
      console.log(
        "   Selectors may not be available. Please run selector collection bot first.",
      );
      process.exit(1);
    }
    ...
    const selectors = await jsonSelectorManager.getSelectors(
      versionResult.ideType,
    ...
    // Choose test mode
    const testMode = process.argv.includes("--monitor")
      ? "monitor"
      : process.argv.includes("--fulltest")
        ? "fulltest"
        : "basic";

    if (testMode === "monitor") {
      // Send eine Test-Nachricht im Monitor-Modus
      await sendTestMessageAndStartMonitoring(
        page,
        responseProcessor,
        versionResult,
        selectors,
...
async function sendTestMessageAndStartMonitoring(
  page,
  responseProcessor,
  versionResult,
  selectors,
...
async function runResponseMonitoring(page, responseProcessor, versionResult) {
  ...
  console.log(
    `🎯 Target: ${versionResult.ideType} ${versionResult.currentVersion}`,
  );
  ...
  const monitorInterval = setInterval(async () => {
    ...
    try {
      ...
      if (
        response &&
        response.length > 0 &&
        response.length === lastResponseLength &&
        timeSinceLastChange > 15000
      ) {
        console.log(
          `[${new Date().toLocaleTimeString()}] 🛑 SMART EXIT: Response unchanged for ${Math.round(timeSinceLastChange / 1000)}s`,
        );
        console.log(
          `[${new Date().toLocaleTimeString()}] 📊 Final Response Stats:`,
        );
        console.log(`   ✓ Length: ${response.length} chars`);
        console.log(
          `   ✓ Stable for: ${Math.round(timeSinceLastChange / 1000)} seconds`,
        );
      ...
      if (
        (isTyping && timeSinceLastLog > 2000) ||
        (!isTyping && response && timeSinceLastLog > 5000)
      ) {
        const timestamp = new Date().toLocaleTimeString();

        if (isTyping) {
          console.log(
            `[${timestamp}] ⌨️  🤖 AI TIPPEN ERKANNT! Generating Response...`,
          );
          lastLogTime = now;
        } else if (response && response.length > 0) {
          console.log(
            `[${timestamp}] 📝 📨 AI RESPONSE: ${response.length} chars`,
          );

          // Vollständige Analyse der Response
          console.log(`[${timestamp}] 📄 FULL Content Analysis:`);
          console.log(
            `[${timestamp}] 📊 Text content: "${response.substring(0, Math.min(200, response.length))}${response.length > 200 ? "..." : ""}"`,
          );
          ...
          try {
            ...
            terminalBlocks =
              await responseProcessor.extractTerminalBlocks(page);
            fileReferences =
              await responseProcessor.extractFileReferences(page);
            urls = await responseProcessor.extractUrls(page);
            structuredData =
              await responseProcessor.extractStructuredData(page);
            listsAndTables =
              await responseProcessor.extractListsAndTables(page);

            if (codeBlocks.length > 0) {
              console.log(
                `[${timestamp}] 🔷 CODE BLOCKS DETECTED: ${codeBlocks.length} blocks`,
              );
              codeBlocks.forEach((block, i) => {
                console.log(
                  `[${timestamp}] 🔷 Block ${i + 1}: ${block.language} (${block.content.length} chars)`,
                );
                console.log(
                  `[${timestamp}] 🔷 Content: "${block.content.substring(0, Math.min(100, block.content.length))}${block.content.length > 100 ? "..." : ""}"`,
                );
              });
            } else {
            ...
            if (inlineCode.length > 0) {
              console.log(
                `[${timestamp}] 📌 INLINE CODE: ${inlineCode.length} snippets`,
              );
              inlineCode.forEach((snippet, i) => {
                console.log(
                  `[${timestamp}] 📌 Snippet ${i + 1}: "${snippet.content}"`,
                );
              });
            } else {
            ...
            if (terminalBlocks.length > 0) {
              console.log(
                `[${timestamp}] 🖥️ TERMINAL BLOCKS: ${terminalBlocks.length} blocks`,
              );
              terminalBlocks.forEach((block, i) => {
                console.log(
                  `[${timestamp}] 💻 Terminal ${i + 1}: "${block.content.substring(0, Math.min(100, block.content.length))}${block.content.length > 100 ? "..." : ""}"`,
                );
              });
            } else {
            ...
            if (fileReferences.length > 0) {
              console.log(
                `[${timestamp}] 📁 FILE REFERENCES: ${fileReferences.length} files`,
              );
              fileReferences.forEach((file, i) => {
                console.log(
                  `[${timestamp}] 📄 File ${i + 1}: "${file.content}"`,
                );
              });
            } else {
            ...
            if (totalStructured > 0) {
              console.log(
                `[${timestamp}] 📊 STRUCTURED DATA: ${totalStructured} blocks`,
              );
              Object.entries(structuredData).forEach(([type, blocks]) => {
                if (blocks.length > 0) {
                  console.log(
                    `[${timestamp}] 📋 ${type.toUpperCase()}: ${blocks.length} blocks`,
                  );
                }
              });
            } else {
            ...
            if (totalLists > 0 || totalTables > 0) {
              console.log(
                `[${timestamp}] 📋 LISTS & TABLES: ${totalLists} lists, ${totalTables} tables`,
              );
            } else {
            ...
            console.log(
              `[${timestamp}] ⚠️ Error in enhanced code detection: ${error.message}`,
            );
            ...
            if (codeBlockMatches) {
              console.log(
                `[${timestamp}] 🔷 CODE BLOCKS (FALLBACK): ${codeBlockMatches.length} blocks`,
              );
              codeBlockMatches.forEach((block, i) => {
                const lines = block.split("\n");
                const lang = lines[0].replace("```", "").trim();
                console.log(
                  `[${timestamp}] 🔷 Block ${i + 1}: ${lang} (${block.length} chars)`,
                );
              });
            } else {
            ...
            if (inlineMatches) {
              console.log(
                `[${timestamp}] 📌 INLINE CODE (FALLBACK): ${inlineMatches.length} snippets`,
              );
          ...
          console.log(
            `[${timestamp}] ⚪ 🔍 Monitoring aktiv - keine AI Activity detected...`,
          );
          lastLogTime = now;
        }
      }
...
async function monitorAndAnalyzeResponse(page, responseProcessor) {
  ...
  console.log(
    "🔍 Monitoring response for 5 minutes (AI kann lange brauchen!)...",
  );
  ...
  while (Date.now() - startTime < 300000) {
    try {
      ...
      if (response && response.length > 0) {
        if (!responseReceived) {
          console.log(
            `📝 Initial response detected: ${response.length} characters`,
          );
        ...
        if (response.length !== lastLength) {
          ...
          stableCount++;
          console.log(
            `⏸️  Response stable (${stableCount}/3) - seit ${Math.round((Date.now() - lastStableTime) / 1000)}s`,
          );
        }
        ...
        const isComplete = await responseProcessor.detectResponseComplete(
          page,
          response,
          lastLength,
        );
        ...
        if (isComplete || (!isTyping && stableDuration > 10)) {
          console.log(
            "✅ Response vollständig! AI hat aufgehört zu tippen + Text stabil lange genug!",
          );
...
if (process.argv.includes("--help")) {
  console.log("📖 Usage:");
  console.log(
    "  node backend/cli/test-ide-message-sending.js           # Basic test",
  );
  console.log(
    "  node backend/cli/test-ide-message-sending.js --fulltest # Full AI response test",
  );
  console.log(
    "  node backend/cli/test-ide-message-sending.js --monitor   # Response monitoring mode",
  );
  console.log(
    "  node backend/cli/test-ide-message-sending.js --help       # This help",
  );
  process.exit(0);
}
