...
class LayerViolationCommands {
  ...
  async fixViolations(options) {
    try {
      ...
      const analysisResult = await this.commandBus.execute(
        "AnalyzeLayerViolationsCommand",
        analyzeCommand,
      );
      ...
      const fixes = analysisResult.analysis.data.fixes.filter(
        (fix) =>
      ...
      spinner.succeed(
        `Layer violation fixes completed (${fixes.length} fixes)`,
      );
    } catch (error) {
  ...
  displayAnalysisResults(result) {
    ...
    console.log(
      `- Critical Fixes: ${data.fixes.filter((f) => f.priority === "critical").length}`,
    );
    console.log(
      `- High Priority Fixes: ${data.fixes.filter((f) => f.priority === "high").length}`,
    );
    console.log(
      `- Medium Priority Fixes: ${data.fixes.filter((f) => f.priority === "medium").length}`,
    );
  ...
  displayFixPlan(fixes, options) {
    ...
    console.log(
      `   layer-violation fix --auto-fix --priority ${options.priority}`,
    );
  ...
  async applyFixes(fixes, options) {
    ...
    fixes.forEach((fix, index) => {
      logger.info(
        `Applying fix ${index + 1}: ${path.basename(fix.file, ".js")}`,
      );
