...
class GenerateScriptsCommand {
  ...
  validateBusinessRules() {
    ...
    if (this.options.scriptTypes && !Array.isArray(this.options.scriptTypes)) {
      ...
      const invalidTypes = this.options.scriptTypes.filter(
        (type) => !validScriptTypes.includes(type),
      );
      if (invalidTypes.length > 0) {
        errors.push(
          `Invalid script types: ${invalidTypes.join(", ")}. Valid types: ${validScriptTypes.join(", ")}`,
        );
      }
    }
