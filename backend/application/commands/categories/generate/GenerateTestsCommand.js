...
class GenerateTestsCommand {
  ...
  validateBusinessRules() {
    ...
    if (
      this.options.testFramework &&
      !validFrameworks.includes(this.options.testFramework)
    ) {
      errors.push(
        `Invalid test framework. Must be one of: ${validFrameworks.join(", ")}`,
      );
    }

    if (
      this.options.coverageTarget &&
      (this.options.coverageTarget < 0 || this.options.coverageTarget > 100)
    ) {
