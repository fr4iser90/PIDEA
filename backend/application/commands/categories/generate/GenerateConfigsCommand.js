...
class GenerateConfigsCommand {
  ...
  validateBusinessRules() {
    ...
    if (this.options.configTypes && !Array.isArray(this.options.configTypes)) {
      ...
      const invalidTypes = this.options.configTypes.filter(
        (type) => !validConfigTypes.includes(type),
      );
      if (invalidTypes.length > 0) {
        errors.push(
          `Invalid config types: ${invalidTypes.join(", ")}. Valid types: ${validConfigTypes.join(", ")}`,
        );
      }
    }
  ...
  getGeneratePriority() {
    ...
    if (
      options.configTypes.includes("docker") ||
      options.configTypes.includes("ci")
    ) {
      return "high";
