...
class AutoRefactorCommand {
  ...
  validate() {
    ...
    if (
      !["complex_tests", "legacy_tests", "slow_tests", "all"].includes(
        this.refactorType,
      )
    ) {
  ...
  validateBusinessRules() {
    ...
    if (this.timeout > 3600000 && this.priority === "critical") {
      errors.push(
        "Critical priority tasks should not have timeout longer than 1 hour",
      );
    }

    if (this.maxConcurrent > 5 && this.refactorType === "complex_tests") {
      errors.push(
        "Complex test refactoring should not exceed 5 concurrent operations",
      );
    }
  ...
  removeDependency(dependency) {
    if (this.metadata.dependencies) {
      this.metadata.dependencies = this.metadata.dependencies.filter(
        (d) => d !== dependency,
      );
      this.updatedAt = new Date();
    }
  ...
  static validateRefactorType(refactorType) {
    return ["complex_tests", "legacy_tests", "slow_tests", "all"].includes(
      refactorType,
    );
  }
