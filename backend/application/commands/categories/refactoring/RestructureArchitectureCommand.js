...
class RestructureArchitectureCommand {
  ...
  validateBusinessRules() {
    ...
    if (this.timeout > 1800000) {
      // 30 minutes
      warnings.push(
        "Architecture restructuring timeout is very high (over 30 minutes)",
      );
    }
    ...
    if (
      this.options.architecturePattern &&
      !validPatterns.includes(this.options.architecturePattern)
    ) {
      errors.push(
        `Invalid architecture pattern. Must be one of: ${validPatterns.join(", ")}`,
      );
    }

    const validStrategies = ["gradual", "complete", "hybrid"];
    if (
      this.options.restructureStrategy &&
      !validStrategies.includes(this.options.restructureStrategy)
    ) {
      errors.push(
        `Invalid restructure strategy. Must be one of: ${validStrategies.join(", ")}`,
      );
    }
