...
class OrganizeModulesCommand {
  ...
  validateBusinessRules() {
    ...
    if (this.timeout > 1200000) {
      // 20 minutes
      warnings.push(
        "Module organization timeout is very high (over 20 minutes)",
      );
    }
    ...
    if (
      this.options.organizationStrategy &&
      !validStrategies.includes(this.options.organizationStrategy)
    ) {
      errors.push(
        `Invalid organization strategy. Must be one of: ${validStrategies.join(", ")}`,
      );
    }
