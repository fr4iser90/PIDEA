...
class GetIDESelectorsCommand {
  ...
  validateParams(params) {
    ...
    if (
      params.selectorType &&
      !["chat", "terminal", "file-explorer", "command-palette", "all"].includes(
        params.selectorType,
      )
    ) {
      errors.push(
        "Selector type must be one of: chat, terminal, file-explorer, command-palette, all",
      );
    }

    if (errors.length > 0) {
      throw new Error(
        `GetIDESelectorsCommand validation failed: ${errors.join(", ")}`,
      );
    }
  }
  ...
  async validate() {
    try {
      ...
      if (
        this.selectorType &&
        this.selectorType !== "all" &&
        !["chat", "terminal", "file-explorer", "command-palette"].includes(
          this.selectorType,
        )
      ) {
        return {
