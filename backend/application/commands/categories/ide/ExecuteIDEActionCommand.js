...
class ExecuteIDEActionCommand {
  ...
  validateParams(params) {
    ...
    if (
      params.actionType &&
      !["keyboard", "menu", "command"].includes(params.actionType)
    ) {
    ...
    if (errors.length > 0) {
      throw new Error(
        `ExecuteIDEActionCommand validation failed: ${errors.join(", ")}`,
      );
    }
  }
    ...
    try {
      ...
      if (!validActions.includes(this.action)) {
        return {
          isValid: false,
          errors: [
            `Action '${this.action}' is not supported. Valid actions: ${validActions.join(", ")}`,
          ],
        };
      }
