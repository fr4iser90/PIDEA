...
class HandlerAudit {
  ...
  cleanupSecurityEvents() {
    ...
    if (this.securityEvents.length > this.options.maxSecurityEvents) {
      this.securityEvents = this.securityEvents.slice(
        -this.options.maxSecurityEvents,
      );
    }
  }
  ...
  detectSuspiciousActivity(data) {
    ...
    const recentFailures = recentRequests.filter(
      (record) => record.type === "HANDLER_FAILURE",
    );
  ...
  async checkComplianceViolations(auditRecord) {
    try {
      ...
      logger.error(
        "HandlerAudit: Compliance violation check failed",
        error.message,
      );
    }
  }
  ...
  getAuditLog(filters = {}) {
    ...
    if (filters.handlerId) {
      filteredLog = filteredLog.filter(
        (record) => record.handlerId === filters.handlerId,
      );
    }
    ...
    if (filters.userId) {
      filteredLog = filteredLog.filter(
        (record) => record.data.userId === filters.userId,
      );
    }

    if (filters.startDate) {
      filteredLog = filteredLog.filter(
        (record) => record.timestamp >= filters.startDate,
      );
    }

    if (filters.endDate) {
      filteredLog = filteredLog.filter(
        (record) => record.timestamp <= filters.endDate,
      );
    }

    if (filters.riskLevel) {
      filteredLog = filteredLog.filter(
        (record) => record.security.riskLevel === filters.riskLevel,
      );
    }
  ...
  getSecurityEvents(filters = {}) {
    ...
    if (filters.handlerId) {
      filteredEvents = filteredEvents.filter(
        (event) => event.data.handlerId === filters.handlerId,
      );
    }

    if (filters.userId) {
      filteredEvents = filteredEvents.filter(
        (event) => event.data.userId === filters.userId,
      );
    }
  ...
  getComplianceRecords(filters = {}) {
    ...
    if (filters.severity) {
      filteredRecords = filteredRecords.filter(
        (record) => record.severity === filters.severity,
      );
    }
