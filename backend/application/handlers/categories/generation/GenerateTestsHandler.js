...
class GenerateTestsHandler {
  ...
  async handle(command) {
    ...
    try {
      ...
      const testableComponents = await this.identifyTestableComponents(
        command.projectPath,
        projectStructure,
      );
      ...
      if (options.generateUnitTests) {
        unitTestResults = await this.generateUnitTests(
          command.projectPath,
          testableComponents,
          options,
        );
      ...
      if (options.generateIntegrationTests) {
        integrationTestResults = await this.generateIntegrationTests(
          command.projectPath,
          testableComponents,
          options,
        );
      ...
      if (options.generateE2ETests) {
        e2eTestResults = await this.generateE2ETests(
          command.projectPath,
          testableComponents,
          options,
        );
      ...
      if (options.testFramework) {
        configResults = await this.createTestConfiguration(
          command.projectPath,
          options,
        );
      }
      ...
      if (options.includeMocks) {
        mockResults = await this.createMocksAndFixtures(
          command.projectPath,
          testableComponents,
        );
      }
      ...
      if (options.testFramework) {
        validationResults = await this.validateGeneratedTests(
          command.projectPath,
          options,
        );
      }
      ...
      this.logger.error(
        `Test generation failed for project ${command.projectPath}:`,
        error,
      );
  ...
  calculateTestPriority(component) {
    if (
      ...
      component.functions.length > 10 ||
      component.classes.length > 2
    ) {
      return "high";
    } else if (
      component.complexity > 10 ||
      component.functions.length > 5 ||
      component.classes.length > 1
    ) {
      return "medium";
    } else {
      return "low";
    ...
    try {
      ...
      for (const priority of ["high", "medium", "low"]) {
        for (const component of testableComponents[priority]) {
          try {
            const testFile = await this.generateUnitTestFile(
              component,
              options,
            );
            const testPath = path.join(
              testDir,
              `${path.basename(component.path, path.extname(component.path))}.test.js`,
            );
  ...
  async createTestConfiguration(projectPath, options) {
    ...
    try {
      ...
      const configPath = path.join(
        projectPath,
        this.getConfigFileName(framework),
      );
  ...
  async validateGeneratedTests(projectPath, options) {
    ...
    try {
      ...
      results.metrics = {
        ...
        averageTestsPerFile:
          testFiles.length > 0
            ? (await this.countTotalTestCases(testFiles)) / testFiles.length
            : 0,
      };
    } catch (error) {
      results.valid = false;
  ...
  async generateOutput(data) {
    ...
    const output = {
      ...
      summary: {
        ...
        integrationTestsGenerated:
          integrationTestResults?.generated?.length || 0,
