...
class EventPublishingService {
  ...
  async publishScriptGenerationStartedEvent(execution, command) {
    try {
      ...
      this.logger.warn(
        "EventPublishingService: Failed to publish script generation started event",
        {
          executionId: execution.id,
          error: error.message,
        },
  ...
  async publishScriptGenerationCompletedEvent(execution, result, command) {
    try {
      ...
      this.logger.warn(
        "EventPublishingService: Failed to publish script generation completed event",
        {
          executionId: execution.id,
          error: error.message,
        },
  ...
  async publishScriptGenerationFailedEvent(error, command) {
    try {
      ...
      this.logger.warn(
        "EventPublishingService: Failed to publish script generation failed event",
        {
