...
class TaskManagementService {
  ...
  async createScriptTask(command, handlerId) {
    try {
      ...
      this.logger.info(
        "TaskManagementService: Created script generation task",
        {
          taskId: task.id,
          projectPath: command.projectPath,
          scriptType: command.scriptType,
      ...
      this.logger.error(
        "TaskManagementService: Failed to create script generation task",
        {
          projectPath: command.projectPath,
          error: error.message,
        },
  ...
  async createExecutionRecord(task, command, handlerId) {
    try {
      ...
      this.logger.error(
        "TaskManagementService: Failed to create execution record",
        {
          taskId: task.id,
          error: error.message,
        },
  ...
  async updateExecutionRecord(execution, result) {
    try {
      ...
      this.logger.error(
        "TaskManagementService: Failed to update execution record",
        {
          executionId: execution.id,
          error: error.message,
        },
