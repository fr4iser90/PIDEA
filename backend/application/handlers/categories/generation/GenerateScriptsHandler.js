...
class GenerateScriptHandler {
  ...
  async handle(command) {
    try {
      ...
      await this.publishScriptGenerationCompletedEvent(
        execution,
        result,
        command,
      );
      ...
      // Log success
      this.logger.info(
        "GenerateScriptHandler: Script generation completed successfully",
        {
          handlerId: this.handlerId,
          commandId: command.commandId,
          projectPath: command.projectPath,
  ...
  async validateCommand(command) {
    ...
    if (command.options) {
      if (
        command.options.autoExecute &&
        typeof command.options.autoExecute !== "boolean"
      ) {
      ...
      if (
        ...
        typeof command.options.overwrite !== "boolean"
      ) {
        errors.push("Overwrite must be a boolean");
      }

      if (
        command.options.scriptName &&
        typeof command.options.scriptName !== "string"
      ) {
      ...
      if (
        ...
        typeof command.options.environment !== "object"
      ) {
  ...
  async analyzeProjectForScripts(command) {
    try {
      this.logger.info(
        "GenerateScriptHandler: Analyzing project for script generation",
        {
          handlerId: this.handlerId,
          projectPath: command.projectPath,
          scriptType: command.scriptType,
      ...
      const projectStructure = await this.projectAnalyzer.getProjectStructure(
        command.projectPath,
      );
      ...
      const projectType = await this.projectAnalyzer.detectProjectType(
        command.projectPath,
      );
      ...
      this.logger.error(
        "GenerateScriptHandler: Failed to analyze project for scripts",
        {
          handlerId: this.handlerId,
          projectPath: command.projectPath,
          error: error.message,
  ...
  async getExistingScripts(projectPath) {
    try {
      ...
      this.logger.warn(
        "GenerateScriptHandler: Failed to get existing scripts",
        {
          handlerId: this.handlerId,
          projectPath,
          error: error.message,
  ...
  async detectBuildTools(projectPath) {
    try {
      ...
      for (const file of configFiles) {
        const exists = await this.fileSystemService.exists(
          `${projectPath}/${file}`,
        );
        if (exists) {
  ...
  async createScriptTask(command) {
    try {
      ...
      this.logger.info(
        "GenerateScriptHandler: Created script generation task",
        {
          handlerId: this.handlerId,
          taskId: task.id,
          projectPath: command.projectPath,
      ...
      this.logger.error(
        "GenerateScriptHandler: Failed to create script generation task",
        {
          handlerId: this.handlerId,
          projectPath: command.projectPath,
          error: error.message,
  ...
  async createExecutionRecord(task, command) {
    try {
      ...
      this.logger.error(
        "GenerateScriptHandler: Failed to create execution record",
        {
          handlerId: this.handlerId,
          taskId: task.id,
          error: error.message,
  ...
  async generateScript(command, projectContext, execution) {
    ...
    try {
      ...
      const processedScript = await this.processScript(
        scriptResult,
        projectContext,
        command,
      );
  ...
  async processScript(scriptResult, projectContext, command) {
    try {
      ...
      const scriptPath = this.determineScriptPath(
        scriptName,
        projectContext,
        command,
      );
      ...
      const enhancedContent = await this.enhanceScriptContent(
      ...
      const finalContent = this.addScriptMetadata(
        enhancedContent,
        scriptName,
        command,
      );
  ...
  async enhanceScriptContent(content, projectContext, command) {
    ...
    enhancedContent = this.addProjectSpecificEnhancements(
      enhancedContent,
      projectContext,
      command,
    );
...
`;
  ...
  async saveScript(script, command) {
    try {
      ...
      if (exists && !command.options?.overwrite) {
        throw new Error(
          `Script file already exists: ${script.path}. Use overwrite option to replace.`,
        );
      }
  ...
  async updateExecutionRecord(execution, result) {
    try {
      ...
      this.logger.error(
        "GenerateScriptHandler: Failed to update execution record",
        {
          handlerId: this.handlerId,
          executionId: execution.id,
          error: error.message,
  ...
  async publishScriptGenerationStartedEvent(execution, command) {
    try {
      ...
      this.logger.warn(
        "GenerateScriptHandler: Failed to publish script generation started event",
        {
          handlerId: this.handlerId,
          executionId: execution.id,
          error: error.message,
  ...
  async publishScriptGenerationCompletedEvent(execution, result, command) {
    try {
      ...
      this.logger.warn(
        "GenerateScriptHandler: Failed to publish script generation completed event",
        {
          handlerId: this.handlerId,
          executionId: execution.id,
          error: error.message,
  ...
  async handleScriptGenerationError(error, command) {
    ...
    try {
      ...
      this.logger.warn(
        "GenerateScriptHandler: Failed to publish script generation failed event",
        {
          handlerId: this.handlerId,
