...
class GenerateDocumentationHandler {
  ...
  async handle(command) {
    ...
    try {
      ...
      const codeDocumentation = await this.extractCodeDocumentation(
        command.projectPath,
        projectStructure,
      );
      ...
      if (options.includeAPI) {
        apiDocsResults = await this.generateAPIDocumentation(
          command.projectPath,
          codeDocumentation,
          options,
        );
      ...
      if (options.includeArchitecture) {
        architectureDocsResults = await this.generateArchitectureDocumentation(
          command.projectPath,
          projectStructure,
          options,
        );
      ...
      if (options.includeExamples) {
        examplesResults = await this.generateExamples(
          command.projectPath,
          codeDocumentation,
          options,
        );
      ...
      if (options.includeDiagrams) {
        diagramsResults = await this.generateDiagrams(
          command.projectPath,
          projectStructure,
          options,
        );
      ...
      if (options.includeChangelog) {
        changelogResults = await this.generateChangelog(
          command.projectPath,
          options,
        );
      }
      ...
      if (options.includeTutorials) {
        tutorialsResults = await this.generateTutorials(
          command.projectPath,
          codeDocumentation,
          options,
        );
      ...
      if (options.autoGenerate) {
        consolidationResults = await this.consolidateDocumentation(
          command.projectPath,
          {
      ...
      this.logger.info(
        `Documentation generation completed successfully for project`,
      );
      ...
      this.logger.error(
        `Documentation generation failed for project ${command.projectPath}:`,
        error,
      );
  ...
  extractFunctions(content) {
    const functions = [];
    const functionRegex =
      /(?:function\s+(\w+)|const\s+(\w+)\s*=\s*(?:async\s*)?\(|(\w+)\s*\([^)]*\)\s*\{)/g;
  ...
  calculateDocumentationCoverage(components) {
    ...
    const documentedComponents = components.filter(
      (comp) =>
  ...
  extractFunctionDocumentation(component, func) {
    const comments = component.comments.filter(
      (c) => c.line <= func.line && c.line >= func.line - 5,
    );
  ...
  extractClassDocumentation(component, cls) {
    const comments = component.comments.filter(
      (c) => c.line <= cls.line && c.line >= cls.line - 5,
    );
  ...
  extractAPIDocumentation(component) {
    ...
    return {
      ...
      documentation:
        component.comments.length > 0 ? component.comments[0].content : "",
    };
  }
  ...
  async generateAPIDocumentation(projectPath, codeDocumentation, options) {
    ...
    try {
      ...
      results.totalEndpoints = codeDocumentation.apis.reduce(
...
### File Types
${Object.entries(projectStructure.metrics.fileTypes)
  ...
  async generateTutorials(projectPath, codeDocumentation, options) {
    ...
    try {
      ...
      const gettingStarted =
        this.generateGettingStartedTutorial(codeDocumentation);
...
${codeDocumentation.functions
  .slice(0, 5)
  .map(
    (func) =>
      `- **${func.name}** - ${func.documentation || "No description available"}`,
  )
  .join("\n")}
...
${codeDocumentation.classes
  .slice(0, 5)
  .map(
    (cls) =>
      `- **${cls.name}** - ${cls.documentation || "No description available"}`,
  )
  .join("\n")}
...
`;
    ...
    const output = {
      ...
      summary: {
        ...
        architectureDocsGenerated:
          architectureDocsResults?.generated?.length || 0,
      ...
      projectStructure: outputConfig.includeRawData
        ? projectStructure
  ...
  async saveResults(command, output) {
    try {
      ...
      this.logger.error(
        "Failed to save documentation generation results:",
        error,
      );
    }
  }
