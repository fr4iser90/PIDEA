  ...
  async handle(command) {
    try {
      ...
      if (!businessValidation.isValid) {
        throw new Error(
          `Business validation failed: ${businessValidation.errors.join(", ")}`,
        );
      }
      ...
      const analysisResult = await this.executeLayerViolationAnalysis(
        command,
        task,
      );
      ...
      let generatedTasks = [];
      if (
        command.options.generateTasks &&
        analysisResult.success &&
        analysisResult.data.tasks
      ) {
        generatedTasks = await this.createTasksFromViolations(
          analysisResult.data.tasks,
          command,
        );
      }
      ...
      await this.eventBus.publish("layer-violation:analysis:completed", {
        ...
        commandId: command.commandId,
        projectId: command.projectId,
        violations: analysisResult.success
          ? analysisResult.data.summary.totalViolations
          : 0,
        criticalViolations: analysisResult.success
          ? analysisResult.data.summary.criticalViolations
          : 0,
  ...
  async canHandle(command) {
    try {
      ...
      if (!step) {
        return {
          canHandle: false,
          reason: "LayerViolationAnalysisStep not found",
        };
      }
