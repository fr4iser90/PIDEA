...
class AdvancedAnalysisHandler {
  constructor(dependencies = {}) {
    ...
    this.advancedAnalysisService =
      dependencies.advancedAnalysisService ||
      new AdvancedAnalysisService(dependencies);
    this.taskRepository = dependencies.taskRepository || new TaskRepository();
    this.executionRepository =
      dependencies.executionRepository || new TaskExecutionRepository();
    this.analysisRepository =
      dependencies.analysisRepository || new AnalysisRepository();
  }
  ...
  async handle(command) {
    ...
    try {
      ...
      this.logger.info(
        "AdvancedAnalysisHandler: Advanced analysis completed successfully",
        {
          handlerId: this.handlerId,
          commandId: command.commandId,
          projectPath: command.projectPath,
  ...
  async validateCommand(command) {
    ...
    if (command.options) {
      if (
        ...
        (typeof command.options.timeout !== "number" ||
          command.options.timeout < 60000)
      ) {
        errors.push("Timeout must be at least 60 seconds");
      }

      if (
        command.options.exportFormat &&
        !["json", "markdown", "html", "pdf"].includes(
          command.options.exportFormat,
        )
      ) {
  ...
  async performAdvancedAnalysis(command, execution) {
    ...
    try {
      this.logger.info(
        "AdvancedAnalysisHandler: Performing advanced analysis",
        {
          handlerId: this.handlerId,
          commandId: command.commandId,
          projectPath: command.projectPath,
      ...
      // Perform advanced analysis
      const analysis =
        await this.advancedAnalysisService.performAdvancedAnalysis(
          command.projectPath,
  ...
  async updateTaskStatus(task, result) {
    ...
    task.result = {
      overallScore: result.analysis.metrics.overallScore,
      violations:
        result.analysis.layerValidation.violations.length +
        result.analysis.logicValidation.violations.length,
    };
  ...
  async saveAnalysisResult(command, result) {
    try {
      ...
      const needsUpdate = await this.shouldUpdateAnalysis(
        existingAnalysis,
        result,
      );
      ...
      if (!needsUpdate) {
        this.logger.info(
          "AdvancedAnalysisHandler: Analysis data is up-to-date, skipping save",
          {
            handlerId: this.handlerId,
            commandId: command.commandId,
            projectPath: command.projectPath,
      ...
      this.logger.info(
        "AdvancedAnalysisHandler: Analysis result saved to repository",
        {
          handlerId: this.handlerId,
      ...
      this.logger.error(
        "AdvancedAnalysisHandler: Failed to save analysis result to repository",
        {
          handlerId: this.handlerId,
          commandId: command.commandId,
          error: error.message,
  ...
  async checkExistingAnalysis(projectPath, analysisType) {
    try {
      ...
      this.logger.warn(
        "AdvancedAnalysisHandler: Failed to check existing analysis",
        {
          projectPath,
          analysisType,
          error: error.message,
  ...
  async shouldUpdateAnalysis(existingAnalysis, newResult) {
    ...
    if (lastUpdate > oneHourAgo) {
      this.logger.info(
        "AdvancedAnalysisHandler: Analysis is recent, checking for data changes",
        {
    ...
    const existingData =
      existingAnalysis.data || existingAnalysis.result_data || {};
    ...
    if (scoreDiff > 5) {
      this.logger.info(
        "AdvancedAnalysisHandler: Significant score change detected, updating analysis",
        {
          existingScore,
          newScore,
          scoreDiff,
    ...
    const existingViolations =
      (existingData.layerValidation?.violations?.length || 0) +
      (existingData.logicValidation?.violations?.length || 0);
    const newViolations =
      (newData.layerValidation?.violations?.length || 0) +
      (newData.logicValidation?.violations?.length || 0);

    if (existingViolations !== newViolations) {
      this.logger.info(
        "AdvancedAnalysisHandler: Violation count changed, updating analysis",
        {
          existingViolations,
          newViolations,
        },
    ...
    if (lastUpdate < oneDayAgo) {
      this.logger.info(
        "AdvancedAnalysisHandler: Analysis is older than 24 hours, updating",
        {
          lastUpdate: lastUpdate.toISOString(),
        },
      );
    ...
    this.logger.info(
      "AdvancedAnalysisHandler: No significant changes detected, skipping update",
      {
        existingScore,
        newScore,
        existingViolations,
