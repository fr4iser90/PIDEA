...
class CreateTaskHandler {
    ...
    try {
      ...
      if (this.taskValidationService) {
        const validationResult =
          await this.taskValidationService.validateTaskCreation({
            title: command.title,
  ...
  async handleWithSuggestion(command, suggestionId) {
    try {
      ...
      const mergedCommand = this.mergeCommandWithSuggestion(
        command,
        suggestion,
      );
  ...
  mergeCommandWithTemplate(command, template) {
    ...
    return new (require("@categories/management/CreateTaskCommand"))(
      mergedParams,
    );
  }
