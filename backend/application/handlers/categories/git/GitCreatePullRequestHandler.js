...
class GitCreatePullRequestHandler {
  ...
  async handle(command) {
    try {
      ...
      this.logger.info(
        "GitCreatePullRequestHandler: Executing gitcreatepullrequestcommand",
        commandData,
      );
      ...
      const prTitle =
        commandData.title ||
        `Merge ${commandData.sourceBranch} into ${commandData.targetBranch}`;
      const prDescription =
        commandData.description ||
        `Automated pull request from ${commandData.sourceBranch}`;
      ...
      try {
        ...
        if (ghCheck.exitCode === 0) {
          ...
          const result = await execAsync(ghCommand, {
            cwd: commandData.projectPath,
          });
        ...
        await execAsync(`git push origin ${commandData.sourceBranch}`, {
          cwd: commandData.projectPath,
        });
      ...
      this.logger.info(
        "GitCreatePullRequestHandler: GitCreatePullRequestCommand completed successfully",
        {
          result: result.stdout,
        },
      );
      ...
      this.logger.error(
        "GitCreatePullRequestHandler: GitCreatePullRequestCommand failed",
        {
          error: error.message,
