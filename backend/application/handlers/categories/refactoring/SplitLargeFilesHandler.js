...
class SplitLargeFilesHandler {
  ...
  async executeFileSplitting(command) {
    ...
    for (const filePath of largeFiles) {
      const splitResult = await this.splitFile(
        filePath,
  ...
  async splitFile(filePath, strategy, options) {
    ...
    try {
      ...
      switch (strategy) {
        case "function":
          splitResult.splits = await this.splitByFunctions(
            content,
            fileName,
            fileExt,
            outputDir,
          ...
          break;
        case "class":
          splitResult.splits = await this.splitByClasses(
            content,
            fileName,
            fileExt,
            outputDir,
          ...
          break;
        case "module":
          splitResult.splits = await this.splitByModules(
            content,
            fileName,
            fileExt,
            outputDir,
      ...
      if (options.createIndexFiles) {
        splitResult.indexFile = await this.createIndexFile(
          splitResult.splits,
          fileName,
          fileExt,
          outputDir,
