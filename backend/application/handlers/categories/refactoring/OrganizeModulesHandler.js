...
class OrganizeModulesHandler {
  ...
  async handle(command) {
    ...
    try {
      ...
      const organizationPlan = await this.generateOrganizationPlan(
        currentStructure,
        options,
      );
      ...
      if (options.moveFiles) {
        executionResults = await this.executeOrganization(
          command.projectPath,
          organizationPlan,
          options,
        );
      ...
      if (options.createIndexFiles) {
        indexResults = await this.createIndexFiles(
          command.projectPath,
          organizationPlan,
        );
      }
      ...
      if (options.validateOrganization) {
        validationResults = await this.validateOrganization(
          command.projectPath,
          organizationPlan,
        );
      }
      ...
      this.logger.info(
        `Module organization completed successfully for project`,
      );
      ...
      this.logger.error(
        `Module organization failed for project ${command.projectPath}:`,
        error,
      );
    ...
    return {
      ...
      averageFileSize:
        structure.files.length > 0
          ? structure.files.reduce((sum, file) => sum + file.size, 0) /
  ...
  generateFeatureBasedMoves(structure) {
    ...
    features.forEach((feature) => {
      const featureFiles = structure.files.filter(
        (file) => this.extractFeatureFromPath(file.path) === feature,
      );
  ...
  generateLayerBasedMoves(structure) {
    ...
    layers.forEach((layer) => {
      const layerFiles = structure.files.filter(
        (file) => file.type === layer || file.path.includes(layer),
      );
  ...
  generateDomainBasedMoves(structure) {
    ...
    domains.forEach((domain) => {
      const domainFiles = structure.files.filter(
        (file) => this.extractDomainFromPath(file.path) === domain,
      );
  ...
  generateRecommendations(structure, plan) {
    ...
    if (structure.metrics.totalFiles > 100) {
      recommendations.push({
        type: "performance",
        message:
          "Large number of files detected. Consider breaking into smaller modules.",
        priority: "high",
      });
    }
    ...
    if (structure.metrics.averageFileSize > 1000) {
      recommendations.push({
        ...
        message:
          "Large average file size detected. Consider splitting large files.",
        priority: "medium",
      });
    }
    ...
    if (plan.moves.length > 50) {
      recommendations.push({
        type: "safety",
        message:
          "Many file moves planned. Consider backing up before execution.",
        priority: "high",
      });
    }
  ...
  async updateImportsInFile(filePath, pathMapping) {
    ...
    for (const [oldPath, newPath] of Object.entries(pathMapping)) {
      ...
      const importRegex = new RegExp(
        `from\\s+['"\`]${this.escapeRegex(oldModule)}['"\`]`,
        "g",
      );
  ...
  async generateOutput(data) {
    ...
    const output = {
      ...
      currentStructure: outputConfig.includeRawData
        ? currentStructure
        : currentStructure.metrics,
      organizationPlan: {
        strategy: organizationPlan.strategy,
