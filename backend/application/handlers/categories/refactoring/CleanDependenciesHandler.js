...
class CleanDependenciesHandler {
  ...
  async handle(command) {
    ...
    try {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Business rule validation failed: ${validation.errors.join(", ")}`,
        );
      }
      ...
      const unusedDependencies = await this.identifyUnusedDependencies(
        command.projectPath,
        currentDependencies,
      );
      ...
      const duplicateDependencies =
        await this.findDuplicateDependencies(currentDependencies);

      // Step 4: Check security vulnerabilities
      let securityIssues = null;
      if (options.checkSecurity) {
        securityIssues =
          await this.checkSecurityVulnerabilities(currentDependencies);
      }
      ...
      if (options.removeUnused || options.consolidateDuplicates) {
        executionResults = await this.executeCleaning(
          command.projectPath,
          cleaningPlan,
          options,
        );
      ...
      if (options.updateVersions) {
        updateResults = await this.updateDependencyVersions(
          command.projectPath,
          cleaningPlan,
        );
      }
      ...
      if (options.validateChanges) {
        validationResults = await this.validateChanges(
          command.projectPath,
          cleaningPlan,
        );
      }
      ...
      this.logger.info(
        `Dependency cleaning completed successfully for project`,
      );
      ...
      this.logger.error(
        `Dependency cleaning failed for project ${command.projectPath}:`,
        error,
      );
  ...
  async analyzeCurrentDependencies(projectPath) {
    ...
    try {
      ...
      const packageLock = packageLockPath
        ? JSON.parse(await fs.readFile(packageLockPath, "utf-8"))
        : null;
      ...
      // Calculate totals
      dependencies.metadata.totalDirect =
        Object.keys(dependencies.direct.dependencies).length +
      ...
      if (packageLock && packageLock.dependencies) {
        dependencies.transitive = this.analyzeTransitiveDependencies(
          packageLock.dependencies,
        );
        dependencies.metadata.totalTransitive = Object.keys(
  ...
  async identifyUnusedDependencies(projectPath, currentDependencies) {
    ...
    try {
      ...
      for (const [depName, depVersion] of Object.entries(allDeps)) {
        ...
        if (!isUsed) {
          const depType = currentDependencies.direct.dependencies[depName]
            ? "dependencies"
            : "devDependencies";
          unused[depType].push({
            name: depName,
      ...
      unused.totalUnused =
        unused.dependencies.length + unused.devDependencies.length;

      return unused;
    } catch (error) {
      throw new Error(
        `Failed to identify unused dependencies: ${error.message}`,
      );
    }
  }
  ...
  isKnownVulnerable(name, version) {
    ...
    return (
      ...
      vulnerablePackages[name].some((range) =>
    ...
    for (const action of cleaningPlan.actions) {
      try {
        if (action.type === "remove_unused") {
          const removed = await this.removeUnusedDependencies(
            projectPath,
            action.dependencies,
          );
          results.removed.push(...removed);
        } else if (action.type === "consolidate_duplicates") {
          const consolidated = await this.consolidateDuplicateDependencies(
            projectPath,
            action.duplicates,
          );
    ...
    try {
      ...
      for (const duplicate of duplicates) {
        ...
        if (
          ...
          packageJson.dependencies[duplicate.name]
        ) {
          ...
          packageJson.devDependencies &&
          packageJson.devDependencies[duplicate.name]
        ) {
      ...
      throw new Error(
        `Failed to consolidate duplicate dependencies: ${error.message}`,
      );
    }
  ...
  async validateChanges(projectPath, cleaningPlan) {
    ...
    try {
      ...
      const newDependencies =
        await this.analyzeCurrentDependencies(projectPath);
  ...
  async generateOutput(data) {
    ...
    const output = {
      ...
      currentDependencies: outputConfig.includeRawData
        ? currentDependencies
        : currentDependencies.metadata,
      unusedDependencies,
      duplicateDependencies,
      securityIssues,
      cleaningPlan: {
