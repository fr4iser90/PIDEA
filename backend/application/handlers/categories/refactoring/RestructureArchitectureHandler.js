...
class RestructureArchitectureHandler {
  ...
  async handle(command) {
    ...
    try {
      ...
      const targetArchitecture = await this.generateTargetArchitecture(
        currentArchitecture,
        options,
      );
      ...
      if (options.backupOriginal) {
        backupResults = await this.createBackup(
          command.projectPath,
          currentArchitecture,
        );
      }
      ...
      if (options.createNewStructure) {
        structureResults = await this.createNewStructure(
          command.projectPath,
          targetArchitecture,
        );
      }
      ...
      if (options.migrateExisting) {
        migrationResults = await this.migrateExistingCode(
          command.projectPath,
          currentArchitecture,
          targetArchitecture,
        );
      ...
      if (options.updateImports) {
        importResults = await this.updateImports(
          command.projectPath,
          targetArchitecture,
        );
      }
      ...
      if (options.createDocumentation) {
        documentationResults = await this.createArchitectureDocumentation(
          command.projectPath,
          targetArchitecture,
        );
      }
      ...
      if (options.validateArchitecture) {
        validationResults = await this.validateArchitecture(
          command.projectPath,
          targetArchitecture,
        );
      }
      ...
      this.logger.info(
        `Architecture restructuring completed successfully for project`,
      );
      ...
      this.logger.error(
        `Architecture restructuring failed for project ${command.projectPath}:`,
        error,
      );
  ...
  async analyzeCurrentArchitecture(projectPath) {
    ...
    try {
      ...
      throw new Error(
        `Failed to analyze current architecture: ${error.message}`,
      );
    }
  }
  ...
  detectArchitecturePattern(architecture) {
    ...
    if (
      layers.domain.length > 0 &&
      layers.application.length > 0 &&
  ...
  hasModelLayer(architecture) {
    return (
      architecture.layers.domain.length > 0 ||
      architecture.layers.infrastructure.some((dir) =>
  ...
  calculateArchitectureMetrics(architecture) {
    return {
      totalLayers: Object.keys(architecture.layers).filter(
        (key) => architecture.layers[key].length > 0,
      ).length,
      ...
      layerDistribution: Object.fromEntries(
        Object.entries(architecture.layers).map(([key, value]) => [
          key,
          value.length,
        ]),
      ),
  ...
  calculateComplexity(architecture) {
    ...
    complexity +=
      Object.keys(architecture.layers).filter(
        (key) => architecture.layers[key].length > 0,
      ).length * 10;
  ...
  generateMigrationPlan(currentArchitecture, options) {
    ...
    plan.estimatedTime = plan.steps.reduce(
  ...
  async createArchitectureDocumentation(projectPath, targetArchitecture) {
    ...
    try {
      ...
      const documentation =
        this.generateArchitectureDocumentation(targetArchitecture);
...
## Layers:
${Object.entries(targetArchitecture.layers)
  .map(
    ([layer, dirs]) =>
...
`;
  ...
  async validateArchitecture(projectPath, targetArchitecture) {
    ...
    try {
      ...
      const newArchitecture =
        await this.analyzeCurrentArchitecture(projectPath);
  ...
  async generateOutput(data) {
    ...
    const output = {
      ...
      currentArchitecture: outputConfig.includeRawData
        ? currentArchitecture
        : currentArchitecture.metrics,
      targetArchitecture: {
        pattern: targetArchitecture.pattern,
  ...
  async saveResults(command, output) {
    try {
      ...
      this.logger.error(
        "Failed to save architecture restructuring results:",
        error,
      );
    }
  }
