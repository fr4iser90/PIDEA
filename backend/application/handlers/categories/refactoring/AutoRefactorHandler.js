...
class AutoRefactorHandler {
  ...
  async handle(command) {
    try {
      ...
      const createdTasks = await this.createRefactoringTasks(
        largeFiles,
        command.projectPath,
      );
  ...
  async runComprehensiveAnalysis(projectPath) {
    try {
      ...
      if (needsRefresh) {
        this.logger.info(
          "🔄 [AutoRefactorHandler] Analysis data needs refresh, running fresh analysis...",
        );
        return await this.createStandardAnalysis(
          projectPath,
      ...
      const latestArchitectureAnalysis =
        await this.projectAnalysisRepository.findLatestByProjectIdAndType(
          projectId,
          "architecture",
        );
      const latestCodeQualityAnalysis =
        await this.projectAnalysisRepository.findLatestByProjectIdAndType(
          projectId,
          "code-quality",
        );
      const latestDependenciesAnalysis =
        await this.projectAnalysisRepository.findLatestByProjectIdAndType(
          projectId,
          "dependencies",
        );
      ...
      if (combinedAnalysis.codeQuality) {
        ...
        if (
          combinedAnalysis.codeQuality.largeFiles &&
          Array.isArray(combinedAnalysis.codeQuality.largeFiles)
        ) {
        ...
        if (
          combinedAnalysis.codeQuality.data &&
          combinedAnalysis.codeQuality.data.largeFiles &&
          Array.isArray(combinedAnalysis.codeQuality.data.largeFiles)
        ) {
        ...
        if (
          combinedAnalysis.codeQuality.realMetrics &&
          combinedAnalysis.codeQuality.realMetrics.largeFiles &&
          Array.isArray(combinedAnalysis.codeQuality.realMetrics.largeFiles)
        ) {
          largeFiles.push(
            ...combinedAnalysis.codeQuality.realMetrics.largeFiles,
          );
      ...
      if (largeFiles.length === 0) {
        this.logger.info(
          "⚠️ [AutoRefactorHandler] No analyses found in database, creating standard analysis...",
        );
      ...
      for (const file of largeFiles) {
        ...
        if (!lines || lines <= 0) {
          try {
            ...
            this.logger.info(
              `📊 [AutoRefactorHandler] Manually counted lines for ${file.file || file.path}: ${lines}`,
            );
          } catch (error) {
            this.logger.warn(
              `⚠️ [AutoRefactorHandler] Could not count lines for ${file.file || file.path}: ${error.message}`,
            );
        ...
        if (lines && lines > 0) {
          validatedLargeFiles.push({
            ...
            package:
              file.package || this.getPackageFromPath(file.file || file.path),
          });
        }
      }

      this.logger.info(
        `✅ [AutoRefactorHandler] Found ${validatedLargeFiles.length} valid large files from analysis data`,
      );
      ...
      this.logger.error(
        "❌ [AutoRefactorHandler] Error reading analysis from database:",
        error,
      );
      throw error;
    }
  ...
  async createStandardAnalysis(projectPath, projectId) {
    try {
      this.logger.info(
        "🔍 [AutoRefactorHandler] Creating standard analysis for project:",
        projectId,
      );
      ...
      const standardAnalysis = new ProjectAnalysis({
        ...
        analysisData: {
          ...
          averageLines:
            largeFiles.length > 0
              ? Math.round(
                  largeFiles.reduce((sum, file) => sum + file.lines, 0) /
                    largeFiles.length,
                )
              : 0,
      ...
      this.logger.info(
        "✅ [AutoRefactorHandler] Standard analysis saved to database",
      );
      ...
      this.logger.error(
        "❌ [AutoRefactorHandler] Error creating standard analysis:",
        error,
      );
      return {
  ...
  identifyLargeFiles(analysisResults) {
    ...
    this.logger.info(
      "🔍 [AutoRefactorHandler] Analyzing results structure for large files",
    );
    ...
    if (analysisResults && typeof analysisResults === "object") {
      ...
      for (const source of possibleLargeFilesSources) {
        if (source && Array.isArray(source)) {
          this.logger.info(
            `📁 [AutoRefactorHandler] Found large files in source:`,
            source.length,
            "files",
          );
          ...
          source.forEach((file) => {
            ...
            if (filePath && !processedPaths.has(filePath)) {
              ...
              this.logger.info(
                `📊 [AutoRefactorHandler] File ${filePath}: lines=${lines}, original=${file.lines}`,
              );
    ...
    this.logger.info(
      `🔍 [AutoRefactorHandler] Found ${largeFiles.length} large files to refactor:`,
      largeFiles.map((f) => `${f.path} (${f.lines} lines)`),
    );
  ...
  async createRefactoringTasks(largeFiles, projectPath) {
    ...
    this.logger.info(
      `🔍 [AutoRefactorHandler] Creating refactoring tasks for ${largeFiles.length} files...`,
    );
    ...
    for (const file of largeFiles) {
      try {
        ...
        this.logger.error(
          `❌ [AutoRefactorHandler] Failed to create task for ${file.path}:`,
          error.message,
        );
      }
    }
    ...
    this.logger.info(
      `✅ [AutoRefactorHandler] Successfully created ${tasks.length} refactoring tasks`,
    );
  ...
  async createRefactoringTask(fileInfo, projectPath) {
    ...
    if (!lines || lines <= 0) {
      ...
      try {
        ...
        this.logger.info(
          `📊 [AutoRefactorHandler] Manually counted lines for ${fileInfo.path}: ${lines}`,
        );
      } catch (error) {
        this.logger.warn(
          `⚠️ [AutoRefactorHandler] Could not count lines for ${fileInfo.path}: ${error.message}`,
        );
        return null;
      }
    }
    ...
    if (!lines || lines <= 0) {
      this.logger.warn(
        `⚠️ [AutoRefactorHandler] Invalid line count for ${fileInfo.path}: ${lines}, skipping...`,
      );
      return null;
    }
    ...
    this.logger.info(
      `✅ [AutoRefactorHandler] Created refactoring task for ${fileInfo.path} (${fileInfo.lines} lines)`,
    );
  ...
  async needsAnalysisRefresh(projectPath) {
    try {
      ...
      if (
        !this.projectAnalysisRepository ||
        typeof this.projectAnalysisRepository.findLatestByProjectId !==
          "function"
      ) {
        this.logger.warn(
          "⚠️ [AutoRefactorHandler] projectAnalysisRepository.findLatestByProjectId not available",
        );
        return true; // Default to refresh if repository not available
      }

      const latestAnalysis =
        await this.projectAnalysisRepository.findLatestByProjectId(projectId);

      if (!latestAnalysis) {
        this.logger.info(
          "🔄 [AutoRefactorHandler] No analysis found, refresh needed",
        );
      ...
      if (analysisAge > thirtyMinutes) {
        this.logger.info(
          `🔄 [AutoRefactorHandler] Analysis data is ${Math.round(analysisAge / 60000)} minutes old, refresh needed`,
        );
        return true;
      }
      ...
      if (analysisData && analysisData.codeQuality) {
        const largeFiles =
          analysisData.codeQuality.largeFiles ||
        ...
        const hasInvalidLineCounts = largeFiles.some(
          (file) =>
            !file.lines ||
            file.lines === 0 ||
        ...
        if (hasInvalidLineCounts) {
          this.logger.info(
            "🔄 [AutoRefactorHandler] Analysis data has invalid line counts, refresh needed",
          );
          return true;
        }
      }
      ...
      this.logger.info(
        "✅ [AutoRefactorHandler] Analysis data is current and valid",
      );
      return false;
    } catch (error) {
      this.logger.error(
        `❌ [AutoRefactorHandler] Error checking analysis refresh need:`,
        error.message,
      );
  ...
  async analyzeDependencies(projectPath) {
    ...
    try {
      ...
      if (packageJsonExists) {
        ...
        analysis.devDependencies = Object.keys(
  ...
  async analyzeArchitecture(projectPath) {
    ...
    try {
      ...
      analysis.structure = {
        ...
        hasComponents:
          dirs.includes("components") || dirs.includes("src/components"),
  ...
  identifyRefactoringOpportunities(analysis) {
    ...
    if (
      analysis.dependencies.hasPackageJson &&
      analysis.dependencies.dependencies.length > 20
    ) {
      opportunities.push({
