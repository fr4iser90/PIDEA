...
class SendMessageHandler {
  ...
  validateDependencies(dependencies) {
    const required = ["browserManager", "ideManager", "eventBus", "logger"];
  ...
  async handleUserSpecificCommand(command) {
    ...
    try {
      ...
      if (sessionId) {
        ...
        session = ChatSession.createSession(userId, "New Chat", {
          ...metadata,
          createdBy: userId,
          idePort: activePort,
        });
      ...
      const ideResponse = await activeIDEService.sendMessage(content, {
  ...
  async handleLegacyCommand(command) {
    ...
    try {
      ...
      // Send to the appropriate IDE and get response with code blocks
      const ideResponse = await activeIDEService.sendMessage(command.content, {
        waitForResponse: true,
      });
