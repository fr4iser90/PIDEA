...
class AutomationOrchestrator {
  ...
  async startAutomaticDetection(options = {}) {
    try {
      ...
      this.isRunning = true;
      this.logger.info(
        `Automatic detection started for ${config.ideTypes.length} IDE types`,
      );
      return true;
    } catch (error) {
  ...
  async scheduleDetection(ideType, port, interval) {
    try {
      ...
      if (this.scheduledTasks.has(taskKey)) {
        this.logger.warn(
          `Detection already scheduled for ${ideType} on port ${port}`,
        );
        return true;
      }

      this.logger.info(
        `Scheduling detection for ${ideType} on port ${port} (interval: ${interval}ms)`,
      );
      ...
      this.logger.info(
        `Detection scheduled successfully for ${ideType} on port ${port}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Failed to schedule detection for ${ideType} on port ${port}:`,
        error.message,
      );
      return false;
    }
  ...
  async runDetectionTask(task) {
    try {
      ...
      this.logger.info(
        `Running detection task for ${task.ideType} on port ${task.port} (run #${task.runCount})`,
      );

      // Run the detection workflow
      const result = await this.versionManagementService.detectAndUpdateVersion(
        task.ideType,
        task.port,
      );
      ...
      if (result.success) {
        task.successCount++;
        this.logger.info(
          `Detection task completed successfully for ${task.ideType} on port ${task.port}`,
        );
        ...
        task.errorCount++;
        this.logger.error(
          `Detection task failed for ${task.ideType} on port ${task.port}: ${result.error}`,
        );
      ...
      task.errorCount++;
      this.logger.error(
        `Detection task error for ${task.ideType} on port ${task.port}:`,
        error.message,
      );
  ...
  async handleDetectionResult(result) {
    try {
      this.logger.info(
        `Handling detection result for ${result.ideType} on port ${result.port}`,
      );

      if (result.success) {
        this.logger.info(
          `Detection successful: ${result.ideType} version ${result.finalVersion}`,
        );

        // Handle successful detection
        if (result.isNewVersion) {
          this.logger.info(
            `New version detected: ${result.ideType} ${result.finalVersion}`,
          );
        ...
        this.logger.error(
          `Detection failed: ${result.ideType} on port ${result.port} - ${result.error}`,
        );
  ...
  getScheduledTasksStatus() {
    ...
    for (const [key, task] of this.scheduledTasks) {
      status.push({
        ...
        successRate:
          task.runCount > 0
            ? ((task.successCount / task.runCount) * 100).toFixed(2) + "%"
            : "0%",
      });
    }
  ...
  getStats() {
    ...
    return {
      ...
      successRate:
        totalRuns > 0
          ? ((totalSuccesses / totalRuns) * 100).toFixed(2) + "%"
          : "0%",
