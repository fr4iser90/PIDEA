...
class GitApplicationService {
  ...
  async getStatus(projectId, projectPath, userId) {
    try {
      this.logger.info("GitApplicationService: Getting Git status", {
        projectId,
        userId,
      });
  ...
  async createBranch(projectPath, branchName, userId) {
    try {
      this.logger.info("GitApplicationService: Creating branch", {
        branchName,
        userId,
      });
  ...
  async switchBranch(projectPath, branchName, userId) {
    try {
      this.logger.info("GitApplicationService: Switching branch", {
        branchName,
        userId,
      });
  ...
  async commit(projectPath, message, files, userId) {
    try {
      this.logger.info("GitApplicationService: Creating commit", {
        message,
        userId,
      });
  ...
  async push(projectPath, remote, branch, userId) {
    try {
      this.logger.info("GitApplicationService: Pushing changes", {
        remote,
        branch,
        userId,
      });
  ...
  async pull(projectPath, remote, branch, userId) {
    try {
      this.logger.info("GitApplicationService: Pulling changes", {
        remote,
        branch,
        userId,
      });
  ...
  async getCurrentBranch(projectPath, userId) {
    try {
      this.logger.info("GitApplicationService: Getting current branch", {
        userId,
      });
  ...
  async getCommitHistory(projectPath, limit, userId) {
    try {
      this.logger.info("GitApplicationService: Getting commit history", {
        limit,
        userId,
      });

      const history = await this.gitService.getCommitHistory(
        projectPath,
        limit,
      );
      return {
  ...
  async addFiles(projectPath, files, userId) {
    try {
      this.logger.info("GitApplicationService: Adding files", {
        files,
        userId,
      });
  ...
  async resetFiles(projectPath, files, userId) {
    try {
      this.logger.info("GitApplicationService: Resetting files", {
        files,
        userId,
      });
