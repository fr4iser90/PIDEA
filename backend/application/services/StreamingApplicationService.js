...
class StreamingApplicationService {
  constructor(dependencies = {}) {
    this.logger =
      dependencies.logger || new Logger("StreamingApplicationService");
  ...
  async startPortStreaming(projectId, port, userId) {
    try {
      this.logger.info("StreamingApplicationService: Starting port streaming", {
        projectId,
        port,
        userId,
      });
  ...
  async stopPortStreaming(projectId, port, userId) {
    try {
      this.logger.info("StreamingApplicationService: Stopping port streaming", {
        projectId,
        port,
        userId,
      });
  ...
  async getStreamingStatus(projectId, userId) {
    try {
      this.logger.info(
        "StreamingApplicationService: Getting streaming status",
        { projectId, userId },
      );
  ...
  async listActiveStreams(userId) {
    try {
      this.logger.info("StreamingApplicationService: Listing active streams", {
        userId,
      });
  ...
  async updateStreamingConfiguration(projectId, config, userId) {
    try {
      this.logger.info(
        "StreamingApplicationService: Updating streaming configuration",
        { projectId, userId },
      );

      const result = await this.portStreamingHandler.updateConfiguration(
        projectId,
        config,
      );
      return {
  ...
  async getPortStatus(port, userId) {
    try {
      this.logger.info("StreamingApplicationService: Getting port status", {
        port,
        userId,
      });
  ...
  async getAllPorts(userId) {
    try {
      this.logger.info("StreamingApplicationService: Getting all ports", {
        userId,
      });
  ...
  async getStreamingStats(userId) {
    try {
      this.logger.info("StreamingApplicationService: Getting streaming stats", {
        userId,
      });
  ...
  async stopAllStreaming(userId) {
    try {
      this.logger.info("StreamingApplicationService: Stopping all streaming", {
        userId,
      });
