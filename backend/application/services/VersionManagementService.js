...
class VersionManagementService {
  ...
  async detectAndUpdateVersion(ideType, port) {
    try {
      this.logger.info(
        `Starting version detection and update workflow for ${ideType} on port ${port}`,
      );
      ...
      if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
        this.logger.info(
          `Using cached workflow result for ${ideType} on port ${port}`,
        );
      ...
      const versionResult = await this.versionDetectionService.detectVersion(
        port,
        ideType,
      );
      ...
      this.logger.info(
        `Detected version: ${currentVersion} (selector: ${selectorVersion})`,
      );

      // Step 2: Check if we need to collect selectors
      if (isNewVersion) {
        this.logger.info(
          `Step 2: New version detected, collecting selectors for ${ideType} ${currentVersion}`,
        );

        const collectionResult = await this.collectSelectorsForNewVersion(
          ideType,
          currentVersion,
          port,
        );
        ...
        if (!collectionResult.success) {
          throw new Error(
            `Selector collection failed: ${collectionResult.error}`,
          );
        }

        // Step 3: Update IDETypes.js with new version and selectors
        this.logger.info(
          `Step 3: Updating IDETypes.js with new version ${currentVersion}`,
        );

        const updateResult = await this.updateIDETypes(
          ideType,
          currentVersion,
          collectionResult.selectors,
        );
        ...
        this.logger.info(
          `Step 2: Version ${currentVersion} is known, no collection needed`,
        );
      ...
      const validationResult = await this.validateWorkflow(
        ideType,
        currentVersion,
        port,
      );
      ...
      if (!validationResult.success) {
        throw new Error(
          `Workflow validation failed: ${validationResult.error}`,
        );
      }
      ...
      this.logger.info(
        `Version detection and update workflow completed successfully for ${ideType} version ${currentVersion}`,
      );
      return workflowResult;
    } catch (error) {
      this.logger.error(
        `Version detection and update workflow failed for ${ideType} on port ${port}:`,
        error.message,
      );
  ...
  async collectSelectorsForNewVersion(ideType, version, port) {
    try {
      this.logger.info(
        `Collecting selectors for new version ${ideType} ${version}`,
      );
      ...
      // Collect selectors
      const selectors = await this.selectorCollectionBot.collectSelectors(
        ideType,
        version,
        port,
      );
      ...
      // Test selectors
      const testResults = await this.selectorCollectionBot.testSelectors(
        ideType,
        version,
        selectors,
        port,
      ...
      if (testResults.failed > testResults.passed) {
        this.logger.warn(
          `Selector collection quality is low: ${testResults.passed}/${testResults.tested} passed`,
        );
      }

      // Save selectors
      await this.selectorCollectionBot.saveSelectors(
        ideType,
        version,
        selectors,
      );
      ...
      this.logger.error(
        `Selector collection failed for ${ideType} ${version}:`,
        error.message,
      );
      return {
  ...
  async updateIDETypes(ideType, version, selectors) {
    try {
      this.logger.info(
        `Updating IDETypes.js for ${ideType} version ${version}`,
      );

      if (!this.ideTypesUpdater) {
        throw new Error(
          `IDETypesUpdater not available for ${ideType} version ${version}. Cannot update IDETypes.js without updater.`,
        );
      }

      // Use IDETypesUpdater to update the file
      const updateResult = await this.ideTypesUpdater.updateVersion(
        ideType,
        version,
        selectors,
      );
      ...
      this.logger.error(
        `IDETypes.js update failed for ${ideType} ${version}:`,
        error.message,
      );
      return {
  ...
  async validateWorkflow(ideType, version, port) {
    try {
      this.logger.info(
        `Validating workflow completion for ${ideType} version ${version}`,
      );

      // Test version detection again
      const versionCheck = await this.versionDetectionService.detectVersion(
        port,
        ideType,
      );
      ...
      if (!versionCheck || versionCheck.currentVersion !== version) {
        throw new Error(
          `Version validation failed: expected ${version}, got ${versionCheck?.currentVersion}`,
        );
      }

      // Test selector availability
      if (this.selectorCollectionBot) {
        const testSelectors = await this.selectorCollectionBot.testSelectors(
          ideType,
          version,
          {},
          port,
      ...
      this.logger.error(
        `Workflow validation failed for ${ideType} ${version}:`,
        error.message,
      );
      return {
