...
class ContentLibraryApplicationService {
  constructor(dependencies = {}) {
    this.logger =
      dependencies.logger || new Logger("ContentLibraryApplicationService");
  ...
  async getContentLibrary(userId) {
    try {
      this.logger.info(
        "ContentLibraryApplicationService: Getting content library",
        { userId },
      );
  ...
  async getContentByCategory(category, userId) {
    try {
      this.logger.info(
        "ContentLibraryApplicationService: Getting content by category",
        { category, userId },
      );
  ...
  async getContentItem(contentId, userId) {
    try {
      this.logger.info(
        "ContentLibraryApplicationService: Getting content item",
        { contentId, userId },
      );
  ...
  async searchContent(query, filters, userId) {
    try {
      this.logger.info("ContentLibraryApplicationService: Searching content", {
        query,
        userId,
      });
  ...
  async createContent(contentData, userId) {
    try {
      this.logger.info("ContentLibraryApplicationService: Creating content", {
        userId,
      });
  ...
  async updateContent(contentId, contentData, userId) {
    try {
      this.logger.info("ContentLibraryApplicationService: Updating content", {
        contentId,
        userId,
      });
  ...
  async deleteContent(contentId, userId) {
    try {
      this.logger.info("ContentLibraryApplicationService: Deleting content", {
        contentId,
        userId,
      });
  ...
  async getCategories(userId) {
    try {
      this.logger.info("ContentLibraryApplicationService: Getting categories", {
        userId,
      });
