...
class AnalysisApplicationService {
  ...
  async getProjectPath(projectId) {
    ...
    if (this.projectRepository) {
      try {
        this.logger.info(
          `ProjectRepository available, searching for project...`,
        );
        ...
        this.logger.error(
          `Failed to get project path for ${projectId}:`,
          error,
        );
      }
    } else {
  ...
  async getAnalysisData(params) {
    ...
    try {
      ...
      if (useCache) {
        const cachedResult = await this.analysisRepository.getCachedAnalysis(
          projectId,
          types,
        );
        if (cachedResult) {
      ...
      const analysis = await this.analysisRepository.getLatestAnalysis(
        projectId,
        types,
      );
      ...
      if (analysis) {
        // Cache result
        if (useCache) {
          await this.analysisRepository.cacheAnalysis(
            projectId,
            types,
            analysis,
          );
  ...
  async getAnalysisStatus(projectId) {
    try {
      const latestAnalysis =
        await this.analysisRepository.getLatestAnalysis(projectId);
  ...
  async getAnalysisHistory(projectId, options = {}) {
    try {
      ...
      const history = await this.analysisRepository.getAnalysisHistory(
        projectId,
        {
          limit,
          offset,
  ...
  async getAnalysisIssues(projectId, analysisType = "code-quality") {
    try {
      const latestAnalysis = await this.analysisRepository.getLatestAnalysis(
        projectId,
        [analysisType],
      );
  ...
  async getAnalysisMetrics(projectId) {
    ...
    try {
      ...
      const metrics =
        await this.analysisOutputService.getAnalysisMetrics(projectPath);
      ...
      this.logger.error(
        `Failed to get analysis metrics for ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async getAnalysisFromDatabase(projectId) {
    try {
      this.logger.info(
        `🔍 [AnalysisApplicationService] Getting analysis from database for project: ${projectId}`,
      );

      if (!this.analysisRepository) {
        this.logger.error(
          `❌ [AnalysisApplicationService] AnalysisRepository not available!`,
        );
      ...
      this.logger.info(
        `🔍 [AnalysisApplicationService] Repository returned ${analyses ? analyses.length : 0} analyses`,
      );

      if (analyses && analyses.length > 0) {
        this.logger.info(
          `🔍 [AnalysisApplicationService] First analysis structure:`,
          JSON.stringify(
            {
              id: analyses[0].id,
              ...
              resultKeys: analyses[0].result
                ? Object.keys(analyses[0].result)
                : [],
            },
            null,
            2,
      ...
      this.logger.error(
        `❌ [AnalysisApplicationService] Error getting analysis from database:`,
        error,
      );
      return { projectId, analysis: [] };
    }
  ...
  async getAnalysisFile(projectId, filename) {
    ...
    try {
      ...
      const file = await this.analysisOutputService.getAnalysisFile(
        projectPath,
        filename,
      );
      ...
      this.logger.error(
        `Failed to get analysis file for ${projectId}/${filename}:`,
        error,
      );
      throw error;
    }
  ...
  async getAnalysisTechStack(projectId) {
    ...
    try {
      ...
      const techStack =
        await this.analysisOutputService.getAnalysisTechStack(projectPath);
      ...
      this.logger.error(
        `Failed to get analysis tech stack for ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async getAnalysisArchitecture(projectId) {
    ...
    try {
      ...
      const architecture =
        await this.analysisOutputService.getAnalysisArchitecture(projectPath);
      ...
      this.logger.error(
        `Failed to get analysis architecture for ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async getAnalysisRecommendations(projectId) {
    ...
    try {
      ...
      const recommendations =
        await this.analysisOutputService.getAnalysisRecommendations(projectId);
      ...
      this.logger.error(
        `Failed to get analysis recommendations for ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async getAnalysisCharts(projectId, type) {
    ...
    try {
      ...
      const charts = await this.analysisOutputService.getAnalysisCharts(
        projectPath,
        type,
      );
      ...
      this.logger.error(
        `Failed to get analysis charts for ${projectId}/${type}:`,
        error,
      );
      throw error;
    }
  ...
  async executeCodeQualityAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `🔍 Executing code quality analysis for project: ${projectId}`,
      );
  ...
  async executeSecurityAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `🔒 Executing security analysis for project: ${projectId}`,
      );
  ...
  async executePerformanceAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `⚡ Executing performance analysis for project: ${projectId}`,
      );
  ...
  async executeManifestAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `📋 Executing manifest analysis for project: ${projectId}`,
      );
  ...
  async executeDependencyAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `🔍 Executing dependency analysis for project: ${projectId}`,
      );
      ...
      this.logger.error(
        `❌ Failed to execute dependency analysis for project ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async executeTechStackAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `🔍 Executing tech stack analysis for project: ${projectId}`,
      );
      ...
      this.logger.error(
        `❌ Failed to execute tech stack analysis for project ${projectId}:`,
        error,
      );
      throw error;
    }
  ...
  async executeArchitectureAnalysis(projectId, options = {}) {
    try {
      this.logger.info(
        `🏗️ Executing architecture analysis for project: ${projectId}`,
      );
  ...
  async executeAnalysisStep(stepName, context) {
    try {
      ...
      this.logger.error(
        `❌ Step ${stepName} execution failed: ${error.message}`,
      );
  ...
  async saveAnalysisResult(projectId, analysisType, result, metadata = {}) {
    try {
      if (!this.analysisRepository) {
        this.logger.warn(
          "Analysis repository not available, skipping database save",
        );
        return null;
      }
  ...
  async checkMemoryUsage() {
    ...
    if (currentUsage > maxMemory * this.memoryThreshold) {
      this.logger.warn(
        `Memory usage approaching limit: ${currentUsage}MB/${maxMemory}MB, triggering cleanup`,
      );
    ...
    if (currentUsage > maxMemory) {
      this.logger.error(
        `Memory usage exceeded limit: ${currentUsage}MB/${maxMemory}MB`,
      );
    ...
    return {
      ...
      peakUsage:
        this.memoryHistory.length > 0
          ? Math.max(...this.memoryHistory.map((h) => h.usage))
          : currentUsage,
      averageUsage:
        this.memoryHistory.length > 0
          ? this.memoryHistory.reduce((sum, h) => sum + h.usage, 0) /
