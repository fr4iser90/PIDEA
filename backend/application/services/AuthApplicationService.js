...
class AuthApplicationService {
  constructor(dependencies = {}) {
    this.authService =
      dependencies.authService || new AuthService(dependencies);
  ...
  async refresh(refreshToken) {
    try {
      this.logger.info(
        "AuthApplicationService: Refreshing authentication with refresh token",
      );
      ...
      const user = await this.authService.userRepository.findById(
        newSession.userId,
      );
  ...
  async getUserProfile(userId) {
    try {
      this.logger.info("AuthApplicationService: Getting user profile", {
        userId,
      });
  ...
  async updateUserProfile(userId, profileData) {
    try {
      this.logger.info("AuthApplicationService: Updating user profile", {
        userId,
      });
  ...
  async getUserSessions(userId) {
    try {
      this.logger.info("AuthApplicationService: Getting user sessions", {
        userId,
      });
