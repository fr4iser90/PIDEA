...
class CodeExplorerApplicationService {
  constructor(dependencies = {}) {
    this.browserManager =
      dependencies.browserManager || new BrowserManager(dependencies);
    this.logger =
      dependencies.logger || new Logger("CodeExplorerApplicationService");
  ...
  async exploreCodeStructure(projectPath, userId) {
    try {
      this.logger.info(
        "CodeExplorerApplicationService: Exploring code structure",
        { projectPath, userId },
      );

      const structure =
        await this.browserManager.exploreCodeStructure(projectPath);
      return {
        data: structure,
      };
    } catch (error) {
  ...
  async analyzeFile(filePath, userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Analyzing file", {
        filePath,
        userId,
      });
  ...
  async searchCode(projectPath, query, filters, userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Searching code", {
        projectPath,
        query,
        userId,
      });

      const results = await this.browserManager.searchCode(
        projectPath,
        query,
        filters,
      );
  ...
  async getDependencyGraph(projectPath, userId) {
    try {
      this.logger.info(
        "CodeExplorerApplicationService: Getting dependency graph",
        { projectPath, userId },
      );
  ...
  async getFileMetrics(filePath, userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Getting file metrics", {
        filePath,
        userId,
      });
  ...
  async getFileTree(userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Getting file tree", {
        userId,
      });
  ...
  async getFileContent(path, userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Getting file content", {
        path,
        userId,
      });
  ...
  async getCurrentFileInfo(userId) {
    try {
      this.logger.info(
        "CodeExplorerApplicationService: Getting current file info",
        { userId },
      );
  ...
  async refreshExplorer(userId) {
    try {
      this.logger.info("CodeExplorerApplicationService: Refreshing explorer", {
        userId,
      });
