...
class IDEMirrorApplicationService {
  constructor(dependencies = {}) {
    ...
    this.logger =
      dependencies.logger || new Logger("IDEMirrorApplicationService");
    this.eventBus = dependencies.eventBus;

    if (!this.ideMirrorService) {
      throw new Error(
        "IDEMirrorApplicationService requires ideMirrorService dependency",
      );
    }
  }
  ...
  async clickElement(selector, coordinates, userId) {
    try {
      this.logger.info("IDEMirrorApplicationService: Clicking element", {
        selector,
        coordinates,
        userId,
      });
  ...
  async switchIDE(port, userId) {
    try {
      this.logger.info("IDEMirrorApplicationService: Switching IDE", {
        port,
        userId,
      });
  ...
  async connectToIDE(userId) {
    try {
      this.logger.info("IDEMirrorApplicationService: Connecting to IDE", {
        userId,
      });
  ...
  async refreshIDEState(userId) {
    try {
      this.logger.info("IDEMirrorApplicationService: Refreshing IDE state", {
        userId,
      });
  ...
  async handleWebSocketInteraction(type, payload, userId) {
    try {
      this.logger.info(
        "IDEMirrorApplicationService: Handling WebSocket interaction",
        { type, userId },
      );
      ...
      switch (type) {
        ...
        case "type":
          return await this.typeTextAdvanced(
            payload.text,
            payload.selector,
            payload.key,
            payload.modifiers,
        ...
        case "focusAndType":
          return await this.focusAndType(
            payload.selector,
            payload.text,
            payload.clearFirst,
            userId,
