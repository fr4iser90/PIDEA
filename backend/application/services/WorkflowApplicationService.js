...
class WorkflowApplicationService {
      ...
      if (this.projectMappingService) {
        try {
          ...
          if (projectInfo && projectInfo.workspacePath) {
            this.logger.info(
              `✅ Using project mapping workspace: ${projectInfo.workspacePath}`,
            );
      ...
      if (this.ideManager) {
        try {
          ...
          if (activeIDE && activeIDE.port) {
            ...
            if (workspacePath) {
              this.logger.info(
                `✅ Using IDE detected workspace: ${workspacePath}`,
              );
  ...
  async executeWorkflow(workflowData, userContext) {
    try {
      ...
      this.logger.info(
        "WorkflowApplicationService: Starting workflow execution",
        {
          projectId,
          mode,
          hasTask: !!task,
      ...
      // Get workspace path
      const workspacePath =
        projectPath || (await this.getProjectWorkspacePath(projectId));
      ...
      const taskOptions =
        workflowData.taskId || options.taskId
          ? {
      ...
      if (clickNewChat && activeIDE) {
        try {
          ...
          this.logger.warn(
            "New chat creation failed, continuing workflow:",
            error.message,
          );
        }
      }
      ...
      if (taskOptions && taskOptions.taskId && this.taskService) {
        try {
          executionResult = await this.executeTaskWorkflow(
            taskOptions,
            workspacePath,
            userContext,
            activeIDE,
      ...
      return {
        data: {
          ...
          standardExecution:
            executionResult?.standardWorkflow || executionResult,
          activeIDE: activeIDE
            ? {
                port: activeIDE.port,
                type: activeIDE.type,
  ...
  async executeTaskWorkflow(
    ...
    try {
      ...
      const taskResult = await this.taskQueueService.enqueue(
        taskOptions.taskId,
        userContext.userId,
        {
  ...
  async executeStandardWorkflow(workflowParams) {
    try {
      ...
      switch (mode) {
        case "analysis":
          workflowResult =
            await this.workflowExecutionService.executeAnalysisWorkflow(
              stepOptions,
            );
          break;
        case "refactoring":
          workflowResult =
            await this.workflowExecutionService.executeRefactoringWorkflow(
              stepOptions,
            );
          break;
        case "full":
        default:
          workflowResult =
            await this.workflowExecutionService.executeFullWorkflow(
              stepOptions,
            );
          break;
      }
  ...
  async getWorkflowStatus(executionId) {
    try {
      ...
      const status =
        await this.workflowExecutionService.getExecutionStatus(executionId);
  ...
  async cancelWorkflow(executionId, userContext) {
    try {
      ...
      const result = await this.workflowExecutionService.cancelExecution(
        executionId,
        userContext.userId,
      );
      ...
      const templates = [
        {
          ...
          description:
            "Comprehensive project analysis including code quality, security, and architecture",
          steps: [
            "project_analysis_step",
            "structure_analysis_step",
            "pattern_analysis_step",
            "coupling_analysis_step",
            "layer_analysis_step",
            "trivy_analysis_step",
            "snyk_analysis_step",
            "semgrep_analysis_step",
            "zap_analysis_step",
            "secret_scanning_step",
            "compliance_analysis_step",
            "memory_analysis_step",
            "cpu_analysis_step",
            "network_analysis_step",
            "database_analysis_step",
          ],
