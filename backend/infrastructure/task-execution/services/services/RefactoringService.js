...
class RefactoringService {
  ...
  async executeRefactoringTask(execution) {
    try {
      ...
      const refactoringPlans = await this.generateRefactoringPlans(
        opportunities,
        execution,
      );
      ...
      if (execution.options.autoApply) {
        appliedRefactoring = await this.applyRefactoring(
          refactoringPlans,
          execution,
        );
      }
      ...
      const validationResults = await this.validateRefactoringChanges(
        appliedRefactoring,
        execution,
      );
      ...
      if (this.fileSystemService.isCodeFile(file)) {
        try {
          ...
          const fileOpportunities =
            await this.refactoringUtils.analyzeFileForRefactoring(
              content,
              file,
              refactoringType,
            );
          ...
          this.logger.error(
            "RefactoringService: Failed to analyze file for refactoring",
            {
              file: file,
              error: error.message,
            },
      ...
      try {
        ...
        switch (opportunity.type) {
          case EXECUTION_CONSTANTS.REFACTORING_TYPES.EXTRACT_METHOD:
            steps =
              await this.refactoringUtils.generateExtractMethodSteps(
                opportunity,
              );
            break;
          case EXECUTION_CONSTANTS.REFACTORING_TYPES.EXTRACT_CLASS:
            steps =
              await this.refactoringUtils.generateExtractClassSteps(
                opportunity,
              );
            break;
          case EXECUTION_CONSTANTS.REFACTORING_TYPES.RENAME:
            steps =
              await this.refactoringUtils.generateRenameSteps(opportunity);
            break;
          case EXECUTION_CONSTANTS.REFACTORING_TYPES.MOVE_METHOD:
            steps =
              await this.refactoringUtils.generateMoveMethodSteps(opportunity);
            break;
        }
        ...
        this.logger.error(
          "RefactoringService: Failed to generate refactoring plan",
          {
    ...
    for (const plan of refactoringPlans) {
      try {
        ...
        for (const step of plan.steps) {
          const stepResult = await this.refactoringUtils.applyRefactoringStep(
            step,
            plan.opportunity,
          );
    ...
    for (const change of appliedRefactoring) {
      if (change.success) {
        for (const stepChange of change.changes) {
          const validation =
            await this.refactoringUtils.validateRefactoringChange(stepChange);
        ...
        validationResults.issues.push(
          `Failed to apply refactoring: ${change.error}`,
        );
      }
    }
  ...
  async getTargetFiles(target, execution) {
    if (typeof target === "string") {
      ...
      return await this.fileSystemService.getAllFiles(
        execution.options.projectPath,
      );
    }
  }
