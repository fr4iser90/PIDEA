...
class DeploymentService {
  ...
  async buildApplication(target, execution) {
    try {
      ...
      if (await this.fileSystemService.exists(packageJsonPath)) {
        ...
        buildCommand =
          'make build || ./build.sh || echo "No build command found"';
      }
      ...
      const result = await this.scriptExecutor.executeScript(buildCommand, {
        cwd: projectPath,
        timeout:
          execution.options.timeout || EXECUTION_CONSTANTS.EXECUTION_TIMEOUT,
      });
  ...
  async runPreDeploymentChecks(target, environment, execution) {
    ...
    try {
      ...
      this.logger.error(
        "DeploymentService: Failed to run pre-deployment checks",
        {
          error: error.message,
        },
      );
  ...
  async runPostDeploymentChecks(target, environment, execution) {
    ...
    try {
      ...
      this.logger.error(
        "DeploymentService: Failed to run post-deployment checks",
        {
          error: error.message,
        },
      );
  ...
  async deployLocal(projectPath, environment) {
    return { type: "local", environment };
  }

  async deployDocker(projectPath, environment) {
    return { type: "docker", environment };
  }

  async deployCloud(projectPath, environment) {
    return { type: "cloud", environment };
  }
