...
class RefactoringUtils {
    ...
    try {
      switch (refactoringType) {
        case EXECUTION_CONSTANTS.REFACTORING_TYPES.EXTRACT_METHOD:
          opportunities.push(
            ...this.findExtractMethodOpportunities(content, filePath),
          );
          break;
        case EXECUTION_CONSTANTS.REFACTORING_TYPES.EXTRACT_CLASS:
          opportunities.push(
            ...this.findExtractClassOpportunities(content, filePath),
          );
          break;
        case EXECUTION_CONSTANTS.REFACTORING_TYPES.RENAME:
          opportunities.push(
            ...this.findRenameOpportunities(content, filePath),
          );
          break;
        case EXECUTION_CONSTANTS.REFACTORING_TYPES.MOVE_METHOD:
          opportunities.push(
            ...this.findMoveMethodOpportunities(content, filePath),
          );
          break;
      }
    } catch (error) {
      this.logger.error(
        "RefactoringUtils: Failed to analyze file for refactoring",
        {
          filePath: filePath,
          error: error.message,
        },
  ...
  findExtractMethodOpportunities(content, filePath) {
    ...
    while ((match = longMethodRegex.exec(content)) !== null) {
      opportunities.push({
        ...
        description:
          "Long method detected - consider extracting smaller methods",
        confidence: 0.8,
      });
    }
  ...
  findExtractClassOpportunities(content, filePath) {
    ...
    while ((match = largeClassRegex.exec(content)) !== null) {
      opportunities.push({
        ...
        line: content.substring(0, match.index).split("\n").length,
        description:
          "Large class detected - consider extracting smaller classes",
        confidence: 0.7,
      });
    }
  ...
  async generateExtractMethodSteps(opportunity) {
    return [
      ...
      },
      {
        ...
        description: "Create new method with extracted code",
      },
      {
  ...
  async generateExtractClassSteps(opportunity) {
    return [
      {
        action: "identify_responsibilities",
        description: "Identify class responsibilities",
      },
      {
        action: "create_new_class",
        description: "Create new class with extracted responsibilities",
      },
      {
        action: "update_original_class",
        description: "Update original class to use new class",
      },
      {
        action: "update_references",
        description: "Update all references to use new class",
      },
    ];
  }
    ...
    return [
      {
        ...
        description: "Find all references to the item",
      },
      {
        action: "update_declaration",
        description: "Update the declaration with new name",
      },
      {
        action: "update_references",
        description: "Update all references with new name",
      },
      {
        action: "run_tests",
        description: "Run tests to ensure nothing is broken",
      },
    ];
  }
  ...
  async generateMoveMethodSteps(opportunity) {
    return [
      {
        action: "identify_target_class",
        description: "Identify the target class for the item",
      },
      { action: "move_method", description: "Move method to target class" },
      {
        action: "update_references",
        description: "Update all references to the moved method",
      },
      {
        action: "remove_original",
        description: "Remove original method declaration",
      },
    ];
  }
