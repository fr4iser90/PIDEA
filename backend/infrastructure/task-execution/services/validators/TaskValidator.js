...
class TaskValidator {
  ...
  validateTask(task) {
    ...
    if (!task.type) {
      ...
      !Object.values(EXECUTION_CONSTANTS.TASK_TYPES).includes(task.type)
    ) {
  ...
  validateTaskOptions(options) {
    ...
    if (options.timeout) {
      ...
      } else if (options.timeout > 3600000) {
        // 1 hour
  ...
  validateExecution(execution) {
    ...
    if (!execution.status) {
      ...
      !Object.values(EXECUTION_CONSTANTS.EXECUTION_STATUS).includes(
        execution.status,
      )
    ) {
