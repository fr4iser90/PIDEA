...
Please analyze the project and execute improvements automatically. Provide detailed progress updates and results.`,
  ...
  async sendPrompt(promptType, data, options = {}) {
    try {
      ...
      if (cachedResponse && !options.skipCache) {
        this.logger.debug("AIService: Using cached response", {
          promptType,
          cacheKey,
        });
      ...
      const aiRequest = {
        ...
        maxTokens:
          options.maxTokens ||
          this.aiModels[options.model || "gpt-4"]?.maxTokens ||
          4096,
        temperature:
          options.temperature ||
          this.aiModels[options.model || "gpt-4"]?.temperature ||
          0.7,
  ...
  async sendToCursorIDE(aiRequest, options = {}) {
    try {
      ...
      if (!this.cursorIDEService) {
        this.logger.warn(
          "AIService: CursorIDEService not available, returning mock response",
        );
        return {
  ...
  async generateTaskSuggestions(context, options = {}) {
    try {
      ...
      if (!this.cursorIDEService) {
        this.logger.warn(
          "AIService: CursorIDEService not available, returning empty suggestions",
        );
        return {
          suggestions: [],
  ...
  getStats() {
    return {
      ...
      rateLimits: Array.from(this.rateLimits.entries()).map(
        ([userId, limits]) => ({
          userId,
