...
class TaskProcessor {
  ...
  async getActiveIDE() {
    if (this.ideManager) {
      try {
        ...
        if (!activeIDE) {
          this.logger.info(
            "TaskProcessor: No active IDE found, getting available IDEs",
          );
        ...
        this.logger.error(
          "TaskProcessor: Error getting active IDE:",
          error.message,
        );
        return null;
      }
  ...
  setupEventListeners() {
    if (!this.eventBus) {
      this.logger.warn(
        "TaskProcessor: No event bus available, skipping event listeners",
      );
      return;
    }
    ...
    this.eventBus.on("queue:item:added", (data) => {
      ...
      if (data.options?.autoExecute !== false) {
        this.logger.info(
          "TaskProcessor: Auto-execute enabled, processing queue immediately",
        );
        this.processExecutionQueue();
      } else {
        this.logger.info(
          "TaskProcessor: Auto-execute disabled, task queued but not executed",
        );
      }
    });
  ...
  async processExecutionQueue() {
    if (!this.taskQueueStore) {
      this.logger.warn(
        "TaskProcessor: No TaskQueueStore available, skipping queue processing",
      );
      return;
    }
    ...
    try {
      ...
      this.logger.info("TaskProcessor: Debug - TaskQueueStore status", {
        ...
        projectQueuesKeys: projectQueues
          ? Array.from(projectQueues.keys())
          : "undefined",
      });

      if (!projectQueues || projectQueues.size === 0) {
        this.logger.warn(
          "TaskProcessor: No project queues available, skipping queue processing",
        );
        return;
      }
      ...
      for (const [projectId, projectQueue] of projectQueues) {
        const queuedTasks = projectQueue.filter(
          (item) =>
            item.status === "queued" && item.options?.autoExecute !== false,
        );
        totalQueuedTasks += queuedTasks.length;

        // Debug: Log each project queue status
        this.logger.info(
          `TaskProcessor: Debug - Project ${projectId} queue status`,
          {
      ...
      this.logger.info(
        `TaskProcessor: Processing execution queue - ${totalQueuedTasks} tasks queued`,
      );
      ...
      for (const [projectId, projectQueue] of projectQueues) {
        ...
        const runningTasks = projectQueue.filter(
          (item) => item.status === "running",
        );

        if (runningTasks.length > 0) {
          this.logger.info(
            `TaskProcessor: ${runningTasks.length} tasks already running in project ${projectId}, skipping`,
          );
          continue;
        }
        ...
        if (queuedTasks.length > 0) {
          ...
          try {
            ...
            if (this.eventBus) {
              ...
              this.logger.warn(
                "TaskProcessor: No eventBus available, cannot emit start event",
              );
            }
            ...
            this.logger.info("TaskProcessor: Calling workflowExecutor.run()", {
              ...
              workflowExecutorMethods: this.workflowExecutor
                ? Object.getOwnPropertyNames(
                    Object.getPrototypeOf(this.workflowExecutor),
                  )
                : "undefined",
            });
            ...
            let projectPath =
              queueItem.context.projectPath || queueItem.options.projectPath;
            if (!projectPath) {
              this.logger.info(
                "TaskProcessor: projectPath not in context, resolving from database...",
              );
            ...
            this.logger.info(
              "TaskProcessor: Passing context to WorkflowExecutor",
              {
                taskId: queueItem.context.taskId,
                projectId: projectId,
                activeIDE: activeIDE
            ...
            if (!this.workflowExecutor.workflowLoaderService) {
              this.logger.warn(
                "TaskProcessor: WorkflowLoaderService not available, loading manually",
              );
              ...
              this.workflowExecutor.workflowLoaderService =
            ...
            this.logger.info(
              "TaskProcessor: workflowExecutor.run() completed",
              {
                taskId: queueItem.context.taskId,
                result: result,
              },
            ...
            if (this.eventBus) {
              this.eventBus.emit(
                EXECUTION_CONSTANTS.EVENTS.EXECUTION_COMPLETE,
                {
                  taskId: queueItem.context.taskId,
                  queueItemId: queueItem.id,
                  projectId: projectId,
                  ...
                  completedAt: queueItem.completedAt,
                  executionTime:
                    Date.now() - new Date(queueItem.startedAt).getTime(),
                },
              );
              this.logger.info(
                "TaskProcessor: Emitted EXECUTION_COMPLETE event",
                {
                  taskId: queueItem.context.taskId,
                  queueItemId: queueItem.id,
                },
              ...
              this.logger.warn(
                "TaskProcessor: No eventBus available, cannot emit completion event",
              );
            }

            // 🔄 SIMPLE: Trigger next task processing after completion
            this.logger.info(
              "TaskProcessor: Task completed, triggering next task processing",
            );
            setTimeout(() => {
              this.processExecutionQueue();
            }, 2000); // 2 second delay
            ...
            if (this.eventBus) {
              ...
              this.logger.warn(
                "TaskProcessor: No eventBus available, cannot emit failure event",
              );
            }

            // 🔄 SIMPLE: Trigger next task processing even after failure
            this.logger.info(
              "TaskProcessor: Task failed, triggering next task processing",
            );
            setTimeout(() => {
              this.processExecutionQueue();
            }, 2000);
  ...
  async resolveProjectPath(projectId) {
    try {
      ...
      if (this.taskQueueStore && this.taskQueueStore.projectRepository) {
        const project =
          await this.taskQueueStore.projectRepository.findById(projectId);
        if (project && project.workspacePath) {
          this.logger.info(
            `TaskProcessor: Found project path in database: ${project.workspacePath}`,
          );
      ...
      this.logger.warn(
        `TaskProcessor: Using fallback project path: ${fallbackPath}`,
      );
      return fallbackPath;
    } catch (error) {
      this.logger.error(
        "TaskProcessor: Failed to resolve project path:",
        error,
      );
      // Fallback to current working directory
      const fallbackPath = process.cwd();
      this.logger.warn(
        `TaskProcessor: Using fallback project path after error: ${fallbackPath}`,
      );
      return fallbackPath;
    }
  }
