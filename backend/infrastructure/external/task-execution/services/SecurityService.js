...
class SecurityService {
  ...
  async executeSecurityTask(execution) {
    try {
      ...
      const automatedChecks = await this.runAutomatedSecurityChecks(
        target,
        scanType,
      );
      ...
      const result = {
        ...
        summary: {
          ...
          highRiskIssues: securityResult.vulnerabilities.filter(
            (v) => v.severity === "high",
          ).length,
  ...
  async collectProjectData(target, execution) {
    ...
    } = require("../../../di/ProjectContextService");
    ...
    const projectStructure =
      await this.fileSystemService.getProjectStructure(projectPath);

    // Get dependency information
    const dependencies =
      await this.fileSystemService.getDependencyInfo(projectPath);

    // Get configuration files
    const configFiles =
      await this.fileSystemService.getConfigurationFiles(projectPath);
  ...
  async runAutomatedSecurityChecks(target, scanType) {
    ...
    try {
      // Scan dependencies
      if (
        scanType === EXECUTION_CONSTANTS.SECURITY_SCAN_TYPES.DEPENDENCIES ||
        scanType === "all"
      ) {
      ...
      // Scan code
      if (
        scanType === EXECUTION_CONSTANTS.SECURITY_SCAN_TYPES.CODE ||
        scanType === "all"
      ) {
      ...
      // Scan configuration
      if (
        scanType === EXECUTION_CONSTANTS.SECURITY_SCAN_TYPES.CONFIGURATION ||
        scanType === "all"
      ) {
      ...
      this.logger.error(
        "SecurityService: Failed to run automated security checks",
        {
          error: error.message,
        },
      );
