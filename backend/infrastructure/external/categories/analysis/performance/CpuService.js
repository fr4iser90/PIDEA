...
class CpuService {
  ...
  async analyze(params) {
    try {
      this.logger.info("Starting CPU analysis", {
  ...
  getSystemCpuInfo() {
    ...
    const usagePercent =
      ((totalCpu.total - totalCpu.idle) / totalCpu.total) * 100;
  ...
  async getProcessCpuInfo(projectPath) {
    try {
      ...
      this.logger.warn("Failed to get process CPU info", {
        error: error.message,
      });
      return [];
    }
  ...
  async getProcessCpu(pid) {
    try {
      const { stdout } = await execAsync(
        `ps -p ${pid} -o pid,pcpu,time --no-headers`,
        { timeout: 5000 },
      );
