...
class PatternAnalysisService {
  ...
  async analyzeCodePatterns(projectPath, config) {
    ...
    for (const file of files) {
      const filePatterns = await this.detectPatternsInFile(
        file,
        config.patterns,
      );
  ...
  detectSingletonPattern(content, filePath) {
    const patterns = [];
    const singletonRegex =
      /class\s+\w+\s*\{[^}]*static\s+getInstance\s*\(\)[^}]*\}/gs;
  ...
  detectFactoryPattern(content, filePath) {
    const patterns = [];
    const factoryRegex =
      /(create|build|make)\w*\s*\([^)]*\)\s*\{[^}]*return\s+new\s+\w+/gi;
  ...
  detectDecoratorPattern(content, filePath) {
    ...
    const decoratorRegex =
      /@\w+|function\s+\w+\s*\([^)]*\)\s*\{[^}]*return\s+function/gi;
  ...
  calculatePatternSummary(patterns) {
    ...
    for (const pattern of patterns) {
      ...
      summary.byConfidence[pattern.confidence] =
        (summary.byConfidence[pattern.confidence] || 0) + 1;
    }
  ...
  generatePatternRecommendations(patterns) {
    ...
    if (summary.totalPatterns === 0) {
      recommendations.push({
        type: "improvement",
        message:
          "No design patterns detected. Consider implementing common patterns for better code organization.",
        severity: "medium",
      });
    }
    ...
    if (summary.byType["singleton"] > 5) {
      recommendations.push({
        type: "warning",
        message:
          "Many singleton patterns detected. Consider if all are necessary.",
        severity: "medium",
      });
    }
    ...
    if (summary.patternScore > 50) {
      recommendations.push({
        type: "positive",
        message:
          "Good use of design patterns detected. Code follows established architectural principles.",
        severity: "low",
      });
    }
