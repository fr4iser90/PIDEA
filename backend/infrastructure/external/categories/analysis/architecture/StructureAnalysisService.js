...
class StructureAnalysisService {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (
            config.fileTypes.includes(ext) ||
            config.fileTypes.includes("*")
          ) {
  ...
  analyzeNode(node, analysis, depth) {
    if (node.type === "file") {
      analysis.totalFiles++;
      analysis.fileTypes[node.extension] =
        (analysis.fileTypes[node.extension] || 0) + 1;
  ...
  generateStructureRecommendations(analysis) {
    ...
    if (analysis.totalFiles > 50) {
      recommendations.push({
        type: "organization",
        message:
          "Consider organizing files into subdirectories to improve project structure.",
        severity: "medium",
      });
    }
    ...
    if (analysis.directoryDepth > 8) {
      recommendations.push({
        type: "complexity",
        message:
          "Very deep directory structure detected. Consider flattening the hierarchy.",
        severity: "medium",
      });
    }
    ...
    if (analysis.largestFiles.length > 5) {
      recommendations.push({
        type: "optimization",
        message:
          "Multiple large files detected. Consider splitting large files into smaller modules.",
        severity: "high",
      });
    }
    ...
    if (analysis.structureScore < 70) {
      recommendations.push({
        type: "improvement",
        message:
          "Project structure could be improved. Review organization and file sizes.",
        severity: "medium",
      });
    }
