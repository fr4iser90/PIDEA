...
class LayerAnalysisService {
  ...
  async analyze(params) {
    try {
      this.logger.info("Starting layer analysis", {
      ...
      const result = await this.analyzeLayerOrganization(
        projectPath,
        layerConfig,
      );
  ...
  async checkLayerViolations(layerPath, layerName, config) {
    ...
    for (const file of files) {
      const fileViolations = await this.checkFileLayerViolations(
        file,
        layerName,
      );
  ...
  async checkFileLayerViolations(filePath, layerName) {
    ...
    try {
      ...
      this.logger.warn("Failed to check file violations", {
        file: filePath,
        error: error.message,
      });
    }
  ...
  checkImportViolation(importPath, currentLayer, filePath) {
    ...
    for (const [importedLayer, allowedLayers] of Object.entries(
      layerDependencies,
    )) {
      if (
        importPath.includes(importedLayer) &&
  ...
  generateLayerRecommendations(layers, violations) {
    ...
    if (summary.totalFiles > 0) {
      ...
      if (avgFilesPerLayer < 3) {
        recommendations.push({
          ...
          message:
            "Some layers have very few files. Consider consolidating layers.",
          severity: "medium",
        });
      }
    }
    ...
    if (summary.layerScore > 80) {
      recommendations.push({
        type: "positive",
        message:
          "Good layer organization detected. Architecture follows clean architecture principles.",
        severity: "low",
      });
    }
