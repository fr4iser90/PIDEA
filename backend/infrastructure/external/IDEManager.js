...
class IDEManager {
  ...
  async initialize() {
    ...
    if (existingIDEs.length > 0) {
      logger.log('[IDEManager] Detecting workspace paths for', existingIDEs.length, 'IDEs (ONCE during startup)');
  ...
  getActiveWorkspacePath() {
    logger.log('[IDEManager] getActiveWorkspacePath called');
    logger.log('[IDEManager] activePort:', this.activePort);
    logger.log('[IDEManager] ideWorkspaces entries:', Array.from(this.ideWorkspaces.entries()));
    
    if (!this.activePort) {
      logger.log('[IDEManager] No active port set');
      return null;
    }
    
    const workspacePath = this.ideWorkspaces.get(this.activePort);
    logger.log('[IDEManager] Workspace path for active port', this.activePort, ':', workspacePath);
  ...
  setWorkspacePath(port, workspacePath) {
    ...
    logger.log('[IDEManager] Set workspace path for port', port, ':', workspacePath);
  }

  // FILE-BASIERTE METHODEN
  async getWorkspaceInfo(port) {
    if (!this.fileDetector) {
      logger.error('[IDEManager] File detector not available');
      return null;
    }
    ...
    try {
      ...
      logger.error('[IDEManager] Error getting workspace info for port', port, ':', error.message);
      return null;
    }
  }
  ...
  async getFilesList(port) {
    ...
    try {
      ...
      logger.error('[IDEManager] Error getting files list for port', port, ':', error.message);
      return [];
    }
  }
  ...
  async getGitStatus(port) {
    ...
    try {
      ...
      logger.error('[IDEManager] Error getting git status for port', port, ':', error.message);
      return null;
    }
  }
  ...
  async executeTerminalCommand(port, command, outputFile = null) {
    ...
    try {
      ...
      logger.error('[IDEManager] Error executing terminal command for port', port, ':', error.message);
      return null;
    }
  }
  ...
  async detectWorkspacePathsForAllIDEs() {
    ...
    if (idesWithoutWorkspace.length === 0) {
      logger.log('[IDEManager] All IDEs already have workspace paths, skipping detection');
      return;
    }
    
    logger.log(`[IDEManager] Detecting workspace paths for ${idesWithoutWorkspace.length} IDEs (${allIDEs.length - idesWithoutWorkspace.length} already have paths)`);
  ...
  async cleanup() {
    ...
    try {
      ...
      logger.log('[IDEManager] Error during cleanup:', error.message);
    }
  }
}
