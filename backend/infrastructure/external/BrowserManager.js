...
class BrowserManager {
  ...
  getPerformanceStats() {
    ...
    return {
      ...
      averageTime:
        this.switchTimes.reduce((a, b) => a + b, 0) / this.switchTimes.length,
      minTime: sorted[0],
  ...
  async connect(port = null) {
    ...
    try {
      logger.info(
        `Getting connection for port ${this.currentPort} from pool...`,
      );
      ...
      logger.error(
        `Connection failed on port ${this.currentPort}:`,
        error.message,
      );
      this.browser = null;
      this.page = null;
  ...
  async switchToPort(port) {
    ...
    try {
      ...
      logger.debug(
        `Connection pool health before switch: ${JSON.stringify(poolHealth)}`,
      );
      ...
      logger.debug(
        `Connection pool health after switch: ${JSON.stringify(poolHealthAfter)}`,
      );
  ...
  async connectToPortForDetection(port) {
    ...
    try {
      ...
      logger.error(
        `Failed to connect to port ${port} for detection:`,
        error.message,
      );
      throw error;
    }
  ...
  async getFileExplorerTree() {
    try {
      ...
      if (!explorerSelector) {
        throw new Error(
          "Explorer container selector not found in configuration",
        );
      }
      ...
      const flatFiles = await page.evaluate((selectors) => {
        const container = document.querySelector(
          selectors.projectManagementSelectors.projectExplorer?.container,
        );
        if (!container) return [];

        const rows = container.querySelectorAll(
          selectors.projectManagementSelectors.projectExplorer?.rows,
        );
        return Array.from(rows)
          .map((item) => {
            const labelElement = item.querySelector(
              selectors.projectManagementSelectors.projectExplorer?.labelName,
            );
            ...
            const isDirectory =
              item.querySelector(
                selectors.projectManagementSelectors.projectExplorer
                  ?.treeItemExpanded,
              ) !== null;
            ...
            let path = "";
            if (ariaLabel) {
              const pathMatch = ariaLabel.match(
                /~\/Documents\/Git\/PIDEA\/(.+)/,
              );
              if (pathMatch) {
                path = pathMatch[1];
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (
          ...
          (ariaLabel && ariaLabel.includes(fileName))
        ) {
          logger.info("Found matching file, clicking:", {
            ariaLabel,
            textContent,
          });
          await row.click();
      ...
      const tabSelector = fileOperationSelectors.fileTabs?.tabByLabel?.replace(
        "{fileName}",
        fileName,
      );
      try {
  ...
  async getCurrentFileContent() {
    try {
      ...
      const chatSelectors = selectors.chatSelectors;
      if (chatSelectors) {
        await page.waitForSelector(chatSelectors.monacoEditor, {
          timeout: 5000,
        });
      }
      ...
      const result = await page.evaluate(
        (selectors) => {
          ...
          const activeTab = document.querySelector(
            selectors.fileOperationSelectors.fileTabs?.activeTab,
          );
          let fileName = "";
          if (activeTab) {
            ...
            for (const editor of allEditors) {
              try {
                ...
                if (
                  ...
                  model.uri.path &&
                  fileName &&
                  model.uri.path.endsWith(fileName)
                ) {
                  targetEditor = editor;
        ...
        {
          fileTabs: fileOperationSelectors.fileTabs,
          monacoEditor: chatSelectors.monacoEditor,
          contentEditable: chatSelectors.contentEditable,
          textarea: chatSelectors.textarea,
          viewLine: chatSelectors.viewLine,
        },
      );
  ...
  async getFileContent(filePath) {
    try {
      ...
      logger.error(
        `Error getting file content for ${filePath}:`,
        error.message,
      );
      throw error;
    }
    ...
    try {
      ...
      const fileInfo = await page.evaluate((selectors) => {
        ...
        const tabTitle = activeTab
          .querySelector(selectors.fileOperationSelectors.fileTabs?.tabTitle)
          ?.textContent?.trim();
  ...
  async refreshExplorer() {
    try {
      ...
      if (
        projectManagementSelectors &&
        projectManagementSelectors.projectExplorer?.refreshButton
      ) {
  ...
  async clickNewChat() {
    try {
      ...
      if (!button) {
        throw new Error(
          `New Chat button not found with selector: ${newChatButtonSelector}`,
        );
      }
      ...
      await button.click();
      logger.info(
        `Successfully clicked New Chat button using selector: ${newChatButtonSelector}`,
      );
  ...
  async handleNewChatModal(page, modal, fileOperationSelectors) {
    try {
      ...
      if (!button) {
        throw new Error(
          `Modal button not found with selector: ${buttonSelector}`,
        );
      }
  ...
  async detectIDEVersion(port) {
    try {
      logger.info(
        `Detecting version on port ${port} using CDP /json/version endpoint`,
      );
      ...
      if (!response.ok) {
        throw new Error(
          `CDP version endpoint failed: ${response.status} ${response.statusText}`,
        );
      }
  ...
  async detectIDEType(port) {
    ...
    throw new Error(
      `Unknown IDE type for port ${port}. Port must be between 9222-9251.`,
    );
  }
  ...
  async getIDESelectors(ideType, version) {
    if (!version) {
      throw new Error(
        `Version is required for ${ideType}. No fallbacks allowed. Please specify exact version.`,
      );
    }
  ...
  async typeMessage(message, send = true) {
    try {
      ...
      logger.info(
        `Detected IDE type: ${ideType} version: ${version} on port ${currentPort}`,
      );
          ...
          if (isVisible) {
            ...
            try {
              ...
              if (!closeButtonSelector) {
                throw new Error(
                  "Modal close button selector not found in configuration",
                );
              }
              ...
              if (!closeButton) {
                throw new Error(
                  `Modal close button not found with selector: ${closeButtonSelector}`,
                );
              }
      ...
      if (send) {
        ...
        if (ideType === "cursor") {
          ...
          if (!sendButtonSelector) {
            throw new Error(
              "VSCode send button selector not found in configuration",
            );
          }
          ...
          if (!sendButton) {
            throw new Error(
              `VSCode send button not found with selector: ${sendButtonSelector}`,
            );
          }
  ...
  isConnected() {
    return (
      this.currentPort !== null && this.browser !== null && this.page !== null
    );
  }
