...
class GitService {
  ...
  async initRepository(repoPath, options = {}) {
    ...
    try {
      this.logger.info("GitService: Initializing repository using step", {
        repoPath,
        bare,
        initialBranch,
      });
      ...
      const result = await this.stepRegistry.executeStep(
        "GitInitRepositoryStep",
        stepContext,
      );
  ...
  async cloneRepository(url, targetPath, options = {}) {
    ...
    try {
      this.logger.info("GitService: Cloning repository using step", {
        url,
        targetPath,
        branch,
        depth,
  ...
  async getCurrentBranch(repoPath, userId, projectId) {
    try {
      ...
      if (result.success) {
        ...
        const currentBranch =
          result.result?.currentBranch || result.currentBranch;
  ...
  async getBranches(repoPath, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Getting branches using step", {
        repoPath,
        includeRemote,
        includeLocal,
      });
      ...
      if (result.success) {
        this.logger.info("GitService: Step execution result", {
  ...
  async createBranch(repoPath, branchName, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Creating branch using step", {
        repoPath,
        branchName,
      });
  ...
  async checkoutBranch(repoPath, branchName, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Checking out branch using step", {
        repoPath,
        branchName,
        createIfNotExists,
      });
  ...
  async getCommitHistory(repoPath, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Getting commit history using step", {
        repoPath,
        limit,
        since,
        until,
  ...
  async getLastCommit(repoPath, userId, projectId) {
    try {
      this.logger.info("GitService: Getting last commit using step", {
        repoPath,
      });
  ...
  async addFiles(repoPath, files = [], userId, projectId) {
    try {
      this.logger.info("GitService: Adding files using step", {
        repoPath,
        files,
      });
  ...
  async commitChanges(repoPath, message, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Committing changes using step", {
        repoPath,
        message,
      });
  ...
  async pushChanges(repoPath, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Pushing changes using step", {
        repoPath,
        remote,
      });
  ...
  async pullChanges(repoPath, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Pulling changes using step", {
        repoPath,
        remote,
        branch,
        rebase,
  ...
  async getRemoteUrl(repoPath, remote = "origin", userId, projectId) {
    try {
      this.logger.info("GitService: Getting remote URL using step", {
        repoPath,
        remote,
      });
  ...
  async addRemote(repoPath, name, url, userId, projectId) {
    try {
      this.logger.info("GitService: Adding remote using step", {
        repoPath,
        name,
        url,
      });
  ...
  async getDiff(repoPath, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Getting diff using step", {
        repoPath,
        staged,
        file,
        commit1,
  ...
  async resetRepository(
    ...
    try {
      this.logger.info("GitService: Resetting repository using step", {
        repoPath,
        mode,
        commit,
      });
  ...
  async mergeBranch(repoPath, branchName, options = {}, userId, projectId) {
    ...
    try {
      this.logger.info("GitService: Merging branch using step", {
        repoPath,
        branchName,
        strategy,
        noFF,
