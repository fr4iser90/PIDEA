...
class ServiceRegistry {
    ...
    this.container.register('workflowOrchestrationService', (cursorIDEService, autoFinishSystem, taskRepository, eventBus, logger) => {
      ...
      return new WorkflowOrchestrationService({
        ...
        this.container.register('integrationSystem', (eventBus, logger) => {
            const { utils } = require('../../domain/workflows/integration');
            return utils.createIntegrationSystem({ eventBus, logger });
        }, { singleton: true, dependencies: ['eventBus', 'logger'] });
        this.container.register('integrationRepository', (databaseConnection) => {
            const IntegrationRepository = require('../database/repositories/IntegrationRepository');
            return new IntegrationRepository(databaseConnection);
