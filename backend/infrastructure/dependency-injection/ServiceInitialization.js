...
class ServiceInitialization {
  ...
  async getInfrastructureServices(serviceRegistry) {
    try {
      const services = {
        ...
        userSessionRepository: serviceRegistry.getService(
          "userSessionRepository",
        ),
        ideWorkspaceDetectionService: serviceRegistry.getService(
      ...
      this.logger.error(
        "Failed to get infrastructure services:",
        error.message,
      );
  ...
  async initializeDomainServices(serviceRegistry, databaseConnection) {
    ...
    if (!validation.isValid) {
      ...
      throw new Error(
        `Dependency validation failed: ${JSON.stringify(validation)}`,
      );
    }
    ...
    this.logger.info(
      `Service startup completed: ${startupResults.started.length} started, ${startupResults.failed.length} failed`,
    );
    ...
    const taskSessionRepository =
      databaseConnection.getRepository("TaskSession");
  ...
  async getDomainServices(serviceRegistry) {
    try {
      const services = {
        ...
        recommendationsService: serviceRegistry.getService(
          "recommendationsService",
        ),
        subprojectDetector: serviceRegistry.getService("subprojectDetector"),
        analysisOutputService: serviceRegistry.getService(
        ...
        projectMappingService: serviceRegistry.getService(
          "projectMappingService",
        ),
        taskRepository: serviceRegistry.getService("taskRepository"),
        taskExecutionRepository: serviceRegistry.getService(
          "taskExecutionRepository",
        ),
        taskService: serviceRegistry.getService("taskService"),
        taskValidationService: serviceRegistry.getService(
      ...
      const dependencyInfo = serviceRegistry
        .getContainer()
  ...
  async initializeWorkflowLoaderService(workflowLoaderService) {
    ...
    this.logger.info(
      "✅ WorkflowLoaderService loaded and initialized from service registry",
    );
  }
  ...
  async initializeFrameworkInfrastructure(stepRegistry) {
    try {
      ...
      const frameworkInfrastructure =
        await initializeFrameworkInfrastructure(stepRegistry);

      // Log initialization results
      this.logger.info("Framework Infrastructure initialized");
      this.logger.info(
        "Framework initialization results:",
        frameworkInfrastructure.initializationResults,
      );
      ...
      if (frameworkInfrastructure.stepRegistry.getHealthStatus) {
        const stepHealthStatus =
          frameworkInfrastructure.stepRegistry.getHealthStatus();
        this.logger.info(
          "Framework Step Registry health status:",
          stepHealthStatus,
        );
      }
      ...
      this.logger.warn(
        "Framework Infrastructure initialization failed, continuing without framework support:",
        error.message,
      );
      ...
      return {
        ...
        validator: {
          validateFramework: () => Promise.resolve({ isValid: true }),
        },
        ...
        stepRegistry: {
          ...
          getHealthStatus: () => ({
            isHealthy: false,
            error: "Framework step registry not initialized",
          }),
        },
  ...
  async setupProjectContext(projectContext) {
    ...
    if (!validation.isValid) {
      this.logger.warn(
        "Project context validation warnings:",
        validation.warnings,
      );
    } else {
  ...
  async initializeApplicationHandlers(serviceRegistry) {
    ...
    const createTaskHandler = new CreateTaskHandler({
      taskRepository: serviceRegistry.getService("taskRepository"),
      taskTemplateRepository: serviceRegistry.getService(
      ...
      taskGenerationService: serviceRegistry.getService(
    ...
    const versionManagementService = serviceRegistry.getService(
    ...
    if (!versionManagementService) {
      ...
      const versionAIIntegration = serviceRegistry.getService(
        "versionAIIntegration",
      );
      this.logger.error(
        "  - versionAIIntegration:",
        versionAIIntegration ? "✅ EXISTS" : "❌ MISSING",
      );

      // Check aiVersionAnalysisService
      const aiVersionAnalysisService = serviceRegistry.getService(
        "aiVersionAnalysisService",
      );
      this.logger.error(
        "  - aiVersionAnalysisService:",
        aiVersionAnalysisService ? "✅ EXISTS" : "❌ MISSING",
      );
      ...
      throw new Error(
        `DI Container failed to resolve versionManagementService. Check logs above for detailed analysis.`,
      );
    }
    ...
    this.logger.info("🔍 Creating VersionManagementHandler with:", {
      ...
      versionManagementServiceMethods: versionManagementService
        ? Object.getOwnPropertyNames(
            Object.getPrototypeOf(versionManagementService),
          )
        : "N/A",
    ...
    this.logger.info("🔍 VersionManagementHandler created. Internal state:", {
      hasVersionManagementService:
        !!versionManagementHandler.versionManagementService,
      versionManagementServiceType:
        typeof versionManagementHandler.versionManagementService,
      versionManagementServiceMethods:
        versionManagementHandler.versionManagementService
          ? Object.getOwnPropertyNames(
