...
class ServiceRegistry {
  ...
  registerInfrastructureServices() {
    ...
    this.container.register(
      ...
      () => {
        const {
          TerminalService,
        } = require("@domain/services/terminal/TerminalService");
  ...
  registerDomainServices() {
    ...
    this.container.register(
      ...
      {
        singleton: true,
        dependencies: ["ideManager", "browserManager", "ideMirrorService"],
      },
    );
    ...
    this.container.register(
      ...
      (fileSystemService, logger, aiVersionAnalysisService) => {
    ...
    this.container.register(
      "hybridVersionDetector",
      (aiVersionAnalysisService, versionManagementService) => {
      ...
      {
        singleton: true,
        dependencies: ["aiVersionAnalysisService", "versionManagementService"],
      },
    );
    ...
    this.container.register(
      "ideService",
      (browserManager, ideManager, eventBus, ideFactory) => {
    ...
    this.container.register(
      "taskAnalysisService",
      (interfaceManager, eventBus, logger, aiService, projectAnalyzer) => {
    ...
    this.container.register(
      ...
      (taskRepository, interfaceManager, eventBus, fileSystemService) => {
    ...
    this.container.register(
      "taskQueueService",
      (taskRepository, queueTaskExecutionService, logger) => {
      ...
      {
        singleton: true,
        dependencies: ["taskRepository", "queueTaskExecutionService", "logger"],
      },
    );
    ...
    this.container.register(
      "queueTaskExecutionService",
      (taskQueueStore, taskRepository, eventBus, logger) => {
    ...
    this.container.register(
      "chatSessionService",
      (chatRepository, browserManager, ideManager, eventBus, logger) => {
    ...
    this.container.register(
      ...
      (browserManager, ideManager, eventBus, logger) => {
      ...
      {
        singleton: true,
        dependencies: ["browserManager", "ideManager", "eventBus", "logger"],
      },
    );
    ...
    this.container.register(
      ...
      (interfaceManager, taskRepository, eventBus, logger) => {
    ...
    this.container.register(
      ...
      (browserManager, ideManager, eventBus, ideFactory) => {
        ...
        const result = factory.createIDE("cursor", {
          browserManager,
          ideManager,
          eventBus,
        });
    ...
    this.container.register(
      ...
      (userSessionRepository, eventBus) => {
    ...
    this.container.register(
      "authService",
      (userRepository, userSessionRepository, sessionActivityService) => {
        const AuthService = require("@domain/services/security/AuthService");
        return new AuthService(
          userRepository,
          userSessionRepository,
          "simple-jwt-secret",
          "simple-refresh-secret",
          sessionActivityService,
        );
      },
      {
    ...
    this.container.register(
      ...
      (
        ...
        return new TaskService(
          ...
          eventBus,
          this,
        ); // Pass serviceRegistry as last parameter
      },
      {
        singleton: true,
        dependencies: [
    ...
    this.container.register(
      ...
      (browserManager, taskService, taskRepository, fileSystemService) => {
        const ManualTasksImportService = require("@domain/services/task/ManualTasksImportService");
        return new ManualTasksImportService(
          browserManager,
          taskService,
          taskRepository,
          fileSystemService,
    ...
    this.container.register(
      ...
      (interfaceManager, taskRepository, logger, eventBus) => {
        const WorkflowOrchestrationService = require("@domain/services/workflow/WorkflowOrchestrationService");
        return new WorkflowOrchestrationService({
          interfaceManager,
          taskRepository,
    ...
    this.container.register(
      "queueHistoryService",
      (queueHistoryRepository, eventBus) => {
  ...
  registerApplicationServices() {
    ...
    this.container.register(
      ...
      (logger, eventBus, contentRepository) => {
      ...
      {
        singleton: true,
        dependencies: ["logger", "eventBus", "contentRepository"],
      },
    );

    // Code Explorer Application Service - coordinates code exploration
    this.container.register(
      "codeExplorerApplicationService",
      (logger, eventBus, browserManager) => {
      ...
      {
        singleton: true,
        dependencies: ["logger", "eventBus", "browserManager"],
      },
    );
    ...
    this.container.register(
      ...
      (ideMirrorService, logger, eventBus) => {
      ...
      {
        singleton: true,
        dependencies: ["ideMirrorService", "logger", "eventBus"],
      },
    );
  ...
  registerExternalServices() {
    ...
    this.container.register(
      "workflowOrchestrationService",
      (taskRepository, eventBus, logger, stepRegistry) => {
        const WorkflowOrchestrationService = require("@domain/services/workflow/WorkflowOrchestrationService");
        return new WorkflowOrchestrationService({
      ...
      {
        singleton: true,
        dependencies: ["taskRepository", "eventBus", "logger", "stepRegistry"],
      },
    );
    ...
    this.container.register(
      ...
      (testOrchestrator, testFixer, eventBus, logger) => {
      ...
      {
        singleton: true,
        dependencies: ["testOrchestrator", "testFixer", "eventBus", "logger"],
      },
    );

    this.container.register(
      "generateTestsHandler",
      (aiService, projectAnalyzer, eventBus, logger) => {
        const GenerateTestsHandler = require("@application/handlers/categories/generate/GenerateTestsHandler");
        return new GenerateTestsHandler(
          aiService,
          projectAnalyzer,
          eventBus,
          logger,
      ...
      {
        singleton: true,
        dependencies: ["aiService", "projectAnalyzer", "eventBus", "logger"],
      },
    );
  ...
  registerStrategyServices() {
    ...
    this.container.register(
      "monorepoStrategy",
      (logger, eventBus, fileSystemService) => {
        // TODO: Replace with RepositoryTypeAnalysisStep after integration
        return {
          isMonorepo: async () => false,
          analyzeMonorepo: async () => ({
            stub: true,
            message: "Replaced with RepositoryTypeAnalysisStep",
          }),
        };
      },
      {
        singleton: true,
        dependencies: ["logger", "eventBus", "fileSystemService"],
      },
    );
  ...
  registerExternalService(serviceName) {
    switch (serviceName) {
      ...
      case "analysisOrchestrator":
        // AnalysisOrchestrator REMOVED - redundant with AnalysisApplicationService
        throw new Error(
          "AnalysisOrchestrator has been removed - use AnalysisApplicationService instead",
        );
        break;
      case "testOrchestrator":
        this.container.register(
          ...
          {
            singleton: true,
            dependencies: ["stepRegistry", "eventBus", "logger"],
          },
        );
        break;
      ...
      case "gitService":
        this.container.register(
          ...
          {
            singleton: true,
            dependencies: ["logger", "eventBus", "stepRegistry"],
          },
        );
        break;
      default:
    ...
    switch (serviceName) {
      ...
      case "ideWorkspaceDetectionService":
        this.container.register(
          ...
          (ideManager, projectRepository) => {
            const IDEWorkspaceDetectionService = require("@domain/services/ide/IDEWorkspaceDetectionService");
            const service = new IDEWorkspaceDetectionService(
              ideManager,
              projectRepository,
            );
          ...
          {
            singleton: true,
            dependencies: ["ideManager", "projectRepository"],
          },
        );
        break;
      ...
      case "workflowExecutor":
        this.container.register(
          "workflowExecutor",
          (stepRegistry, taskRepository, workflowLoaderService, logger) => {
            const WorkflowExecutor = require("@domain/services/WorkflowExecutor");
            return new WorkflowExecutor({
              stepRegistry,
              taskRepository,
      ...
      case "ideFactory":
        this.container.register(
          ...
          () => {
            const {
              getIDEFactory,
            } = require("@domain/services/ide/IDEFactory");
            return getIDEFactory();
          },
          { singleton: true },
        );
      ...
      case "getChatHistoryHandler":
        this.container.register(
          ...
          (chatRepository, ideManager, serviceRegistry, chatCacheService) => {
      ...
      case "sessionActivityService":
        this.container.register(
          ...
          {
            singleton: true,
            dependencies: ["userSessionRepository", "eventBus"],
          },
        );
        break;
      case "authService":
      ...
      case "queueHistoryService":
        this.container.register(
          ...
          {
            singleton: true,
            dependencies: ["queueHistoryRepository", "eventBus"],
          },
        );
        break;
  ...
  registerHandlerService(serviceName) {
    throw new Error(
      `Handler service '${serviceName}' should be managed by HandlerRegistry, not ServiceRegistry`,
    );
  }
  ...
  collectServiceDefinitions() {
    ...
    this.addServiceDefinition(
      "interfaceManager",
      ["browserManager", "ideManager", "eventBus", "stepRegistry"],
      "domain",
    );
    ...
    this.addServiceDefinition(
      "manualTasksImportService",
      ["browserManager", "taskService", "taskRepository"],
      "domain",
    );
  ...
  getServicesByCategory(category) {
    ...
    for (const [
      serviceName,
      serviceCategory,
    ] of this.serviceOrderResolver.serviceCategories.entries()) {
    ...
    try {
      ...
      try {
        this.projectContextService.initialize({
          projectMappingService: this.container.resolve(
            "projectMappingService",
          ),
          workspacePathDetector: this.container.resolve(
            "workspacePathDetector",
          ),
        });
      } catch (error) {
  ...
  registerServiceByName(serviceName) {
    const category =
      this.serviceOrderResolver.serviceCategories.get(serviceName);
    ...
    switch (category) {
      ...
      default:
        throw new Error(
          `Unknown service category for '${serviceName}': ${category}`,
        );
    }
  }
