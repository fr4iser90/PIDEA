...
class ServiceOrderResolver {
  ...
  addService(serviceName, dependencies = [], category = "unknown") {
    ...
    this.logger.debug(
      `Added service: ${serviceName} (${category}) with dependencies: [${dependencies.join(", ")}]`,
    );
  }
  ...
  resolveOrder() {
    try {
      ...
      this.logger.info(
        `Service order resolved successfully: ${sortedServices.length} services in ${registrationPlan.length} categories`,
      );
  ...
  getReadyServices(registeredServices) {
    ...
    for (const serviceName of this.dependencyGraph.nodes.keys()) {
      if (!registeredServices.includes(serviceName)) {
        const validation = this.validateServiceRegistration(
          serviceName,
          registeredServices,
        );
  ...
  getServicesByCategory(category) {
    ...
    for (const [
      serviceName,
      serviceCategory,
    ] of this.serviceCategories.entries()) {
