...
class ProjectContextService {
  ...
  async getProjectId() {
    ...
    if (!projectId) {
      ...
      if (projectPath && this.projectMappingService) {
        projectId =
          await this.projectMappingService.getProjectIdFromWorkspace(
            projectPath,
          );
        this.container.setProjectContext({ projectId });
      }
  ...
  async autoDetectProjectPath() {
    try {
      ...
      const logger = new ServiceLogger("PIDEAProjectContextService");
      logger.info(
        "🔍 No cached project info found, performing live detection...",
      );
      ...
      if (monorepoSubdirs.includes(currentDirName)) {
        ...
        const hasMultipleSubdirs =
          monorepoSubdirs.filter((subdir) => parentFiles.includes(subdir))
            .length >= 2;

        if (hasMonorepoIndicators && hasMultipleSubdirs) {
          logger.info(
            "🏗️ Detected monorepo, using parent directory:",
            parentDir,
          );
  ...
  async getCachedProjectInfo(projectPath) {
    try {
      ...
      const parentProject =
        await projectRepository.findByWorkspacePath(parentDir);
  ...
  async cacheProjectInfo(projectPath, projectType) {
    try {
      ...
      if (!projectRepository) {
        logger.warn(
          "ProjectRepository not available in DI container yet, skipping cache",
        );
        return;
      }

      logger.info("Attempting to cache project info:", {
        projectPath,
        projectType,
      });
      ...
      const result = await projectRepository.findOrCreateByWorkspacePath(
        projectPath,
        {
          id: projectId,
          name: projectName,
  ...
  async validateProjectContext() {
    ...
    if (context.projectPath) {
      try {
        ...
        if (!stats.isDirectory()) {
          ...
          result.errors.push(
            `Project path is not a directory: ${context.projectPath}`,
          );
        }
      } catch (error) {
        result.isValid = false;
        result.errors.push(
          `Project path does not exist: ${context.projectPath}`,
        );
      }
    } else {
    ...
    if (
      ...
      context.workspacePath !== context.projectPath
    ) {
      try {
        const stats = await fs.stat(context.workspacePath);
        if (!stats.isDirectory()) {
          result.warnings.push(
            `Workspace path is not a directory: ${context.workspacePath}`,
          );
        }
      } catch (error) {
        result.warnings.push(
          `Workspace path does not exist: ${context.workspacePath}`,
        );
      }
    }
