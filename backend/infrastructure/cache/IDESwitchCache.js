...
class IDESwitchCache {
  constructor(options = {}) {
    ...
    logger.info(
      `IDESwitchCache initialized with TTL: ${this.ttl}ms, MaxSize: ${this.maxSize}, CleanupInterval: ${this.cleanupInterval}ms`,
    );
  }
  ...
  async getCachedSwitch(port) {
    try {
      ...
      if (
        this.performanceMetrics.totalMisses % 50 === 0 &&
        this.performanceMetrics.totalMisses > 0
      ) {
        logger.debug(
          `IDESwitchCache Debug - Cache miss #${this.performanceMetrics.totalMisses}, Cache size: ${this.cache.size}/${this.maxSize}`,
        );
      }
      ...
      this.performanceMetrics.totalMisses++;
      logger.error(
        `Error getting cached switch for port ${port}:`,
        error.message,
      );
      return null;
    }
  ...
  setCachedSwitch(port, result) {
    try {
      ...
      if (this.cache.size >= this.maxSize) {
        logger.warn(
          `IDESwitchCache full (${this.cache.size}/${this.maxSize}), evicting oldest entries`,
        );
        this.evictOldest();
      }
      ...
      logger.debug(
        `Cached switch result for port ${port}, cache size: ${this.cache.size}, total sets: ${this.performanceMetrics.totalSets}`,
      );
    } catch (error) {
  ...
  cleanup() {
    try {
      ...
      if (heapUsageMB > 50) {
        // If heap usage > 50MB
        logger.warn(
          `High memory usage in cache: ${heapUsageMB.toFixed(2)}MB, but keeping cache for IDE switches`,
        );
      }
  ...
  evictOldest() {
    try {
      ...
      if (this.cache.size >= this.maxSize * 0.8) {
        logger.warn(
          `IDESwitchCache very full (${this.cache.size}/${this.maxSize}), but keeping cache for IDE switches`,
        );
        return;
      }
