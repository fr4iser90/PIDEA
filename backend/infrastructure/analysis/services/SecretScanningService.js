...
class SecretScanningService {
  constructor() {
    ...
    this.baseUrl =
      process.env.SECRET_SCAN_API_URL || "https://api.trufflesecurity.com";
  ...
  async analyze(params) {
    try {
      this.logger.info("Starting secret scanning analysis", {
  ...
  async scanWithExternalAPI(projectPath, config) {
    ...
    try {
      ...
      this.logger.error("External secret scanning failed", {
        error: error.message,
      });
      return [];
    }
    ...
    const patterns = [
      {
        name: "API Key",
        pattern:
          /(api[_-]?key|apikey)\s*[:=]\s*['"`]?([a-zA-Z0-9]{32,})['"`]?/gi,
        severity: "high",
      },
      {
        name: "JWT Token",
      ...
      },
      {
        name: "Access Token",
        pattern:
          /(access[_-]?token|token)\s*[:=]\s*['"`]?([a-zA-Z0-9]{32,})['"`]?/gi,
        severity: "high",
      },
    ];
