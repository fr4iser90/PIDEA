...
class FrameworkLoader {
  ...
  async initialize() {
    try {
      ...
      try {
        this.domainFrameworkSystem = await initializeFrameworks(
          this.frameworkConfigs,
        );
      } catch (domainError) {
        logger.warn(
          `⚠️ Domain framework system initialization failed:`,
          domainError.message,
        );
      }
  ...
  async discoverFrameworks() {
    try {
      ...
      if (process.env.LOG_LEVEL === "debug") {
        logger.debug(
          `🔍 Discovered ${this.frameworkPaths.size} framework directories`,
        );
      }
    } catch (error) {
    ...
    try {
      for (const [frameworkName, frameworkPath] of this.frameworkPaths) {
        try {
          ...
          logger.error(
            `❌ Failed to load framework "${frameworkName}":`,
            error.message,
          );
        }
      }
  ...
  async loadFrameworkConfig(frameworkName, frameworkPath) {
    try {
      ...
      try {
        ...
        logger.warn(
          `⚠️ No config file found for framework "${frameworkName}", creating default config`,
        );
      ...
      logger.debug(
        `❌ Failed to load framework config "${frameworkName}":`,
        error.message,
      );
    ...
    try {
      ...
      if (frameworkInfo.domainRegistered) {
        try {
          ...
          logger.info(
            `✅ Unregistered framework "${frameworkName}" from domain registry`,
          );
        } catch (registryError) {
          logger.warn(
            `⚠️ Failed to unregister framework "${frameworkName}" from domain registry:`,
      ...
      logger.error(
        `❌ Failed to unload framework "${frameworkName}":`,
        error.message,
      );
      throw error;
    }
  ...
  getFrameworksByCategory(category) {
    return Array.from(this.loadedFrameworks.values()).filter(
      (framework) => framework.config.category === category,
    );
  }
  ...
  getHealthStatus() {
    ...
    const loadedFrameworks = Array.from(this.loadedFrameworks.values()).filter(
      (f) => f.status === "loaded",
    ).length;
    const failedFrameworks = Array.from(this.loadedFrameworks.values()).filter(
      (f) => f.status === "failed",
    ).length;
    ...
    return {
      ...
      isHealthy:
        failedFrameworks === 0 && loadedFrameworks > 0 && domainInitialized,
    };
  }
}
