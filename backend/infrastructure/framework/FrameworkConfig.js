...
class FrameworkConfig {
  ...
  getDefaultConfig() {
    return {
      ...
      security: {
        allowedPaths: ["framework/", "domain/", "infrastructure/"],
        forbiddenPaths: [
          "node_modules/",
          ".git/",
          "package.json",
  ...
  validateConfig() {
    ...
    if (
      typeof this.config.framework.maxFrameworks !== "number" ||
      this.config.framework.maxFrameworks <= 0
    ) {
      errors.push("framework.maxFrameworks must be a positive number");
    }

    if (
      typeof this.config.framework.timeout !== "number" ||
      this.config.framework.timeout <= 0
    ) {
      errors.push("framework.timeout must be a positive number");
    }

    if (
      typeof this.config.framework.retryAttempts !== "number" ||
      this.config.framework.retryAttempts < 0
    ) {
    ...
    if (
      typeof this.config.security.maxFileSize !== "number" ||
      this.config.security.maxFileSize <= 0
    ) {
    ...
    if (
      typeof this.config.performance.maxParallelLoads !== "number" ||
      this.config.performance.maxParallelLoads <= 0
    ) {
  ...
  setupEnvironmentVariables() {
    ...
    process.env.FRAMEWORK_MAX_FRAMEWORKS =
      this.config.framework.maxFrameworks.toString();
    process.env.FRAMEWORK_TIMEOUT = this.config.framework.timeout.toString();
    process.env.FRAMEWORK_RETRY_ATTEMPTS =
      this.config.framework.retryAttempts.toString();
    process.env.FRAMEWORK_CACHE_ENABLED =
      this.config.framework.cacheEnabled.toString();
    process.env.FRAMEWORK_CACHE_TIMEOUT =
      this.config.framework.cacheTimeout.toString();
    process.env.FRAMEWORK_VALIDATION_ENABLED =
      this.config.framework.validationEnabled.toString();
    process.env.FRAMEWORK_SECURITY_ENABLED =
      this.config.framework.securityEnabled.toString();
    ...
    process.env.FRAMEWORK_LAZY_LOADING =
      this.config.performance.lazyLoading.toString();
    process.env.FRAMEWORK_PARALLEL_LOADING =
      this.config.performance.parallelLoading.toString();
    process.env.FRAMEWORK_MAX_PARALLEL_LOADS =
      this.config.performance.maxParallelLoads.toString();
    process.env.FRAMEWORK_MEMORY_LIMIT =
      this.config.performance.memoryLimit.toString();
    process.env.FRAMEWORK_CPU_LIMIT =
      this.config.performance.cpuLimit.toString();

    // Set logging settings
    process.env.FRAMEWORK_LOGGING_ENABLED =
      this.config.logging.enabled.toString();
    process.env.FRAMEWORK_LOG_LEVEL = this.config.logging.level;
    process.env.FRAMEWORK_LOG_FORMAT = this.config.logging.format;
    process.env.FRAMEWORK_LOG_MAX_FILES =
      this.config.logging.maxFiles.toString();
  ...
  async updateConfig(updates) {
    try {
      ...
      logger.error(
        "❌ Failed to update framework configuration:",
        error.message,
      );
      throw error;
    }
  ...
  async resetConfig() {
    try {
      ...
      logger.error(
        "❌ Failed to reset framework configuration:",
        error.message,
      );
      throw error;
    }
  ...
  getEnvironmentVariables() {
    ...
    envVars.FRAMEWORK_MAX_FRAMEWORKS =
      this.config.framework.maxFrameworks.toString();
    envVars.FRAMEWORK_TIMEOUT = this.config.framework.timeout.toString();
    envVars.FRAMEWORK_RETRY_ATTEMPTS =
      this.config.framework.retryAttempts.toString();
    envVars.FRAMEWORK_CACHE_ENABLED =
      this.config.framework.cacheEnabled.toString();
    envVars.FRAMEWORK_CACHE_TIMEOUT =
      this.config.framework.cacheTimeout.toString();
    envVars.FRAMEWORK_VALIDATION_ENABLED =
      this.config.framework.validationEnabled.toString();
    envVars.FRAMEWORK_SECURITY_ENABLED =
      this.config.framework.securityEnabled.toString();
    ...
    envVars.FRAMEWORK_LAZY_LOADING =
      this.config.performance.lazyLoading.toString();
    envVars.FRAMEWORK_PARALLEL_LOADING =
      this.config.performance.parallelLoading.toString();
    envVars.FRAMEWORK_MAX_PARALLEL_LOADS =
      this.config.performance.maxParallelLoads.toString();
    envVars.FRAMEWORK_MEMORY_LIMIT =
      this.config.performance.memoryLimit.toString();
