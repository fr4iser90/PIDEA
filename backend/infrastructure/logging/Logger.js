...
class Logger {
    ...
    return printf(({ level, message, service, ...meta }) => {
      ...
      if (meta && Object.keys(meta).length) {
        ...
        if (Object.keys(cleanMeta).length > 0) {
          ...
          const isStringSerialized = keys.every(
            (key) => /^\d+$/.test(key) && cleanMeta[key].length === 1,
          );
  ...
  apiRequest(method, path, statusCode, duration, meta = {}) {
    ...
    this.logger[level](
      `${emoji} API ${method} ${path} - ${statusCode} (${duration}ms)`,
      meta,
    );
  }
...
function getLogger(serviceName = "PIDEA") {
  ...
  return {
    info: (message, meta = {}) =>
      singletonLogger.info(message, { ...meta, service: serviceName }),
    warn: (message, meta = {}) =>
      singletonLogger.warn(message, { ...meta, service: serviceName }),
    error: (message, meta = {}) =>
      singletonLogger.error(message, { ...meta, service: serviceName }),
    debug: (message, meta = {}) =>
      singletonLogger.debug(message, { ...meta, service: serviceName }),
    success: (message, meta = {}) =>
      singletonLogger.success(message, { ...meta, service: serviceName }),
    failure: (message, meta = {}) =>
      singletonLogger.failure(message, { ...meta, service: serviceName }),
    ...
    get isDevelopment() {
      return singletonLogger.isDevelopment;
    },
    get isProduction() {
      return singletonLogger.isProduction;
    },
  };
}
