...
class PostgreSQLTaskExecutionRepository {
  ...
  async initTable() {
    ...
    await this.databaseConnection.execute(
      `CREATE INDEX IF NOT EXISTS idx_task_executions_task_id ON ${this.tableName} (task_id)`,
    );
    await this.databaseConnection.execute(
      `CREATE INDEX IF NOT EXISTS idx_task_executions_status ON ${this.tableName} (status)`,
    );
    await this.databaseConnection.execute(
      `CREATE INDEX IF NOT EXISTS idx_task_executions_started_at ON ${this.tableName} (started_at)`,
    );
  }
  ...
  async create(taskExecution, now) {
    ...
    const params = [
      ...
      taskExecution.completedAt
        ? taskExecution.completedAt.toISOString()
        : null,
      ...
      taskExecution.resourceUsage
        ? JSON.stringify(taskExecution.resourceUsage)
        : null,
  ...
  async findCompleted(limit = 100) {
    ...
    const rows = await this.databaseConnection.query(selectSQL, [
