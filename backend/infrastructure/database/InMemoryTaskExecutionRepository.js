...
class InMemoryTaskExecutionRepository {
    ...
    try {
      ...
      if (options.status) {
        executions = executions.filter(
          (exec) => exec.status === options.status,
        );
      }

      if (options.taskId) {
        executions = executions.filter(
          (exec) => exec.taskId === options.taskId,
        );
      }

      if (options.startDate) {
        executions = executions.filter(
          (exec) => new Date(exec.createdAt) >= new Date(options.startDate),
        );
      }

      if (options.endDate) {
        executions = executions.filter(
          (exec) => new Date(exec.createdAt) <= new Date(options.endDate),
        );
      }
  ...
  async deleteByTaskId(taskId) {
    try {
      ...
      throw new Error(
        `Failed to delete executions by task ID: ${error.message}`,
      );
    }
  }
  ...
  async count(filter = {}) {
    try {
      ...
      if (filter.startDate) {
        executions = executions.filter(
          (exec) => new Date(exec.createdAt) >= new Date(filter.startDate),
        );
      }

      if (filter.endDate) {
        executions = executions.filter(
          (exec) => new Date(exec.createdAt) <= new Date(filter.endDate),
        );
      }
    ...
    try {
      ...
      if (allExecutions.length > 0) {
        ...
        const recentExecutions = allExecutions.filter(
          (exec) => new Date(exec.createdAt) >= last24Hours,
        );
  ...
  async findByDateRange(startDate, endDate) {
    try {
      ...
      throw new Error(
        `Failed to find executions by date range: ${error.message}`,
      );
    }
  }
  ...
  async findLongRunning(thresholdMinutes = 30) {
    try {
      ...
      for (const execution of this.executions.values()) {
        if (execution.status === "running") {
          const startTime = new Date(
            execution.startTime || execution.createdAt,
          );
      ...
      throw new Error(
        `Failed to find long running executions: ${error.message}`,
      );
    }
  }
