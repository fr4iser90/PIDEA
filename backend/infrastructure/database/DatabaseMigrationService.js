...
class DatabaseMigrationService {
  ...
  async initialize() {
    ...
    try {
      ...
      this.logger.error(
        "❌ Database migration service initialization failed:",
        error,
      );
      throw error;
    }
  ...
  async getAppliedMigrations() {
    try {
      ...
      if (result.rows) {
        ...
        this.logger.warn(
          "🔍 [getAppliedMigrations] Unexpected result format:",
          result,
        );
        rows = [];
      }
  ...
  async runMigration(migrationFile) {
    ...
    try {
      ...
      if (this.databaseConnection.getType() === "sqlite") {
        ...
        this.logger.info(
          `🔄 Found ${statements.length} SQL statements in migration`,
        );
        ...
        for (let i = 0; i < statements.length; i++) {
          ...
          if (!statement.trim() || statement.trim().startsWith("--")) {
            this.logger.debug(
              `🔧 Skipping empty/comment statement ${i + 1}: ${statement.substring(0, 50)}...`,
            );
            continue;
          }
        ...
        try {
          ...
          if (
            ...
            this.logger.warn(
              `⚠️ Migration ${migrationFile} skipped - constraint/table already exists: ${error.message}`,
            );
      ...
      this.logger.info(
        `✅ Migration completed: ${migrationFile} (${executionTime}ms)`,
      );
    } catch (error) {
      const executionTime = Date.now() - startTime;
      ...
      if (this.databaseConnection.getType() === "postgresql") {
        try {
          ...
          this.logger.warn(
            "⚠️ Could not reset transaction after error:",
            rollbackError.message,
          );
        }
      }
  ...
  parseSQLStatements(sql) {
    ...
    while (remainingSQL.length > 0) {
      ...
      if (doBlockMatch) {
        ...
        remainingSQL = remainingSQL.substring(
          doBlockMatch.index + doBlock.length,
        );
      } else {
        // No more DO blocks, split by semicolons normally
        const semicolonIndex = remainingSQL.indexOf(";");
        if (semicolonIndex >= 0) {
          const statement = remainingSQL
            .substring(0, semicolonIndex + 1)
            .trim();
