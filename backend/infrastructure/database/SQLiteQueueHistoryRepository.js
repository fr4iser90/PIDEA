/**
 * SQLiteQueueHistoryRepository - SQLite implementation for queue history operations
...
class SQLiteQueueHistoryRepository {
  constructor(databaseConnection) {
    this.logger = new Logger('SQLiteQueueHistoryRepository');
    ...
    this.logger.info('SQLiteQueueHistoryRepository initialized');
  }
  ...
  async deleteOlderThan(cutoffDate) {
    try {
      ...
      // Delete items
      const deleteSql = `DELETE FROM ${this.tableName} WHERE id IN (${deletedIds.map(() => '?').join(',')})`;
  ...
  async getStatistics(filters = {}) {
    try {
      ...
      const typeDistribution = typeResults.reduce((acc, row) => {
        acc[row.workflow_type] = row.count;
        return acc;
      }, {});
      ...
      const statistics = {
        ...
        uniqueTypes: stats.unique_types || 0,
        uniqueUsers: stats.unique_users || 0,
        typeDistribution: typeDistribution,
        successRate: stats.total_items > 0 ? (stats.completed_count / stats.total_items) * 100 : 0
      };
  ...
  buildWhereClause(filters) {
    ...
    if (filters.search) {
      conditions.push('(workflow_id LIKE ? OR error_message LIKE ?)');
  ...
  parseHistoryItem(row) {
    return {
      ...
      stepsData: row.steps_data ? JSON.parse(row.steps_data) : [],
...
module.exports = SQLiteQueueHistoryRepository; 