...
describe("FrameworkValidator", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should initialize successfully", async () => {
      ...
      expect(frameworkValidator.validationRules).to.be.an("object");
      expect(frameworkValidator.validationRules.requiredFields).to.be.an(
        "array",
      );
  ...
  describe("Framework Validation", () => {
    ...
    it("should detect missing required fields", async () => {
      ...
      const result =
        await frameworkValidator.validateFramework(invalidFramework);

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("version"))).to.be
        .true;
      expect(result.errors.some((error) => error.includes("description"))).to.be
        .true;
      expect(result.errors.some((error) => error.includes("category"))).to.be
        .true;
    });
    ...
    it("should validate field types", async () => {
      ...
      expect(result.errors.some((error) => error.includes("workflows"))).to.be
        .true;
      expect(result.errors.some((error) => error.includes("dependencies"))).to
        .be.true;
    });
    ...
    it("should validate step structure", async () => {
      ...
      const result = await frameworkValidator.validateFramework(
        frameworkWithInvalidSteps,
      );

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("invalid_step"))).to
        .be.true;
    });
    ...
    it("should validate workflow structure", async () => {
      ...
      const result = await frameworkValidator.validateFramework(
        frameworkWithInvalidWorkflows,
      );

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("invalid_workflow")))
        .to.be.true;
    });
    ...
    it("should validate activation settings", async () => {
      ...
      const result = await frameworkValidator.validateFramework(
        frameworkWithInvalidActivation,
      );

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("auto_load"))).to.be
        .true;
      expect(
        result.errors.some((error) => error.includes("requires_confirmation")),
      ).to.be.true;
      expect(result.errors.some((error) => error.includes("fallback_to_core")))
        .to.be.true;
    });
    ...
    it("should validate settings structure", async () => {
      ...
      const result = await frameworkValidator.validateFramework(
        frameworkWithInvalidSettings,
      );

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("max_file_size"))).to
        .be.true;
      expect(result.errors.some((error) => error.includes("backup_enabled"))).to
        .be.true;
      expect(
        result.errors.some((error) => error.includes("validation_enabled")),
      ).to.be.true;
      expect(result.errors.some((error) => error.includes("undo_enabled"))).to
        .be.true;
    });
  });
  ...
  describe("Dependency Validation", () => {
    ...
    it("should validate circular dependencies", async () => {
      ...
      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("circular"))).to.be
        .true;
    });
    ...
    it("should validate workflow step dependencies", async () => {
      ...
      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("missing_step"))).to
        .be.true;
    });
  });
  ...
  describe("Activation Validation", () => {
    ...
    it("should validate activation with missing dependencies", async () => {
      ...
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(
        result.errors.some((error) => error.includes("missing_dependency")),
      ).to.be.true;
    });
  });
  ...
  describe("Custom Validation Rules", () => {
    ...
    it("should apply custom validation rules", async () => {
      const customRule = {
        ...
        validate: (framework) => {
          if (framework.name.length < 5) {
            return {
              valid: false,
              error: "Framework name must be at least 5 characters",
            };
          }
          return { valid: true };
        },
      ...
      const result =
        await frameworkValidator.validateFramework(shortNameFramework);

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("5 characters"))).to
        .be.true;
    });
    ...
    it("should handle multiple custom validation rules", async () => {
      ...
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(
        result.errors.some((error) => error.includes("Author is required")),
      ).to.be.true;
  ...
  describe("Error Handling", () => {
    ...
    it("should handle dependency validation errors gracefully", async () => {
      ...
      sandbox
        .stub(frameworkValidator, "checkDependency")
        .rejects(new Error("Dependency check failed"));
    ...
    it("should handle activation validation errors gracefully", async () => {
      ...
      sandbox
        .stub(frameworkValidator, "validateDependencies")
        .rejects(new Error("Activation validation failed"));
