...
describe("FrameworkConfig", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should load configuration from file if exists", async () => {
      ...
      expect(frameworkConfig.isInitialized).to.be.true;
      expect(frameworkConfig.config.frameworkBasePath).to.equal(
  ...
  describe("Configuration Management", () => {
    ...
    it("should update individual configuration settings", async () => {
      ...
      const result = await frameworkConfig.updateSetting(
        "autoLoadFrameworks",
        true,
      );
    ...
    it("should validate configuration before updating", async () => {
      ...
      expect(result.errors.some((error) => error.includes("frameworkBasePath")))
        .to.be.true;
      expect(
        result.errors.some((error) => error.includes("autoLoadFrameworks")),
      ).to.be.true;
    });
    ...
    it("should reset configuration to defaults", async () => {
      ...
      expect(result.success).to.be.true;
      expect(frameworkConfig.config.frameworkBasePath).to.not.equal(
        "/custom/path",
      );
  ...
  describe("Configuration Validation", () => {
    ...
    it("should detect invalid boolean fields", () => {
      ...
      expect(
        result.errors.some((error) => error.includes("requireConfirmation")),
      ).to.be.true;
      expect(result.errors.some((error) => error.includes("fallbackToCore"))).to
        .be.true;
    });
    ...
    it("should detect invalid numeric fields", () => {
      ...
      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("maxFileSize"))).to.be
        .true;
    });
    ...
    it("should validate path accessibility", async () => {
      ...
      const result = await frameworkConfig.validateConfigAsync(
        configWithInvalidPath,
      );

      expect(result.valid).to.be.false;
      expect(result.errors).to.be.an("array").that.is.not.empty;
      expect(result.errors.some((error) => error.includes("not accessible"))).to
        .be.true;
    });
  });
