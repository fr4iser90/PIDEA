...
describe("Version Management Integration", () => {
  ...
  describe("VersionManagementService Integration", () => {
    it("should complete full workflow for new version", async () => {
      ...
      mockSelectorCollectionBot.collectSelectors.mockResolvedValue(
        mockSelectors,
      );
      ...
      // Run workflow
      const result = await versionManagementService.detectAndUpdateVersion(
        "cursor",
        9222,
      );
    ...
    it("should use cache for repeated requests", async () => {
      ...
      expect(mockVersionDetectionService.detectVersion).toHaveBeenCalledTimes(
        1,
      );
    });
  });
  ...
  describe("AutomationOrchestrator Integration", () => {
    ...
    it("should schedule detection tasks", async () => {
      ...
      versionManagementService.detectAndUpdateVersion = jest
        .fn()
        .mockResolvedValue({
          finalVersion: "1.7.0",
          isNewVersion: false,
      ...
      // Schedule detection
      const result = await automationOrchestrator.scheduleDetection(
        "cursor",
        9222,
        60000,
      );
      ...
      expect(automationOrchestrator.scheduledTasks.has("cursor:9222")).toBe(
        true,
      );
    ...
    it("should handle event handlers", () => {
      ...
      // Add event handlers
      automationOrchestrator.addEventHandler(
        "detection_success",
        successHandler,
      );
      ...
      // Emit events
      automationOrchestrator.emitEvent("detection_success", {
        ideType: "cursor",
      });
      automationOrchestrator.emitEvent("detection_error", {
        ideType: "cursor",
        error: "test",
      });
      ...
      automationOrchestrator.removeEventHandler(
        "detection_success",
        successHandler,
      );
      automationOrchestrator.emitEvent("detection_success", {
  ...
  describe("VersionController Integration", () => {
    ...
    it("should handle selector collection request", async () => {
      ...
      // Mock version management service
      versionManagementService.collectSelectorsForNewVersion = jest
        .fn()
        .mockResolvedValue({
    ...
    it("should handle automation start request", async () => {
      ...
      automationOrchestrator.startAutomaticDetection = jest
        .fn()
    ...
    it("should handle automation status request", async () => {
      ...
      automationOrchestrator.getScheduledTasksStatus = jest
        .fn()
        .mockReturnValue([{ ideType: "cursor", port: 9222, runCount: 5 }]);
