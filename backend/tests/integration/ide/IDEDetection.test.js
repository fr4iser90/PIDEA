const IDEDetectorFactory = require("@api/ide/IDEDetectorFactory");
const CursorDetector = require("@api/ide/detectors/CursorDetector");
const VSCodeDetector = require("@api/ide/detectors/VSCodeDetector");
const WindsurfDetector = require("@api/ide/detectors/WindsurfDetector");
...
describe("IDE Detection Integration", () => {
  ...
  describe("Detector Registration", () => {
    ...
    it("should reject invalid detector", () => {
      ...
      expect(() =>
        detectorFactory.registerDetector("invalid", invalidDetector),
  ...
  describe("Multi-IDE Detection", () => {
    it("should detect all IDE types concurrently", async () => {
      ...
      cursorDetector.scanForIDEs = jest
        .fn()
        .mockResolvedValue([
          { port: 9222, status: "running", url: "http://127.0.0.1:9222" },
        ]);

      vscodeDetector.scanForIDEs = jest
        .fn()
        .mockResolvedValue([
    ...
    it("should handle detection errors gracefully", async () => {
      ...
      windsurfDetector.scanForIDEs = jest
        .fn()
        .mockResolvedValue([
  ...
  describe("Type-Specific Detection", () => {
    ...
    it("should throw error for unknown IDE type", async () => {
      await expect(detectorFactory.detectByType("unknown")).rejects.toThrow(
        "Unsupported IDE type: unknown",
      );
    });
  });
  ...
  describe("Port Management", () => {
    ...
    it("should check port availability", async () => {
      ...
      expect(typeof result === "object" || typeof result === "boolean").toBe(
        true,
      );
    });
    ...
    it("should throw error for unsupported port operations", async () => {
      ...
      await expect(
        detectorFactory.findAvailablePort("limited"),
      ).rejects.toThrow(
        "Detector for limited does not support findAvailablePort",
      );
    });
  });
  ...
  describe("Configuration Integration", () => {
    it("should get detector configuration", () => {
      ...
      expect(cursorConfig === null || typeof cursorConfig === "object").toBe(
        true,
      );
      expect(vscodeConfig === null || typeof vscodeConfig === "object").toBe(
        true,
      );
      expect(
        windsurfConfig === null || typeof windsurfConfig === "object",
      ).toBe(true);
    });

    it("should throw error for unknown IDE type", () => {
      expect(() => detectorFactory.getDetectorConfig("unknown")).toThrow(
        "Unsupported IDE type: unknown",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle unsupported IDE type gracefully", () => {
      expect(() => detectorFactory.createDetector("unsupported")).toThrow(
        "Unsupported IDE type: unsupported",
      );
    });
