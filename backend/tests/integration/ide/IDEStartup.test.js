...
describe("IDE Startup Integration", () => {
  ...
  describe("Starter Registration", () => {
    it("should register custom starter", () => {
      const customStarter = {
        startIDE: jest
          .fn()
          .mockResolvedValue({ port: 9000, status: "running" }),
    ...
    it("should reject invalid starter", () => {
      ...
      expect(() =>
        starterFactory.registerStarter("invalid", invalidStarter),
  ...
  describe("IDE Startup", () => {
    it("should start cursor IDE successfully", async () => {
      ...
      const result = await starterFactory.startIDE(
        "cursor",
        9222,
        "/test/path",
      );
    ...
    it("should prevent starting IDE on occupied port", async () => {
      ...
      await expect(starterFactory.startIDE("cursor", 9222)).rejects.toThrow(
        "IDE already running on port 9222",
      );
    });
  ...
  describe("IDE Shutdown", () => {
    ...
    it("should throw error if no IDE running on port", async () => {
      await expect(starterFactory.stopIDE(9222)).rejects.toThrow(
        "No IDE process found on port 9222",
      );
    });
  ...
  describe("Configuration Integration", () => {
    ...
    it("should throw error for unknown IDE type", () => {
      expect(() => starterFactory.getStarterConfig("unknown")).toThrow(
  ...
  describe("Error Handling", () => {
    it("should handle unsupported IDE type gracefully", () => {
      expect(() => starterFactory.createStarter("unsupported")).toThrow(
