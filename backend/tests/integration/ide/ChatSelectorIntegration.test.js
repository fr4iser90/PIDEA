...
describe("Chat Selector Collection Integration", () => {
  ...
  describe("End-to-End Chat Selector Collection", () => {
    it("should collect and categorize chat selectors through full workflow", async () => {
      ...
      jest
        .spyOn(collector, "collectSelectors")
        .mockResolvedValue(mockSelectors);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining(
          "Collecting selectors for cursor version 1.5.7",
        ),
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
    ...
    it("should handle collection errors gracefully", async () => {
      ...
      await expect(
        bot.collectSelectors("cursor", "1.5.7", 3000),
      ).rejects.toThrow("Selector collection failed for cursor version 1.5.7");

      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "Selector collection failed for cursor version 1.5.7",
        ),
      );
    });
    ...
    it("should cache results correctly", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining(
          "Using cached selectors for cursor version 1.5.7",
        ),
      );
    });
  });
  ...
  describe("Chat Selector Testing Integration", () => {
    it("should test chat selectors through full workflow", async () => {
      ...
      const categorizedSelectors = await bot.collectSelectors(
        "cursor",
        "1.5.7",
        3000,
      );
      const testResults = await bot.testSelectors(
        "cursor",
        "1.5.7",
      ...
      expect(testResults.details.chat.passed).toBe(
        testResults.details.chat.total,
      );
  ...
  describe("Version Manager Integration", () => {
    it("should integrate with SelectorVersionManager for new versions", async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockSelectors);
      jest
        .spyOn(versionManager, "collectSelectorsForVersion")
        .mockResolvedValue({
          selectors: mockSelectors,
          testResults: { passed: 3, failed: 0, tested: 3 },
        });

      const result = await versionManager.collectSelectorsForVersion(
        "cursor",
        "1.5.7",
        3000,
      );
