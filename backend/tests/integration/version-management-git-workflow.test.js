...
describe("Version Management Git Workflow Integration", () => {
  ...
  describe("Complete Version Management Workflow", () => {
    it("should handle complete feature development workflow", async () => {
      ...
      const bumpType = await versionService.determineBumpType(
        task,
        context.projectPath,
        context,
      );
      ...
      // Step 3: Bump version
      const bumpResult = await versionService.bumpVersion(
        task,
        context.projectPath,
        bumpType,
        context,
    ...
    it("should handle breaking change workflow with major bump", async () => {
      const breakingTask = {
        ...
        description:
          "BREAKING CHANGE: Refactor API interface to use new authentication system",
        type: { value: "refactor" },
        priority: { value: "high" },
        category: "backend",
      };
  ...
  describe("Git Integration", () => {
    ...
    it("should create git tags with proper metadata", async () => {
      ...
      const tagResult = await versionService.createGitTag(
        "/test/project",
        "1.1.0",
        task,
      );
  ...
  describe("File System Integration", () => {
    it("should update package files correctly", async () => {
      const updatedFiles = await versionService.updatePackageFiles(
        "/test/project",
        "1.1.0",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle file system errors gracefully", async () => {
      mockFileSystemService.readJsonFile.mockRejectedValue(
    ...
    it("should handle version bump errors gracefully", async () => {
      ...
      const result = await versionService.bumpVersion(
        task,
        "/test/project",
        "minor",
      );
