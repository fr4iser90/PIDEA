...
class AdvancedAnalysisTester {
  ...
  async testCommandCreation() {
    ...
    logger.info(
      `  ✓ Invalid command validation: ${invalidValidation.isValid ? "FAILED" : "PASSED"}`,
    );
  ...
  async testServiceIntegration() {
    ...
    try {
      ...
      logger.info(
        `  ✓ Total Violations: ${(result.layerValidation.violations || []).length + (result.logicValidation.violations || []).length}`,
      );
      logger.info(
        `  ✓ Insights Generated: ${result.integratedInsights.length}`,
      );
      logger.info(
      ...
      logger.info(
        `  ✓ Report Summary: ${report.summary.overallScore}/100 score, ${report.summary.totalViolations} violations`,
      );
    } catch (error) {
      logger.info(
        `  ⚠ Analysis failed (this might be expected for some projects): ${error.message}`,
      );
    }
  ...
  async testHandlerWorkflow() {
    ...
    const mockEventBus = {
      emit: (event, data) => {
        logger.info(
          `  📡 Event: ${event}`,
          data ? `(${Object.keys(data).length} properties)` : "",
        );
      },
    };
    ...
    try {
      ...
      logger.info(
        `  ⚠ Handler workflow failed (this might be expected): ${error.message}`,
      );
    }
  ...
  async createTestProject(projectPath) {
    try {
      ...
      await fs.mkdir(path.join(projectPath, "src/presentation"), {
        recursive: true,
      });
