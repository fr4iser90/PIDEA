...
This is a test task for integration testing.`;
      ...
      const contentHash =
        await contentHashService.generateContentHash(markdownContent);

      // Extract status from content
      const extractedStatus =
        await contentHashService.extractStatusFromContent(markdownContent);

      expect(extractedStatus).toBe("in_progress");

      // Validate consistency
      const validationResult =
        await statusValidator.validateTaskStatusConsistency(taskId);

      expect(validationResult.isValid).toBe(false);
      expect(validationResult.issues).toContain(
        "Status mismatch: file shows 'in_progress', database shows 'pending'",
      );
      ...
      expect(fixResult.fixesApplied).toContain(
    ...
    it("should handle file movement during status transition", async () => {
      ...
      jest
        .spyOn(fileLocationService, "findExistingTaskFile")
        .mockResolvedValue(oldPath);
      jest
        .spyOn(fileLocationService, "getTaskFilePathForStatus")
        .mockReturnValue(newPath);

      // Move files
      const moveResult = await fileLocationService.moveTaskFiles(
        oldPath,
        newPath,
      );
...
- Progress: 50%`;
      ...
      const originalHash =
        await contentHashService.generateContentHash(originalContent);
      const modifiedHash =
        await contentHashService.generateContentHash(modifiedContent);

      expect(originalHash).not.toBe(modifiedHash);

      // Validate original content against original hash
      const isValidOriginal = await contentHashService.validateContentHash(
        originalContent,
        originalHash,
      );
      ...
      const isValidModified = await contentHashService.validateContentHash(
        modifiedContent,
        originalHash,
      );
      ...
      const isValidModifiedNew = await contentHashService.validateContentHash(
        modifiedContent,
        modifiedHash,
      );
  ...
  describe("File Location Service Integration", () => {
    it("should resolve task file paths consistently", () => {
      ...
      expect(filePath).toMatch(
        /docs\/09_roadmap\/completed\/2025-q1\/backend\/test-task$/,
      );
    });

    it("should normalize task names for file system compatibility", () => {
      const normalizedName = fileLocationService.normalizeTaskName(
  ...
  describe("Batch Operations", () => {
    it("should validate multiple tasks for consistency", async () => {
      ...
      const batchResult =
        await statusValidator.validateBatchTaskStatusConsistency(taskIds);
      ...
      const task1Result = batchResult.results.find(
        (r) => r.taskId === "task-1",
      );
      const task2Result = batchResult.results.find(
        (r) => r.taskId === "task-2",
      );
      const task3Result = batchResult.results.find(
        (r) => r.taskId === "task-3",
      );
  ...
  describe("Error Handling", () => {
    it("should handle database connection failures gracefully", async () => {
      ...
      await expect(
        eventStore.recordStatusChangeEvent(
          "task-123",
          "pending",
          "in_progress",
        ),
    ...
    it("should handle file system errors gracefully", async () => {
      ...
      await expect(
        contentHashService.generateFileHash("/nonexistent/file.md"),
    ...
    it("should handle missing task gracefully", async () => {
      ...
      const result =
        await statusValidator.validateTaskStatusConsistency("nonexistent-task");
