...
describe("Layer Architecture Integration", () => {
  ...
  describe("Complete Layer Workflow", () => {
    it("should handle complete layer task workflow", async () => {
      ...
      mockDatabaseConnection.execute
        ...
        .mockResolvedValueOnce({
          rows: [
            { id: "assignment-1", layer_id: "layer-1", task_id: "task-1" },
          ],
        }) // assignTaskToLayer
        .mockResolvedValueOnce({
          rows: [
            { id: "assignment-2", layer_id: "layer-2", task_id: "task-1" },
          ],
        }) // assignTaskToLayer
        .mockResolvedValueOnce({
          rows: [
      ...
      const distributionResult =
        await layerTaskHandler.handleTaskDistribution("task-1");
      const statusResult = await layerTaskHandler.handleStatusUpdate(
        "task-1",
        "layer-1",
        "in_progress",
      );
    ...
    it("should handle layer creation and task assignment", async () => {
      ...
      const assignmentResult = await layerTaskHandler.handleTaskAssignment(
        "task-1",
        "layer-1",
      );
  ...
  describe("Error Handling Integration", () => {
    it("should handle layer deletion with assigned tasks", async () => {
      ...
      await expect(
        layerTaskHandler.handleLayerDeletion("layer-1"),
