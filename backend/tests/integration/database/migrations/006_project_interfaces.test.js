...
class MockDatabase {
  ...
  hasColumn(tableName, columnName) {
    return (
      ...
      this.tables[tableName].hasOwnProperty(columnName)
    );
  }
...
describe("Migration 006 - Project Interfaces Table", () => {
  ...
  test("should include proper table and column comments", () => {
    ...
    expect(migrationContent).toContain(
      "COMMENT ON COLUMN project_interfaces.id IS",
    );
    expect(migrationContent).toContain(
      "COMMENT ON COLUMN project_interfaces.project_id IS",
    );
    expect(migrationContent).toContain(
      "COMMENT ON COLUMN project_interfaces.interface_name IS",
    );
  });

  test("should validate interface status constraint", () => {
    // Check constraint definition
    const constraintMatch = migrationContent.match(
