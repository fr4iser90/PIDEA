...
`;
  ...
  afterAll(async () => {
    ...
    try {
      ...
      console.warn(
        "Failed to clean up test framework directory:",
        error.message,
      );
    }
  });
  ...
  describe("Infrastructure-Domain Separation", () => {
    ...
    it("should validate steps in infrastructure layer only", async () => {
      ...
      const testFramework = loader
        .getAllFrameworks()
        .find((f) => f.name === "test-framework");
  ...
  describe("Framework Step Registry Integration", () => {
    it("should register framework steps with main step registry", () => {
      ...
      const testStep = frameworkSteps.find((step) =>
        step.includes("test-step"),
      );
