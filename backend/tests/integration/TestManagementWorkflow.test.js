...
describe("Test Management Workflow Integration", () => {
  ...
  beforeEach(async () => {
    ...
    updateTestStatusHandler = new UpdateTestStatusHandler(
  ...
  describe("Complete Test Management Workflow", () => {
    it("should handle complete test lifecycle from registration to reporting", async () => {
      ...
      const test1 = await testManagementService.registerTest(
        testFile1,
        "User Service",
        {
          framework: "jest",
      ...
      const test2 = await testManagementService.registerTest(
        testFile2,
        "Legacy Service",
        {
          framework: "jest",
      ...
      const command1 = UpdateTestStatusCommand.create(
        testFile1,
        "User Service",
        "passing",
        1500,
      );
      const command2 = UpdateTestStatusCommand.create(
        testFile2,
        "Legacy Service",
        "failing",
        3000,
      ...
      expect(
        legacyResults.files.some((f) => f.fileName === "legacy.test.js"),
      ).toBe(true);
      ...
      const trackingResults =
        await testStatusTracker.trackTestResults(mockJestResults);
      expect(trackingResults.trackedTests).toBe(3);
      expect(trackingResults.updatedTests).toBe(3);

      // Step 6: Version a test
      const versionResult = await testVersioner.versionTest(
        testFile1,
        "2.0.0",
        "Major refactor",
      );
  ...
  describe("Test Status Tracking", () => {
    it("should track test status changes over time", async () => {
      ...
      // Register test
      const test = await testManagementService.registerTest(
        testFile,
        "Status Tracking",
      );
      ...
      // Update to passing
      await testManagementService.updateTestStatus(
        testFile,
        "Status Tracking",
        "passing",
        1000,
      ...
      const updatedTest1 =
        await testMetadataRepository.findByFilePath(testFile);
  ...
  describe("Legacy Detection", () => {
    it("should detect legacy patterns in test files", async () => {
      ...
      const legacyFileResult = results.files.find(
        (f) => f.fileName === "legacy-patterns.test.js",
      );
  ...
  describe("Test Versioning", () => {
    it("should version test files and track changes", async () => {
      ...
      // Version 1.0.0
      const version1 = await testVersioner.versionTest(
        testFile,
        "1.0.0",
        "Initial version",
      );
      ...
      // Version 2.0.0
      const version2 = await testVersioner.versionTest(
        testFile,
        "2.0.0",
        "Added new test",
      );
      ...
      // Compare versions
      const comparison = await testVersioner.compareVersions(
        testFile,
        "1.0.0",
        "2.0.0",
      );
  ...
  describe("Test Reporting", () => {
    it("should generate comprehensive reports", async () => {
      ...
      const files = [
        {
          name: "fast.test.js",
          content:
            'describe("Fast", () => { it("should be fast", () => expect(true).toBe(true)); });',
        },
        {
          name: "slow.test.js",
          content:
            'describe("Slow", () => { it("should be slow", () => { setTimeout(() => {}, 1000); expect(true).toBe(true); }); });',
        },
        {
          name: "failing.test.js",
          content:
            'describe("Failing", () => { it("should fail", () => expect(true).toBe(false)); });',
        },
      ];

      for (const file of files) {
        await fs.writeFile(path.join(tempDir, file.name), file.content);
        await testManagementService.registerTest(
      ...
      await testManagementService.updateTestStatus(
        path.join(tempDir, "fast.test.js"),
        "fast.test.js",
        "passing",
        100,
      );
      await testManagementService.updateTestStatus(
        path.join(tempDir, "slow.test.js"),
        "slow.test.js",
        "passing",
        5000,
      );
      await testManagementService.updateTestStatus(
        path.join(tempDir, "failing.test.js"),
        "failing.test.js",
        "failing",
        2000,
        "Test failed",
      ...
      const markdownReport = await testReporter.generateReport({
        format: "markdown",
      });
      ...
      expect(markdownReport.formattedReport).toContain(
  ...
  describe("API Integration", () => {
    it("should handle API operations correctly", async () => {
      ...
      const testsByStatus =
        await testMetadataRepository.findByStatus("unknown");
  ...
  describe("Error Handling", () => {
    it("should handle errors gracefully", async () => {
      ...
      await expect(
        testManagementService.updateTestStatus(
          "/path/test.js",
          "Test",
          "invalid-status",
        ),
  ...
  describe("Performance", () => {
    it("should handle large numbers of tests efficiently", async () => {
      ...
      for (const testFile of tests) {
        await testManagementService.registerTest(
          testFile,
          `Bulk Test ${tests.indexOf(testFile)}`,
        );
      }
