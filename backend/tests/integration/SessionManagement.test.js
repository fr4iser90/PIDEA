...
describe("Session Management Integration Tests", () => {
  ...
  describe("Session Extension Flow", () => {
    it("should extend session successfully", async () => {
      const mockExtendSession = jest
        .spyOn(sessionActivityService, "extendSession")
        .mockResolvedValue({
  ...
  describe("Session Status Monitoring", () => {
    it("should get session status successfully", async () => {
      const mockGetStats = jest
        .spyOn(sessionActivityService, "getSessionActivityStats")
        .mockResolvedValue({
          sessionId: testSession.id,
          activityCount: 10,
  ...
  describe("Activity Recording", () => {
    it("should record user activity successfully", async () => {
      const mockRecordActivity = jest
        .spyOn(sessionActivityService, "recordActivity")
        .mockResolvedValue({
          sessionId: testSession.id,
          timestamp: Date.now(),
          type: "mouse-move",
  ...
  describe("Session Analytics", () => {
    it("should get session analytics successfully", async () => {
      const mockGetUserStats = jest
        .spyOn(sessionActivityService, "getUserActivityStats")
        .mockResolvedValue({
          userId: testUser.id,
          sessionCount: 5,
  ...
  describe("Admin Functions", () => {
    ...
    it("should get monitoring data for admin", async () => {
      const mockGetStatus = jest
        .spyOn(sessionActivityService, "getStatus")
        .mockReturnValue({
          isRunning: true,
    ...
    it("should trigger manual cleanup for admin", async () => {
      const mockCleanup = jest
        .spyOn(sessionActivityService, "cleanupExpiredSessions")
        .mockResolvedValue({
          cleanedCount: 3,
          errors: [],
        });
    ...
    it("should update configuration for admin", async () => {
      const mockUpdateConfig = jest
        .spyOn(sessionActivityService, "updateConfig")
        .mockImplementation(() => {});
