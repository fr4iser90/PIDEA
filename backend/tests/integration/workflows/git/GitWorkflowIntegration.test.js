...
describe("Git Workflow Integration", () => {
  ...
  describe("Service Integration", () => {
    it("should integrate WorkflowGitService with GitWorkflowManager", async () => {
      ...
      expect(branchResult.branchName).toMatch(
    ...
    it("should integrate TaskService with enhanced git workflow", async () => {
      ...
      const result = await enhancedTaskService.executeTask("task-1", {
        projectPath: "/test/project",
      });
    ...
    it("should integrate AutoFinishSystem with enhanced git workflow", async () => {
      ...
      autoFinishSystem.cursorIDE.sendMessage = jest
        .fn()
        .mockResolvedValue("Task completed successfully");
  ...
  describe("Error Handling Integration", () => {
    it("should handle git service failures gracefully across all services", async () => {
      ...
      mockGitService.createBranch.mockRejectedValue(
        new Error("Git service unavailable"),
      );
      ...
      const orchestrationResult =
        await workflowOrchestrationService.executeWorkflow(task, {});
    ...
    it("should handle invalid task data across all services", async () => {
      ...
      await expect(
        gitWorkflowManager.executeWorkflow(context),
      ).rejects.toThrow();
    });
  });
