...
describe('Cross-Database Compatibility Integration Tests', () => {
  ...
  describe('Query Translation', () => {
    ...
    test('should translate PostgreSQL functions to SQLite equivalents', () => {
      const postgresqlQuery = 'SELECT NOW(), EXTRACT(YEAR FROM created_at) FROM users';
      const params = [];

      const result = sqlTranslator.translate(postgresqlQuery, params);

      expect(result.query).toBe('SELECT datetime(\'now\'), strftime(\'%Y\', created_at) FROM users');
      expect(result.params).toEqual([]);
    });

    test('should translate PostgreSQL data types to SQLite', () => {
      const postgresqlQuery = 'CREATE TABLE users (id SERIAL PRIMARY KEY, name VARCHAR(255), active BOOLEAN)';
      const params = [];

      const result = sqlTranslator.translate(postgresqlQuery, params);

      expect(result.query).toBe('CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, active INTEGER)');
      expect(result.params).toEqual([]);
    });
    ...
    test('should handle complex PostgreSQL queries', () => {
      ...
      expect(result.query).toContain('SELECT u.name, p.title, COUNT(*) as post_count');
  ...
  describe('Function Compatibility', () => {
    ...
    test('should translate EXTRACT with different fields', () => {
      const testCases = [
        { input: 'EXTRACT(MONTH FROM created_at)', expected: 'strftime(\'%m\', created_at)' },
        { input: 'EXTRACT(DAY FROM created_at)', expected: 'strftime(\'%d\', created_at)' },
        { input: 'EXTRACT(HOUR FROM created_at)', expected: 'strftime(\'%H\', created_at)' },
        { input: 'EXTRACT(MINUTE FROM created_at)', expected: 'strftime(\'%M\', created_at)' },
        { input: 'EXTRACT(SECOND FROM created_at)', expected: 'strftime(\'%S\', created_at)' }
      ];
  ...
  describe('PostgreSQL-Specific Features', () => {
    ...
    test('should handle PostgreSQL-specific data types', () => {
      const postgresqlQuery = 'CREATE TABLE test_table (id SERIAL PRIMARY KEY, data JSONB)';
      const result = sqlTranslator.translate(postgresqlQuery, []);

      expect(result.query).toBe('CREATE TABLE test_table (id INTEGER PRIMARY KEY AUTOINCREMENT, data TEXT)');
    });
  ...
  describe('Cross-Database Query Execution', () => {
    test('should execute translated queries on SQLite', async () => {
      ...
      const result = await sqliteConnection.query(translated.query, translated.params);
    ...
    test('should handle parameter binding differences', async () => {
      ...
      expect(translated.query).toBe('SELECT * FROM users WHERE id = ? AND name = ?');
    ...
    test('should handle complex translated queries', async () => {
      ...
      expect(translated.query).toContain('SELECT u.name, COUNT(p.id) as post_count');
      expect(translated.query).toContain('FROM users u');
      expect(translated.query).toContain('LEFT JOIN posts p ON u.id = p.user_id');
  ...
  describe('Migration Support', () => {
    ...
    test('should handle schema differences during migration', () => {
      ...
      expect(translatedSchema.query).toContain('INTEGER PRIMARY KEY AUTOINCREMENT');
