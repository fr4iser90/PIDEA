...
describe("Services Integration Tests", () => {
    ...
    describe("Project CRUD Operations", () => {
      it("should create and retrieve a project", async () => {
        ...
        const createdProject =
          await projectApplicationService.createProject(projectData);
        ...
        // Retrieve project
        const retrievedProject = await projectApplicationService.getProject(
          createdProject.id,
        );
      ...
      it("should list projects with pagination", async () => {
        ...
        // List projects
        const result = await projectApplicationService.listProjects({
          page: 1,
          limit: 10,
        });
      ...
      it("should update a project", async () => {
        ...
        const updatedProject = await projectApplicationService.updateProject(
          createdProject.id,
          updateData,
        );
      ...
      it("should delete a project", async () => {
        ...
        await expect(
          projectApplicationService.getProject(createdProject.id),
      ...
      it("should handle project search functionality", async () => {
        ...
        // Search for projects
        const searchResult = await projectApplicationService.listProjects({
          search: "Alpha",
        });
        expect(searchResult.projects.length).toBeGreaterThan(0);
        expect(
          searchResult.projects.some((p) => p.name.includes("Alpha")),
        ).toBe(true);
      });
    });
    ...
    describe("Project-Interface Integration", () => {
      ...
      it("should create interfaces for a project", async () => {
        ...
        const createdInterface =
          await projectApplicationService.createInterface(
            testProjectId,
            interfaceData,
          );
      ...
      it("should list interfaces for a project", async () => {
        ...
        // List interfaces
        const interfaces =
          await projectApplicationService.listInterfaces(testProjectId);
      ...
      it("should get a specific interface", async () => {
        ...
        const retrievedInterface = await projectApplicationService.getInterface(
          testProjectId,
          createdInterface.id,
        );
      ...
      it("should update an interface", async () => {
        ...
        const updatedInterface =
          await projectApplicationService.updateInterface(
            testProjectId,
            createdInterface.id,
            updateData,
          );
      ...
      it("should delete an interface", async () => {
        ...
        await projectApplicationService.deleteInterface(
          testProjectId,
          createdInterface.id,
        );

        await expect(
          projectApplicationService.getInterface(
            testProjectId,
            createdInterface.id,
          ),
  ...
  describe("InterfaceManager Integration", () => {
    describe("Interface Lifecycle Management", () => {
      it("should manage interface lifecycle", async () => {
        ...
        // Create interface
        const createdInterface = await interfaceManager.createInterface(
          "test-project",
          interfaceData,
        );
        expect(createdInterface).toHaveProperty("id");
        ...
        // Start interface
        const startResult = await interfaceManager.startInterface(
          createdInterface.id,
        );
        ...
        const status = await interfaceManager.getInterfaceStatus(
          createdInterface.id,
        );
        ...
        // Stop interface
        const stopResult = await interfaceManager.stopInterface(
          createdInterface.id,
        );
        ...
        // Restart interface
        const restartResult = await interfaceManager.restartInterface(
          createdInterface.id,
        );
      ...
      it("should handle interface errors gracefully", async () => {
        ...
        // Test error handling
        await expect(
          interfaceManager.startInterface("nonexistent-id"),
        ).rejects.toThrow("Interface not found");

        await expect(
          interfaceManager.getInterfaceStatus("nonexistent-id"),
        ).rejects.toThrow("Interface not found");
      });
      ...
      it("should track interface statistics", async () => {
        ...
        expect(updatedStats.totalCreated).toBeGreaterThan(
          initialStats.totalCreated,
        );
        expect(updatedStats.activeCount).toBeGreaterThan(
  ...
  describe("Database Integration", () => {
    it("should persist project data correctly", async () => {
      ...
      const persistedProject = await projectRepository.findById(
        createdProject.id,
      );
    ...
    it("should handle database errors gracefully", async () => {
      ...
      await expect(
        projectApplicationService.createProject(invalidData),
      ).rejects.toThrow();
    });
  });
