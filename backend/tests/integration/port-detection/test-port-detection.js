  ...
  try {
    ...
    if (availableIDEs.length > 1) {
      ...
      for (const ide of availableIDEs) {
        ...
        try {
          ...
          logger.info(
            `❌ Failed to switch to port ${ide.port}: ${error.message}`,
          );
        }
        logger.info("");
      }
    ...
    if (availableIDEs.length > 0) {
      ...
      try {
        ...
        logger.info(
          `BrowserManager current port: ${browserManager.getCurrentPort()}`,
        );
      } catch (error) {
        logger.info(
          `❌ Failed to connect to port ${firstIDE.port}: ${error.message}`,
        );
      }
    }
  } catch (error) {
