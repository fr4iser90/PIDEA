  ...
  describe("POST /api/projects/:projectId/tasks/enqueue - task-review mode", () => {
    ...
    it("successfully executes task review workflow", async () => {
      ...
      expect(response.body.message).toContain(
        "Task review completed: 2/2 tasks processed successfully",
      );
    ...
    it("processes tasks sequentially", async () => {
      ...
      expect(mockStepRegistry.executeStep).toHaveBeenNthCalledWith(
        1,
        "IDESendMessageStep",
        {
          projectId,
    ...
    it("returns failure when all tasks fail", async () => {
      mockStepRegistry.executeStep.mockRejectedValue(
        new Error("All tasks failed"),
      );
    ...
    it("uses default workflow prompt when not provided", async () => {
      ...
      expect(mockStepRegistry.executeStep).toHaveBeenCalledWith(
        "IDESendMessageStep",
