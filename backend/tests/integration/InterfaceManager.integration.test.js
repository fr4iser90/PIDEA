...
describe("InterfaceManager Integration", () => {
  ...
  beforeEach(() => {
    ...
    interfaceFactory.registerDefaultConfig("ide", {
      ideFactoryConfig: "value",
    });
  });
  ...
  describe("Interface Lifecycle Integration", () => {
    test("should complete full interface lifecycle", async () => {
      ...
      // Remove interface
      const removed = await interfaceManager.removeInterface(
        interfaceInstance.id,
      );
    ...
    test("should handle multiple interfaces concurrently", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const interfaceInstance = await interfaceManager.createInterface(
          "test",
          {
            projectId: `project-${i}`,
            index: i,
      ...
      const runningInterfaces = interfaceManager
        .getAllInterfaces()
      ...
      const stoppedInterfaces = interfaceManager
        .getAllInterfaces()
        .filter((i) => i.isStopped());
  ...
  describe("Factory Integration", () => {
    ...
    test("should create multiple interfaces through factory", async () => {
      const specifications = [
        ...
        {
          type: "ide",
          config: { projectId: "project-3", workspacePath: "/test" },
        },
      ];

      const result =
        await interfaceFactory.createMultipleInterfaces(specifications);
    ...
    test("should handle factory creation errors", async () => {
      ...
      interfaceManager.registerInterface(
        "error",
  ...
  describe("Registry Integration", () => {
    ...
    test("should categorize interfaces in registry", () => {
      ...
      const developmentTypes =
        interfaceRegistry.getInterfaceTypesByCategory("development");
  ...
  describe("Project Integration", () => {
    test("should manage interfaces for specific projects", async () => {
      ...
      const project1Filtered = allInterfaces.filter(
        (i) => i.config.projectId === "project-1",
      );
      const project2Filtered = allInterfaces.filter(
        (i) => i.config.projectId === "project-2",
      );
      ...
      project2Filtered.forEach((interfaceInstance) => {
        ...
        expect(interfaceInstance.config.workspacePath).toMatch(
          /\/project-2\/workspace-\d+/,
        );
      });
    });
  ...
  describe("Error Handling Integration", () => {
    test("should handle interface creation errors", async () => {
      ...
      await expect(interfaceManager.createInterface("error")).rejects.toThrow(
        "Constructor error",
      );
