...
const {
  DatabaseConnection,
} = require("../../infrastructure/database/DatabaseConnection");
const {
  AnalysisRepository,
} = require("../../domain/repositories/AnalysisRepository");
const {
  AnalysisStepRepository,
} = require("../../domain/repositories/AnalysisStepRepository");
...
describe("Analysis Integration Tests - Techstack & Recommendations", () => {
  ...
  describe("Analysis Queue Integration", () => {
    it("should process techstack analysis through queue", async () => {
      ...
      const analysis = await analysisRepository.findById(
        createResponse.body.analysisId,
      );
      expect(analysis).toBeTruthy();
