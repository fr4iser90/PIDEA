...
describe("Terminal Log Capture Integration", () => {
  ...
  beforeEach(() => {
    ...
    mockIDEMirrorService.prototype.clickElementInIDE = jest
      .fn()
      .mockResolvedValue();
  ...
  describe("End-to-End Terminal Log Capture Flow", () => {
    test("should capture terminal output and make it readable", async () => {
      ...
      expect(fs.mkdir).toHaveBeenCalledWith(`/tmp/IDEWEB/${port}/logs`, {
        recursive: true,
      });

      // Step 2: Execute command with capture
      await terminalLogCaptureService.executeCommandWithCapture(
        port,
        testCommand,
      );
      ...
      terminalLogCaptureService.processLogFile = jest
        .fn()
        .mockImplementation(async (port) => {
          ...
          for (const line of lines) {
            ...
            await terminalLogCaptureService.writeEncryptedLogEntry(
              encryptedLogPath,
              logEntry,
            );
          }
      ...
      expect(fs.readFile).toHaveBeenCalledWith(
        `/tmp/IDEWEB/${port}/logs/terminal.log`,
        "utf8",
      );
      ...
      logEncryptionService.decryptLogEntry = jest
        .fn()
        .mockResolvedValue(expectedLogEntry);
  ...
  describe("Security Integration", () => {
    ...
    test("should validate file paths to prevent traversal attacks", () => {
      ...
      expect(() =>
        logPermissionManager.validateLogPath(validPath),
      ).not.toThrow();

      // Invalid path should throw
      expect(() => logPermissionManager.validateLogPath(invalidPath)).toThrow(
  ...
  describe("API Integration", () => {
    test("should handle search functionality", async () => {
      ...
      logEncryptionService.decryptLogEntries = jest
        .fn()
    ...
    test("should handle export functionality", async () => {
      ...
      // Mock log reading
      terminalLogReader.getRecentLogs = jest
        .fn()
        .mockResolvedValue(mockLogEntries);

      const exportedData = await terminalLogReader.exportLogs(port, format, {
        lines: 10,
      });
  ...
  describe("Error Handling Integration", () => {
    ...
    test("should handle encryption errors gracefully", async () => {
      ...
      logEncryptionService.encryptLogEntry = jest
        .fn()
        .mockRejectedValue(new Error("Encryption failed"));

      await expect(
        logEncryptionService.encryptLogEntry(logEntry),
    ...
    test("should handle permission errors gracefully", async () => {
      ...
      await expect(
        logPermissionManager.createSecureLogDirectory(logDir),
