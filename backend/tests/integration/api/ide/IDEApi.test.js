...
app.use(
  "/api/ide/selection",
  IDESelectionController(mockIDESelectionService, mockEventBus),
);
app.use(
  "/api/ide/mirror",
  IDEMirrorController(mockIDEMirrorService, mockEventBus),
);
app.use(
  "/api/ide/features",
  IDEFeatureController(mockIDEService, mockEventBus),
);
...
describe("IDE API Integration Tests", () => {
  ...
  describe("GET /api/ide/list", () => {
    it("should return list of available IDEs", async () => {
      ...
      const response = await request(app).get("/api/ide/list").expect(200);
    ...
    it("should handle service errors", async () => {
      mockIDEService.listIDEs.mockRejectedValue(
        new Error("Service unavailable"),
      );

      const response = await request(app).get("/api/ide/list").expect(500);
  ...
  describe("GET /api/ide/status", () => {
    ...
    it("should return 400 for missing port parameter", async () => {
      const response = await request(app).get("/api/ide/status").expect(400);
  ...
  describe("POST /api/ide/start", () => {
    it("should start IDE successfully", async () => {
      ...
      const mockResult = { port: 9222, status: "running" };
  ...
  describe("POST /api/ide/stop", () => {
    it("should stop IDE successfully", async () => {
      ...
      const mockResult = { port: 9222, status: "stopped" };
  ...
  describe("GET /api/ide/selection", () => {
    it("should return current IDE selection", async () => {
      ...
      mockIDESelectionService.getCurrentSelection.mockResolvedValue(
        mockSelection,
      );

      const response = await request(app).get("/api/ide/selection").expect(200);
  ...
  describe("POST /api/ide/selection", () => {
    it("should set IDE selection successfully", async () => {
      ...
      expect(mockIDESelectionService.setSelection).toHaveBeenCalledWith(
        selectionData,
      );
    });
  ...
  describe("POST /api/ide/mirror/interact", () => {
    it("should handle IDE interaction", async () => {
      ...
      expect(mockIDEMirrorService.interactWithIDE).toHaveBeenCalledWith(
        interactionData,
      );
    });
  });
  ...
  describe("GET /api/ide/features", () => {
    ...
    it("should return 400 for missing port parameter", async () => {
      const response = await request(app).get("/api/ide/features").expect(400);
  ...
  describe("Event Bus Integration", () => {
    ...
    it("should emit selection change events", async () => {
      ...
      const mockResult = { currentPort: 9222 };
      mockIDESelectionService.setSelection.mockResolvedValue(mockResult);
