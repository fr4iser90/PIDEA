...
const {
  GetChatHistoryStep,
} = require("@domain/steps/categories/chat/get_chat_history_step");
...
describe("GetChatHistoryStep Cache Integration", () => {
  ...
  describe("Cache Hit Scenario", () => {
    test("should return cached messages when available", async () => {
      ...
      // Pre-populate cache
      getChatHistoryStep.chatCacheService.setChatHistory(
        "9222",
        cachedMessages,
      );
    ...
    test("should include cache statistics in response", async () => {
      const cachedMessages = [
        { id: "1", content: "Test", sender: "user", type: "text" },
      ];
      getChatHistoryStep.chatCacheService.setChatHistory(
        "9222",
        cachedMessages,
  ...
  describe("Cache Miss Scenario", () => {
    test("should extract from IDE and cache results on cache miss", async () => {
      const extractedMessages = [
        ...
        {
          id: "2",
          content: "Hi there from IDE!",
          sender: "assistant",
          type: "text",
        },
      ];

      mockCursorIDEService.extractChatHistory.mockResolvedValue(
        extractedMessages,
      );
      ...
      const cachedMessages =
        getChatHistoryStep.chatCacheService.getChatHistory("9222");
      expect(cachedMessages).toEqual(extractedMessages);
    });

    test("should handle IDE extraction errors gracefully", async () => {
      mockCursorIDEService.extractChatHistory.mockRejectedValue(
  ...
  describe("Port-based Caching", () => {
    test("should cache separately for different ports", async () => {
      const messages9222 = [
        { id: "1", content: "Port 9222 message", sender: "user", type: "text" },
      ];
      const messages9224 = [
        { id: "2", content: "Port 9224 message", sender: "user", type: "text" },
      ];
    ...
    test("should handle port number and string formats", async () => {
      const messages = [
        { id: "1", content: "Test message", sender: "user", type: "text" },
      ];
  ...
  describe("Error Handling", () => {
    test("should handle cache service errors gracefully", async () => {
      ...
      const originalGetChatHistory =
        getChatHistoryStep.chatCacheService.getChatHistory;
      getChatHistoryStep.chatCacheService.getChatHistory = jest
        .fn()
        .mockImplementation(() => {
