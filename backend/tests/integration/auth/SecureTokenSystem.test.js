...
describe("Secure Token System Integration", () => {
  ...
  describe("Token Creation and Validation Flow", () => {
    ...
    it("should reject invalid access token", async () => {
      ...
      const invalidToken =
        "test.jwt.token.dummy.header.test.jwt.token.dummy.different.payload.test.jwt.token.dummy.different.signature";

      await expect(
        authService.validateAccessToken(invalidToken),
  ...
  describe("Refresh Token Validation", () => {
    it("should validate refresh token successfully", () => {
      ...
      const result = tokenValidator.validateRefreshToken(
        "refresh-token",
        session,
      );
  ...
  describe("Token Hash Operations", () => {
    ...
    it("should validate token against stored hash", () => {
      ...
      const isInvalid = tokenHasher.validateToken(
        "different-token",
        hashResult.hash,
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle empty tokens gracefully", () => {
      expect(() => tokenHasher.hashToken("")).toThrow(
        "Token is required for hashing",
      );
      expect(() => tokenHasher.hashToken(null)).toThrow(
        "Token is required for hashing",
      );
    });

    it("should handle validation errors gracefully", () => {
      const result = tokenValidator.validateToken(
        "invalid.token",
        "prefix",
        "hash",
      );
