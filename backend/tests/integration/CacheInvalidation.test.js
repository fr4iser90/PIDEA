...
describe("Cache Invalidation Integration", () => {
  ...
  describe("Event-Driven Invalidation", () => {
    ...
    it("should handle IDE switch events", () => {
      ...
      const ideSwitchHandler = mockEventCoordinator.on.mock.calls.find(
        (call) => call[0] === "ide:switch",
      )[1];
    ...
    it("should handle project change events", () => {
      ...
      const projectChangeHandler = mockEventCoordinator.on.mock.calls.find(
        (call) => call[0] === "project:change",
      )[1];
    ...
    it("should handle analysis complete events", () => {
      // Set up test data
      cacheService.set(
        "analysis:codeQuality:proj1",
        { analysis: "data1" },
        "codeQuality",
        "analysis",
      );
      cacheService.set(
        "analysis:security:proj1",
        { analysis: "data2" },
        "security",
        "analysis",
      );
      ...
      const analysisCompleteHandler = mockEventCoordinator.on.mock.calls.find(
      ...
      expect(cacheService.memoryCache.has("analysis:codeQuality:proj1")).toBe(
        false,
      );
      expect(cacheService.memoryCache.has("analysis:security:proj1")).toBe(
        true,
      );
      expect(cacheService.memoryCache.has("project:proj1")).toBe(true);
    });
    ...
    it("should handle chat events", () => {
      ...
      // Simulate chat event
      const chatHandler = mockEventCoordinator.on.mock.calls.find(
        (call) => call[0] === "chat:new",
      )[1];
  ...
  describe("Error Handling", () => {
    ...
    it("should handle missing event handlers gracefully", () => {
      ...
      const missingHandler = mockEventCoordinator.on.mock.calls.find(
        (call) => call[0] === "missing:event",
      );
  ...
  describe("Integration Scenarios", () => {
    ...
    it("should handle project analysis workflow", () => {
      ...
      expect(cacheService.memoryCache.has("analysis:performance:proj1")).toBe(
        true,
      );
      expect(cacheService.memoryCache.has("project:proj1")).toBe(true);
    });
