...
class PlaywrightTestRunner {
  ...
  async executeTest(testFile, options = {}) {
    ...
    for (const browserName of this.config.browsers || ["chromium"]) {
      try {
        const result = await this.executeTestOnBrowser(
          testFile,
          browserName,
          options,
        );
    ...
    try {
      ...
      return {
        ...
        error: browserCrashed
          ? "Browser crashed after successful tests (NixOS issue)"
          : stderr,
        command: command,
