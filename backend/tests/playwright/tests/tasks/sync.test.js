...
test.describe("PIDEA Tasks Sync Tests", () => {
  ...
  test("should create new task and sync to database", async ({ page }) => {
    ...
    try {
      const tasksLink = page
        .locator('[data-testid="tasks-link"], a[href*="tasks"], .tasks-nav')
        .first();
    ...
    if (addTaskButton) {
      ...
      if (await taskInList.isVisible()) {
        ...
        console.log(
          "⚠️ Task creation verification failed - might need different selectors",
        );
      }
    } else {
      console.log(
        "⚠️ Add task button not found - task creation UI might be different",
      );
    }
  });
  ...
  test("should update task and verify sync", async ({ page }) => {
    ...
    const taskItems = page.locator(
      '.task-item, .task-row, [data-testid="task-item"]',
    );
  ...
  test("should handle sync errors gracefully", async ({ page }) => {
    ...
    try {
      const refreshButton = page.locator(
        '[data-testid="refresh"], .refresh-btn, button:has-text("Refresh")',
      );
