...
test.describe('Session Persistence', () => {
  ...
  test('should maintain session after page refresh', async ({ page, context }) => {
    ...
    const accessTokenAfterLogin = cookiesAfterLogin.find(cookie => cookie.name === 'accessToken');
    const refreshTokenAfterLogin = cookiesAfterLogin.find(cookie => cookie.name === 'refreshToken');
    
    console.log('🍪 Cookies after login:');
    console.log('  - accessToken:', accessTokenAfterLogin ? 'EXISTS' : 'MISSING');
    console.log('  - refreshToken:', refreshTokenAfterLogin ? 'EXISTS' : 'MISSING');
    ...
    page.on('console', msg => {
      ...
      } else if (logText.includes('AuthStore') || logText.includes('auth') || logText.includes('cookie') || logText.includes('token') || logText.includes('initialize') || logText.includes('App') || logText.includes('React') || logText.includes('mount') || logText.includes('component') || logText.includes('useEffect') || logText.includes('store') || logText.includes('zustand')) {
    ...
    const accessTokenAfterRefresh = cookiesAfterRefresh.find(cookie => cookie.name === 'accessToken');
    const refreshTokenAfterRefresh = cookiesAfterRefresh.find(cookie => cookie.name === 'refreshToken');
    
    console.log('🍪 Cookies after refresh:');
    console.log('  - accessToken:', accessTokenAfterRefresh ? 'EXISTS' : 'MISSING');
    console.log('  - refreshToken:', refreshTokenAfterRefresh ? 'EXISTS' : 'MISSING');
    ...
    const errorMessages = await page.locator('.error, .auth-error, [class*="error"]').allTextContents();
    ...
    const authStoreLogs = consoleLogs.filter(log => log.includes('AuthStore'));
    if (authStoreLogs.length === 0) {
      console.log('⚠️  WARNING: No AuthStore logs found - initialize() might not be called');
    } else {
    ...
    const appLogs = consoleLogs.filter(log => log.includes('App') || log.includes('React'));
    if (appLogs.length === 0) {
    ...
    const cookieLogs = consoleLogs.filter(log => log.includes('cookie') || log.includes('Cookie'));
    ...
    const tokenLogs = consoleLogs.filter(log => log.includes('token') || log.includes('Token'));
    ...
    const initLogs = consoleLogs.filter(log => log.includes('initialize') || log.includes('init'));
    ...
    if (authStoreLogs.length === 0) {
      ...
      console.log('  🔍 This means the App component is not calling AuthStore.initialize()');
    } else {
    ...
    if (loginFormVisible && !appRootVisible) {
      ...
      console.log('🔍 DIAGNOSIS: Cookies exist but frontend AuthStore failed to validate them');
