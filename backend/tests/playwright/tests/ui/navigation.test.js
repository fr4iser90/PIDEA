...
test.describe("PIDEA UI Navigation Tests", () => {
  ...
  test("should navigate between different sections", async ({ page }) => {
    ...
    const navLinks = [
      {
        selector: '[data-testid="projects-link"], a[href*="projects"]',
        text: "Projects",
      },
      {
        selector: '[data-testid="tasks-link"], a[href*="tasks"]',
        text: "Tasks",
      },
      {
        selector: '[data-testid="settings-link"], a[href*="settings"]',
        text: "Settings",
      },
    ];
    ...
    for (const link of navLinks) {
      try {
        ...
        console.log(
          `⚠️ Navigation link ${link.text} not found or not clickable`,
        );
      }
    }
  });
  ...
  test("should show project information in header", async ({ page }) => {
    ...
    const projectInfo = page.locator(
      "header .project-info, header .project-name, header h1",
    );
  ...
  test("should handle responsive navigation", async ({ page }) => {
    ...
    const mobileNav = page.locator(
      '.mobile-nav, .hamburger-menu, [data-testid="mobile-menu"]',
    );
  ...
  test("should show loading states during navigation", async ({ page }) => {
    ...
    try {
      const projectsLink = page
        .locator('[data-testid="projects-link"], a[href*="projects"]')
        .first();
      ...
      console.log(
        "⚠️ Could not test loading states - navigation might be instant",
      );
    }
  });
