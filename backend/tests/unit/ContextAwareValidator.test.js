...
describe("ContextAwareValidator", () => {
  ...
  describe("Constructor", () => {
    ...
    test("should initialize with appropriateness patterns", () => {
      ...
      expect(
        validator.appropriatenessPatterns.addressesOriginalQuestion,
      ).toBeDefined();
      expect(
        validator.appropriatenessPatterns.providesCompleteSolution,
      ).toBeDefined();
  ...
  describe("validateResponse", () => {
    test("should validate response with context successfully", async () => {
      const response =
        "Here is the complete solution for your code review request. The code has been analyzed and all issues have been identified.";
  ...
  describe("validateIntentMatch", () => {
    test("should detect code review intent", () => {
      const response =
        "Code review complete. Found 3 issues that need attention.";
    ...
    test("should detect debugging intent", () => {
      const response =
        "The error has been fixed. The issue was in the variable declaration.";
    ...
    test("should detect explanation intent", () => {
      const response =
        "Here is how the function works: it takes an input and processes it through several steps.";
    ...
    test("should detect refactoring intent", () => {
      const response =
        "The code has been refactored for better performance and readability.";
  ...
  describe("validateContextRelevance", () => {
    test("should detect builds on previous context", () => {
      const response =
        "As mentioned in the previous message, here is the extended solution.";
      const conversationHistory = [
        { sender: "user", content: "Create a function" },
        { sender: "ai", content: "Here is a basic function" },
      ];

      const result = validator.validateContextRelevance(
        response,
        conversationHistory,
      );

      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "builds_on_previous")).toBe(
        true,
      );
    });
    ...
    test("should detect maintains context", () => {
      const response =
        "In this context, the solution should be implemented as follows.";
      const conversationHistory = [
      ...
      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "maintains_context")).toBe(
        true,
      );
    });
    ...
    test("should detect continuity indicators", () => {
      ...
      expect(result.score).toBeGreaterThan(0);
      expect(
        result.factors.some((f) => f.type === "continuity_indicators"),
      ).toBe(true);
    });
  });
  ...
  describe("validateCompleteness", () => {
    test("should detect comprehensive response", () => {
      const response =
        "Here is a comprehensive solution that covers all aspects of your request.";
      const userIntent = "Create a complete solution";

      const result = validator.validateCompleteness(response, userIntent);

      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "response_coverage")).toBe(
        true,
      );
    });
    ...
    test("should detect complete solution", () => {
      const response =
        "Complete solution provided. You can now implement this code.";
      ...
      expect(result.score).toBeGreaterThan(0);
      expect(
        result.factors.some((f) => f.type === "solution_completeness"),
      ).toBe(true);
    });

    test("should detect explanation quality", () => {
      const response =
        "Here is a detailed explanation with examples of how this works.";
      const userIntent = "Explain this";

      const result = validator.validateCompleteness(response, userIntent);

      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "explanation_quality")).toBe(
        true,
      );
    });
  });
  ...
  describe("validateAppropriateness", () => {
    test("should detect addresses question", () => {
      const response =
        "This answer addresses your original question about the implementation.";
      const userIntent = "How do I implement this?";

      const result = validator.validateAppropriateness(response, userIntent);

      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "addresses_question")).toBe(
        true,
      );
    });
    ...
    test("should detect helpful suggestions", () => {
      const response =
        "You might want to consider adding error handling to this code.";
      const userIntent = "Improve this code";

      const result = validator.validateAppropriateness(response, userIntent);

      expect(result.score).toBeGreaterThan(0);
      expect(result.factors.some((f) => f.type === "helpful_suggestions")).toBe(
        true,
      );
    });
  });
  ...
  describe("responseAddressesIntent", () => {
    test("should detect code review response", () => {
      ...
      const result = validator.responseAddressesIntent(
        response,
        detectedIntent,
      );
  ...
  describe("Integration Tests", () => {
    test("should provide comprehensive validation for code review scenario", async () => {
      const response =
        "Code review complete. I found 3 issues: 1) Missing error handling, 2) Inefficient loop, 3) Unused variable. Here are the fixes for each issue.";
      const userIntent = "Please review this code and find any issues";
      const conversationHistory = [
        { sender: "user", content: "Can you review this code?" },
        { sender: "ai", content: "I'll analyze the code for you." },
    ...
    test("should provide comprehensive validation for code generation scenario", async () => {
      const response =
        "Here is the complete implementation of the function you requested. The code includes proper error handling and follows best practices. You can now use this function in your project.";
