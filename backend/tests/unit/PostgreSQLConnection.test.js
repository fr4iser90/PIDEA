...
describe('PostgreSQLConnection Unit Tests', () => {
  ...
  describe('Connection Management', () => {
    test('should connect successfully with valid configuration', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('🗄️ Connecting to PostgreSQL database: test_db');
    });
    ...
    test('should handle connection errors gracefully', async () => {
      ...
      await expect(postgresConnection.connect()).rejects.toThrow('Connection failed');
      expect(postgresConnection.isConnected).toBe(false);
    });
    ...
    test('should handle disconnect errors gracefully', async () => {
      ...
      await expect(postgresConnection.disconnect()).rejects.toThrow('Disconnect failed');
    });
  });
  ...
  describe('Query Execution', () => {
    ...
    test('should execute SELECT query with parameters', async () => {
      ...
      const result = await postgresConnection.query('SELECT * FROM test WHERE id = $1', [1]);
    ...
    test('should execute INSERT statement successfully', async () => {
      ...
      const result = await postgresConnection.execute('INSERT INTO test (name) VALUES ($1)', ['test']);
    ...
    test('should execute UPDATE statement successfully', async () => {
      ...
      const result = await postgresConnection.execute('UPDATE test SET name = $1 WHERE id = $2', ['updated', 1]);

      expect(result).toEqual(mockResult);
      expect(mockClient.query).toHaveBeenCalledWith('UPDATE test SET name = $1 WHERE id = $2', ['updated', 1]);
    });
    ...
    test('should execute DELETE statement successfully', async () => {
      ...
      const result = await postgresConnection.execute('DELETE FROM test WHERE id = $1', [1]);

      expect(result).toEqual(mockResult);
      expect(mockClient.query).toHaveBeenCalledWith('DELETE FROM test WHERE id = $1', [1]);
    });

    test('should throw error when not connected', async () => {
      postgresConnection.isConnected = false;

      await expect(postgresConnection.query('SELECT * FROM test')).rejects.toThrow('Database not connected');
    });
    ...
    test('should handle query errors gracefully', async () => {
      ...
      await expect(postgresConnection.query('SELECT * FROM test')).rejects.toThrow('Query failed');
    });
    ...
    test('should handle execute errors gracefully', async () => {
      ...
      await expect(postgresConnection.execute('INSERT INTO test VALUES ($1)', ['test'])).rejects.toThrow('Execute failed');
    });
  });
  ...
  describe('Table Verification', () => {
    ...
    test('should verify all required tables exist', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith("SELECT tablename FROM pg_tables WHERE schemaname = 'public'");
      expect(mockLogger.info).toHaveBeenCalledWith('✅ All 12 required tables verified successfully');
    });
    ...
    test('should throw error when tables are missing', async () => {
      ...
      await expect(postgresConnection.verifyTablesCreated()).rejects.toThrow('Database initialization incomplete. Missing tables: user_sessions, tasks, analysis, chat_sessions, chat_messages, workflows, workflow_executions, task_templates, task_suggestions, task_sessions');
    });
    ...
    test('should handle table verification errors gracefully', async () => {
      ...
      await expect(postgresConnection.verifyTablesCreated()).rejects.toThrow('Table verification failed');
    });
  });
  ...
  describe('Database Initialization', () => {
    ...
    test('should initialize database successfully', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('✅ PostgreSQL database initialized successfully');
    });
    ...
    test('should handle initialization errors gracefully', async () => {
      ...
      await expect(postgresConnection.initializeDatabase()).rejects.toThrow('Initialization failed');
    });
  });
  ...
  describe('Connection Pool Management', () => {
    ...
    test('should handle pool connection errors gracefully', async () => {
      ...
      await expect(postgresConnection.connect()).rejects.toThrow('Pool connection failed');
    });
  });
  ...
  describe('Transaction Management', () => {
    ...
    test('should handle transaction errors gracefully', async () => {
      ...
      await expect(postgresConnection.beginTransaction()).rejects.toThrow('Transaction failed');
    });
  });
  ...
  describe('Error Handling', () => {
    test('should handle invalid connection parameters gracefully', async () => {
      ...
      await expect(invalidConnection.connect()).rejects.toThrow('Connection refused');
    });
    ...
    test('should handle database not found errors gracefully', async () => {
      ...
      await expect(postgresConnection.query('SELECT * FROM test')).rejects.toThrow('database "nonexistent" does not exist');
    });
    ...
    test('should handle permission denied errors gracefully', async () => {
      ...
      await expect(postgresConnection.query('SELECT * FROM test')).rejects.toThrow('permission denied for table test');
    });
  });
