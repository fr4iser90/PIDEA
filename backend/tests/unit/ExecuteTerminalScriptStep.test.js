...
} = require("@domain/steps/categories/terminal/execute_terminal_script_step");
...
describe("ExecuteTerminalScriptStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate interpreter types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid interpreter: invalid-interpreter. Valid interpreters: bash, sh, zsh, python, python3, node, npm, npx, ruby, php, perl, java, go, rust",
      );
    });
    ...
    test("should provide warnings for relative paths", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Relative working directory may cause issues",
      );
    });
  });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate interpreter", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid interpreter: invalid. Valid interpreters: bash, sh, zsh, python, python3, node, npm, npx, ruby, php, perl, java, go, rust",
      );
    });
  ...
  describe("Successful Execution", () => {
    ...
    test("should call IDEAutomationService with correct parameters", async () => {
      ...
      expect(
        mockIDEAutomationService.executeTerminalScript,
  ...
  describe("Error Handling", () => {
    ...
    test("should publish failure event on error", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "terminal.script.executing.failed",
        {
          stepId: expect.any(String),
          userId: "test-user-123",
          script: 'echo "Hello World"',
