...
const {
  getServiceRegistry,
} = require("../../backend/infrastructure/dependency-injection/ServiceRegistry");
const {
  getServiceContainer,
} = require("../../backend/infrastructure/dependency-injection/ServiceContainer");
...
describe("Dependency Injection Ordering Fix", () => {
  ...
  describe("DependencyGraph", () => {
    ...
    it("should validate dependencies", () => {
      ...
      assert(
        validation.missingDependencies["A"],
        "Should report missing B for A",
      );
      assert(
        validation.missingDependencies["C"],
        "Should report missing D for C",
      );
    });
  });
  ...
  describe("ServiceOrderResolver", () => {
    ...
    it("should detect circular dependencies in service order", () => {
      ...
      assert(
        !resolution.success,
        "Resolution should fail with circular dependency",
      );
      assert(
        resolution.error.includes("Circular dependencies detected"),
    ...
    it("should group services by category", () => {
      ...
      const resolution = serviceOrderResolver.resolveOrder();
      assert(
        resolution.registrationPlan.length > 0,
        "Should have registration plan",
      );

      const infrastructurePlan = resolution.registrationPlan.find(
        (p) => p.category === "infrastructure",
      );
      const domainPlan = resolution.registrationPlan.find(
        (p) => p.category === "domain",
      );
      const handlersPlan = resolution.registrationPlan.find(
        (p) => p.category === "handlers",
      );
  ...
  describe("ServiceContainer", () => {
    ...
    it("should provide dependency information", () => {
      ...
      assert(
        ...
        "Should have correct dependency count",
      );
    });
  });
  ...
  describe("ServiceRegistry Integration", () => {
    it("should register services with automatic dependency resolution", () => {
      ...
      assert(
        serviceRegistry.serviceOrderResolver,
        "Should have service order resolver",
      );
      assert(
        typeof serviceRegistry.collectServiceDefinitions === "function",
        "Should have collectServiceDefinitions method",
      );
      assert(
        typeof serviceRegistry.registerServiceByName === "function",
        "Should have registerServiceByName method",
      );
    });
    ...
    it("should resolve WorkflowOrchestrationService dependencies", () => {
      ...
      serviceRegistry.addServiceDefinition(
      ...
      serviceRegistry.addServiceDefinition(
        "cursorIDEService",
        ["browserManager", "ideManager", "eventBus"],
        "domain",
      );
      ...
      assert(
        resolution.orderedServices.includes("cursorIDEService"),
        "Should include cursorIDEService",
      );
      assert(
        resolution.orderedServices.includes("workflowOrchestrationService"),
        "Should include workflowOrchestrationService",
      );

      // cursorIDEService should come before workflowOrchestrationService
      const cursorIndex =
        resolution.orderedServices.indexOf("cursorIDEService");
      const workflowIndex = resolution.orderedServices.indexOf(
        "workflowOrchestrationService",
      );
      assert(
        cursorIndex < workflowIndex,
        "cursorIDEService should be resolved before workflowOrchestrationService",
      );
    });
  });
