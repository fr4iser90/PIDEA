...
} = require("@domain/steps/categories/terminal/monitor_terminal_output_step");
...
describe("MonitorTerminalOutputStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate filter types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid filter: invalid-filter. Valid filters: all, stdout, stderr, error, warning, info",
      );
    });
    ...
    test("should provide warnings for large maxLines", () => {
      ...
      expect(result.warnings).toContain(
        "Large max lines may impact performance",
      );
    });
    ...
    test("should provide warnings for all filter", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Monitoring all output may generate large results",
      );
    });
  });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate filter", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid filter: invalid. Valid filters: all, stdout, stderr, error, warning, info",
      );
    });
  ...
  describe("Successful Execution", () => {
    ...
    test("should call IDEAutomationService with correct parameters", async () => {
      ...
      expect(
        mockIDEAutomationService.monitorTerminalOutput,
  ...
  describe("Error Handling", () => {
    ...
    test("should publish failure event on error", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "terminal.output.monitoring.failed",
        {
          stepId: expect.any(String),
          userId: "test-user-123",
          sessionId: "session-123",
