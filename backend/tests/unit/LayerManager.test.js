...
describe("LayerManager", () => {
  ...
  describe("createLayer", () => {
    it("should create a new layer successfully", async () => {
      ...
      const result = await layerManager.createLayer(
        "Test Layer",
        LayerType.DOMAIN,
        "Test description",
        1,
    ...
    it("should throw error for duplicate name", async () => {
      ...
      await expect(
        layerManager.createLayer("Test Layer", LayerType.DOMAIN),
  ...
  describe("getLayer", () => {
    ...
    it("should throw error for non-existent layer", async () => {
      ...
      await expect(layerManager.getLayer("non-existent")).rejects.toThrow(
  ...
  describe("updateLayer", () => {
    it("should update layer successfully", async () => {
      ...
      const result = await layerManager.updateLayer(mockLayer.id, {
        name: "Updated Layer",
      });
    ...
    it("should throw error for non-existent layer", async () => {
      ...
      await expect(
        layerManager.updateLayer("non-existent", { name: "Updated" }),
  ...
  describe("initializeDefaultLayers", () => {
    it("should initialize default layers", async () => {
      mockDatabaseConnection.execute
        ...
        .mockResolvedValue({
          rows: [
            Layer.create("Domain Layer", LayerType.DOMAIN).toDatabaseRow(),
          ],
        }); // create calls
