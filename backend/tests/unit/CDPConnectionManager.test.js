...
describe("CDPConnectionManager", () => {
  ...
  describe("initialize", () => {
    it("should initialize successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "CDPConnectionManager initialized successfully",
      );
    });
    ...
    it("should handle initialization failure", async () => {
      ...
      await expect(cdpManager.initialize()).rejects.toThrow(
        "Connection failed",
      );
      expect(cdpManager.isInitialized).toBe(false);
    });
    ...
    it("should not initialize twice", async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "CDPConnectionManager already initialized",
      );
    });
  });
  ...
  describe("testCDPAvailability", () => {
    it("should return true when CDP is available", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining("CDP availability test passed"),
      );
    });
    ...
    it("should return false when CDP is not available", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        "CDP availability test failed on all test ports",
      );
    });
  });
  ...
  describe("getWorkspaceDetectionConnection", () => {
    ...
    it("should get connection successfully", async () => {
      const mockConnection = {
        browser: {
          contexts: () => [{ pages: () => [{ title: () => "Test" }] }],
        },
        page: { title: () => "Test" },
      };

      // Mock connection pool
      cdpManager.connectionPool.getConnection = jest
        .fn()
      ...
      expect(cdpManager.connectionPool.getConnection).toHaveBeenCalledWith(
        9222,
      );
    });
    ...
    it("should throw error for invalid connection", async () => {
      ...
      await expect(
        cdpManager.getWorkspaceDetectionConnection(9222),
  ...
  describe("executeWorkspaceDetection", () => {
    ...
    it("should execute operation successfully", async () => {
      ...
      cdpManager.getWorkspaceDetectionConnection = jest
        .fn()
        .mockResolvedValue(mockConnection);

      const result = await cdpManager.executeWorkspaceDetection(
        9222,
        mockOperation,
      );
    ...
    it("should handle operation failure", async () => {
      ...
      await expect(
        cdpManager.executeWorkspaceDetection(9222, mockOperation),
  ...
  describe("extractWorkspaceInfo", () => {
    ...
    it("should handle extraction failure", async () => {
      ...
      await expect(cdpManager.extractWorkspaceInfo(connection)).rejects.toThrow(
  ...
  describe("caching", () => {
    ...
    it("should cache workspace detection result", () => {
      ...
      expect(cdpManager.workspaceDetectionCache.has("workspace-9222")).toBe(
        true,
      );
  ...
  describe("destroy", () => {
    it("should destroy successfully", async () => {
      ...
      expect(cdpManager.isInitialized).toBe(false);
      expect(mockLogger.info).toHaveBeenCalledWith(
        "CDPConnectionManager destroyed successfully",
      );
    });
