...
} = require("@domain/steps/categories/terminal/terminal_session_management_step");
...
describe("TerminalSessionManagementStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate action types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid action: invalid-action. Valid actions: create, read, update, delete, restart, cleanup",
      );
    });
    ...
    test("should provide warnings for destructive actions", () => {
      ...
      destructiveActions.forEach((action) => {
        ...
        expect(result.warnings).toContain(
          `Action ${action} may cause data loss`,
        );
      });
    });
    ...
    test("should provide warnings for restart action", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Restart action may interrupt ongoing operations",
      );
    });
  });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate action", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid action: invalid. Valid actions: create, read, update, delete, restart, cleanup",
      );
    });
  ...
  describe("Successful Execution", () => {
    ...
    test("should call IDEAutomationService with correct parameters", async () => {
      ...
      expect(
        mockIDEAutomationService.manageTerminalSession,
  ...
  describe("Error Handling", () => {
    ...
    test("should publish failure event on error", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "terminal.session.managing.failed",
        {
          stepId: expect.any(String),
          userId: "test-user-123",
          action: "create",
