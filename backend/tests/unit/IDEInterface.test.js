...
describe("IDEInterface", () => {
  ...
  describe("Initialization", () => {
    test("should initialize IDE interface successfully", async () => {
      ...
      expect(
        mockDependencies.workspaceService.validateWorkspacePath,
      ).toHaveBeenCalledWith("/test/workspace");
      expect(mockDependencies.portManager.initializePort).toHaveBeenCalledWith(
        3000,
        {
    ...
    test("should handle workspace validation failure", async () => {
      mockDependencies.workspaceService.validateWorkspacePath.mockResolvedValue(
        false,
      );

      await expect(ideInterface.initialize()).rejects.toThrow(
        "Invalid workspace path: /test/workspace",
      );
      expect(ideInterface.status).toBe("error");
    });

    test("should handle port initialization failure", async () => {
      mockDependencies.portManager.initializePort.mockRejectedValue(
        new Error("Port initialization failed"),
      );
    ...
    test("should handle missing workspace path gracefully", async () => {
      const interfaceWithoutWorkspace = new IDEInterface(
        "test-ide-3",
        "ide",
        {},
        mockDependencies,
    ...
    test("should handle missing port gracefully", async () => {
      const interfaceWithoutPort = new IDEInterface(
        "test-ide-4",
        "ide",
        { workspacePath: "/test" },
        mockDependencies,
  ...
  describe("Lifecycle Management", () => {
    ...
    test("should destroy IDE interface successfully", async () => {
      ...
      expect(mockDependencies.portManager.releasePort).toHaveBeenCalledWith(
        3000,
      );
    });

    test("should handle connection failure during start", async () => {
      mockDependencies.browserManager.connect.mockRejectedValue(
    ...
    test("should handle disconnect failure gracefully", async () => {
      await ideInterface.start();
      mockDependencies.browserManager.disconnect.mockRejectedValue(
  ...
  describe("IDE-Specific Operations", () => {
    ...
    test("should get workspace information", async () => {
      ...
      expect(
        mockDependencies.workspaceService.getWorkspaceInfo,
      ).toHaveBeenCalledWith("/test/workspace");
    });
    ...
    test("should throw error when getting workspace info without workspace path", async () => {
      ...
      await expect(
        interfaceWithoutWorkspace.getWorkspaceInfo(),
      ).rejects.toThrow("No workspace path configured");
    });

    test("should execute command successfully", async () => {
      const commandResult = { output: "Command executed" };
      mockDependencies.browserManager.executeCommand.mockResolvedValue(
        commandResult,
      );
      ...
      expect(
        mockDependencies.browserManager.executeCommand,
      ).toHaveBeenCalledWith("test-command", {
        port: 3000,
        workspacePath: "/test/workspace",
        option: "value",
      });
    ...
    test("should handle command execution failure", async () => {
      mockDependencies.browserManager.executeCommand.mockRejectedValue(
    ...
    test("should throw error when executing command without connection", async () => {
      const disconnectedInterface = new IDEInterface(
        "test-ide-6",
        "ide",
        {},
        mockDependencies,
      ...
      await expect(
        disconnectedInterface.executeCommand("test-command"),
      ).rejects.toThrow("IDE interface is not connected");
    });

    test("should send message successfully", async () => {
      const messageResult = { response: "Message sent" };
      mockDependencies.browserManager.sendMessage.mockResolvedValue(
        messageResult,
      );
      ...
      expect(mockDependencies.browserManager.sendMessage).toHaveBeenCalledWith(
        "test-message",
        {
          port: 3000,
          workspacePath: "/test/workspace",
    ...
    test("should handle message send failure", async () => {
      mockDependencies.browserManager.sendMessage.mockRejectedValue(
    ...
    test("should throw error when sending message without connection", async () => {
      ...
      await expect(
        disconnectedInterface.sendMessage("test-message"),
      ).rejects.toThrow("IDE interface is not connected");
    });
  });
  ...
  describe("Event Publishing", () => {
    ...
    test("should handle missing event bus gracefully", async () => {
      const interfaceWithoutEventBus = new IDEInterface(
        "test-ide-8",
        "ide",
        {},
        {
  ...
  describe("Error Handling", () => {
    test("should handle initialization errors", async () => {
      mockDependencies.workspaceService.validateWorkspacePath.mockRejectedValue(
    ...
    test("should handle start errors", async () => {
      await ideInterface.initialize();
      mockDependencies.browserManager.start.mockRejectedValue(
    ...
    test("should handle stop errors", async () => {
      await ideInterface.initialize();
      await ideInterface.start();
      mockDependencies.browserManager.stop.mockRejectedValue(
    ...
    test("should handle destroy errors", async () => {
      await ideInterface.initialize();
      await ideInterface.start();
      mockDependencies.browserManager.cleanup.mockRejectedValue(
  ...
  describe("Configuration Management", () => {
    test("should handle custom IDE configuration", () => {
      ...
      const customInterface = new IDEInterface(
        "test-ide-9",
        "ide",
        { ideConfig: customConfig },
        mockDependencies,
      );
    ...
    test("should use default IDE configuration when not provided", () => {
      const defaultInterface = new IDEInterface(
        "test-ide-10",
        "ide",
        {},
        mockDependencies,
  ...
  describe("Edge Cases", () => {
    test("should handle missing browser manager gracefully", async () => {
      const interfaceWithoutBrowserManager = new IDEInterface(
        "test-ide-11",
        "ide",
        {},
        {
      ...
      const result =
        await interfaceWithoutBrowserManager.executeCommand("test-command");
      expect(result.success).toBe(true);
      expect(result.data.command).toBe("test-command");
    });

    test("should handle missing port manager gracefully", async () => {
      const interfaceWithoutPortManager = new IDEInterface(
        "test-ide-12",
        "ide",
        {
          workspacePath: "/test",
    ...
    test("should handle missing workspace service gracefully", async () => {
      const interfaceWithoutWorkspaceService = new IDEInterface(
        "test-ide-13",
        "ide",
        {
          workspacePath: "/test",
    ...
    test("should handle special characters in configuration", () => {
      ...
      const specialInterface = new IDEInterface(
        "test-ide-14",
        "ide",
        specialConfig,
        mockDependencies,
