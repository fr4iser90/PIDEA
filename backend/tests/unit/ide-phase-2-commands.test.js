...
describe("IDE Phase 2: Terminal & Analysis Commands", () => {
  ...
  beforeEach(() => {
    ...
    mockDependencies = {
      ideAutomationService: {
        ...
        monitorTerminalOutput: async () => ({
         
          result: "http://localhost:3000",
        }),
        restartUserApp: async () => ({
         
          result: "app restarted",
        }),
        captureTerminalLogs: async () => ({
         
          logs: [],
          count: 0,
        }),
        analyzeProject: async () => ({ analysis: {} }),
        analyzeAgain: async () => ({ analysis: {} }),
        getWorkspaceInfo: async () => ({ workspaceInfo: {} }),
        detectPackageJson: async () => ({
         
          packageJsonUrl: "http://localhost:3000",
        }),
      },
      eventBus: {
        publish: async () => true,
  ...
  describe("Terminal Commands", () => {
    describe("OpenTerminalCommand", () => {
      ...
      it("should validate required parameters", () => {
        expect(() => new OpenTerminalCommand({})).to.throw(
          "User ID is required",
        );
        expect(
          () => new OpenTerminalCommand({ userId: "test", ideType: "invalid" }),
        ).to.throw("Invalid IDE type");
      });
    ...
    describe("ExecuteTerminalCommand", () => {
      ...
      it("should validate required parameters", () => {
        expect(() => new ExecuteTerminalCommand({ userId: "test" })).to.throw(
        ...
        ).to.throw("Wait time must be a number between 0 and 30000");
      });
    ...
    describe("MonitorTerminalOutputCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () =>
            new MonitorTerminalOutputCommand({ userId: "test", duration: 500 }),
        ).to.throw("Duration must be a number between 1000 and 60000");
        expect(
          () =>
            new MonitorTerminalOutputCommand({ userId: "test", interval: 50 }),
        ).to.throw("Interval must be a number between 100 and 10000");
      });
    });
    ...
    describe("RestartUserAppCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () => new RestartUserAppCommand({ userId: "test", appName: 123 }),
        ).to.throw("App name must be a string");
        expect(
          () =>
            new RestartUserAppCommand({ userId: "test", forceRestart: "true" }),
        ).to.throw("forceRestart must be a boolean");
      });
    });
    ...
    describe("TerminalLogCaptureCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () => new TerminalLogCaptureCommand({ userId: "test", maxLines: 0 }),
        ).to.throw("Max lines must be a number between 1 and 10000");
        expect(
          () =>
            new TerminalLogCaptureCommand({
              userId: "test",
              filterLevel: "invalid",
            }),
        ).to.throw(
          "Filter level must be one of: all, error, warning, info, debug",
        );
      });
    });
  });
  ...
  describe("Analysis Commands", () => {
    describe("AnalyzeProjectCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          ...
          "Analysis type must be one of: full, quick, dependencies, structure",
        );
        expect(
          () =>
            new AnalyzeProjectCommand({ userId: "test", includeCache: "true" }),
        ).to.throw("includeCache must be a boolean");
      });
    });
    ...
    describe("AnalyzeAgainCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () => new AnalyzeAgainCommand({ userId: "test", clearCache: "true" }),
        ).to.throw("clearCache must be a boolean");
        expect(
          () =>
            new AnalyzeAgainCommand({ userId: "test", forceRefresh: "true" }),
        ).to.throw("forceRefresh must be a boolean");
      });
    });
    ...
    describe("GetWorkspaceInfoCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () =>
            new GetWorkspaceInfoCommand({
              userId: "test",
              includeDetails: "true",
            }),
        ).to.throw("includeDetails must be a boolean");
        expect(
          () =>
            new GetWorkspaceInfoCommand({
              userId: "test",
        ...
        ).to.throw("includeProjects must be a boolean");
      });
    });
    ...
    describe("DetectPackageJsonCommand", () => {
      ...
      it("should validate parameters", () => {
        expect(
          () =>
            new DetectPackageJsonCommand({
              userId: "test",
              detectDevServer: "true",
            }),
        ).to.throw("detectDevServer must be a boolean");
        expect(
          () =>
            new DetectPackageJsonCommand({
              userId: "test",
        ...
        ).to.throw("analyzeScripts must be a boolean");
      });
    });
  });
  ...
  describe("Terminal Handlers", () => {
    describe("OpenTerminalHandler", () => {
      it("should create handler with valid dependencies", () => {
        ...
        expect(handler.ideAutomationService).to.equal(
          mockDependencies.ideAutomationService,
        );
        expect(handler.eventBus).to.equal(mockDependencies.eventBus);
      });

      it("should validate dependencies", () => {
        expect(() => new OpenTerminalHandler({})).to.throw(
          "IDEAutomationService is required",
        );
        expect(
          () => new OpenTerminalHandler({ ideAutomationService: {} }),
        ).to.throw("EventBus is required");
      });
      ...
      it("should validate command type", async () => {
        ...
        try {
          ...
          expect(error.message).to.include(
            "Invalid command type for OpenTerminalHandler",
          );
        }
      });
    });
