...
describe("QueryOptimizer", () => {
  ...
  describe("constructor", () => {
    ...
    it("should set performance thresholds", () => {
      ...
      expect(queryOptimizer.performanceThresholds.highMemoryUsage).toBe(
        100 * 1024 * 1024,
      );
    });
  });
  ...
  describe("optimizeQuery", () => {
    ...
    it("should handle optimization errors gracefully", async () => {
      ...
      await expect(queryOptimizer.optimizeQuery(query, params)).rejects.toThrow(
        "Invalid SQL",
      );
    });
  ...
  describe("generateOptimizations", () => {
    it("should generate optimizations for slow queries", async () => {
      ...
      const optimizations =
        await queryOptimizer.generateOptimizations(analysis);
    ...
    it("should generate optimizations for SELECT * queries", async () => {
      ...
      expect(optimizations).toBeInstanceOf(Array);
      expect(optimizations.some((opt) => opt.type === "select_star")).toBe(
        true,
      );
    });
  });
  ...
  describe("applyOptimizations", () => {
    it("should apply SELECT * optimizations", async () => {
      ...
      const result = await queryOptimizer.applyOptimizations(
        query,
        optimizations,
      );
  ...
  describe("analyzeQueryStructure", () => {
    it("should analyze query structure correctly", () => {
      const query =
        "SELECT * FROM tasks WHERE status = ? JOIN users ON tasks.user_id = users.id";
    ...
    it("should determine complexity correctly", () => {
      ...
      const complexQuery =
        "SELECT * FROM tasks JOIN users ON tasks.user_id = users.id JOIN projects ON users.project_id = projects.id";

      expect(queryOptimizer.analyzeQueryStructure(simpleQuery).complexity).toBe(
        "low",
      );
      expect(queryOptimizer.analyzeQueryStructure(mediumQuery).complexity).toBe(
        "medium",
      );
      expect(
        queryOptimizer.analyzeQueryStructure(complexQuery).complexity,
      ).toBe("high");
    });
  });
  ...
  describe("calculateEstimatedImprovement", () => {
    it("should calculate improvement for slow queries", () => {
      ...
      const improvement = queryOptimizer.calculateEstimatedImprovement(
        query,
        executionTime,
        structure,
      );
