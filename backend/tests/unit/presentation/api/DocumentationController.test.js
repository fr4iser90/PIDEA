...
describe("DocumentationController", () => {
  ...
  describe("analyzeAllProjects", () => {
    ...
    it("should successfully analyze all projects", async () => {
      ...
      jest
        .spyOn(controller, "executeDocumentationTasksBatch")
        .mockResolvedValue({
          batchId: "batch_123",
          status: "completed",
          progress: 100,
    ...
    it("should handle case when no IDEs are available", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        ...
        error:
          "No IDE instances found. Please open projects in Cursor IDE first.",
      });
    });
    ...
    it("should handle errors during analysis", async () => {
      mockCursorIDEService.sendMessage.mockRejectedValue(
    ...
    it("should handle errors in response collection", async () => {
      mockChatRepository.getMessagesByPort.mockRejectedValue(
  ...
  describe("findProjectIDE", () => {
    ...
    it("should handle case when IDE manager is not available", async () => {
      ...
      const result =
        await controllerWithoutIDEManager.findProjectIDE("/path/to/project1");
  ...
  describe("analyzeDocumentation", () => {
    ...
    it("should handle case when no IDE is found for project", async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        ...
        error:
          "No IDE found for this project. Please open the project in Cursor IDE first.",
      });
    });
  ...
  describe("processDocumentationAnalysis", () => {
    it("should process analysis text successfully", () => {
      ...
      const result = controller.processDocumentationAnalysis(
        analysisText,
        "test-project",
      );
  ...
  describe("generateAnalysisSummary", () => {
    it("should generate summary with all data", () => {
      ...
      const priorities = [{ level: "high", items: ["Item 1", "Item 2"] }];
      ...
      expect(summary).toEqual({
        ...
        highPriorityCount: 2,
        message:
          "Found 2 documentation tasks. Average coverage: 68%. 2 high priority areas identified.",
      });
    });
    ...
    it("should handle empty data", () => {
      ...
      expect(summary).toEqual({
        ...
        highPriorityCount: 0,
        message:
          "Found 0 documentation tasks. Average coverage: 0%. 0 high priority areas identified.",
      });
    });
  });
  ...
  describe("createTasksFromAnalysis", () => {
    ...
    it("should create tasks from analysis successfully", async () => {
      ...
      const result = await controller.createTasksFromAnalysis(
        analysisResult,
        "test-project",
      );
  ...
  describe("getProjectIdFromPath", () => {
    it("should extract project ID from workspace path", () => {
      expect(controller.getProjectIdFromPath("/path/to/my-project")).toBe(
        "myproject",
      );
      expect(controller.getProjectIdFromPath("/path/to/My Project")).toBe(
        "myproject",
      );
      expect(controller.getProjectIdFromPath("/path/to/project-123")).toBe(
        "project123",
      );
    });
  ...
  describe("createOrGetChatSession", () => {
    it("should create chat session for project and IDE port", async () => {
      const session = await controller.createOrGetChatSession(
        "test-project",
        3000,
      );
  ...
  describe("saveChatMessage", () => {
    it("should save chat message and return message object", async () => {
      const message = await controller.saveChatMessage(
        "session1",
        "Test message",
        "assistant",
        "documentation",
  ...
  describe("runSingleProjectAnalysis", () => {
    ...
    it("should run single project analysis successfully", async () => {
      const result = await controller.runSingleProjectAnalysis(
        "test-project",
        "/path/to/project",
        3000,
      );
    ...
    it("should handle missing prompt file", async () => {
      ...
      await expect(
        controller.runSingleProjectAnalysis(
          "test-project",
          "/path/to/project",
          3000,
        ),
    ...
    it("should handle AI response failure", async () => {
      ...
      await expect(
        ...
        "AI analysis timed out or failed for project test-project",
      );
    });
  });
  ...
  describe("sendQuickPrompt", () => {
    ...
    it("should send quick prompt successfully", async () => {
      const result = await controller.sendQuickPrompt(
        "test-project",
        "/path/to/project",
        3000,
      );
  ...
  describe("getLatestChatResponse", () => {
    ...
    it("should get latest chat response successfully", async () => {
      const result = await controller.getLatestChatResponse(
        "test-project",
        3000,
      );
    ...
    it("should handle case when chat repository is not available", async () => {
      ...
      const result = await controllerWithoutChatRepo.getLatestChatResponse(
        "test-project",
        3000,
      );
  ...
  describe("collectResponseOnly", () => {
    ...
    it("should collect response only successfully", async () => {
      const result = await controller.collectResponseOnly(
        "test-project",
        "/path/to/project",
        3000,
      );
  ...
  describe("executeDocumentationTasksBatch", () => {
    ...
    it("should execute documentation tasks batch successfully", async () => {
      ...
      const result = await controller.executeDocumentationTasksBatch(
        tasks,
        "/path/to/project",
      );
  ...
  describe("executeDocumentationTask", () => {
    it("should execute single documentation task successfully", async () => {
      ...
      const result = await controller.executeDocumentationTask(
        task,
        "/path/to/project",
      );
  ...
  describe("sendTasksToIDE", () => {
    ...
    it("should send tasks to IDE successfully", async () => {
      ...
      const result = await controller.sendTasksToIDE(
        tasks,
        "/path/to/project",
        3000,
      );
