...
describe("WorkflowController", () => {
  ...
  describe("executeAutoMode", () => {
    ...
    it("should execute workflow successfully with full mode", async () => {
      ...
      expect(VibeCoderModeCommand).toHaveBeenCalledWith(
    ...
    it("should get workspace path from project mapping service", async () => {
      ...
      mockApplication.projectMappingService.getWorkspaceFromProjectId.mockReturnValue(
        "/mapped/path",
      );
      ...
      expect(
        mockApplication.projectMappingService.getWorkspaceFromProjectId,
    ...
    it("should get workspace path from IDE manager when project mapping fails", async () => {
      ...
      mockIdeManager.getWorkspaceInfo.mockResolvedValue({
    ...
    it("should use project root as final fallback", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        "WorkflowController: File-based detection failed, using project root",
        expect.any(Object),
      );
      expect(VibeCoderModeCommand).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it("should handle task execution with taskId", async () => {
      ...
      mockApplication.taskService.executeTask.mockResolvedValue({
       
      });

      await controller.executeAutoMode(mockReq, mockRes);

      expect(mockApplication.gitService.createBranch).toHaveBeenCalledWith(
        "/test/path",
        "feature/test",
      );
      expect(mockIdeManager.clickNewChat).toHaveBeenCalledWith(3000);
      expect(mockApplication.taskService.executeTask).toHaveBeenCalledWith(
        "task-123",
        {
          projectPath: "/test/path",
          userId: "user-123",
    ...
    it("should handle Git branch creation failure", async () => {
      ...
      mockApplication.gitService.createBranch.mockRejectedValue(
        new Error("Git error"),
      );
      mockApplication.taskService.executeTask.mockResolvedValue({
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to create Git branch",
        expect.any(Object),
      );
    ...
    it("should handle new chat click failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to click new chat",
        expect.any(Object),
      );
      expect(mockApplication.taskService.executeTask).toHaveBeenCalled();
    });
    ...
    it("should handle task execution failure", async () => {
      ...
      mockReq.body.taskId = "task-123";
      mockApplication.taskService.executeTask.mockRejectedValue(
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to execute task",
        expect.any(Object),
      );
    ...
    it("should handle command execution failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to execute workflow",
        expect.any(Object),
      );
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "autoMode:failed",
  ...
  describe("getAutoModeStatus", () => {
    ...
    it("should get workflow status successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "WorkflowController: Workflow status retrieved",
        {
          sessionId: "session-123",
          userId: "user-123",
        },
    ...
    it("should handle query execution failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to get workflow status",
        {
          sessionId: "session-123",
          error: "Query failed",
          userId: "user-123",
  ...
  describe("getAutoModeProgress", () => {
    ...
    it("should get workflow progress successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "WorkflowController: Workflow progress retrieved",
        {
          sessionId: "session-123",
          userId: "user-123",
        },
    ...
    it("should handle query execution failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to get workflow progress",
        {
          sessionId: "session-123",
          error: "Query failed",
          userId: "user-123",
  ...
  describe("getAutoModeResults", () => {
    ...
    it("should get workflow results successfully", async () => {
      const mockResult = {
        results: { analysis: {}, refactor: {}, generate: {} },
      };
      mockQueryBus.execute.mockResolvedValue(mockResult);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "WorkflowController: Workflow results retrieved",
        {
          sessionId: "session-123",
          userId: "user-123",
        },
    ...
    it("should handle query execution failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to get workflow results",
        {
          sessionId: "session-123",
          error: "Query failed",
          userId: "user-123",
  ...
  describe("getAutoModeSessions", () => {
    ...
    it("should get workflow sessions successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "WorkflowController: Workflow sessions retrieved",
        {
          count: 2,
          userId: "user-123",
        },
    ...
    it("should handle query execution failure", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "WorkflowController: Failed to get workflow sessions",
        {
          error: "Query failed",
          userId: "user-123",
        },
  ...
  describe("getAutoModeStats", () => {
    ...
    it("should get workflow statistics successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "WorkflowController: Workflow statistics retrieved",
        {
          userId: "user-123",
        },
      );
  ...
  describe("edge cases and error handling", () => {
    it("should handle missing dependencies gracefully", async () => {
      ...
      await expect(
        controllerWithMissingDeps.executeAutoMode(mockReq, mockRes),
    ...
    it("should handle undefined request body gracefully", async () => {
      ...
      await expect(
        controller.executeAutoMode(mockReq, mockRes),
      ).rejects.toThrow();
    });
