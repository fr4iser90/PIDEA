...
describe("TaskController - Manual Tasks", () => {
  ...
  describe("getManualTasks", () => {
    it("should return manual tasks only", async () => {
      ...
      mockTaskApplicationService.getProjectTasks = jest
        .fn()
      ...
      expect(mockTaskApplicationService.getProjectTasks).toHaveBeenCalledWith(
        "test",
        { type: "documentation" },
      );
  ...
  describe("getManualTaskDetails", () => {
    it("should return specific manual task", async () => {
      ...
      mockTaskApplicationService.getTask = jest
        .fn()
        .mockResolvedValue(mockTask);

      const req = {
        params: { projectId: "test", id: "1" },
        user: { id: "user1" },
      };
      const res = { json: jest.fn(), status: jest.fn().mockReturnThis() };
      ...
      expect(mockTaskApplicationService.getTask).toHaveBeenCalledWith(
        "1",
        "test",
      );
      expect(res.json).toHaveBeenCalledWith(
    ...
    it("should handle task not found", async () => {
      mockTaskApplicationService.getTask = jest.fn().mockResolvedValue(null);

      const req = {
        params: { projectId: "test", id: "999" },
        user: { id: "user1" },
      };
      const res = { json: jest.fn(), status: jest.fn().mockReturnThis() };
    ...
    it("should handle errors correctly", async () => {
      const error = new Error("Database error");
      mockTaskApplicationService.getTask = jest.fn().mockRejectedValue(error);
  ...
  describe("syncManualTasks", () => {
    it("should sync manual tasks successfully", async () => {
      ...
      mockTaskApplicationService.syncManualTasks = jest
        .fn()
      ...
      expect(mockTaskApplicationService.syncManualTasks).toHaveBeenCalledWith(
        "test-project",
        "user1",
      );
      expect(res.json).toHaveBeenCalledWith(
  ...
  describe("cleanManualTasks", () => {
    it("should clean manual tasks successfully", async () => {
      ...
      mockTaskApplicationService.cleanManualTasks = jest
        .fn()
        .mockResolvedValue(mockResult);
      ...
      expect(mockTaskApplicationService.cleanManualTasks).toHaveBeenCalledWith(
        "test-project",
        "user1",
      );
      expect(res.json).toHaveBeenCalledWith(
