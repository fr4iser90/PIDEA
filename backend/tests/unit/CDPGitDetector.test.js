...
describe("CDPGitDetector", () => {
  ...
  describe("initialize", () => {
    it("should initialize successfully", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "CDPGitDetector initialized successfully",
      );
    });

    it("should handle initialization failure", async () => {
      mockCDPManager.initialize.mockRejectedValue(
        new Error("CDP initialization failed"),
      );

      await expect(gitDetector.initialize()).rejects.toThrow(
    ...
    it("should not initialize twice", async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "CDPGitDetector already initialized",
      );
    });
  });
  ...
  describe("extractGitInfoFromCDP", () => {
    ...
    it("should handle page evaluation failure", async () => {
      const mockPage = {
        evaluate: jest
          .fn()
          .mockRejectedValue(new Error("Page evaluation failed")),
      };
  ...
  describe("extractGitInfoLocally", () => {
    ...
    it("should return non-Git repo info for directory without .git", async () => {
      ...
      const result =
        await gitDetector.extractGitInfoLocally("/path/to/workspace");
    ...
    it("should extract Git info for valid repository", async () => {
      const fs = require("fs");
      fs.existsSync = jest.fn().mockImplementation((path) => {
        if (path === "/path/to/workspace" || path === "/path/to/workspace/.git")
          return true;
        return false;
      });
  ...
  describe("getGitInformation", () => {
    ...
    it("should use cached result", async () => {
      ...
      expect(gitDetector.getCachedGitInfo).toHaveBeenCalledWith(
        "/path/to/workspace",
      );
    });
    ...
    it("should try CDP extraction first when port is provided", async () => {
      ...
      const result = await gitDetector.getGitInformation(
        "/path/to/workspace",
        9222,
      );

      expect(result).toEqual(mockCDPResult);
      expect(mockCDPManager.executeWorkspaceDetection).toHaveBeenCalledWith(
        9222,
        expect.any(Function),
      );
    });
    ...
    it("should fallback to local extraction when CDP fails", async () => {
      ...
      gitDetector.getCachedGitInfo = jest.fn().mockReturnValue(null);
      mockCDPManager.executeWorkspaceDetection.mockRejectedValue(
        new Error("CDP failed"),
      );
      gitDetector.extractGitInfoLocally = jest
        .fn()
        .mockResolvedValue(mockLocalResult);
      ...
      expect(gitDetector.extractGitInfoLocally).toHaveBeenCalledWith(
        "/path/to/workspace",
      );
    });
    ...
    it("should skip CDP extraction when disabled", async () => {
      ...
      detector.extractGitInfoLocally = jest
        .fn()
        .mockResolvedValue(mockLocalResult);

      const result = await detector.getGitInformation(
        "/path/to/workspace",
        9222,
      );
    ...
    it("should cache the result", async () => {
      ...
      expect(gitDetector.cacheGitInfo).toHaveBeenCalledWith(
        "/path/to/workspace",
        mockResult,
        1000,
      );
    ...
    it("should record extraction history", async () => {
      ...
      expect(gitDetector.recordExtractionHistory).toHaveBeenCalledWith(
        "/path/to/workspace",
        expect.any(Object),
      );
    });
  ...
  describe("caching", () => {
    ...
    it("should clear Git cache for specific workspace", () => {
      gitDetector.gitCache.set("/path/to/workspace", {
        gitInfo: {},
        timestamp: Date.now(),
        ttl: 1000,
      });
    ...
    it("should clear all Git cache", () => {
      gitDetector.gitCache.set("/path/to/workspace1", {
        gitInfo: {},
        timestamp: Date.now(),
        ttl: 1000,
      });
      gitDetector.gitCache.set("/path/to/workspace2", {
        gitInfo: {},
        timestamp: Date.now(),
        ttl: 1000,
      });
  ...
  describe("extraction history", () => {
    ...
    it("should keep only last 10 extractions", () => {
      ...
      // Add 11 extractions
      for (let i = 0; i < 11; i++) {
        gitDetector.recordExtractionHistory("/path/to/workspace", {
          ...result,
          index: i,
        });
      }
    ...
    it("should clear extraction history for specific workspace", () => {
      gitDetector.extractionHistory.set("/path/to/workspace", [
        { test: "data" },
      ]);

      gitDetector.clearExtractionHistory("/path/to/workspace");

      expect(gitDetector.extractionHistory.has("/path/to/workspace")).toBe(
        false,
      );
    });

    it("should clear all extraction history", () => {
      gitDetector.extractionHistory.set("/path/to/workspace1", [
        { test: "data1" },
      ]);
      gitDetector.extractionHistory.set("/path/to/workspace2", [
        { test: "data2" },
      ]);
  ...
  describe("destroy", () => {
    it("should destroy successfully", async () => {
      ...
      expect(gitDetector.isInitialized).toBe(false);
      expect(mockLogger.info).toHaveBeenCalledWith(
        "CDPGitDetector destroyed successfully",
      );
    });
