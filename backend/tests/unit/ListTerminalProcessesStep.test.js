...
} = require("@domain/steps/categories/terminal/list_terminal_processes_step");
...
describe("ListTerminalProcessesStep", () => {
  ...
  beforeEach(() => {
    ...
    context.getService.mockImplementation((serviceName) => {
      if (serviceName === "IDEAutomationService")
  ...
  describe("Validation", () => {
    ...
    test("should validate filter types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid filter: invalid-filter. Valid filters: all, running, sleeping, stopped, zombie",
      );
    });
    ...
    test("should validate sortBy fields", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid sort field: invalid-field. Valid fields: pid, name, status, cpu, memory",
      );
    });
    ...
    test("should validate sort order", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid sort order: invalid-order. Valid orders: asc, desc",
      );
    });
    ...
    test("should provide warnings for all filter", () => {
      ...
      expect(result.warnings).toContain(
        "Listing all processes may return large results",
      );
    });
  });
  ...
  describe("Service Validation", () => {
    test("should require IDEAutomationService", () => {
      ...
      expect(() => step.validateServices(context)).toThrow(
        "IDEAutomationService not available in context",
      );
    });
  ...
  describe("Context Validation", () => {
    test("should validate required userId", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "User ID is required",
      );
    });
    ...
    test("should validate filter", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid filter: invalid. Valid filters: all, running, sleeping, stopped, zombie",
      );
    });
    ...
    test("should validate sortBy", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid sort field: invalid. Valid fields: pid, name, status, cpu, memory",
      );
    });
    ...
    test("should validate sortOrder", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid sort order: invalid. Valid orders: asc, desc",
      );
    });
    ...
    test("should validate includeDetails boolean", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Include details must be a boolean",
      );
    });
  ...
  describe("Successful Execution", () => {
    ...
    test("should call IDEAutomationService with correct parameters", async () => {
      ...
      expect(
        mockIDEAutomationService.listTerminalProcesses,
  ...
  describe("Error Handling", () => {
    ...
    test("should publish failure event on error", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "terminal.processes.listing.failed",
        {
          stepId: expect.any(String),
          userId: "test-user-123",
          error: "Service error",
