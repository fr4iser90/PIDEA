...
describe("FrameworkStepRegistry Unit Tests", () => {
  ...
  describe("Initialization", () => {
    test("should initialize successfully", async () => {
      jest
        .spyOn(frameworkStepRegistry, "discoverFrameworks")
        .mockResolvedValue();
      jest
        .spyOn(frameworkStepRegistry, "loadFrameworkSteps")
        .mockResolvedValue();
      jest
        .spyOn(frameworkStepRegistry, "registerFrameworkSteps")
        .mockResolvedValue();

      await frameworkStepRegistry.initialize(
        mockFrameworkBasePath,
        mockStepRegistry,
      );

      expect(frameworkStepRegistry.frameworkBasePath).toBe(
        mockFrameworkBasePath,
      );
    ...
    test("should handle initialization errors", async () => {
      ...
      await expect(
        frameworkStepRegistry.initialize(
          mockFrameworkBasePath,
          mockStepRegistry,
        ),
  ...
  describe("Framework Discovery", () => {
    test("should discover framework directories", async () => {
      ...
      expect(frameworkStepRegistry.frameworkDirectories.has("framework1")).toBe(
        true,
      );
      expect(frameworkStepRegistry.frameworkDirectories.has("framework2")).toBe(
        true,
      );
    });
  ...
  describe("Step Loading", () => {
    ...
    test("should load framework steps successfully", async () => {
      ...
      jest.doMock(
        "/path/to/framework/steps/test_step.js",
        () => mockStepModule,
      );

      await frameworkStepRegistry.loadFrameworkStepsFromDirectory(
        "test_framework",
        frameworkStepRegistry.frameworkDirectories.get("test_framework"),
      );

      expect(frameworkStepRegistry.loadedFrameworks.has("test_framework")).toBe(
        true,
      );
    });
  ...
  describe("Step Registration", () => {
    ...
    test("should register step with new interface", async () => {
      await frameworkStepRegistry.registerFrameworkStep(
    ...
    test("should handle step execution errors", async () => {
      const stepInfo = frameworkStepRegistry.frameworkSteps.get(
        "test_framework.test_step",
      );
  ...
  describe("Framework Management", () => {
    ...
    test("should get steps by framework name", () => {
      ...
      expect(steps).toEqual(["framework1.step1", "framework1.step2"]);
    });

    test("should check if step is framework step", () => {
      expect(frameworkStepRegistry.isFrameworkStep("framework1.step1")).toBe(
        true,
      );
    ...
    test("should get framework step info", () => {
      const stepInfo =
        frameworkStepRegistry.getFrameworkStepInfo("framework1.step1");
  ...
  describe("Framework Reloading", () => {
    test("should reload framework successfully", async () => {
      ...
      frameworkStepRegistry.frameworkDirectories.set(
        "test_framework",
        mockFrameworkInfo,
      );
      jest
        .spyOn(frameworkStepRegistry, "loadFrameworkStepsFromDirectory")
        .mockResolvedValue();
      jest
        .spyOn(frameworkStepRegistry, "registerFrameworkSteps")
        .mockResolvedValue();
      ...
      expect(
        frameworkStepRegistry.loadFrameworkStepsFromDirectory,
      ).toHaveBeenCalledWith("test_framework", mockFrameworkInfo);
    ...
    test("should handle reloading non-existent framework", async () => {
      await expect(
        frameworkStepRegistry.reloadFramework("non_existent"),
