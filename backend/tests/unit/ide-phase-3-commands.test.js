...
describe("IDE Phase 3: Browser/IDE Commands & Handlers", () => {
  ...
  describe("SwitchIDEPortCommand", () => {
    ...
    it("should reject invalid port numbers", async () => {
      ...
      expect(validation.errors).to.include(
        "Port must be in IDE range (9222-9251)",
      );
    });
  ...
  describe("SwitchIDEPortHandler", () => {
    ...
    it("should handle command successfully", async () => {
      ...
      expect(mockDependencies.ideAutomationService.switchIDEPort.called).to.be
        .true;
    });
  });
  ...
  describe("OpenFileExplorerHandler", () => {
    ...
    it("should handle command successfully", async () => {
      ...
      expect(result.ideType).to.equal("cursor");
      expect(result.path).to.equal("/path/to/project");
      expect(mockDependencies.browserManager.openFileExplorer.called).to.be
        .true;
      expect(mockDependencies.ideAutomationService.openFileExplorer.called).to
        .be.true;
    });
  });
  ...
  describe("OpenCommandPaletteHandler", () => {
    ...
    it("should handle command successfully", async () => {
      ...
      expect(result.ideType).to.equal("cursor");
      expect(result.searchTerm).to.equal("test search");
      expect(mockDependencies.browserManager.openCommandPalette.called).to.be
        .true;
      expect(mockDependencies.ideAutomationService.openCommandPalette.called).to
        .be.true;
    });
  });
  ...
  describe("ExecuteIDEActionHandler", () => {
    ...
    it("should handle command successfully", async () => {
      ...
      expect(result.action).to.equal("save");
      expect(result.actionType).to.equal("keyboard");
      expect(mockDependencies.browserManager.executeIDEAction.called).to.be
        .true;
      expect(mockDependencies.ideAutomationService.executeIDEAction.called).to
        .be.true;
    });
  });
  ...
  describe("GetIDESelectorsHandler", () => {
    ...
    it("should handle command successfully", async () => {
      ...
      expect(mockDependencies.ideAutomationService.getIDESelectors.called).to.be
        .true;
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle service errors gracefully", async () => {
      ...
      mockDependencies.browserManager.switchToPort.rejects(
  ...
  describe("Event Publishing", () => {
    it("should publish events for successful operations", async () => {
      ...
      expect(calls.some((call) => call.args[0] === "ide.port.switching")).to.be
        .true;
      expect(calls.some((call) => call.args[0] === "ide.port.switched")).to.be
        .true;
    });
    ...
    it("should publish events for failed operations", async () => {
      ...
      expect(calls.some((call) => call.args[0] === "ide.port.switching")).to.be
        .true;
      expect(calls.some((call) => call.args[0] === "ide.port.switch.failed")).to
        .be.true;
    });
  });
});
