...
describe("StatusCoordinator", () => {
  ...
  beforeEach(() => {
    ...
    statusCoordinator = new StatusCoordinator(
      mockDatabaseConnection,
      mockEventBus,
    );
  });
  ...
  describe("coordinateStatus", () => {
    it("should coordinate status synchronously", async () => {
      ...
      const result = await statusCoordinator.coordinateStatus(
        "task-1",
        "layer-1",
        "in_progress",
        "sync",
    ...
    it("should throw error for no layer assignments", async () => {
      ...
      await expect(
        statusCoordinator.coordinateStatus("task-1", "layer-1", "in_progress"),
      ).rejects.toThrow("No layer assignments found for task");
    });

    it("should throw error for unknown coordination type", async () => {
      const mockAssignments = [{ layer_id: "layer-1", task_id: "task-1" }];
      ...
      await expect(
        statusCoordinator.coordinateStatus(
          "task-1",
          "layer-1",
          "in_progress",
          "invalid",
  ...
  describe("updateLayerTaskStatus", () => {
    it("should update layer task status", async () => {
      ...
      const result = await statusCoordinator.updateLayerTaskStatus(
        "task-1",
        "layer-1",
        "in_progress",
      );
    ...
    it("should throw error for non-existent assignment", async () => {
      ...
      await expect(
        statusCoordinator.updateLayerTaskStatus(
          "task-1",
          "layer-1",
          "in_progress",
        ),
  ...
  describe("getCoordinationHistory", () => {
    it("should return coordination history", async () => {
      ...
      const result = await statusCoordinator.getCoordinationHistory(
        "task-1",
        10,
      );
  ...
  describe("resolvePendingCoordination", () => {
    it("should resolve pending coordination", async () => {
      ...
      const result =
        await statusCoordinator.resolvePendingCoordination("coord-1");
    ...
    it("should throw error for non-existent coordination", async () => {
      ...
      await expect(
        statusCoordinator.resolvePendingCoordination("non-existent"),
  ...
  describe("validateStatusTransition", () => {
    it("should validate valid status transition", async () => {
      ...
      const result = await statusCoordinator.validateStatusTransition(
        "task-1",
        "layer-1",
        "in_progress",
      );
    ...
    it("should throw error for invalid status transition", async () => {
      ...
      await expect(
        statusCoordinator.validateStatusTransition(
          "task-1",
          "layer-1",
          "in_progress",
        ),
      ...
      const mockDependencies = [
        {
          ...
          target_layer_name: "Domain Layer",
        },
      ];
