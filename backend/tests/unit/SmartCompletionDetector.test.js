...
describe("SmartCompletionDetector", () => {
  ...
  describe("detectCompletion", () => {
    test("should detect explicit completion", async () => {
      const response =
        "Task complete. The implementation is done and ready for testing.";
    ...
    test("should detect implicit completion", async () => {
      const response =
        "Here is the solution for your problem. You can now implement this code.";
      ...
      expect(result.indicators.some((i) => i.type === "implicit_pattern")).toBe(
        true,
      );
    });
    ...
    test("should detect partial completion", async () => {
      const response =
        "I'm still working on this. Need more time to complete the implementation.";
    ...
    test("should detect completion with summary", async () => {
      const response =
        "Summary of changes: Added error handling, improved performance, and updated documentation.";
      const context = { userIntent: "Refactor this code" };

      const result = await detector.detectCompletion(response, context);

      expect(result.isComplete).toBe(true);
      expect(result.indicators.some((i) => i.type === "summary_pattern")).toBe(
        true,
      );
    });
    ...
    test("should detect completion with next steps", async () => {
      const response =
        "The code is ready. Next step is to test the implementation.";
      ...
      expect(result.isComplete).toBe(true);
      expect(
        result.indicators.some((i) => i.type === "next_steps_pattern"),
      ).toBe(true);
    });
  ...
  describe("checkExplicitCompletion", () => {
    test("should detect explicit keywords", () => {
      ...
      expect(result.indicators.some((i) => i.type === "explicit_keyword")).toBe(
        true,
      );
    });
    ...
    test("should detect explicit patterns", () => {
      ...
      expect(result.isComplete).toBe(true);
      expect(result.indicators.some((i) => i.type === "explicit_pattern")).toBe(
        true,
      );
    });
  ...
  describe("checkPartialCompletion", () => {
    test("should detect incomplete indicators", () => {
      ...
      expect(result.isPartial).toBe(true);
      expect(
        result.indicators.some((i) => i.type === "incomplete_indicator"),
      ).toBe(true);
      expect(
        result.suggestions.some((s) => s.includes("still in progress")),
      ).toBe(true);
    });
    ...
    test("should detect pending indicators", () => {
      ...
      expect(result.isPartial).toBe(true);
      expect(
        result.indicators.some((i) => i.type === "pending_indicator"),
      ).toBe(true);
      expect(result.suggestions.some((s) => s.includes("pending items"))).toBe(
        true,
      );
    });
    ...
    test("should detect question indicators", () => {
      ...
      expect(result.isPartial).toBe(true);
      expect(
        result.indicators.some((i) => i.type === "question_indicator"),
      ).toBe(true);
      expect(result.suggestions.some((s) => s.includes("clarification"))).toBe(
        true,
      );
    });
  });
  ...
  describe("calculateOverallConfidence", () => {
    ...
    test("should calculate confidence with appropriate length", () => {
      ...
      const context = {
        response:
          "This is a response with appropriate length for testing purposes.",
      };
  ...
  describe("detectSemanticCompletion", () => {
    test("should detect semantic completion with keyword overlap", () => {
      ...
      expect(
        result.indicators.some((i) => i.type === "semantic_intent_match"),
      ).toBe(true);
    });
    ...
    test("should detect solution completeness", () => {
      ...
      expect(result.isSemanticallyComplete).toBe(true);
      expect(
        result.indicators.some((i) => i.type === "solution_completeness"),
      ).toBe(true);
    });
  ...
  describe("calculateKeywordOverlap", () => {
    test("should calculate overlap between keyword sets", () => {
      ...
      const result = detector.calculateKeywordOverlap(
        intentKeywords,
        responseKeywords,
      );
  ...
  describe("detectPartialCompletionWithSuggestions", () => {
    test("should provide context-specific suggestions", () => {
      ...
      const result = detector.detectPartialCompletionWithSuggestions(
        response,
        context,
      );

      expect(result.suggestions.some((s) => s.includes("error handling"))).toBe(
        true,
      );
      expect(result.suggestions.some((s) => s.includes("documentation"))).toBe(
        true,
      );
    });
    ...
    test("should provide code-specific suggestions", () => {
      ...
      expect(
        result.suggestions.some((s) => s.includes("completion status")),
      ).toBe(true);
    });
  ...
  describe("Integration Tests", () => {
    test("should provide comprehensive completion detection for code generation", async () => {
      const response =
        "Here is the complete solution for your function. The code includes error handling and is ready for testing. Next step is to run the tests.";
      const context = {
    ...
    test("should provide comprehensive completion detection for partial work", async () => {
      const response =
        "I've started implementing the function but still need to add error handling. TODO: Add try-catch blocks. Is this the right approach?";
      const context = {
