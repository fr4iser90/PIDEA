...
} = require("@domain/steps/categories/terminal/terminal_log_capture_step");
...
describe("TerminalLogCaptureStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate maxLines range", () => {
      ...
      expect(result.errors).toContain(
        "Max lines must be a number between 1 and 10000",
      );
    });
    ...
    test("should validate filter levels", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid filter level: invalid-level. Valid levels: all, error, warning, info, debug",
      );
    });
    ...
    test("should validate format types", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid format: invalid-format. Valid formats: text, json, csv, xml",
      );
    });
    ...
    test("should provide warnings for large maxLines", () => {
      ...
      expect(result.warnings).toContain(
        "Large max lines value may impact performance",
      );
    });
    ...
    test("should provide warnings for all filter level", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Capturing all log levels may result in large output",
      );
    });
    ...
    test("should provide warnings for structured formats", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Structured formats may increase processing time",
      );
    });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate includeTimestamps boolean", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Include timestamps must be a boolean",
      );
    });
    ...
    test("should validate filter level", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid filter level: invalid. Valid levels: all, error, warning, info, debug",
      );
    });
    ...
    test("should validate format", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid format: invalid. Valid formats: text, json, csv, xml",
      );
    });
  });
  ...
  describe("Log Data Handling", () => {
    ...
    test("should handle different log formats", async () => {
      const textLogs = [
        ...
        "2024-01-01 10:01:00 ERROR: Test log 2",
      ];
