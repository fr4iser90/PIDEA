...
describe("TaskStatusUpdateStep", () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      getService: jest.fn((serviceName) => {
        ...
        if (serviceName === "statusTransitionService")
  ...
  describe("Configuration", () => {
    ...
    it("should have correct instance properties", () => {
      ...
      expect(step.description).toBe(
        "Updates task status and organizes files accordingly",
      );
  ...
  describe("Task Information Retrieval", () => {
    it("should get current task info successfully", async () => {
      ...
      const result = await step.getCurrentTaskInfo(
        "task-123",
        mockTaskRepository,
      );
  ...
  describe("Status Transition Validation", () => {
    ...
    it("should reject invalid transitions", () => {
      ...
      expect(step.validateStatusTransition("cancelled", "in-progress")).toBe(
        false,
      );
  ...
  describe("File Movement", () => {
    it("should move files to completed using status transition service", async () => {
      mockStatusTransitionService.moveTaskToCompleted.mockResolvedValue({
       
      });

      const currentTask = { id: "task-123", status: "in-progress" };

      await step.moveTaskFiles(
        "task-123",
        currentTask,
        "completed",
        {},
      ...
      expect(
        mockStatusTransitionService.moveTaskToCompleted,
      ).toHaveBeenCalledWith("task-123");
    });

    it("should move files to in-progress using status transition service", async () => {
      mockStatusTransitionService.moveTaskToInProgress.mockResolvedValue({
      ...
      expect(
        mockStatusTransitionService.moveTaskToInProgress,
      ).toHaveBeenCalledWith("task-123");
    });

    it("should move files to cancelled with reason", async () => {
      mockStatusTransitionService.moveTaskToCancelled.mockResolvedValue({
      ...
      expect(
        mockStatusTransitionService.moveTaskToCancelled,
      ).toHaveBeenCalledWith("task-123", "No longer needed");
    });
  });
  ...
  describe("Complete Execution Flow", () => {
    ...
    it("should handle missing services in context", async () => {
      ...
      await expect(step.execute(invalidContext, options)).rejects.toThrow(
