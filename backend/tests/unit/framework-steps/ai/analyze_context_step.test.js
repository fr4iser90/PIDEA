...
const {
  config,
  execute,
} = require("../../../../domain/steps/categories/ai/analyze_context_step");
...
describe("Analyze Context Step", () => {
  ...
  describe("Analysis Results", () => {
    ...
    test("should analyze requirements context", async () => {
      ...
      expect(result.data.analysis.requirements).toBeDefined();
      expect(
        Array.isArray(result.data.analysis.requirements.dependencies),
      ).toBe(true);
      expect(
        Array.isArray(result.data.analysis.requirements.devDependencies),
      ).toBe(true);
    });
