...
describe('TaskDistributionManager', () => {
  ...
  describe('distributeTask', () => {
    it('should distribute task to target layers', async () => {
      ...
      const result = await taskDistributionManager.distributeTask(mockTask, mockLayers);
    ...
    it('should throw error for no valid target layers', async () => {
      ...
      await expect(taskDistributionManager.distributeTask(mockTask))
  ...
  describe('determineTargetLayers', () => {
    it('should determine layers based on task category', async () => {
      ...
      const result = await taskDistributionManager.determineTargetLayers(mockTask);
    ...
    it('should determine layers based on task type', async () => {
      const mockTask = { id: 'task-1', category: 'application', type: 'feature' };
      const mockLayers = [
        Layer.create('Domain Layer', LayerType.DOMAIN),
        Layer.create('Application Layer', LayerType.APPLICATION)
      ];
  ...
  describe('assignTaskToLayer', () => {
    it('should assign task to layer successfully', async () => {
      ...
      const result = await taskDistributionManager.assignTaskToLayer(mockTask, mockLayer);
  ...
  describe('getTaskLayerAssignments', () => {
    it('should return task layer assignments', async () => {
      ...
      const result = await taskDistributionManager.getTaskLayerAssignments('task-1');
  ...
  describe('getLayerTasks', () => {
    it('should return layer tasks', async () => {
      const mockTasks = [
        { id: 'assignment-1', layer_id: 'layer-1', task_id: 'task-1', title: 'Test Task' }
      ];
    ...
    it('should filter by status', async () => {
      const mockTasks = [
        { id: 'assignment-1', layer_id: 'layer-1', task_id: 'task-1', status: 'pending' }
      ];
      ...
      const result = await taskDistributionManager.getLayerTasks('layer-1', 'pending');
  ...
  describe('updateTaskLayerStatus', () => {
    it('should update task layer status', async () => {
      ...
      const result = await taskDistributionManager.updateTaskLayerStatus('task-1', 'layer-1', 'in_progress');

      expect(result.status).toBe('in_progress');
      expect(mockEventBus.emit).toHaveBeenCalledWith('task.layer.status.updated', expect.any(Object));
    });
    ...
    it('should throw error for non-existent assignment', async () => {
      ...
      await expect(taskDistributionManager.updateTaskLayerStatus('task-1', 'layer-1', 'in_progress'))
  ...
  describe('createDistributionRule', () => {
    it('should create distribution rule', async () => {
      ...
      const result = await taskDistributionManager.createDistributionRule(ruleData);
  ...
  describe('applyDistributionRules', () => {
    it('should apply applicable distribution rules', async () => {
      ...
      taskDistributionManager.distributeTask = jest.fn().mockResolvedValue([
        { layerId: 'layer-1', status: 'assigned' }
      ]);
