...
describe("ExecuteTerminalStep", () => {
  ...
  describe("Configuration", () => {
    test("should have correct configuration", () => {
      ...
      expect(config.description).toBe(
        "Execute terminal commands with validation and monitoring",
      );
  ...
  describe("Successful Execution", () => {
    test("should execute terminal command successfully", async () => {
      ...
      mockIDEAutomationService.executeTerminalCommand.mockResolvedValue(
        mockTerminalResult,
      );
    ...
    test("should handle command with default values", async () => {
      ...
      expect(result.success).toBe(true);
      expect(
        mockIDEAutomationService.executeTerminalCommand,
      ).toHaveBeenCalledWith("ls -la", {
        waitTime: 2000,
        workingDirectory: undefined,
        environment: undefined,
      });
  ...
  describe("Error Handling", () => {
    test("should handle terminal command failure", async () => {
      ...
      mockIDEAutomationService.executeTerminalCommand.mockRejectedValue(
  ...
  describe("Validation Method", () => {
    ...
    test("should detect command too long", () => {
      ...
      expect(validation.errors).toContain(
        "Terminal command too long (max 1000 characters)",
      );
    });
    ...
    test("should warn about dangerous commands", () => {
      ...
      expect(validation.warnings).toContain(
        "Potentially dangerous command detected: rm -rf",
      );
    });
    ...
    test("should warn about multiple dangerous commands", () => {
      ...
      expect(validation.isValid).toBe(true);
      expect(validation.warnings).toContain(
        "Potentially dangerous command detected: mkfs",
      );
    });
  });
  ...
  describe("Step Registry Integration", () => {
    ...
    test("should export ExecuteTerminalStep class", () => {
      ...
      expect(typeof executeTerminalStepModule.ExecuteTerminalStep).toBe(
        "function",
      );
    });
  });
