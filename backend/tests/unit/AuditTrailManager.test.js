...
describe('AuditTrailManager Unit Tests', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should handle initialization errors gracefully', async () => {
      ...
      await expect(auditTrailManager.initialize()).rejects.toThrow('Initialization failed');
    });
  ...
  describe('Audit Trail Queries', () => {
    ...
    test('should get audit trail for specific record', async () => {
      ...
      const result = await auditTrailManager.getAuditTrailForRecord('users', recordId);

      expect(result).toEqual(mockAuditTrail);
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM audit_trail WHERE table_name = ? AND record_id = ?'),
        ['users', recordId]
      );
    });
    ...
    test('should get audit trail for specific table', async () => {
      ...
      expect(result).toEqual(mockAuditTrail);
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM audit_trail WHERE table_name = ?'),
        [tableName]
      );
    });
    ...
    test('should get audit trail with date range', async () => {
      ...
      const result = await auditTrailManager.getAuditTrailByDateRange(startDate, endDate);

      expect(result).toEqual(mockAuditTrail);
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM audit_trail WHERE timestamp BETWEEN ? AND ?'),
        [startDate, endDate]
      );
    });
    ...
    test('should get audit trail with pagination', async () => {
      ...
      expect(result).toEqual(mockAuditTrail);
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM audit_trail ORDER BY timestamp DESC LIMIT ? OFFSET ?'),
        [10, 0]
      );
    });
  });
  ...
  describe('Compliance Features', () => {
    ...
    test('should generate compliance report', async () => {
      ...
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT operation, COUNT(*) as count FROM audit_trail')
      );
    });
    ...
    test('should check data retention compliance', async () => {
      ...
      const expiredCount = await auditTrailManager.checkDataRetentionCompliance(retentionDays);

      expect(expiredCount).toBe(2);
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT id, timestamp FROM audit_trail WHERE timestamp < ?'),
        expect.any(Array)
      );
    });
    ...
    test('should purge expired audit records', async () => {
      ...
      const purgedCount = await auditTrailManager.purgeExpiredRecords(retentionDays);
  ...
  describe('Performance', () => {
    ...
    test('should handle high-volume audit logging efficiently', async () => {
      ...
      const promises = Array(1000).fill().map(() => 
        auditTrailManager.logOperation(auditData)
      );
    ...
    test('should handle large audit trail queries efficiently', async () => {
      const mockAuditTrail = Array(10000).fill().map((_, i) => ({
        id: i,
        table_name: 'users',
        operation: 'INSERT',
  ...
  describe('Error Handling', () => {
    test('should handle database connection errors gracefully', async () => {
      ...
      await expect(auditTrailManager.initialize()).rejects.toThrow('Database connection failed');
    });
    ...
    test('should handle audit logging errors gracefully', async () => {
      ...
      await expect(auditTrailManager.logOperation(auditData)).rejects.toThrow('Audit logging failed');
    });
    ...
    test('should handle query errors gracefully', async () => {
      ...
      await expect(auditTrailManager.getAuditTrailForTable('users')).rejects.toThrow('Query failed');
    });
  });
