...
describe("RoadmapStatusMigration", () => {
  ...
  describe("discoverTaskFiles", () => {
    ...
    it("should handle file system errors", async () => {
      ...
      await expect(migration.discoverTaskFiles()).rejects.toThrow(
        "Permission denied",
      );
    });
  });
...
Task description here.
  ...
  describe("migrateTaskFile", () => {
    it("should successfully migrate a task file", async () => {
      ...
      expect(migration.parseTaskMetadata).toHaveBeenCalledWith(
        "/source/path/task.md",
      );
    ...
    it("should handle migration errors gracefully", async () => {
      migration.parseTaskMetadata = jest
        .fn()
  ...
  describe("migrateAllTasks", () => {
    it("should complete full migration process", async () => {
      migration.discoverTaskFiles = jest
        .fn()
    ...
    it("should handle migration errors and continue", async () => {
      migration.discoverTaskFiles = jest
        .fn()
        .mockResolvedValue(["file1.md", "file2.md"]);
      migration.migrateTaskFile = jest
        .fn()
