...
describe("PlaywrightTestHandler", () => {
  ...
  beforeEach(() => {
    ...
    PlaywrightTestApplicationService.mockImplementation(
      () => mockPlaywrightTestService,
    );
  ...
  describe("handleExecuteTests", () => {
    ...
    test("should handle missing project ID", async () => {
      ...
      await expect(handler.handleExecuteTests(invalidCommand)).rejects.toThrow(
        "Project ID is required",
      );
    });

    test("should handle service errors", async () => {
      mockPlaywrightTestService.executeTests.mockRejectedValue(
        new Error("Service error"),
      );

      await expect(handler.handleExecuteTests(command)).rejects.toThrow(
        "Service error",
      );
    });
  });
  ...
  describe("handleGetTestResults", () => {
    ...
    test("should handle get test results command successfully", async () => {
      ...
      expect(result.result).toBe(mockResult);
      expect(mockPlaywrightTestService.getTestResults).toHaveBeenCalledWith(
        command.testId,
      );
    });
    ...
    test("should handle missing test ID", async () => {
      ...
      await expect(
        handler.handleGetTestResults(invalidCommand),
  ...
  describe("handleStopTests", () => {
    ...
    test("should handle stop tests command successfully", async () => {
      ...
      expect(result.result).toBe(mockResult);
      expect(mockPlaywrightTestService.stopTests).toHaveBeenCalledWith(
        command.testId,
      );
    });
  ...
  describe("handleValidateLoginCredentials", () => {
    ...
    test("should handle validate login credentials command successfully", async () => {
      mockPlaywrightTestService.validateLoginCredentials.mockResolvedValue(
        mockResult,
      );
      ...
      expect(result.result).toBe(mockResult);
      expect(
        mockPlaywrightTestService.validateLoginCredentials,
      ).toHaveBeenCalledWith(command.projectId, command.credentials);
    });
    ...
    test("should handle missing project ID", async () => {
      ...
      await expect(
        handler.handleValidateLoginCredentials(invalidCommand),
  ...
  describe("handleConfigurationCommand", () => {
    ...
    test("should handle get configuration command", async () => {
      ...
      mockPlaywrightTestService.detectWorkspacePath = jest
        .fn()
        .mockResolvedValue("/workspace");
      mockPlaywrightTestService.loadProjectConfiguration = jest
        .fn()
    ...
    test("should handle unknown configuration action", async () => {
      ...
      await expect(handler.handleConfigurationCommand(command)).rejects.toThrow(
        "Unknown configuration action: unknown",
      );
    });
  ...
  describe("handleProjectCommand", () => {
    ...
    test("should handle list projects command", async () => {
      ...
      mockPlaywrightTestService.detectWorkspacePath = jest
        .fn()
        .mockResolvedValue("/workspace");
      mockPlaywrightTestService.discoverProjectTests = jest
        .fn()
        .mockResolvedValue(mockTestFiles);
    ...
    test("should handle unknown project action", async () => {
      ...
      await expect(handler.handleProjectCommand(command)).rejects.toThrow(
        "Unknown project action: unknown",
      );
    });
