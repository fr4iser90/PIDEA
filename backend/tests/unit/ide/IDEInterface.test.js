...
describe("IDEInterface", () => {
  ...
  describe("Interface Methods", () => {
    ...
    it("should have detectDevServerFromPackageJson method", () => {
      expect(typeof ideInterface.detectDevServerFromPackageJson).toBe(
        "function",
      );
    });
  ...
  describe("Method Implementations", () => {
    it("should throw error when detect is called", async () => {
      await expect(ideInterface.detect()).rejects.toThrow(
        "detect() method must be implemented",
      );
    });

    it("should throw error when start is called", async () => {
      await expect(ideInterface.start()).rejects.toThrow(
        "start() method must be implemented",
      );
    });

    it("should throw error when stop is called", async () => {
      await expect(ideInterface.stop()).rejects.toThrow(
        "stop() method must be implemented",
      );
    });

    it("should throw error when getStatus is called", async () => {
      await expect(ideInterface.getStatus()).rejects.toThrow(
        "getStatus() method must be implemented",
      );
    });

    it("should throw error when getVersion is called", async () => {
      await expect(ideInterface.getVersion()).rejects.toThrow(
        "getVersion() method must be implemented",
      );
    });

    it("should throw error when getFeatures is called", async () => {
      await expect(ideInterface.getFeatures()).rejects.toThrow(
        "getFeatures() method must be implemented",
      );
    });

    it("should throw error when executeCommand is called", async () => {
      await expect(ideInterface.executeCommand("test")).rejects.toThrow(
        "executeCommand() method must be implemented",
      );
    });

    it("should throw error when getDOM is called", async () => {
      await expect(ideInterface.getDOM()).rejects.toThrow(
        "getDOM() method must be implemented",
      );
    });

    it("should throw error when interact is called", async () => {
      await expect(ideInterface.interact("selector", "action")).rejects.toThrow(
        "interact() method must be implemented",
      );
    });

    it("should throw error when sendMessage is called", async () => {
      await expect(ideInterface.sendMessage("test")).rejects.toThrow(
        "sendMessage() method must be implemented",
      );
    });

    it("should throw error when getWorkspacePath is called", async () => {
      await expect(ideInterface.getWorkspacePath()).rejects.toThrow(
        "getWorkspacePath() method must be implemented",
      );
    });

    it("should throw error when switchToPort is called", async () => {
      await expect(ideInterface.switchToPort(3000)).rejects.toThrow(
        "switchToPort() method must be implemented",
      );
    });

    it("should throw error when getActivePort is called", () => {
      expect(() => ideInterface.getActivePort()).toThrow(
        "getActivePort() method must be implemented",
      );
    });

    it("should throw error when monitorTerminalOutput is called", async () => {
      await expect(ideInterface.monitorTerminalOutput()).rejects.toThrow(
        "monitorTerminalOutput() method must be implemented",
      );
    });

    it("should throw error when getUserAppUrlForPort is called", async () => {
      await expect(ideInterface.getUserAppUrlForPort()).rejects.toThrow(
        "getUserAppUrlForPort() method must be implemented",
      );
    });

    it("should throw error when detectDevServerFromPackageJson is called", async () => {
      await expect(
        ideInterface.detectDevServerFromPackageJson(),
      ).rejects.toThrow(
    ...
    it("should throw error when applyRefactoring is called", async () => {
      await expect(
        ideInterface.applyRefactoring("file.js", "code"),
      ).rejects.toThrow("applyRefactoring() method must be implemented");
    });

    it("should throw error when sendTask is called", async () => {
      await expect(ideInterface.sendTask({})).rejects.toThrow(
        "sendTask() method must be implemented",
      );
    });

    it("should throw error when sendAutoModeTasks is called", async () => {
      await expect(ideInterface.sendAutoModeTasks([], {})).rejects.toThrow(
        "sendAutoModeTasks() method must be implemented",
      );
    });
  });
  ...
  describe("Interface Contract", () => {
    ...
    it("should have all required methods", () => {
      const requiredMethods = [
        ...
        "detectDevServerFromPackageJson",
        "applyRefactoring",
        "sendTask",
        "sendAutoModeTasks",
      ];
