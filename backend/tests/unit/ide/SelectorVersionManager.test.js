...
describe("SelectorVersionManager", () => {
  ...
  describe("getSelectors", () => {
    ...
    it("should return version-specific selectors when version is provided", async () => {
      ...
      expect(result).toBe(mockSelectors);
      expect(mockIDETypes.getSelectorsForVersion).toHaveBeenCalledWith(
        "cursor",
        "0.42.0",
      );
    });
  ...
  describe("clearCache", () => {
    it("should clear version cache", () => {
      ...
      versionManager.versionCache.set("cursor:0.42.0", {
        selectors: {},
        timestamp: Date.now(),
      });
      versionManager.versionCache.set("vscode:1.85.0", {
        selectors: {},
        timestamp: Date.now(),
      });
