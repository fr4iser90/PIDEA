...
describe("Token Value Object", () => {
  ...
  const validJWT =
    "test.jwt.token.dummy.header.test.jwt.token.dummy.payload.test.jwt.token.dummy.signature";
  ...
  describe("Constructor", () => {
    ...
    it("should throw error for token shorter than prefix length", () => {
      expect(() => new Token("short", 10)).toThrow(
        "Token must be at least 10 characters long",
      );
    });

    it("should throw error for invalid JWT format", () => {
      expect(() => new Token("invalid.jwt")).toThrow(
        "Invalid JWT format: must have 3 parts separated by dots",
      );
    });

    it("should throw error for JWT with empty parts", () => {
      expect(() => new Token("header..signature")).toThrow(
        "Invalid JWT format: all parts must be non-empty",
      );
    });
  });
  ...
  describe("Token Operations", () => {
    ...
    it("should correctly identify expired token", () => {
      ...
      const expiredJWT =
        "test.jwt.token.dummy.header.test.jwt.token.dummy.expired.payload.test.jwt.token.dummy.expired.signature";
    ...
    it("should correctly identify non-expired token", () => {
      ...
      const futureJWT =
        "test.jwt.token.dummy.header.test.jwt.token.dummy.future.payload.test.jwt.token.dummy.future.signature";
    ...
    it("should return null for expiration time when no exp claim", () => {
      ...
      const noExpJWT =
        "test.jwt.token.dummy.header.test.jwt.token.dummy.noexp.payload.test.jwt.token.dummy.noexp.signature";
    ...
    it("should return null for user ID when not present", () => {
      ...
      const noUserIdJWT =
        "test.jwt.token.dummy.header.test.jwt.token.dummy.nouser.payload.test.jwt.token.dummy.nouser.signature";
  ...
  describe("Factory Methods", () => {
    ...
    it("should throw error when token does not match prefix", () => {
      ...
      expect(() => Token.fromPrefix(prefix, validJWT)).toThrow(
        "Token does not match the provided prefix",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle malformed base64 in payload", () => {
      ...
      const malformedJWT =
        "test.jwt.token.dummy.header.invalid-base64.test.jwt.token.dummy.signature";
    ...
    it("should handle invalid JSON in payload", () => {
      ...
      const invalidJSONJWT =
        "test.jwt.token.dummy.header.invalid-json.test.jwt.token.dummy.signature";
