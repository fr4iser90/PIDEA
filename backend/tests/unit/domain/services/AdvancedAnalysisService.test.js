...
describe("AdvancedAnalysisService", () => {
  ...
  describe("constructor", () => {
    ...
    test("should create default instances when dependencies not provided", () => {
      ...
      expect(defaultService.layerValidationService).toBeInstanceOf(
        LayerValidationService,
      );
      expect(defaultService.logicValidationService).toBeInstanceOf(
        LogicValidationService,
      );
      expect(defaultService.taskAnalysisService).toBeInstanceOf(
        TaskAnalysisService,
      );
    });
  });
  ...
  describe("performAdvancedAnalysis", () => {
    ...
    test("should perform comprehensive advanced analysis", async () => {
      ...
      mockTaskAnalysisService.analyzeProject.mockResolvedValue(
        mockStandardAnalysis,
      );
      mockLayerValidationService.validateLayers.mockResolvedValue(
        mockLayerValidation,
      );
      mockLogicValidationService.validateLogic.mockResolvedValue(
        mockLogicValidation,
      );

      const result = await service.performAdvancedAnalysis(
        projectPath,
        options,
      );
      ...
      logger.debug(
        "Debug - Layer validation overall:",
        result.layerValidation.overall,
      );
      logger.debug(
        "Debug - Logic validation overall:",
        result.logicValidation.overall,
      );
      ...
      expect(mockTaskAnalysisService.analyzeProject).toHaveBeenCalledWith(
        projectPath,
        options,
      );
      expect(mockLayerValidationService.validateLayers).toHaveBeenCalledWith(
        projectPath,
        options,
      );
      expect(mockLogicValidationService.validateLogic).toHaveBeenCalledWith(
        projectPath,
        options,
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Starting advanced analysis with layer and logic validation...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 1: Performing standard analysis...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 2: Performing layer validation...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 3: Performing logic validation...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 4: Generating integrated insights...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 5: Generating comprehensive recommendations...",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Phase 6: Calculating overall metrics...",
      );
    });
          ...
          violations: [
            {
              type: "business-logic",
              file: "src/ui/App.jsx",
              message: "Business logic in UI layer",
            },
          ],
        },
      };
      ...
      expect(insights).toContainEqual(
        expect.objectContaining({
          ...
          description: expect.stringContaining(
            "Layer violation in src/ui/App.jsx correlates with logic issue",
          ),
        }),
      );
    });
    ...
    test("should detect business logic in wrong layers", async () => {
          ...
          violations: [
            {
              type: "business-logic",
              file: "src/ui/App.jsx",
              message: "Business logic in UI",
            },
            {
              type: "business-logic",
              file: "src/infrastructure/Database.js",
              message: "Business logic in infrastructure",
            },
          ],
        },
      };
      ...
      expect(insights).toContainEqual(
        expect.objectContaining({
          ...
          description:
            "Found 2 instances of business logic in inappropriate layers",
        }),
      );
    });
      ...
      const analysis = {
        logicValidation: {
          security: {
            ...
            violations: [{ type: "sql-injection", severity: "critical" }],
          },
        },
      };
  ...
  describe("areViolationsRelated", () => {
    test("should detect related violations in same file", () => {
      ...
      const result = service.areViolationsRelated(
        layerViolation,
        logicViolation,
      );
  ...
  describe("generateComprehensiveRecommendations", () => {
    test("should generate recommendations for critical violations", async () => {
      ...
      const recommendations =
        await service.generateComprehensiveRecommendations(analysis);
      ...
      expect(recommendations).toContainEqual(
        expect.objectContaining({
          ...
          description:
            "Found 2 critical violations that must be addressed immediately",
        }),
      );
    });
    ...
    test("should generate recommendations for high priority issues", async () => {
      ...
      expect(recommendations).toContainEqual(
        expect.objectContaining({
          ...
          description:
            "Found 2 high priority issues that should be addressed soon",
        }),
      );
    });
          ...
          recommendations: [
            {
              ...
              description: "Better separation needed",
            },
          ],
        },
      };
  ...
  describe("generateAnalysisReport", () => {
    test("should generate comprehensive analysis report", () => {
      const analysis = {
        ...
        recommendations: [{ priority: "high", category: "architecture" }],
      };
