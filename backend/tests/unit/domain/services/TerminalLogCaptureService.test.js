...
describe("TerminalLogCaptureService", () => {
  ...
  beforeEach(() => {
    ...
    mockIDEManager.prototype.getActiveIDE = jest
      .fn()
      .mockResolvedValue({ port: 9222 });
  ...
  describe("initializeCapture", () => {
    ...
    test("should handle initialization errors", async () => {
      ...
      await expect(service.initializeCapture(port)).rejects.toThrow(
  ...
  describe("createSecureLogDirectory", () => {
    ...
    test("should handle directory creation errors", async () => {
      ...
      await expect(service.createSecureLogDirectory(logDir)).rejects.toThrow(
        "Permission denied",
      );
    });
  });
  ...
  describe("openTerminal", () => {
    ...
    test("should handle terminal opening errors", async () => {
      ...
      await expect(service.openTerminal()).rejects.toThrow(
  ...
  describe("setupLogCapture", () => {
    test("should execute setup commands", async () => {
      ...
      expect(
        mockIDEMirrorService.prototype.clickElementInIDE,
      ).toHaveBeenCalledWith(".xterm-helper-textarea");
      expect(mockIDEMirrorService.prototype.typeInIDE).toHaveBeenCalledWith(
  ...
  describe("executeCommandWithCapture", () => {
    ...
    test("should execute command with output capture", async () => {
      ...
      expect(service.executeTerminalCommand).toHaveBeenCalledWith(
        `${command} >> /tmp/IDEWEB/${port}/logs/terminal.log 2>&1`,
      );
    });
    ...
    test("should switch IDE if not on target port", async () => {
      ...
      service.ideManager.getActiveIDE = jest
        .fn()
        .mockResolvedValue({ port: 9223 });
