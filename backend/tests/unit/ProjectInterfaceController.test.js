...
describe("ProjectInterfaceController", () => {
  ...
  beforeEach(() => {
    ...
    controller = new ProjectInterfaceController(
      mockInterfaceManager,
      mockProjectApplicationService,
    );
  });
  ...
  describe("createInterface", () => {
    it("should create an interface successfully", async () => {
      ...
      mockProjectApplicationService.getProject.mockResolvedValue({
        id: "project1",
      });
      ...
      expect(mockInterfaceManager.createInterface).toHaveBeenCalledWith(
        "project1",
        expect.objectContaining(interfaceData),
      );
  ...
  describe("getInterface", () => {
    it("should return interface if found", async () => {
      ...
      expect(mockInterfaceManager.getInterface).toHaveBeenCalledWith(
        "project1",
        "1",
      );
  ...
  describe("startInterface", () => {
    it("should start interface successfully", async () => {
      ...
      expect(mockInterfaceManager.startInterface).toHaveBeenCalledWith(
        "project1",
        "1",
      );
    ...
    it("should return error if start fails", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({
        error: "Interface already running",
      });
    });
  });
  ...
  describe("stopInterface", () => {
    it("should stop interface successfully", async () => {
      ...
      expect(mockInterfaceManager.stopInterface).toHaveBeenCalledWith(
        "project1",
        "1",
      );
      expect(res.json).toHaveBeenCalledWith({
    ...
    it("should return error if stop fails", async () => {
      const result = { error: "Interface not running" };
      mockInterfaceManager.stopInterface.mockResolvedValue(result);
  ...
  describe("restartInterface", () => {
    it("should restart interface successfully", async () => {
      ...
      expect(mockInterfaceManager.restartInterface).toHaveBeenCalledWith(
        "project1",
        "1",
      );
      expect(res.json).toHaveBeenCalledWith({
  ...
  describe("listInterfaces", () => {
    it("should list interfaces with pagination", async () => {
      ...
      expect(mockInterfaceManager.listInterfaces).toHaveBeenCalledWith(
        "project1",
        {
          page: 1,
          limit: 10,
  ...
  describe("getInterfaceStatus", () => {
    it("should return interface status", async () => {
      ...
      expect(mockInterfaceManager.getInterfaceStatus).toHaveBeenCalledWith(
        "project1",
        "1",
      );
  ...
  describe("getInterfaceLogs", () => {
    it("should return interface logs", async () => {
      ...
      expect(mockInterfaceManager.getInterfaceLogs).toHaveBeenCalledWith(
        "project1",
        "1",
        {
          lines: 50,
  ...
  describe("validateInterfaceData", () => {
    ...
    it("should reject invalid interface types", () => {
      ...
      expect(result.errors).toContain(
        "Interface type must be one of: cursor, vscode, windsurf, jetbrains, sublime",
      );
    });
