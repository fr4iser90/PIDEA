...
describe("VersionManagementService", () => {
  ...
  describe("getCurrentVersion", () => {
    it("should get current version from package.json", async () => {
      ...
      expect(
        mockDependencies.fileSystemService.readJsonFile,
      ).toHaveBeenCalledWith("/test/project/package.json");
    });

    it("should return 0.0.0 when no version found", async () => {
      mockDependencies.fileSystemService.readJsonFile.mockRejectedValue(
  ...
  describe("bumpVersion", () => {
    ...
    it("should bump version successfully", async () => {
      const result = await service.bumpVersion(
        mockTask,
        "/test/project",
        "minor",
      );
      ...
      expect(
        mockDependencies.fileSystemService.writeJsonFile,
      ).toHaveBeenCalled();
  ...
  describe("determineBumpType", () => {
    ...
    it("should determine major bump for breaking changes", async () => {
      const taskWithBreaking = {
        ...mockTask,
        description:
          "This is a breaking change that will affect existing functionality",
      };

      const bumpType = await service.determineBumpType(
        taskWithBreaking,
        "/test/project",
      );
  ...
  describe("detectBreakingChanges", () => {
    ...
    it("should not detect breaking changes in normal tasks", () => {
      const task = {
        ...
        description:
          "This adds a new feature without breaking existing functionality",
      };
