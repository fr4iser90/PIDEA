...
describe("TaskService - Phase Grouping", () => {
  ...
  describe("getTasksByPhases", () => {
    ...
    it("should handle repository errors", async () => {
      taskRepository.findByProject.mockRejectedValue(
        new Error("Database error"),
      );

      await expect(taskService.getTasksByPhases("project1")).rejects.toThrow(
        "Failed to get tasks by phases: Database error",
      );
    });
  });
  ...
  describe("getTasksByPhase", () => {
    ...
    it("should return empty array for non-existent phase", async () => {
      ...
      const result = await taskService.getTasksByPhase(
        "project1",
        "nonexistent",
      );
  ...
  describe("executePhase", () => {
    it("should execute all tasks in a phase using Auto-Finish System", async () => {
      ...
      const mockAutoFinishResult = { status: "completed" };
    ...
    it("should throw error for non-existent phase", async () => {
      ...
      await expect(
        taskService.executePhase("project1", "nonexistent"),
      ).rejects.toThrow("No tasks found for phase: nonexistent");
    });
    ...
    it("should handle individual task execution errors", async () => {
      ...
      autoFinishSystem.processTask.mockRejectedValue(
