...
describe('SchemaVersionManager Unit Tests', () => {
  ...
  describe('Constructor', () => {
    test('should create instance with database connection', () => {
      expect(schemaVersionManager.databaseConnection).toBe(mockDatabaseConnection);
  ...
  describe('Version Initialization', () => {
    ...
    test('should handle initialization errors gracefully', async () => {
      ...
      await expect(schemaVersionManager.initialize()).rejects.toThrow('Initialization failed');
    });

    test('should get current version after initialization', async () => {
      const mockVersion = { version: '1.0.0', applied_at: '2023-01-01T00:00:00Z' };
      mockDatabaseConnection.execute.mockResolvedValue({ changes: 1 });
  ...
  describe('Version Management', () => {
    ...
    test('should get version by number', async () => {
      ...
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM schema_versions WHERE version = ?'),
        [version]
      );
    });
    ...
    test('should list all versions', async () => {
      ...
      expect(mockDatabaseConnection.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM schema_versions ORDER BY applied_at DESC')
      );
    });
  ...
  describe('Version Validation', () => {
    ...
    test('should validate version format', () => {
      ...
      expect(schemaVersionManager.validateVersionFormat('1.0.0-beta')).toBe(true);
      expect(schemaVersionManager.validateVersionFormat('1.0.0-alpha.1')).toBe(true);
    });

    test('should reject invalid version formats', () => {
      expect(schemaVersionManager.validateVersionFormat('1.0')).toBe(false);
      expect(schemaVersionManager.validateVersionFormat('1.0.0.0.0')).toBe(false);
  ...
  describe('Migration Management', () => {
    ...
    test('should apply migration', async () => {
      ...
      await schemaVersionManager.applyMigration(version, description, migrationSQL);
    ...
    test('should rollback migration', async () => {
      ...
      expect(mockDatabaseConnection.execute).toHaveBeenCalledWith(
        expect.stringContaining('DELETE FROM schema_versions WHERE version = ?'),
        [version]
      );
    });
    ...
    test('should handle migration errors gracefully', async () => {
      ...
      await expect(schemaVersionManager.applyMigration(version, description, migrationSQL))
    ...
    test('should handle rollback errors gracefully', async () => {
      ...
      await expect(schemaVersionManager.rollbackMigration(version, rollbackSQL))
  ...
  describe('Version Comparison', () => {
    ...
    test('should handle pre-release versions', () => {
      expect(schemaVersionManager.compareVersions('1.0.0', '1.0.0-alpha')).toBe(1);
      expect(schemaVersionManager.compareVersions('1.0.0-alpha', '1.0.0-beta')).toBe(-1);
      expect(schemaVersionManager.compareVersions('1.0.0-beta', '1.0.0')).toBe(-1);
    });
  ...
  describe('Schema Validation', () => {
    ...
    test('should validate required tables exist', async () => {
      ...
      const isValid = await schemaVersionManager.validateRequiredTables(requiredTables);
  ...
  describe('Error Handling', () => {
    test('should handle database connection errors gracefully', async () => {
      ...
      await expect(schemaVersionManager.initialize()).rejects.toThrow('Database connection failed');
    });
    ...
    test('should handle query errors gracefully', async () => {
      ...
      await expect(schemaVersionManager.getCurrentVersion()).rejects.toThrow('Query failed');
    });
    ...
    test('should handle invalid version operations gracefully', async () => {
      ...
      await expect(schemaVersionManager.setVersion('', 'Invalid version')).rejects.toThrow();
    });
  });
  ...
  describe('Performance', () => {
    ...
    test('should handle large version histories efficiently', async () => {
      const mockVersions = Array(1000).fill().map((_, i) => ({
        version: `1.${i}.0`,
  ...
  describe('Configuration', () => {
    test('should handle custom version table name', () => {
      ...
      const manager = new SchemaVersionManager(mockDatabaseConnection, customTableName);
