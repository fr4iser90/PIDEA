...
describe("TaskContentHashService", () => {
  ...
  beforeEach(() => {
    ...
    service = new TaskContentHashService(
      mockFileSystemService,
      mockCacheService,
    );
  });
  ...
  describe("generateContentHash", () => {
    ...
    it("should throw error for invalid content", async () => {
      await expect(service.generateContentHash(null)).rejects.toThrow(
        "Content must be a non-empty string",
      );
      await expect(service.generateContentHash("")).rejects.toThrow(
        "Content must be a non-empty string",
      );
      await expect(service.generateContentHash(123)).rejects.toThrow(
        "Content must be a non-empty string",
      );
    });
  });
  ...
  describe("generateFileHash", () => {
    ...
    it("should throw error for invalid file path", async () => {
      await expect(service.generateFileHash(null)).rejects.toThrow(
        "File path must be a non-empty string",
      );
      await expect(service.generateFileHash("")).rejects.toThrow(
        "File path must be a non-empty string",
      );
    });
  });
  ...
  describe("extractStatusFromContent", () => {
    ...
    it("should infer status from progress indicators", async () => {
      ...
      expect(await service.extractStatusFromContent(completedContent)).toBe(
        "completed",
      );
      expect(await service.extractStatusFromContent(inProgressContent)).toBe(
        "in_progress",
      );
      expect(await service.extractStatusFromContent(pendingContent)).toBe(
        "pending",
      );
    });
  ...
  describe("error handling", () => {
    it("should handle file read errors gracefully", async () => {
      ...
      await expect(service.generateFileHash(filePath)).rejects.toThrow(
        "File not found",
      );
    });
