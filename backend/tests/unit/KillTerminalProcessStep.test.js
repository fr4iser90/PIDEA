...
} = require("@domain/steps/categories/terminal/kill_terminal_process_step");
...
describe("KillTerminalProcessStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate signal types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid signal: INVALID_SIGNAL. Valid signals: SIGTERM, SIGKILL, SIGINT, SIGQUIT, SIGHUP, SIGUSR1, SIGUSR2, SIGSTOP, SIGCONT, SIGTSTP",
      );
    });
    ...
    test("should provide warnings for dangerous signals", () => {
      ...
      dangerousSignals.forEach((signal) => {
        ...
        expect(result.warnings).toContain(
          `Signal ${signal} may cause data loss or system instability`,
        );
      });
    });
    ...
    test("should provide warnings for force kill", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "Force kill may cause data loss or system instability",
      );
    });
    ...
    test("should provide warnings for system processes", () => {
      ...
      systemPids.forEach((pid) => {
        ...
        expect(result.isValid).toBe(true);
        expect(result.warnings).toContain(
          `Process ID ${pid} may be a system process`,
        );
      });
    });
  });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate signal", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid signal: INVALID. Valid signals: SIGTERM, SIGKILL, SIGINT, SIGQUIT, SIGHUP, SIGUSR1, SIGUSR2, SIGSTOP, SIGCONT, SIGTSTP",
      );
    });
  ...
  describe("Error Handling", () => {
    ...
    test("should publish failure event on error", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "terminal.process.killing.failed",
        {
          stepId: expect.any(String),
          userId: "test-user-123",
          processId: 1234,
