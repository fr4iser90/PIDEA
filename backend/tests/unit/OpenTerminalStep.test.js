...
const {
  OpenTerminalStep,
} = require("@domain/steps/categories/terminal/open_terminal_step");
...
describe("OpenTerminalStep", () => {
  ...
  describe("Validation", () => {
    ...
    test("should validate valid IDE types", () => {
      ...
      expect(result.errors).toContain(
        "Invalid IDE type: invalid-ide. Valid types: cursor, vscode, windsurf",
      );
    });
    ...
    test("should validate shell types", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid shell: invalid-shell. Valid shells: bash, zsh, sh, fish, powershell, cmd",
      );
    });
    ...
    test("should provide warnings for PowerShell with Cursor", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain(
        "PowerShell with Cursor IDE may have compatibility issues",
      );
    });
  });
  ...
  describe("Context Validation", () => {
    ...
    test("should validate IDE type", () => {
      ...
      expect(() => step.validateContext(invalidContext)).toThrow(
        "Invalid IDE type: invalid. Valid types: cursor, vscode, windsurf",
      );
    });
