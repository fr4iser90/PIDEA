...
describe("SessionActivityService", () => {
  ...
  beforeEach(() => {
    ...
    sessionActivityService =
      new (require("../../domain/services/security/SessionActivityService"))(
        dependencies,
      );
  });
  ...
  describe("Initialization", () => {
    ...
    it("should start cleanup tasks on initialization", () => {
      const mockStartCleanupTasks = jest.spyOn(
        sessionActivityService,
        "startCleanupTasks",
      );
  ...
  describe("Activity Recording", () => {
    it("should record activity successfully", async () => {
      ...
      const result = await sessionActivityService.recordActivity(
        sessionId,
        activityData,
      );
    ...
    it("should throw error for missing session ID", async () => {
      await expect(
        sessionActivityService.recordActivity(null, {}),
    ...
    it("should update activity cache", async () => {
      ...
      await sessionActivityService.recordActivity(sessionId, {
        type: "scroll",
      });

      const cachedActivity =
        sessionActivityService.activityCache.get(sessionId);
    ...
    it("should handle activity recording errors", async () => {
      ...
      await expect(
        sessionActivityService.recordActivity("session-1", {}),
  ...
  describe("Session Extension", () => {
    ...
    it("should extend session when activity threshold is met", async () => {
      ...
      const result =
        await sessionActivityService.checkSessionExtension(sessionId);
    ...
    it("should handle session extension errors", async () => {
      mockUserSessionRepository.findById.mockRejectedValue(
        new Error("Database error"),
      );

      await expect(
        sessionActivityService.checkSessionExtension("session-1"),
  ...
  describe("Session Analytics", () => {
    it("should get session activity stats", async () => {
      ...
      const stats =
        await sessionActivityService.getSessionActivityStats(sessionId);
    ...
    it("should get user activity stats", async () => {
      ...
      const stats = await sessionActivityService.getUserActivityStats(
        userId,
        timeRange,
      );
    ...
    it("should handle missing session stats", async () => {
      const stats = await sessionActivityService.getSessionActivityStats(
        "nonexistent-session",
      );
  ...
  describe("Session Cleanup", () => {
    it("should cleanup expired sessions", async () => {
      ...
      expect(mockUserSessionRepository.delete).toHaveBeenCalledWith(
        "expired-session",
      );
    });

    it("should handle cleanup errors gracefully", async () => {
      mockUserSessionRepository.findByUserId.mockRejectedValue(
        new Error("Database error"),
      );
  ...
  describe("Configuration Management", () => {
    it("should update configuration", () => {
      ...
      expect(sessionActivityService.activityThreshold).toBe(
        newConfig.activityThreshold,
      );
      expect(sessionActivityService.sessionTimeout).toBe(
        newConfig.sessionTimeout,
      );
      expect(sessionActivityService.maxExtensions).toBe(
        newConfig.maxExtensions,
      );
    });
  ...
  describe("Memory Management", () => {
    it("should cleanup activity cache periodically", () => {
      const mockCleanupCache = jest.spyOn(
        sessionActivityService,
  ...
  describe("Integration", () => {
    it("should work with AuthService", async () => {
      ...
      const result = await sessionActivityService.extendSession(
        "session-1",
        "activity",
      );
    ...
    it("should emit events for external listeners", async () => {
      ...
      await sessionActivityService.recordActivity("session-1", {
        type: "test",
      });
