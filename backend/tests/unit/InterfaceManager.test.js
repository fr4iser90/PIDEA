...
describe("InterfaceManager", () => {
  ...
  describe("Interface Creation", () => {
    ...
    test("should create interface instance successfully", async () => {
      ...
      expect(interfaceManager.activeInterfaces.has(interfaceInstance.id)).toBe(
        true,
      );
  ...
  describe("Interface Lifecycle Management", () => {
    ...
    beforeEach(async () => {
      interfaceManager.registerInterface("test", TestInterface);
      interfaceInstance = await interfaceManager.createInterface(
        "test",
        {},
        "test-1",
      );
    ...
    test("should handle lifecycle errors", async () => {
      ...
      interfaceManager.registerInterface("error", ErrorInterface);
      const errorInstance = await interfaceManager.createInterface(
        "error",
        {},
        "error-1",
      );

      await expect(interfaceManager.startInterface("error-1")).rejects.toThrow(
        "Start error",
      );
  ...
  describe("Interface Removal", () => {
    ...
    test("should handle removal errors", async () => {
      ...
      await expect(interfaceManager.removeInterface("error-1")).rejects.toThrow(
        "Destroy error",
      );
  ...
  describe("Registry Information", () => {
    test("should provide registry info", () => {
      interfaceManager.registerInterface("test1", TestInterface, {
        config1: "value1",
      });
      interfaceManager.registerInterface("test2", TestInterface, {
        config2: "value2",
      });
  ...
  describe("Event Bus Integration", () => {
    test("should handle missing event bus gracefully", () => {
      const managerWithoutEventBus = new InterfaceManager({
        logger: mockLogger,
      });
