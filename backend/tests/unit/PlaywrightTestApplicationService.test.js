...
describe("PlaywrightTestApplicationService", () => {
  ...
  describe("executeTests", () => {
    ...
    const mockResults = [
      { testFile: "login.test.js", duration: 1000 },
      { testFile: "dashboard.test.js", error: "Test failed" },
    ];

    beforeEach(() => {
      mockWorkspaceDetector.detectWorkspacePath.mockResolvedValue(
        workspacePath,
      );
    ...
    test("should handle configuration loading failure", async () => {
      mockTestManager.loadTestConfig.mockRejectedValue(
    ...
    test("should use provided workspace path in options", async () => {
      ...
      expect(mockTestManager.loadTestConfig).toHaveBeenCalledWith(
        providedPath,
        options,
      );
    });
  ...
  describe("getTestResults", () => {
    ...
    test("should handle test results not found", async () => {
      ...
      await expect(service.getTestResults(testId)).rejects.toThrow(
  ...
  describe("detectWorkspacePath", () => {
    ...
    test("should use workspace detector when available", async () => {
      ...
      expect(mockWorkspaceDetector.detectWorkspacePath).toHaveBeenCalledWith(
        "project-id",
      );
    });
    ...
    test("should use project mapper when workspace detector fails", async () => {
      ...
      expect(mockProjectMapper.getProjectPath).toHaveBeenCalledWith(
        "project-id",
      );
    });
  ...
  describe("loadProjectConfiguration", () => {
    ...
    test("should load configuration successfully", async () => {
      ...
      expect(mockTestManager.validateTestConfig).toHaveBeenCalledWith(
        mockConfig,
      );
    });
    ...
    test("should handle configuration validation failure", async () => {
      ...
      await expect(
        service.loadProjectConfiguration(workspacePath),
    ...
    test("should merge with provided options", async () => {
      ...
      const result = await service.loadProjectConfiguration(
        workspacePath,
        options,
      );
  ...
  describe("validateLoginCredentials", () => {
    ...
    test("should handle missing credentials", async () => {
      await expect(
        service.validateLoginCredentials(projectId, {}),
