...
describe("IDEConfigManager", () => {
  ...
  describe("getIDEConfig", () => {
    ...
    it("should throw error for unknown IDE", () => {
      expect(() => configManager.getIDEConfig("unknown")).toThrow(
        "Unknown IDE type: unknown",
      );
    });
  });
  ...
  describe("updateIDEConfig", () => {
    ...
    it("should update specific IDE config", async () => {
      ...
      expect(configManager.config.ideTypes.cursor.portRange).toEqual(
        newCursorConfig.portRange,
      );
    ...
    it("should throw error for unknown IDE", async () => {
      await expect(
        configManager.updateIDEConfig("unknown", {}),
      ).rejects.toThrow("Unknown IDE type: unknown");
    });
  });
  ...
  describe("validateConfig", () => {
    ...
    it("should detect invalid port ranges", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((error) => error.includes("port range"))).toBe(
        true,
      );
    });
  });
