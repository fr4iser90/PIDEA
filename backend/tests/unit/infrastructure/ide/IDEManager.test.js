...
describe("IDEManager", () => {
  ...
  describe("switchToIDE", () => {
    ...
    it("should throw error for non-existent IDE", async () => {
      ...
      await expect(manager.switchToIDE(9999)).rejects.toThrow(
        "No IDE found on port 9999",
      );
    });

    it("should throw error for non-running IDE", async () => {
      const mockIDEs = [{ port: 9222, status: "stopped", ideType: "cursor" }];

      mockDetectorFactory.detectAll.mockResolvedValue(mockIDEs);
      mockStarterFactory.getRunningIDEs.mockReturnValue([]);

      await expect(manager.switchToIDE(9222)).rejects.toThrow(
        "IDE on port 9222 is not running",
      );
    });
  });
  ...
  describe("stopIDE", () => {
    ...
    it("should switch to another IDE if stopping active IDE", async () => {
      ...
      const mockIDEs = [{ port: port2, status: "running", ideType: "vscode" }];
