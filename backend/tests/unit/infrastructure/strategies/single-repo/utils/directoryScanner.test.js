...
describe("DirectoryScanner", () => {
  ...
  describe("constructor", () => {
    ...
    it("should handle undefined logger gracefully", () => {
      const scannerWithoutLogger = new DirectoryScanner(
        undefined,
        mockFileUtils,
      );
    ...
    it("should handle undefined fileUtils gracefully", () => {
      const scannerWithoutFileUtils = new DirectoryScanner(
        mockLogger,
        undefined,
      );
  ...
  describe("scanDirectory", () => {
    ...
    it("should handle undefined logger gracefully", async () => {
      ...
      await expect(
        scannerWithoutLogger.scanDirectory("/test", mockStructure, 0, 3),
    ...
    it("should handle depth tracking correctly", async () => {
      const mockEntries = [{ name: "src", isDirectory: true, isFile: false }];
    ...
    it("should handle relative path calculation correctly", async () => {
      ...
      await directoryScanner.scanDirectory(
        "/test/project",
        mockStructure,
        0,
        3,
  ...
  describe("scanForTestFiles", () => {
    ...
    it("should handle nested directory structure", async () => {
      ...
      expect(testFiles).toContain(
        path.join("/test", "src", "components", "Button.test.js"),
      );
    });
  });
  ...
  describe("edge cases and error handling", () => {
    it("should handle null and undefined parameters gracefully", async () => {
      ...
      await expect(
        directoryScanner.scanDirectory(null, mockStructure, 0, 3),
      ).resolves.not.toThrow();

      await expect(
        directoryScanner.scanDirectory(undefined, mockStructure, 0, 3),
      ).resolves.not.toThrow();

      await expect(
        directoryScanner.scanForTestFiles(null, []),
      ).resolves.not.toThrow();

      await expect(
        directoryScanner.scanForTestFiles(undefined, []),
      ).resolves.not.toThrow();
    });
