...
describe("OptimizationService", () => {
  ...
  describe("optimizeSingleRepo", () => {
    ...
    it("should optimize single repository successfully", async () => {
      const result = await optimizationService.optimizeSingleRepo(
        projectPath,
        defaultOptions,
      );
      ...
      expect(result).toEqual({
        ...
        build: { message: "Build optimization completed" },
        testing: { message: "Testing optimization completed" },
        linting: { message: "Linting optimization completed" },
        security: { message: "Security optimization completed" },
        performance: {
    ...
    it("should handle optimization errors gracefully", async () => {
      ...
      jest
        .spyOn(optimizationService, "optimizeStructure")
        .mockRejectedValue(new Error("Structure optimization failed"));

      await expect(
        optimizationService.optimizeSingleRepo(projectPath, defaultOptions),
      ).rejects.toThrow(
        "Failed to optimize single repository: Structure optimization failed",
      );
  ...
  describe("optimizeDependencies", () => {
    ...
    it("should handle undefined options", async () => {
      const result =
        await optimizationService.optimizeDependencies(projectPath);
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("error handling scenarios", () => {
    it("should handle logger errors gracefully", async () => {
      ...
      try {
        const result = await serviceWithBrokenLogger.optimizeSingleRepo(
          "/test/path",
          {},
        );
        expect(result).toBeDefined();
    ...
    it("should handle multiple optimization failures", async () => {
      ...
      jest
        .spyOn(optimizationService, "optimizeStructure")
        .mockRejectedValue(new Error("Structure failed"));
      jest
        .spyOn(optimizationService, "optimizeDependencies")
        .mockRejectedValue(new Error("Dependencies failed"));

      await expect(
        optimizationService.optimizeSingleRepo("/test/path", {}),
      ).rejects.toThrow(
        "Failed to optimize single repository: Structure failed",
      );
    });
    ...
    it("should handle synchronous errors in optimization methods", async () => {
      ...
      await expect(
        ...
        "Failed to optimize single repository: Synchronous error",
      );
    });
  });
