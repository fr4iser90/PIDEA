...
jest.mock("@/infrastructure/templates/modules", () => {
  ...
  return {
    ...
    TemplateSearch: {
      ...
      getTemplateCounts: jest
        .fn()
        .mockReturnValue({ build: 1, test: 1, deploy: 1 }),
    },
...
describe("ScriptTemplates", () => {
  ...
  describe("generateScript", () => {
    it("should generate script successfully", () => {
      const result = scriptTemplates.generateScript("build", "testTemplate", {
        NAME: "World",
      });
    ...
    it("should use empty variables object when not provided", () => {
      ...
      expect(scriptTemplates.generateScript("build", "testTemplate")).toBe(
        'echo "Hello World"',
      );
    });
  });
  ...
  describe("searchTemplates", () => {
    it("should search templates with keyword", () => {
      const mockResults = [{ category: "build", name: "test", template: {} }];
      const { TemplateSearch } = require("@templates/modules");
  ...
  describe("createCustomTemplate", () => {
    ...
    it("should create custom template without eventBus when not provided", () => {
      const scriptTemplatesWithoutEventBus = new ScriptTemplates({
        logger: mockLogger,
      });
      const mockTemplate = { name: "Custom Template" };
      const result =
        scriptTemplatesWithoutEventBus.createCustomTemplate(mockTemplate);
      expect(result).toBeDefined();
      expect(result.id).toBe("custom_123");
    });
  });
