...
describe("TokenHasher", () => {
  ...
  describe("validateToken", () => {
    ...
    it("should reject invalid token", () => {
      const hashResult = tokenHasher.hashToken(testToken);
      const isValid = tokenHasher.validateToken(
        "different-token",
        hashResult.hash,
      );
  ...
  describe("extractPrefix", () => {
    ...
    it("should throw error for empty token", () => {
      expect(() => tokenHasher.extractPrefix("")).toThrow(
        "Token is required for prefix extraction",
      );
    });

    it("should throw error for null token", () => {
      expect(() => tokenHasher.extractPrefix(null)).toThrow(
        "Token is required for prefix extraction",
      );
    });
  });
  ...
  describe("getTokenInfo", () => {
    ...
    it("should throw error for empty token", () => {
      expect(() => tokenHasher.getTokenInfo("")).toThrow(
        "Token is required for info extraction",
      );
    });

    it("should throw error for null token", () => {
      expect(() => tokenHasher.getTokenInfo(null)).toThrow(
        "Token is required for info extraction",
      );
    });
  });
  ...
  describe("hashTokens (Batch Operations)", () => {
    ...
    it("should throw error for non-array input", () => {
      expect(() => tokenHasher.hashTokens("not-an-array")).toThrow(
