...
describe("Task Management System Security Tests", () => {
  ...
  describe("Data Protection", () => {
    test("should encrypt sensitive task data", async () => {
      ...
      const savedTask = await application.taskRepository.findById(
        result.task.id,
      );
  ...
  describe("Rate Limiting", () => {
    test("should prevent task creation spam", async () => {
      ...
      logger.info(
        `Rate limiting: ${successful} successful, ${failed} rejected`,
      );
    });
    ...
    test("should prevent AI service abuse", async () => {
      ...
      logger.info(
        `AI rate limiting: ${successful} successful, ${failed} rejected`,
      );
    });
  });
  ...
  describe("Session Security", () => {
    test("should prevent session hijacking", async () => {
      ...
      const expiredSession = await application.createSession("test-user", {
        expiresIn: "1ms",
      });
    ...
    test("should prevent session fixation", async () => {
      ...
      await expect(
        application.createSession("test-user", {
    ...
    test("should enforce session timeout", async () => {
      ...
      await expect(application.validateSession(session.id)).rejects.toThrow();
    });
  });
  ...
  describe("File System Security", () => {
    ...
    test("should validate file extensions", async () => {
      const dangerousExtensions = [".exe", ".bat", ".sh", ".ps1", ".vbs"];
  ...
  describe("Network Security", () => {
    test("should prevent unauthorized network access", async () => {
      ...
      for (const url of maliciousUrls) {
        await expect(application.networkService.fetch(url)).rejects.toThrow();
      }
    });
  ...
  describe("Data Validation", () => {
    ...
    test("should prevent prototype pollution", async () => {
      ...
      for (const obj of maliciousObjects) {
        await expect(application.processObject(obj)).rejects.toThrow();
      }
    });
  });
});
