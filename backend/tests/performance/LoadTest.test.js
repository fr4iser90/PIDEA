...
describe("Task Management System Performance Tests", () => {
  ...
  describe("Concurrent Task Creation", () => {
    test("should handle 100 concurrent task creations", async () => {
      ...
      const results = await Promise.all(
        tasks.map((task) =>
          application.commandBus.execute("CreateTaskCommand", task),
        ),
      );
      ...
      logger.info(
        `Created ${numTasks} tasks in ${duration}ms (${duration / numTasks}ms per task)`,
      );
    });
  ...
  describe("Concurrent Task Execution", () => {
    test("should handle 50 concurrent task executions", async () => {
      ...
      logger.info(
        `Executed ${numTasks} tasks in ${duration}ms (${duration / numTasks}ms per task)`,
      );
    });
  ...
  describe("Concurrent Auto Mode Operations", () => {
    test("should handle 10 concurrent auto mode operations", async () => {
      ...
      const results = await Promise.all(
        operations.map((op) =>
          application.commandBus.execute("AutoModeCommand", op),
        ),
      );
      ...
      logger.info(
        `Completed ${numOperations} auto mode operations in ${duration}ms`,
      );
    });
    ...
    test("should handle concurrent auto mode with different modes", async () => {
      ...
      logger.info(
        `Completed ${numOperations} mixed mode auto operations in ${duration}ms`,
      );
    });
  });
  ...
  describe("Database Performance", () => {
    ...
    test("should handle filtered queries efficiently", async () => {
      ...
      const result = await application.queryBus.execute(
        "GetTasksQuery",
        highPriorityQuery,
      );
      ...
      logger.info(
        `Filtered ${result.tasks.length} high priority tasks in ${duration}ms`,
      );
    });
  });
  ...
  describe("AI Service Performance", () => {
    test("should handle concurrent AI requests efficiently", async () => {
      ...
      const results = await Promise.all(
        requests.map((req) =>
          application.commandBus.execute("AnalyzeProjectCommand", req),
        ),
      );
      ...
      logger.info(
        `Completed ${numRequests} AI analysis requests in ${duration}ms`,
      );
    });
    ...
    test("should handle AI model switching efficiently", async () => {
      ...
      logger.info(
        `Completed ${numRequests} multi-model AI requests in ${duration}ms`,
      );
    });
  });
  ...
  describe("Memory Usage", () => {
    ...
    test("should handle memory cleanup after large operations", async () => {
      ...
      logger.info(
        `Memory after cleanup: ${Math.round(memoryIncrease / 1024 / 1024)}MB increase`,
      );
    });
  });
  ...
  describe("Response Time Performance", () => {
    ...
    test("should respond to task execution within 500ms", async () => {
      // Create task first
      const createResult = await application.commandBus.execute(
        "CreateTaskCommand",
        {
