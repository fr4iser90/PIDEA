...
test.describe("Rapid IDE Switching Performance", () => {
  ...
  test("should handle rapid IDE switching without performance degradation", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await page.waitForSelector('[data-testid="ide-status-active"]', {
        timeout: 2000,
      });
    ...
    const averageSwitchTime =
      ideSwitches.reduce((a, b) => a + b, 0) / ideSwitches.length;
  ...
  test("should cache IDE information and improve subsequent requests", async ({
    page,
  }) => {
  ...
  test("should handle network errors gracefully during IDE switching", async ({
    page,
  }) => {
  ...
  test("should maintain UI responsiveness during rapid switching", async ({
    page,
  }) => {
    ...
    expect(slowInteractions.length).toBeLessThan(
      responsivenessChecks.length * 0.2,
    ); // Less than 20% slow interactions
  });
  ...
  test("should show loading states during IDE switching", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const loadingVisible = await page.isVisible(
  ...
  test("should prevent multiple simultaneous IDE switches", async ({
    page,
  }) => {
    ...
    const activeIDEs = await page
      .locator('[data-testid="ide-status-active"]')
      .count();
    expect(activeIDEs).toBe(1);
  });

  test("should maintain state consistency during rapid switching", async ({
    page,
  }) => {
    // Get initial state
    const initialState = await page.evaluate(() => {
      return {
        activeIDE: document.querySelector('[data-testid="active-ide"]')
          ?.textContent,
        availableIDEs: Array.from(
          document.querySelectorAll('[data-testid="ide-option"]'),
  ...
  test("should provide visual feedback for successful switches", async ({
    page,
  }) => {
    // Perform a switch
    await page.click('[data-testid="ide-option-0"]');
    ...
    const successText = await page.textContent(
