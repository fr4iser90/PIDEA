...
describe("Selector Versioning System E2E Tests", () => {
  ...
  describe("Complete versioning workflow", () => {
    it("should handle complete selector versioning workflow for Cursor", async () => {
      ...
      const versionSelectors = IDETypes.getSelectorsForVersion(
        "cursor",
        "0.42.0",
      );
      expect(versionSelectors).toBeDefined();
      expect(versionSelectors.chatSelectors).toBeDefined();
      expect(versionSelectors.chatSelectors.input).toContain(
        "aislash-editor-input",
      );

      // Step 5: Use SelectorVersionManager to get selectors
      const managerSelectors = await versionManager.getSelectors(
        "cursor",
        "0.42.0",
      );
      ...
      const selectorManagerSelectors = await selectorManager.getSelectors(
        "cursor",
        "0.42.0",
      );
      ...
      expect(versionSelectors.chatSelectors.input).toBe(
        managerSelectors.chatSelectors.input,
      );
      expect(managerSelectors.chatSelectors.input).toBe(
        selectorManagerSelectors.chatSelectors.input,
      );
    });
  ...
  describe("Fallback mechanisms", () => {
    ...
    it("should fallback to hardcoded selectors when versioned selectors not available", async () => {
      ...
      const selectors = await versionManager.getSelectors(
        "jetbrains",
        "2023.1",
      );
  ...
  describe("Real-world usage scenarios", () => {
    ...
    it("should handle IDE version updates gracefully", async () => {
      ...
      const oldSelectors = await selectorManager.getSelectors(
        ideType,
        oldVersion,
      );
      ...
      const newSelectors = await selectorManager.getSelectors(
        ideType,
        newVersion,
      );
