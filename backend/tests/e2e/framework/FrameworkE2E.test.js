...
describe("Framework E2E Tests", () => {
  ...
  describe("Complete Framework Workflow", () => {
    beforeEach(async () => {
      ...
      sandbox
        .stub(fs, "readdir")
        .resolves(["refactoring_management", "testing_management"]);
        ...
        JSON.stringify({
          ...
          workflows: {
            complete_refactoring: {
              ...
              description:
                "Complete refactoring workflow with analysis and task generation",
            },
          },
          activation: {
            auto_load: false,
      ...
      // Mock core services
      sandbox
        .stub(require("../../../domain/services/TaskService"), "default")
        .returns({
          createTask: sandbox
            .stub()
      ...
      sandbox
        .stub(require("../../../domain/services/IDEService"), "default")
        .returns({
      ...
      sandbox
        .stub(require("../../../domain/services/CursorService"), "default")
        .returns({
    ...
    it("should complete full framework workflow from initialization to step execution", async () => {
      ...
      expect(frameworks.map((f) => f.name)).to.include(
      ...
      expect(
        frameworkSteps.some((step) => step.includes("refactoring_management")),
      ).to.be.true;

      // 4. Activate refactoring framework
      const activationResult = await app.frameworkManager.activateFramework(
        "refactoring_management",
      );
      expect(activationResult.success).to.be.true;
      expect(activationResult.framework.name).to.equal(
        "refactoring_management",
      );
      ...
      expect(executionCall.args[0]).to.have.property(
        "projectPath",
        "/test/project",
      );
      expect(executionCall.args[0]).to.have.property(
    ...
    it("should handle framework workflow execution", async () => {
      ...
      const framework = app.frameworkLoader.getFramework(
        "refactoring_management",
      );
      ...
      stepKeys.forEach((stepKey) => {
        const stepInfo =
          app.frameworkStepRegistry.getFrameworkStepInfo(stepKey);
        if (stepInfo && stepInfo.module) {
          stepInfo.module.execute = mockExecute;
        }
      });
      ...
      for (const stepName of workflow.steps) {
        ...
        const result = await app.stepRegistry.executeStep(
          stepKey,
          workflowContext,
        );
    ...
    it("should handle framework deactivation and cleanup", async () => {
      ...
      const deactivationResult = await app.frameworkManager.deactivateFramework(
        "refactoring_management",
      );
    ...
    it("should handle framework errors and fallback to core", async () => {
      ...
      if (stepInfo && stepInfo.module) {
        stepInfo.module.execute = sandbox
          .stub()
          .rejects(new Error("Framework step failed"));
      }
  ...
  describe("Framework Configuration and Settings", () => {
    ...
    it("should respect framework configuration settings", async () => {
      ...
      if (config.activation.auto_load) {
        ...
        const activeFrameworks = app.frameworkManager.getAllActiveFrameworks();
        expect(
          activeFrameworks.some((f) => f.name === "refactoring_management"),
        ).to.be.true;
      }
    });
    ...
    it("should handle framework validation settings", async () => {
      ...
      if (framework.config.settings.validation_enabled) {
        const validationResult =
          await app.frameworkValidator.validateFramework(framework);
  ...
  describe("Framework Performance and Scalability", () => {
    beforeEach(async () => {
      ...
      const frameworkDirs = Array.from(
        { length: 10 },
        (_, i) => `framework_${i}`,
      );
