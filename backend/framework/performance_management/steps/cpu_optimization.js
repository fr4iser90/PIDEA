...
class CpuOptimizationStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const enableProfiling =
        options.enableProfiling || config.settings.enableProfiling;
      ...
      result.optimization.optimizations =
        await this.applyOptimizations(optimizationLevel);
      ...
      this.logger.info(
        `✅ CPU optimization completed. Applied ${result.optimization.optimizations.length} optimizations.`,
      );
  ...
  async analyzeCpuUsage() {
    return {
      ...
      loadAverage:
        process.platform === "linux" ? require("os").loadavg() : [0, 0, 0],
...
module.exports = {
  config,
  execute: CpuOptimizationStep.prototype.execute.bind(
    new CpuOptimizationStep(),
  ),
};
