...
class GeneratePerformanceReportStep {
  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const includeCharts =
        options.includeCharts || config.settings.includeCharts;
      ...
      result.report.recommendations = await this.generateRecommendations(
        result.report,
      );
  ...
  async generateRecommendations(report) {
    ...
    if (report.metrics.memory.usage.heapUsed > 100 * 1024 * 1024) {
      // 100MB
...
module.exports = {
  config,
  execute: GeneratePerformanceReportStep.prototype.execute.bind(
    new GeneratePerformanceReportStep(),
  ),
};
