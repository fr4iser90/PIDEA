  ...
  async execute(context = {}, options = {}) {
    try {
      ...
      const concurrentUsers =
        options.concurrentUsers || config.settings.concurrentUsers;
      ...
      // Run load tests
      result.testing.results = await this.runLoadTests(
        testDuration,
        concurrentUsers,
      );

      // Generate recommendations
      result.testing.recommendations = await this.generateRecommendations(
        result.testing,
      );

      this.logger.info(
        `✅ Load testing completed. Tested ${concurrentUsers} concurrent users for ${testDuration}ms`,
      );
