...
async function execute(context, options = {}) {
  ...
  logger.info(
    `🔍 [RefactorGenerateTask] Received ${largeFiles.length} large files:`,
    largeFiles.slice(0, 3),
  ); // Debug first 3 files
  ...
  try {
    ...
    for (const fileInfo of largeFiles) {
      try {
        const task = await createRefactoringTask(
          fileInfo,
          projectPath,
          taskRepository,
          context,
        ...
        logger.error(
          `❌ [RefactorGenerateTask] Failed to create task for ${fileInfo.path}:`,
          error.message,
        );
      }
    }
    ...
    logger.info(
      `✅ [RefactorGenerateTask] Successfully created ${createdTasks.length} refactoring tasks`,
    );
...
async function createRefactoringTask(
  fileInfo,
  projectPath,
  taskRepository,
  context,
  ...
  try {
    if (
      taskRepository &&
      typeof taskRepository.findByProjectPath === "function"
    ) {
      existingTasks = await taskRepository.findByProjectPath(projectPath);
    }
  } catch (error) {
    logger.warn(
      "⚠️ [RefactorGenerateTask] Error checking existing tasks:",
      error.message,
    );
  }
  ...
  const existingTask = existingTasks.find(
    (task) =>
      ...
      task.title.includes(
        path.basename(fileInfo.path, path.extname(fileInfo.path)),
      ),
  );

  if (existingTask) {
    logger.info(
      `⚠️ [RefactorGenerateTask] Task already exists for ${fileInfo.path}, skipping...`,
    );
    return existingTask;
  }
  ...
  if (!lines || lines <= 0) {
    logger.warn(
      `⚠️ [RefactorGenerateTask] Invalid line count for ${fileInfo.path}: ${lines}, skipping...`,
    );
    return null;
  }
  ...
  if (!projectId && projectPath) {
    try {
      ...
      } = require("@domain/services/shared/ProjectMappingService");
      const projectMappingService = new ProjectMappingService();
      projectId =
        await projectMappingService.getProjectIdFromWorkspace(projectPath);
    } catch (error) {
      logger.warn(
        "⚠️ [RefactorGenerateTask] Failed to get projectId from ProjectMappingService:",
        error.message,
      );
  ...
  logger.info(
    "🔍 [RefactorGenerateTask] Task created with projectId:",
    task.projectId,
  );
  ...
  try {
    if (taskRepository && typeof taskRepository.create === "function") {
      ...
      logger.info(
        `✅ [RefactorGenerateTask] Task saved to repository: ${taskId}`,
      );
    } else {
      logger.warn(
        "⚠️ [RefactorGenerateTask] taskRepository.create not available, task created in memory only",
      );
    }
  } catch (error) {
    logger.error(
      `❌ [RefactorGenerateTask] Failed to save task ${taskId}:`,
      error.message,
    );
  }

  logger.info(
    `✅ [RefactorGenerateTask] Created refactoring task for ${fileInfo.path} (${fileInfo.lines} lines)`,
  );
