...
async function execute(context, options = {}) {
  ...
  try {
    ...
    const analyzeResult = await stepRegistry.executeStep(
      "RefactorAnalyze",
      context,
      options,
    );
    ...
    const largeFiles =
      analyzeResult.result?.largeFiles || analyzeResult.largeFiles || [];

    logger.info(`🔍 [RefactoringStep] Analysis result:`, {
      success: analyzeResult.success,
      largeFilesCount: largeFiles.length,
      resultKeys: analyzeResult.result
        ? Object.keys(analyzeResult.result).length
        : 0,
    });
    ...
    logger.info(`🔍 [RefactoringStep] Task context:`, {
      largeFilesCount: taskContext.largeFiles
        ? taskContext.largeFiles.length
        : 0,
      hasLargeFiles: !!taskContext.largeFiles,
    });

    const taskResult = await stepRegistry.executeStep(
      "RefactorGenerateTask",
      taskContext,
      options,
    );
    ...
    logger.info(
      `✅ [RefactoringStep] Refactoring orchestration completed successfully!`,
    );
    ...
    logger.error(
      "❌ [RefactoringStep] Refactoring orchestration failed:",
      error,
    );
    throw error;
  }
