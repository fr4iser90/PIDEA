...
/**
 * Roadmap File Reorganization Script - FIXED VERSION
...
const logger = new Logger('RoadmapFileReorganizationFixed');

class RoadmapFileReorganizerFixed {
    ...
    async reorganize() {
        try {
            logger.info('🚀 Starting roadmap file reorganization (FIXED VERSION)...');
    ...
    extractTaskName(filename) {
        ...
        const suffixes = [
            '-index', '-implementation', '-phase-1', '-phase-2', '-phase-3', 
            '-phase-4', '-phase-5', '-phase-6', '-phase-7', '-report', 
    ...
    async generateReport() {
        ...
        const reportPath = path.join(this.workspacePath, 'logs', 'roadmap-reorganization-fixed-report.json');
...
if (require.main === module) {
    ...
    const reorganizer = new RoadmapFileReorganizerFixed(workspacePath);
...
module.exports = RoadmapFileReorganizerFixed;
