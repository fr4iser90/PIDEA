...
/**
 * Roadmap File Reorganization Script
 * Reorganizes all roadmap files into the new status-based structure
 * 
 * NEW STRUCTURE: docs/09_roadmap/[status]/[priority]/[category]/[task-name]/[file].md
 * OLD STRUCTURE: docs/09_roadmap/[status]/[category]/[file].md
 */
...
const logger = new Logger('RoadmapFileReorganization');

class RoadmapFileReorganizer {
    ...
    async reorganize() {
        try {
            logger.info('🚀 Starting roadmap file reorganization...');
    ...
    async analyzeFileStructure() {
        ...
        for (const filePath of allFiles) {
            ...
            let priority, category, filename;
            
            // Parse different structures
            if (status === 'completed' && parts[1].includes('q')) {
                // completed/2025-q3/category/file - KEEP QUARTER STRUCTURE
                const quarter = parts[1];
                category = parts[2];
                filename = parts[3];
                
                // For completed tasks, use quarter instead of priority
                const groupKey = `${status}/${quarter}/${category}/${this.extractTaskName(filename)}`;
                ...
                if (!fileGroups.has(groupKey)) {
                    fileGroups.set(groupKey, {
                        ...
                        quarter,
                        category,
                        taskName: this.extractTaskName(filename),
                        files: []
                    });
                }
                ...
                continue; // Skip the normal processing for completed tasks
                
            } else if (['pending', 'in-progress', 'failed'].includes(status)) {
                // pending/priority/category/file or pending/category/file
            ...
            // Create group key
            const groupKey = `${status}/${priority}/${category}/${taskName}`;
    ...
    extractTaskName(filename) {
        // Remove file extensions and type suffixes
        let taskName = filename.replace('.md', '');
        
        // Remove common suffixes
        taskName = taskName.replace(/-index$/, '');
        taskName = taskName.replace(/-implementation$/, '');
        taskName = taskName.replace(/-phase-\d+$/, '');
        taskName = taskName.replace(/-report$/, '');
        taskName = taskName.replace(/-summary$/, '');
        taskName = taskName.replace(/-analysis$/, '');
        taskName = taskName.replace(/-validation$/, '');
    ...
    async reorganizeTaskGroup(group) {
        ...
        // Create new directory structure - different for completed vs others
        let newDir;
        if (status === 'completed' && quarter) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                logger.debug(`📁 Moved: ${file.filename} -> ${path.relative(this.roadmapDir, newPath)}`);
    ...
    async generateReport() {
        ...
        const reportPath = path.join(this.workspacePath, 'logs', 'roadmap-reorganization-report.json');
        ...
        if (this.errors.length > 0) {
            logger.warn('⚠️ Some errors occurred during reorganization:');
            this.errors.forEach(error => {
                logger.warn(`  - ${error.file || error.group}: ${error.error}`);
            });
        }
    }
}
...
async function main() {
    ...
    const reorganizer = new RoadmapFileReorganizer(workspacePath);
...
module.exports = RoadmapFileReorganizer;
